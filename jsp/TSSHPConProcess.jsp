		                 ...
		                 if (rsFndId.next())
		                    {
							   ...
							   out.println("<table bgcolor='#FFFFCC'><TR bgcolor='#FFFFCC'><TD colspan=3><font color='#000099'>Processor Request Message </FONT></TD><TD colspan=3>"+devStatus+"("+devMessage+")"+"</TD></TR></table>");	
							   rsFndId.close();
                               stateFndId.close();
						       //out.println("sqlfnd="+sqlfnd);	
			                  ...
			                  String sqlSEAC=  " select * from RCV_TRANSACTIONS_INTERFACE "+
			                                   " where OE_ORDER_LINE_ID in ( select LINE_ID from OE_ORDER_LINES_ALL "+
                                                                           " where HEADER_ID in (  select HEADER_ID from OE_ORDER_HEADERS_ALL where ORDER_NUMBER like '1152%' "+
                                                                                                 " and to_char(creation_date, 'YYYYMMDD') between '20060802' and '20061231' ) "+ 
							  ...
							  if (rsSEAC.next()) 
							  {
								   ...
								   String sSqlMailUser = "select DISTINCT USERMAIL,USERNAME,WEBID from ORADDMAN.WSUSER where LOCKFLAG='N' and USERNAME in ('kerwin','suming','jingker') "; // 不需要通知做ShipConfirm 的人
	                               ...
	                               while(rsMail.next())
	                               {
                                        ...
                                        {  
		                                 ...
		                                 urAddress = "prod.ts.com.tw:8000/OA_HTML/AppsLocalLogin.jsp?langCode=US";		
