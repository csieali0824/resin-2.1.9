...
{  
	   ...
	   //{
		 ...
		 //String c_in_array=request.getParameter("MONTH"+ac+"-"+subac);//取前一頁之輸入欄位
          // sql="insert into PSALES_FORE_COST(FCCOMP,FCYEAR,FCMONTH,FCREG,FCCOUN,FCTYPE,FCPRJCD,FCCOST,FCMVER,FCLUSER,FCCURR) values(?,?,?,?,?,?,?,?,?,?,?)";
           sql="insert into MRPRODCLS(PCLS_CODE,PROD_CLASS,CLASS_DESC) values(?,?,?)";
  ...
  out.println("輸入產品別代碼新增成功 !!!<BR>");
  out.println("<A HREF=/wins/WinsMainMenu.jsp>首頁</A>&nbsp;&nbsp;<A HREF=../jsp/WSMRProdClassInput.jsp?BACK=TRUE>產品別代碼資料新增</A>&nbsp;&nbsp;<A HREF='../jsp/WSMRProdClassInquiry.jsp'>產品別代碼資料查詢</A> ");
