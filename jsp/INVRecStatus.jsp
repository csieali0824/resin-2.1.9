...
{   
  location.href="../jsp/INVRecStatus.jsp?STATUSID="+statusID+"&PAGEURL="+pageURL+"&SEARCHSTRING="+document.MYFORM.SEARCHSTRING.value  
}
...
{  //out.println("Step1"); 
   ...
   //取得資料總筆數  
	...
	{ //out.println("Step2");
	  ...
	  {
	    sql = "select count(*) from INV_M_REC where  RECSTATID='"+statusID+"' and (RECCREATENO like '"+searchString+"%')";	
	  } 
	  else 
	  {
	    sql = "select count(*) from INV_M_REC where  RECSTATID='"+statusID+"'";
	  }	
 	  rs=statement.executeQuery(sql);	 
    } 
	else 
	{	//out.println("Step3");
	   if (statusID.equals("002")) //為倉管確認中,可看到其所管轄倉庫之所有入庫申請案件
  	   { //out.println("Step4");
	     if (searchString!=null) //如果有搜尋特定單號則另下SQL
	     {
	       sql = "select count(*) from INV_M_REC where  RECSTATID='"+statusID+"' and RECCENTERNO='"+userActCenterNo+"' and (RECCREATENO like '"+searchString+"%')";	
	     } 
		 else 
		 {
	       sql = "select count(*) from INV_M_REC where  RECSTATID='"+statusID+"' and RECCENTERNO='"+userActCenterNo+"'";
	     }	
 	     rs=statement.executeQuery(sql);	 
       }
	}	  //out.println("Step8");    
...
<strong><font color="#0080C0" size="5">入庫案件處理</font></strong> <FONT COLOR=RED SIZE=4>&nbsp;&nbsp;狀態:<%=statusName%>(<%=statusDesc%>)</FONT><FONT COLOR=BLACK SIZE=2>(總共<%=maxrow%>筆記錄)</FONT>
<table width="75%" border="0">
  <tr>
    ...
    <td><strong><font color="#400040" size="2">入庫單號:</font></strong>
...
<A HREF="../jsp/INVRecStatus.jsp?<%=firstpageurl%>"><font size="2"><strong><font color="#FF0080">第一頁</font></strong></font></A>&nbsp;&nbsp<A HREF="../jsp/INVRecStatus.jsp?<%=lastpageurl%>"><font size="2"><strong><font color="#FF0080">最後一頁</font></strong></font></A><font color="#FF0080"><strong><font size="2">&nbsp;&nbsp;<A HREF="../jsp/INVRecStatus.jsp?<%=nextpageurl%>">下一頁</A>&nbsp;&nbsp;<A HREF="../jsp/INVRecStatus.jsp?<%=prevpageurl%>">上一頁</A>&nbsp;&nbsp;(第<%=currentPageNumber%>頁/共<%=totalPageNumber%>頁)</font></strong></font> 
<%    
  try
  {   
   ...
   String sql=null;
	...
	{ 
	  if (searchString!=null) //如果有搜尋特定單號則另下SQL
	  {
	    sql = "select RECCREATENO,RECCENTERNO,RECCREATEDATE,RECCREATEUSER,RECSTATID,RECSTAT from INV_M_REC where RECSTATID='"+statusID+"' and (RECCREATENO like '"+searchString+"%') order by RECCREATENO";	     
      } 
	  else 
	  {
	    sql = "select RECCREATENO,RECCENTERNO,RECCREATEDATE,RECCREATEUSER,RECSTATID,RECSTAT from INV_M_REC where RECSTATID='"+statusID+"' order by RECCREATENO";
	  }		
	  rs=statement.executeQuery(sql);	  
    } 
	else 
	{
	   ...
	   { 
	     if (searchString!=null) //如果有搜尋特定單號則另下SQL
	     {
	       sql = "select RECCREATENO,RECCENTERNO,RECCREATEDATE,RECCREATEUSER,RECSTATID,RECSTAT from INV_M_REC where RECSTATID='"+statusID+"' and RECCENTERNO='"+userActCenterNo+"' and (RECCREATENO like '"+searchString+"%') order by RECCREATENO";	     
         } 
		 else 
		 {
	       sql = "select RECCREATENO,RECCENTERNO,RECCREATEDATE,RECCREATEUSER,RECSTATID,RECSTAT from INV_M_REC where RECSTATID='"+statusID+"' and RECCENTERNO='"+userActCenterNo+"' order by RECCREATENO";
	     }		
	     rs=statement.executeQuery(sql);	    	  
	   }      
	}		
...
/////////////////////////////////////////////////////////
   ...
   qryAllChkBoxEditBean.setSearchKey("RECCREATENO");
 ...
 %>
倉別: 
      <INPUT NAME="WHS" TYPE="text" value="CE" size="2">
      &nbsp;&nbsp  架位: 
      <INPUT TYPE="text" NAME="LOC" size="6">
      &nbsp;&nbsp     
      <%
	  try
      {       
	   if (statusID.equals("002"))
	   {
         Statement statement=con.createStatement();
...
<BR>
	...
	<input name="ACTCENTERNO" type="HIDDEN" value="<%= ACTCENTERNO %>" >
	<input name="RECCREATEDATE" type="HIDDEN" value="<%= RECCREATEDATE %>" >    
	<input name="RECCREATEUSER" type="HIDDEN" value="<%= RECCREATEUSER %>" >  
	<!--<input name="WHS" type="HIDDEN" value="<%= WHS %>" > 
