...
<%@ page import="bean.ComboBoxAllBean,bean.DateBean,bean.ArrayComboBoxBean,java.text.DecimalFormat" %>
...
<body>
<FORM ACTION="../jsp/WSForeShpPRRpt.jsp" METHOD="post" NAME="MYFORM">
...
try
{
  ...
  sWhere= " where a.MCOUNTRY=l.LOCALE and a.MPROJ=b.FMPRJCD and a.MCOUNTRY=r.LOCALE and r.REGION in ("+regionString+") and b.FMCOLOR=c.COLORCODE"+          			                 		 
		  " and (MPROJ in ("+modelString+") or MPROJ||c.COLORDESC in ("+shipString+") or MPROJ in ("+prString+") )";
  if (regionNo!=null && !regionNo.equals("--")) //若有選擇REGION,則只取出有隸屬該REGION的國家
  {
  ...
  while (rs.next())  //main while loop
  {
   ...
   //取得已核准之購料需求   
   subRs=subStat.executeQuery("select sum(l.RQQTY) as rqqty,h.RQYEAR||h.RQMONTH as yearmonth from PSALES_FORE_APP_LN l,PSALES_FORE_APP_HD h where l.DOCNO=h.DOCNO and h.CANCEL!='Y' and h.APPROVED='Y' and h.RQYEAR||h.RQMONTH between '"+yearMonthString[0][0]+yearMonthString[0][1]+"' and '"+yearMonthString[5][0]+yearMonthString[5][1]+"' and h.COUNTRY="+countryNo+" and h.TYPE='001' and l.PRJCD='"+modelNo+"' and l.COLOR='"+color+"' group by h.RQYEAR||h.RQMONTH order by h.RQYEAR||h.RQMONTH");	     
   if (subRs.next())  
   {
   ...
   subRs.close();  
   	// 2005.06.20 Ivy Yang改自Data Center取得出貨統計
   		//自維修系統中取得出貨資訊   
   		//subRs=repairStat.executeQuery("select sum(PCOUNT) as shqty,PYEAR||PMONTH as yearmonth "
		//+" from PISSHIPCNT where PCLASS='MO' and COUNTRY='"+countryNo+"' and PMODEL='"+modelNo+"' and PCOLOR='"+color+"'"
