...
<%@ page import="bean.ComboBoxBean,bean.ArrayComboBoxBean,bean.ForePriCostInputBean,bean.DateBean,bean.RsCountBean" %>
<!--=============以下區段為取得連結池==========-->
...
<title>Sales Forecast Price & Cost Entry Form</title>
</head>
<body>
...
<strong> Price &amp; Cost of Forecast  Entry</strong></font>
...
<table width="100%" border="0">
    <tr bgcolor="#D0FFFF">
        ...
        <% 		 		 		 
         ...
         <%		 		  		  
          ...
          {     
		   ...
		   out.println("<select NAME='REGION' onChange='showCountry(document.MYFORM.COUNTRY.length)'>");
           out.println("<OPTION VALUE='--'>--");   
		   for (int i=0;i<regionArray.length;i++)		   		   		     			            
		   {
              if (regionArray[i].equals(regionNo)) 
              {
                out.println("<OPTION VALUE='"+regionArray[i]+"' SELECTED>"+regionArray[i]);                                     
              } else {
                out.println("<OPTION VALUE='"+regionArray[i]+"'>"+regionArray[i]);
              }     
		   }	  
		   out.println("</select>"); 	
         } //end of try
	 ...
	 <% 
	   ...
	   {
   	      out.println("<input name='submit1' type='submit' value='Enter Price' onClick='return setSubmit("+'"'+"../jsp/WSForecastPriceInput.jsp"+'"'+")'> ");
	   }  
	 %> 
	<% 
	    if (UserRoles.indexOf("admin")>=0 || UserRoles.indexOf("fc_editor")>=0)
	   {
   	      out.println("<input name='submit1' type='submit' value='Enter Cost' onClick='return setSubmit("+'"'+"../jsp/WSForecastCostInput.jsp"+'"'+")'> ");
	   }  
	 %>    	  
</td> 
	</tr>	
