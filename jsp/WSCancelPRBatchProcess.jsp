...
<html>
<head>
<title>Cancel PR Batch Process</title>
  ...
  {   
    ...
    if (status.equals("COMPLETE") || status.equals("REJECT"))	
    {
	  ...
	  nextPrcsMan="--";
  	  ...
  	  {
	     mailSubject="Cancel PR("+choice[i]+") has been approved by "+UserName+"!!"; 
	  } else {
	     mailSubject="Cancel PR("+choice[i]+") has been rejected by "+UserName+"!!"; 
	  }   
    } else {
      //*********取得下一個流程資訊**********************************
      docOrders++;     
    ...
    //************END OF 取得下一個流程資訊**************************
	  mailMan=nextPrcsMan;
	  mailSubject="Cancel PR("+choice[i]+") coming from "+UserName+" need to approve!!";  
    }	
    ...
    if (!nextPrcsManMail.equals(""))
    {
      ...
      sendMailBean.setUrlAddr(serverName+"/wins/jsp/WSCancelPRApprove.jsp?DOCNO="+choice[i]);		 
      sendMailBean.sendMail();
    }//enf of nextPrcsManMail if--------------------------------	   
	
	//若已核准則通知MCUser物管人員,同時將原PR之CANCEL欄位給UPDATE成Y
	if (status.equals("COMPLETE")) 
    {
	  ...
	  statement.executeUpdate("update PSALES_FORE_APP_HD set CANCEL='Y',CANCEL_PR='"+choice[i]+"' where DOCNO='"+origin_PR+"'");
    }
  } //end of choice for         	       
  statement.close();
  out.println("Cancel PR has been Processed !!<BR>");
