...
function setSubmit(URL)
{
   ...
   {     
     ...
     { 
      alert("Before you want to add , please do not let the Monthly COGs of forecast data be Null !!");   
      return(false);
      } 
   ...
   {     
     ...
     { 
	  c=txt.charAt(j);
	    ...
	    {
            alert("Monthly COGs data should be numerical!!");   
            return(false);
		 }
      } 
   } //end of for  null check
...
{    
   ...
   {     
      for (j=1;j<dim2;j++)
	  {
		 ...
		 { 
           alert("Before you want to save , please do not let the Monthly COGs  detail be Null !!");   
           return(false);
		 }  
	  } //enf for of jj             
   } //end of for null check
      ...
      for (k=1;k<dim2;k++)
	  {
         ...
         { 
	       c=txt.charAt(j);
	        if ("0123456789.".indexOf(c,0)<0) 
	       {
             alert("Monthly COGs detail should be numerical!!");   
             return(false);
		    }
         }
	   } //enf for of k
...
<title>Monthly COGs Input Form</title>
</head>
<body>
<FORM ACTION="WSForeCogsInput.jsp" METHOD="post" NAME="MYFORM">
...
<strong>COGs Input (Detail)</strong></font> 
...
{    
  ...
  if ( bringLast!=null   && bringLast.equals("Y"))  //若要帶出前一版本資料則執行以下動作
  {
    ...
    ResultSet blrs=null;
				...
				{			   				  				
				  blrs=blstat.executeQuery("select FCCOGS,FCMVER FROM PSALES_FORE_COGS where FCCOMP='"+comp+"' and FCTYPE='"+type+"' and FCREG='"+region+"' and FCCOUN='"+country+"' and FCPRJCD='"+chooseItem+"' and FCMONTH='"+dateBean.getMonthString()+"' and FCYEAR='"+dateBean.getYearString()+"' and FCCURR='"+curr+"' order by FCMVER DESC");                  				  
				  if (blrs!=null)
				  {				    					 
					 if ( blrs.next())
					 {
				       allMonth[jj]=blrs.getString("FCCOGS");					  					    						
					 } else {
					    allMonth[jj]="0";
					 }				  				    
				  } //end if of blrs!=null				  
     ...
     <% 				 		 
      ...
      <td width="24%"><font size="2"> <font color="#333399" face="Arial Black"><strong>Country: 
       ...
       %> 
        ...
        <INPUT TYPE="button"  value="Add" onClick='setSubmit("../jsp/WSForeCogsInput.jsp")' >
	  </font></td>     
  </tr>
</table>
<table width="100%" border="1" cellspacing="0" cellpadding="0">
  ...
  <tr>  
    <td colspan="14"><a href='javaScript:setSubmit("../jsp/WSForeCogsInput.jsp?BRINGLAST=Y")'>Last version of this model</a><BR><HR SIZE=2></td>
    </tr>
  <tr bgcolor="#FFCC33">
...
<% 
     int div1=0,div2=0;      //做為運算共有多少個row和column輸入欄位的變數
      ...
      {	
       ...
       %>
<BR>
<INPUT name="button2" TYPE="button" onClick='setSubmit("../jsp/WSForeCogsInput.jsp")'  value="DELETE" >
------------------------------------------------------------------------------------------------------------
<INPUT TYPE="button"  value="SAVE" onClick='setSubmit2("../jsp/WSForeCogsInsert.jsp",<%=div1%>,<%=div2%>)' >
---------------------
<!-- 表單參數 -->  
