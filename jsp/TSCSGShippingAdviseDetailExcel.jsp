...
<body>
<FORM ACTION="../jsp/TSCSGShippingAdviseDetailExcel.jsp" METHOD="post" name="MYFORM">
<%
...
String package_mode="",basic_package="內外P/N",shipping_mark="";
try 
{ 	
	...
	String status="";
	RPTName = "TSCSG_Shipping_Advise_Detail";
	...
	sql = " select tsa.PC_ADVISE_ID,"+
          ...
          //" tsa.SHIPPING_REMARK,"+
		  " tssg_ship_pkg.GET_SO_SHIPPING_MARKS(tsa.advise_no,tsa.so_no) SHIPPING_REMARK,"+  //modify by Peggy 20200417
          " tsa.CUST_PO_NUMBER,"+
          " tsa.SHIPPING_METHOD,"+
          " pha.segment1 PONO,"+
          ""+(UserRoles.indexOf("TEW_QC")<0?"tspp.PO_UNIT_PRICE":"'' PO_UNIT_PRICE")+","+
		  " (nvl(tspp.order_qty,tsa.ship_qty)/1000) order_qty,"+
		  " tsa.ADVISE_NO,"+
		  ...
		  //" nvl((select count(1) from tsc.tsc_shipping_advise_lines tsal where tsal.pc_advise_id = tsa.pc_advise_id ),0) box_cnt,"+
	...
	sql +=") po_cnt"+
		  ...
		  ",tcl.lov_meaning market_group_desc"+  
	      ",TSC_INV_CATEGORY(tsa.inventory_item_id,49,23) TSC_Package"+ 
          ",case  mp.organization_code when 'SG1' then '內銷' when 'SG2' then '外銷' else mp.organization_code end as organization_name"+
		  ",tsa.cust_label"+ //add by Peggy 20200415
		  ",tsa.chk_rpt_name"+ //add by Peggy 20200714
		  ",TSC_GET_SHIPPING_NOTE(tsa.so_line_id,NULL) SHIPPING_NOTE"+ //add by Peggy 20200730
          " FROM "+
		  " (select a.*,TSC_LABEL_PKG.TSC_GET_CUST_LABEL_GROUP (a.so_line_id) cust_label from tsc.tsc_shipping_advise_pc_sg a"+
		  " where a.PC_SCHEDULE_SHIP_DATE between TO_DATE(nvl('"+SDATE+"','20200201'),'yyyymmdd') and  TO_DATE(nvl('"+EDATE+"','20991231'),'yyyymmdd')";
	if (!ADVISE_NO.equals(""))
	{
		sql += " and a.ADVISE_NO = '"+ADVISE_NO+"'";
	}	
	if (!MO.equals(""))
	{
		sql += " and a.SO_NO like '"+MO+"%'";
	}
	if (!ITEMNAME.equals(""))
	{
		sql += " and (a.ITEM_NO like '"+ITEMNAME +"%' OR a.ITEM_DESC like '"+ ITEMNAME+"%')";
	}	
	if (!SALESAREA.equals("") && !SALESAREA.equals("--"))
	{
		sql += " and a.REGION_CODE='"+ SALESAREA+"'";
	}	
	...
	{
		sql += " AND a.PRODUCT_GROUP='"+TSCPRODGROUP+"'";
	}	
	if (!CHK_PRT.equals(""))
	{
		sql += " AND exists (select 1 from tsc.tsc_shipping_advise_pc_sg x where x.CHK_RPT_NAME like '%"+CHK_PRT+"%' and x.advise_no=a.advise_no)";
	}
	...
	{ 	
		if (reccnt==0)
		{
			...
			ws.addCell(new jxl.write.Label(col, row, "Delivery Instruction" , ACenterBLB));
			...
			//包裝方式
			ws.addCell(new jxl.write.Label(col, row, "包裝方式" , ACenterBLB));
			ws.setColumnView(col,25);	
			col++;
		...
		col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("ORGANIZATION_NAME")==null?"":rs.getString("ORGANIZATION_NAME")), ACenterL));
		col++;	
		...
		ws.addCell(new jxl.write.Label(col, row, rs.getString("SHIPPING_METHOD")+ (rs.getString("shipping_note")==null?"":"\r\n("+rs.getString("shipping_note")+")"), ALeftL));
		col++;
		...
		if (rs.getString("to_tw").equals("是") && (shipping_mark.indexOf("元超")<0 || rs.getString("SO_NO").startsWith("1121"))) package_mode="";
		if ((shipping_mark.indexOf("元超")>=0 && !rs.getString("SO_NO").startsWith("1121")) || (!rs.getString("to_tw").equals("是") && (rs.getString("QC_RPT_FLAG").equals("Y") || rs.getString("QC_RPT_FLAG").indexOf("PDF")>=0 || shipping_mark.toUpperCase().indexOf("CHANNEL WELL") >=0)))
		{
			package_mode+="\r\n附"+(rs.getString("QC_RPT_FLAG").equals("PDF")?"電子檔":(rs.getString("QC_RPT_FLAG").equals("PPDF")?"電子檔及紙本":"出貨"))+"檢驗報告"+(rs.getString("QC_RPT_FLAG").indexOf("PDF")>=0?"(檔名:"+rs.getString("CHK_RPT_NAME")+")":"");  
		}
		...
		col++;		
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("CUST_LABEL")==null?"":rs.getString("CUST_LABEL")), ALeftL));
		col++;
		row++;
