...
<title>TEW Pick Lot Allot</title>
<!--=============以下區段為安全認證機制==========-->
...
<jsp:useBean id="PickBean" scope="session" class="bean.Array2DimensionInputBean"/>
<%
String sql = "";
...
<FORM ACTION="TEWPickAllot.jsp" METHOD="post" NAME="MYFORM">
<strong><font style="font-size:20px;color:#006666">TEW 批號分配</font></strong>
<BR>
...
<table cellSpacing="0" bordercolordark="#5C7671"  cellPadding="1" width="100%" align="center" borderColorLight="#CFDAD8" border='1' bgcolor="#DCE6E7">
	<tr>
		...
		&nbsp;&nbsp;&nbsp;&nbsp;<INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>' onClick='setSubmit("../jsp/TEWPickAllot.jsp")' ></td>
	</tr>
</table> 
<HR>
<%
try
{
	sql = " select a.* from ( select  distinct a.tew_advise_no advise_no"+
		  ...
		  ",d.INVOICE_NO"+
		  " from tsc.tsc_shipping_advise_lines a,tsc.tsc_shipping_advise_headers b,ap.ap_supplier_sites_all c,apps.TSC_VENDOR_INVOICE_LINES d"+
		  " where  b.SHIPPING_FROM=? "+
		  " and b.advise_header_id = a.advise_header_id "+
		  " and a.vendor_site_id = c.vendor_site_id(+)"+
		  " and a.tew_advise_no is not null"+
		  " and a.tew_advise_no = d.tew_advise_no(+)"+
		  " and not exists (select 1 from oraddman.tew_lot_allot_detail x where x.advise_line_id = a.advise_line_id)"+
		  " and not exists (select 1 from tsc.TSC_PICK_CONFIRM_LINES y where y.advise_header_id = b.advise_header_id)";
...
%>
		<table cellSpacing="0" bordercolordark="#5C7671"  cellPadding="1" width="100%" align="center" borderColorLight="#CFDAD8" border='1'>
			<tr bgcolor="#C8E3E8" style="font-family:'細明體';font-size:12px" align="center">
				...
				<td width="13%" align="center">供應商</td>
				<td width="15%" align="center">供應商發票號碼</td>
...
%>
		<tr>
			...
			<td><input type="button" name="btn_<%=rs.getString("ADVISE_NO")%>" value="批號分配" onClick="setSubmit1('../jsp/TEWPickAllotDetail.jsp?ADVISENO=<%=rs.getString("ADVISE_NO")%>')"></td>
