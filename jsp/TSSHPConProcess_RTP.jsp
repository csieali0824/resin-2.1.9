...
// 因關聯 訂單主檔及明細檔,故需呼叫SET Client Information Procedure
 ...
 //java.sql.Time shippingTime = null; //將SHIPDATE轉換成日期格式以符丟入API格式 Update By Kerwin 2006/07/28
...
if (tsInvoiceNo!=null)
{
  ...
  // else if (shipDate > systemDate )
   ...
   else if (periodStatus=="OPEN" || periodStatus.equals("OPEN"))
   {
			  ...
			  String sql1a=  " select B.SALESORDERNO||'-'|| B.LINE_NO as SELECTFLAG, A.TSINVOICENO,B.RCVQTY ,A.CREATE_BY,B.PO_HEADER_ID,B.PO_LINE_ID,B.PO_LOCATION_LINE_ID, "+
			    " A.EMPLOYEE_ID,B.ORGANIZATION_ID,B.ORDER_HEADER_ID,B.ORDER_LINE_ID  "+
 ...
 %>
			   ...
			   <input type="text" size="15" name="ORDERHADERID" value="<%=rs.getInt("ORDER_HEADER_ID")%>" readonly>
			   <input type="text" size="15" name="ORDERLINEID" value="<%=rs.getInt("ORDER_LINE_ID")%>" readonly>			   
...
<%					  
					  if ((errorMessageHeader==null || errorMessageHeader=="" || errorMessageHeader.equals("&nbsp;") ) && (errorMessageLine==null || errorMessageLine=="" || errorMessageLine.equals("&nbsp;"))) 
					  {
				        ...
				        String sql1=" update APPS.TSC_DROPSHIP_SHIP_HEADER "+
                                    " set STATUS='CLOSED',CONFIRM_DATE=SYSDATE,SHIPDATE="+shipDate+dateBean.getHourMinuteSecond()+",CONFIRM_BY ='"+UserName+"' "+
						...
						// 2006/07/31 增加 判斷若為TSC_TERM_RULE內規定的SEA(C) ,則不執行Concurrent--> Receiving Transaction Processor
			           String sqlRTP=  " select a.SHIPMETHOD , a.TSINVOICENO , a.FOBPOINT, "+
					                          " b.ORDER_HEADER_ID, b.ORDER_LINE_ID, b.INVENTORY_ITEM_ID, b.RCVQTY "+
                                       " from TSC_DROPSHIP_SHIP_HEADER a, TSC_DROPSHIP_SHIP_LINE b, TSC_TERM_RULE c "+
                                       " where a.STATUS = 'CLOSED' and NVL(a.PRINTED_FLAG,'N') = 'Y' and b.LINE_STATUS = 'CLOSED' "+ 
									   "   and a.TSINVOICENO = b.TSINVOICENO and a.DS_HEADER_ID = b.DS_HEADER_ID"+ 
					           		   "   and a.SHIPMETHOD = c.SHIPPING_METHOD and a.FOBPOINT = c.SHIPPING_TERM "+
                                       "   and a.TSINVOICENO = upper('"+tsInvoiceNo+"')" ; 
					   Statement stateRTP=con.createStatement();				 
					   ResultSet rsRTP=stateRTP.executeQuery(sqlRTP);
					   if (rsRTP.next())
					   {	
                          ...
                          if (rsRTP.getString("SHIPMETHOD")!="SEA(C)" && !rsRTP.getString("SHIPMETHOD").equals("SEA(C)"))
						  { // 不為SEA(C)..直接執行呼叫 Receiving Transaction Processor 的 Request
						    
						  } // end of if (rsRTP.getString("SHIPMETHOD")!="SEA(C)" && !rsRTP.getString("SHIPMETHOD").equals("SEA(C)"))	 
						  else if (rsRTP.getString("SHIPMETHOD").equals("SEA(C)"))
						  { // ***** 如為 SEA(C)..則更改RCV_TRANSACTION_INTERFACE 的 PROCESS_STATUS_CODE = 'ERROR' 使其先不執行入帳動作  ****
						       String sqlErr=" update RCV_TRANSACTIONS_INTERFACE set PROCESSING_STATUS_CODE='PENDING' "+
                                              " where QUANTITY = '"+rsRTP.getString("RCVQTY")+"'  and PROCESSING_STATUS_CODE ='PENDING' "+
										      "   and ITEM_ID = '"+rsRTP.getString("INVENTORY_ITEM_ID")+"' "+
										      "   and OE_ORDER_HEADER_ID = '"+rsRTP.getString("ORDER_HEADER_ID")+"' and OE_ORDER_LINE_ID = '"+rsRTP.getString("ORDER_LINE_ID")+"' ";								   
                              // PreparedStatement pstmtErr=con.prepareStatement(sqlErr);
							  // pstmtErr.executeUpdate(); 
                              // pstmtErr.close();	
						  } // end of else if
					   } // End of if (rsRTP.next())
					   rsRTP.close();
					   stateRTP.close();		 
					   // // 2006/07/31 增加 判斷若為TSC_TERM_RULE內規定的SEA(C) ,則不執行Concurrent--> Receiving Transaction Processor	 
					  }							 
