...
//out.println("CHECK_FUNCTION="+CHECK_FUNCTION);
...
try
{
	...
	{
		sql= " update oraddman.ts_label_all  "+
			 " set LABEL_NAME=?"+
			 ",LABEL_GROUP_CODE=?"+
			 ",LABEL_TYPE_CODE=?"+
			 ...
			 ",SHIPPING_MARK=?"+
			 ",CHECK_FUNCTION=?"+ //add by Peggy 20240507
			 " where LABEL_CODE = ?";
		//out.println(sql);
		...
		st1.setString(7,SHIPPING_MARK); //add by Peggy 20160401
		st1.setString(8,CHECK_FUNCTION); //add by Peggy 20240507
		st1.setString(9,LABEL_CODE);
		st1.executeUpdate();
		st1.close();
	...
	{
		//sql = " select  'TS'|| LPAD(NVL(MAX(REPLACE(UPPER(LABEL_CODE),'TS','')),0)+1,3,'0') AS SEQ from oraddman.ts_label_all a ";
		sql = " select  'TS'|| LPAD(NVL(MAX(TO_NUMBER(REPLACE(UPPER(LABEL_CODE),'TS',''))),0)+1,4,'0') AS SEQ from oraddman.ts_label_all a ";
		//out.println(sql);
		...
		sql = " insert into oraddman.ts_label_all "+
			  " (label_code"+
			  ", label_name"+
			  ", label_file"+
			  ", label_group_code"+
			  ...
			  ", last_update_date"+
			  ", last_updated_by"+
			  ", SHIPPING_MARK"+ //add by Peggy 20160401
			  ", CHECK_FUNCTION"+//add by Peggy 20240507
			  " )"+
			  " values ("+
			  " ?"+
		...
		st1.setString(9,SHIPPING_MARK); //add by Peggy 20160401
		st1.setString(10,CHECK_FUNCTION); //add by Peggy 20160401		
		st1.executeUpdate();
		st1.close();
	...
	if  (!upload_file.isMissing()) 
	{
		...
		ResultSet rs7=stmt.executeQuery("select label_file from oraddman.ts_label_all where LABEL_CODE='"+LABEL_CODE+"' for UPDATE");  	
		if (rs7.next())
		{
			BLOB myblob=null;
...
%>
		<script language="javascript">
			alert("修改成功!!");
			location.replace("/oradds/jsp/TSLabelQuery.jsp?LABEL_GROUP=<%=LABEL_GROUP_CODE%>")
		</script>	
<%
	}
	else
...
%>
		<script language="javascript">
			alert("新增成功!!");
			location.replace("/oradds/jsp/TSLabelAdd.jsp?LABEL_GROUP=<%=LABEL_GROUP_CODE%>")
		</script>
<%
	}
}
