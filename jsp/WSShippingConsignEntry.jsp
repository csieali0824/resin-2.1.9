...
<%@ page import="bean.ComboBoxBean,bean.ArrayComboBoxBean,bean.DateBean,bean.ArrayListCheckBoxBean" %>
...
<font color="#000000" size="+2" face="Arial"><strong>寄售出庫</strong></font>
<BR>
...
%> 
<FORM ACTION="WSShippingConsignInsert.jsp" METHOD="post" NAME="MYFORM">
...
<table border="1" cellspacing="0" cellpadding="0" bgcolor="#CCFFCC">
	...
	<tr >
		...
		<td>
			...
			try // get whs
			{
				...
				out.println("<select NAME='WAREHOUSE' onChange='setSubmit("+'"'+"../jsp/WSShippingConsignEntry.jsp?RESET=1"+'"'+")'>");
				out.println("<OPTION VALUE=-->--");
				String sSql = "SELECT BLWHS FROM wsshp_center WHERE centerno ='"+userActCenterNo+"' ORDER BY BLWHS";
	...
	<tr>
		<td bgcolor="#CCFFCC"><font color="#333399" font size="2" face="Arial Black">Customer </font></td>
		<td bgcolor="#CCFFCC"><font color="#333399" font size="2" face="Arial Black"> 
		  <%
			...
			try // get customer
			{
				...
				{
					String sLoc="";
					if (Warehouse.equals("71")) { sLoc = "('200','210')" ; }
					else if (Warehouse.equals("72")) { sLoc = "('230')"; }
					else if (Warehouse.equals("73")) { sLoc = "('250')"; }
					else { sLoc = "('')" ; }
					String sqlCust = "SELECT CCUST,CNME FROM RCM WHERE CLOC IN "+sLoc+" ORDER BY CCUST";
					Statement statCust=bpcscon.createStatement();
					ResultSet rsCust=statCust.executeQuery(sqlCust);
					while(rsCust.next())
					{
						String s1 = rsCust.getString("CCUST");
						String s2 = rsCust.getString("CNME");
			...
			try  // get sales rep.
			{
				...
				{
					String sqlCust = "SELECT CNME,CSAL,SNAME,CCOUN,trim(CAD1)||trim(CAD2)||trim(CAD3) as ADDR FROM RCM,SSM"+
						" WHERE SSAL=CSAL AND CCUST="+CustNo;
					Statement statCust=bpcscon.createStatement();
					ResultSet rsCust=statCust.executeQuery(sqlCust);
					while(rsCust.next())
					{
						SalesRepNo = rsCust.getString("CSAL");
						SalesRepName = rsCust.getString("SNAME");
						CustName = rsCust.getString("CNME");
						Addr = rsCust.getString("ADDR");
						//out.println(SalesRepNo);
						//out.println(SalesRepName);
					} // end of while					
					rsCust.close();    
					statCust.close();
		...
		<td colspan="2"><font color="#333399" font size="2" face="Arial Black"><%=SalesRepNo+"-"+SalesRepName%></font></td>
			<input name="SALESREPNO" type="hidden" value="<%=SalesRepNo%>"> 
			<input name="SALESREPNAME" type="hidden" value="<%=SalesRepName%>"> 
			<input name="CUSTNAME" type="hidden" value="<%=CustName%>"> 		
	<tr>
	
		<td><font color="#333399" font size="2" face="Arial Black">Address</font></td>
		<td colspan="4" ><font color="#333399" font size="2" face="Arial Black"><%=Addr%></font></td>
			
			<input name="ADDR" type="hidden" size="30" value="<%=Addr%>">
	...
	<tr>
		<td colspan="5"><font color="#333399" font size="2" face="Arial Black">Carton No./IMEI No.
            ...
            <input name="button3" type="button" onClick='setSubmit("../jsp/WSShippingConsignEntry.jsp?RESET=1")' value="Add">
</font>
		</td>
	</tr>
			...
			<% 
				...
				if (carIMEI!=null && !carIMEI.equals(""))
				{
					...
					if (sFlag.equals("Y"))  // check imei has been shipped
					{
						...
						" WHERE (SHP_NOTES IS NULL OR SHP_NOTES = 'X' OR SHP_NOTES = 'S')"; // null = shipped, X = DOA/DAP exchange out S =  Consign out
...
<BR>
<input name="del" type="button" onClick='setSubmit("../jsp/WSShippingConsignEntry.jsp?RESET=1")'  value="DELETE" >

<div align="center">
<input name="confirm" type="submit" value="確認送出" >
</div>
