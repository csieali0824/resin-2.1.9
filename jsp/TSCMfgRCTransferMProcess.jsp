<!--20090426 liling 新增 txnDate+sTime 系統時間,丟手key的異動日,存於YEW_RUNCARD_ALL \RES_EMPLOYEE_OP,執行完畢清為空白->
<!--20091123 liling performance tuning -->
<!--20151224 liling 042->046 增加 txnDate+sTime 系統時間,丟手key的異動日 -->
<!--20160203 liling 若有超額完工改抓工單上QUEUE的數量來計算OVER的數量 -->
...
<FORM ACTION="TSCMfgRCTransferMProcess.jsp" METHOD="post" NAME="MPROCESSFORM">
<%
...
if(dateCodeSet==null || dateCodeSet.equals("")) dateCodeSet="";
...
pstmtNLS.close();
//抓取系統日期
String systemDate ="",txnDate="";  //20090426
String organCode ="";
...
if (rsOrgCode.next())
{
    ...
    sTime=rsOrgCode.getString("STIME");  //20090426
}
rsOrgCode.close();
stateOrgCode.close();
	    ...
	    { //out.println("aMFGRCExpTransCode[i][0]="+aMFGRCExpTransCode[i][0]);	   
		  ...
		  if (Float.parseFloat(aMFGRCExpTransCode[i][2])>0) // 若設定移站數量大於0才進行移站及報廢
		  {
		   ...
		   java.text.DecimalFormat nf = new java.text.DecimalFormat("###,##0.000"); // 取小數後三位      
						 ...
						 String sqlMQty = " select b.QTY_IN_QUEUE,RES_EMPLOYEE_OP from APPS.YEW_WORKORDER_ALL a, APPS.YEW_RUNCARD_ALL b "+
 									     " where a.WO_NO = b.WO_NO and a.WO_NO = '"+woNo+ "' "+
						 ...
						 if (rsMQty.next())
						 {
						   ...
						   txnDate = rsMQty.getString("RES_EMPLOYEE_OP");		// 20090426 移站日期		
		     ...
		     // {
			    rcScrapQty = Float.parseFloat(aMFGRCExpTransCode[i][11]);  // 2007/03/30 測試以手動輸入的報廢數 By Kerwin
			 // }
	       //抓取移站數量的單位,並計算本站報廢數量_迄

		 java.sql.Date txnDateTime = null; //將txnDATE轉換成日期格式以符丟入API格式
		 if (txnDate!=null && txnDate.length()>=8)
		 {
		  txnDateTime = new java.sql.Date(Integer.parseInt(txnDate.substring(0,4))-1900,Integer.parseInt(txnDate.substring(4,6))-1,Integer.parseInt(txnDate.substring(6,8)));  // 給Receiving Date
		   String txnTime = dateBean.getHourMinuteSecond();  // 給System Time
          Calendar calendar1 = Calendar.getInstance(); 
		  calendar1.set(dateBean.getYear(),dateBean.getMonth(),dateBean.getDay(),dateBean.getHour(), dateBean.getMinute(), dateBean.getSecond() );  // 設定日期的格式(年,月,日,時,分,秒)
          String sqlDate="  select TO_DATE('"+txnDate+txnTime+"','YYYYMMDDHH24MISS') from DUAL   ";  	
         // out.print("sqlDate"+sqlDate);			
          Statement stateDate=con.createStatement();
          ResultSet rsDate=stateDate.executeQuery(sqlDate);
	 	  if (rsDate.next())
		   { 
             txnDateTime  = rsDate.getDate(1,calendar1); 
		     out.println("<br>txnDateTime = "+txnDateTime);
		   }
		  rsDate.close();
          stateDate.close();	   
         }    // 得到入庫執行日期..日期型態

		 String getErrScrapBuffer = "",overSRFlag="";
		 ...
		 {		 
		   ...
		   int scrpAccID = 0;
						 ...
						 rsScrp.close();
						 stateScrp.close();
                         //out.print("sqlScrp="+sqlScrp+"  "+scrpAccID);
		   ...
		   // 2007/04/17 報廢時超額數量判段_By Kerwin_起
		             // 判斷是否已超過該站可移站數,若是,則亦表示OverComp_起--表示工令可用移站數
                      ...
                      ResultSet rsRM=stateRM.executeQuery(" select QUANTITY_IN_QUEUE from WIP_OPERATIONS where WIP_ENTITY_ID = "+entityId+" and organization_id= "+organizationID+" and OPERATION_SEQ_NUM = "+aMFGRCExpTransCode[i][4]+" ");
			          if (rsRM.next()) { remainQueueQty = rsRM.getFloat("QUANTITY_IN_QUEUE"); }
			          rsRM.close();
	   		          stateRM.close();
			         // 判斷是否已超過該站可移站數,若是,則亦表示OverComp_迄--表示工令可用移站數
			          if (Float.parseFloat(aMFGRCExpTransCode[i][11]) > remainQueueQty)//若報廢數大於可移數,表示報廢overcompletion
		              {
					     overSRQty = Float.parseFloat(aMFGRCExpTransCode[i][11])-remainQueueQty; 
						 // 取到小數後3位,四捨五入
          ...
          //  out.print("<br>2007/04/17 報廢時超額數量判段_By Kerwin_迄");
	      ...
	      // seqstmt.setDate(3,processDateTime);  out.println("processDateTime="+processDateTime); // LAST_UPDATE_DATE 
		   ...
		   scrapstmt.setFloat(7,rcScrapQty); out.println("流程卡:"+aMFGRCExpTransCode[i][1]+"(報廢數量="+rcScrapQty+")<BR>");        	   
		   ...
		   if (overSRFlag == "Y" || overSRFlag.equals("Y"))
           { scrapstmt.setFloat(19,overSRQty); }  // OVERCOMPLETION_TRANSACTION_QTY
		...
		{              
		    // 再判斷是否已超過該站可移站數,若是,則亦表示OverComp_起--表示工令可用移站數
				     float remainQueueQty=0;
			          ...
			          rsRM.close();
	   		          stateRM.close();
           //out.print("<br>getRetScrapCode="+getRetScrapCode+"  remainQueueQty="+remainQueueQty);
			// 再判斷是否已超過該站可移站數,若是,則亦表示OverComp_迄--表示工令可用移站數
			...
			if (Float.parseFloat(aMFGRCExpTransCode[i][2]) > runCardQtyf && remainQueueQty > Float.parseFloat(aMFGRCExpTransCode[i][2]) )//若移站數大於流程卡數,表示overcompletion
            {   // 條件是:本次移站數>流程卡數且工令可供移站數亦>本次移站數 
            ...
            } else  { 	// 否則,即以本次移站數-	工令可供移站數 作為超額移站數	  
...
//out.println("****Step2");
	    ...
	    //   seqstmt.setDate(7,txnDateTime);  // TRANSACTION_DATE 
//out.print("txnDateTime"+txnDateTime);
		   ...
		   int opSeqNo = 0;              
						String sqlGrp = " select GROUP_ID, TO_OPERATION_SEQ_NUM  from WIP.WIP_MOVE_TXN_INTERFACE "+
                                       //20091123 Liling Performance Issue : add organization_od
 									   //  " where WIP_ENTITY_NAME = '"+woNo+ "' and WIP_ENTITY_ID = "+entityId+"   "+  
 									     " where WIP_ENTITY_NAME = '"+woNo+ "' and WIP_ENTITY_ID = "+entityId+" AND ORGANIZATION_ID="+organizationID+"  "+  
										 "   and ATTRIBUTE2 = '"+aMFGRCExpTransCode[i][1]+"' "+ // 2006/11/17 By RunCardNo
										 "   and PROCESS_STATUS = 2 "+ // 2006/12/08 -- By Process Status=1
										 "   and TO_INTRAOPERATION_STEP_TYPE = 1 "; // 取此次移站的Group ID
                         //out.print("<br>sqlGrp="+sqlGrp);
						 Statement stateGrp=con.createStatement();
                         ResultSet rsGrp=stateGrp.executeQuery(sqlGrp);
						 if (rsGrp.next())
						 {
		...
		{   // 取成功移站後,前一站,本站,下一站相關資訊RUNCARD_ALL資料表更新
		    ...
		    String sqlp = " select OPERATION_SEQ_NUM, OPERATION_SEQUENCE_ID, STANDARD_OPERATION_ID,  "+
					      ...
					      "  from WIP_OPERATIONS "+
                         // 20091123 Liling Performance Issue : add organization_id
          ...
          { 
            String sqlOver=" select OVERCOMPLETION_PRIMARY_QTY from wip_move_transactions where TO_INTRAOPERATION_STEP_TYPE= 1 "+
			  			   "    AND ORGANIZATION_ID="+organizationID+" AND WIP_ENTITY_ID="+entityId+" and ATTRIBUTE2= '"+aMFGRCExpTransCode[i][1]+"' and FM_OPERATION_SEQ_NUM = "+aMFGRCExpTransCode[i][4]+" ";
...
//out.println("Step3");
		   String SqlQueueTrans="insert into APPS.YEW_RUNCARD_TRANSACTIONS( "+
				           ...
				           "     values(?,YEW_RCARD_TRANSACTIONS_S.NEXTVAL,?,?,?,?,to_date('"+txnDate+"'||'"+sTime+"','yyyymmddhh24miss'),?,?,?,?,?,?,?,?,?,?,?,?,SYSDATE,?, '"+actionID+"', '"+actionName+"', '"+aMFGRCExpTransCode[i][11]+"', '"+aMFGRCExpTransCode[i][2]+"') ";  						
		 ...
		 {		 
		  ...
		  // %%%%%%%%%%%%%%%%%%% 寫入Run card Scrap Transaction %%%%%%%%%%%%%%%%%%%_迄 
           ...
           String rcSql=" update APPS.YEW_RUNCARD_ALL set LAST_UPDATED_BY=?, LAST_UPDATE_DATE=?, STATUSID=?, STATUS=?, "+
						...
						" QTY_IN_QUEUE=?, QTY_IN_INPUT=?, RC_DATE_CODE= '"+dateCodeSet+"', RES_EMPLOYEE_OP='' "+   //20090428 liling add RES_EMPLOYEE_OP清為空白,移站時才會抓系統日
...
//	if (!nextOpSeqNum.equals("0") && !interfaceErr) // 若下一站不等於最後一站且Interface無異常
	if ( !interfaceErr) // 若下一站不等於最後一站且Interface無異常	 //20150413 liling for 部份後段只剩二站
	{
	 %>
	 ...
	 <%	
	          ...
	          %>
	           ...
	           //   alert("Oracle移站異常..請洽MIS查明原因!!!");
	              alert(interfaceErr+" Oracle移站異常..請洽MIS查明原因!!!");  //20150413 liling show message				  
	             </script>
	          <%	
	       }
		   ...
		   //抓取移站數量的單位_起	                     
						 String sqlUOM = " select DISTINCT a.WO_UOM, b.WIP_ENTITY_ID, b.PRIMARY_ITEM_ID ,a.OE_ORDER_NO, b.QTY_IN_QUEUE, "+
						             //    "        a.ALTERNATE_ROUTING, a.SUPPLIER_LOT_NO, a.ORGANIZATION_ID "+ // 2007/01/18 若判斷為後段外購工令,則以廠商批號寫入MMT
						                 "        a.ALTERNATE_ROUTING, a.WAFER_LOT_NO, a.ORGANIZATION_ID "+ //20160201 liling 修正抓wafer lot 寫入mmt							 
						                 "  from APPS.YEW_WORKORDER_ALL a, APPS.YEW_RUNCARD_ALL b "+
 									     " where a.WO_NO = b.WO_NO and a.WO_NO = '"+woNo+ "'";
						 ...
						 if (rsUOM.next())
						 {
						  ...
						  // opSupplierLot = rsUOM.getString("SUPPLIER_LOT_NO");
						   opSupplierLot = rsUOM.getString("WAFER_LOT_NO");  //20160201 liling
	    ...
	    { //out.println("aMFGRCExpTransCode[i][0]="+aMFGRCExpTransCode[i][0]);	   
		  ...
		  if (Float.parseFloat(aMFGRCExpTransCode[i][2])>0) // 若設定移站數量大於0才進行移站及報廢
		  {
		   ...
		   java.text.DecimalFormat nf = new java.text.DecimalFormat("###,##0.000"); // 取小數後三位 
					     ...
					     String sqlMQty = " select b.QTY_IN_QUEUE, b.RUNCARD_QTY,RES_EMPLOYEE_OP,"+  // 展開即完工入庫,故不關連WIP_MOVE_TRANSACTIONS  //20151224 RES_EMPLOYEE_OP
						 ...
						 if (rsMQty.next())
						 {
						   ...
						   txnDate = rsMQty.getString("RES_EMPLOYEE_OP");		// 20151124 移站日期
		 ...
		 {		 
		   ...
		   // 2007/04/17 報廢時超額數量判段_By Kerwin_起
		             // 判斷是否已超過該站可移站數,若是,則亦表示OverComp_起--表示工令可用移站數
				      float remainQueueQty=0;
		         	  Statement stateRM=con.createStatement();
                      ResultSet rsRM=stateRM.executeQuery(" select QUANTITY_IN_QUEUE from WIP_OPERATIONS where WIP_ENTITY_ID = "+entityId+" and OPERATION_SEQ_NUM = "+aMFGRCExpTransCode[i][4]+" and ORGANIZATION_ID="+organizationID+" ");  //20091123 Liling add organization_id
			          if (rsRM.next()) { remainQueueQty = rsRM.getFloat("QUANTITY_IN_QUEUE"); }
			          rsRM.close();
	   		          stateRM.close();
			         // 判斷是否已超過該站可移站數,若是,則亦表示OverComp_迄--表示工令可用移站數
		...
		{  
		  ...
		  // out.print("<br>move qty="+aMFGRCExpTransCode[i][2]); 
			...
			if (Float.parseFloat(aMFGRCExpTransCode[i][2]) > runCardQtyf )   //若移站數大於流程卡數,表示overcompletion
            {
                ...
                //20160203 liling 若有超額完工先抓工單上QUEUE的數量來計算OVER的數量,避免有SCRAP QTY扣掉.
				      float wipQueueQty=0;
					  Statement stateRMQ=con.createStatement();
                      ResultSet rsRMQ=stateRMQ.executeQuery(" select QUANTITY_IN_QUEUE from WIP_OPERATIONS where WIP_ENTITY_ID = "+entityId+" and OPERATION_SEQ_NUM = "+aMFGRCExpTransCode[i][4]+" and organization_id="+organizationID+" ");  //20091123 liling organization_id
			          if (rsRMQ.next()) { wipQueueQty = rsRMQ.getFloat("QUANTITY_IN_QUEUE"); }
			          rsRMQ.close();
	   		          stateRMQ.close();					
				//20160203 liling			
               // overQty = Float.parseFloat(aMFGRCExpTransCode[i][2]) - runCardQtyf ;    //移站數-流程卡數=超出數量
                overQty = Float.parseFloat(aMFGRCExpTransCode[i][2]) - wipQueueQty ;    //移站數-流程卡數=超出數量		20160203 liling 改判斷Queue-移站數		
				// 取到小數後3位,四捨五入
				String strOverQty = nf.format(overQty);
				java.math.BigDecimal bd = new java.math.BigDecimal(strOverQty);
				java.math.BigDecimal overCompQty = bd.setScale(3, java.math.BigDecimal.ROUND_HALF_UP);
				...
				// out.print("<br>overQty="+overQty);
					  ...
					  float remainQueueQty=0;
					  Statement stateRM=con.createStatement();
                      ResultSet rsRM=stateRM.executeQuery(" select QUANTITY_IN_QUEUE from WIP_OPERATIONS where WIP_ENTITY_ID = "+entityId+" and OPERATION_SEQ_NUM = "+aMFGRCExpTransCode[i][4]+" and organization_id="+organizationID+" ");  //20091123 liling organization_id
			          if (rsRM.next()) { remainQueueQty = rsRM.getFloat("QUANTITY_IN_QUEUE"); }
			          rsRM.close();
	   		          stateRM.close();	
					  	//out.print("<br>remainQueueQty="+remainQueueQty);			  
          ...
          { Sqlrc=Sqlrc1+Sqlrcv5+Sqlrc3+Sqlrcv5; }
          // out.print("<br>Sqlrc="+Sqlrc);
           PreparedStatement seqstmt=con.prepareStatement(Sqlrc); 
           seqstmt.setString(1,fndUserName);    //out.println("fndUserName="+fndUserName); // CREATED_BY_NAME
          // seqstmt.setDate(2,processDateTime);  out.println("processDateTime="+processDateTime); // CREATION_DATE
	      // seqstmt.setDate(3,processDateTime);  out.println("processDateTime="+processDateTime); // LAST_UPDATE_DATE 
	       seqstmt.setString(2,fndUserName);    //out.println("fndUserName="+fndUserName);  // LAST_UPDATED_BY_NAME
   // out.println("<br>organCode="+organCode+"  id:"+Integer.parseInt(organizationID));		   
           seqstmt.setString(3,organCode);      //out.println("organCode="+organCode);   // ORGANIZATION_CODE
	       seqstmt.setInt(4,Integer.parseInt(organizationID)); //out.println("organizationID="+organizationID);   // ORGANIZATION_ID
	       seqstmt.setInt(5,1);      //PROCESS_PHASE   1=Move Validation, 2=Move Processing, 3= Operation Backflush Setup
		...
		{   // 取成功移站後,前一站,本站,下一站相關資訊RUNCARD_ALL資料表更新
		    ...
		    String sqlp = " select OPERATION_SEQ_NUM, OPERATION_SEQUENCE_ID, STANDARD_OPERATION_ID,  "+
        			      "       DESCRIPTION, PREVIOUS_OPERATION_SEQ_NUM, NEXT_OPERATION_SEQ_NUM   "+
					      "  from WIP_OPERATIONS "+
						  "  where PREVIOUS_OPERATION_SEQ_NUM is null and WIP_ENTITY_ID ="+entityId+" and organization_id="+organizationID+" ";	// 投產移站即完工(只有一站)  //20091123 liling organization_id
	        ...
	        if (rsp.next())
	        {
			    ...
			    if (nextOpSeqNum==null || nextOpSeqNum.equals(""))
				{
				   ...
				   //out.println("下一站代碼="+nextOpSeqNum);
				     ...
				     nextOpSeqNum	  = "0"; 
					 // 本站即為最後一站,故需更新狀態至 046 (完工入庫)	
					  //  out.print("<br>organizationID:"+organizationID+"  entityId:"+entityId);					 
					 Statement stateMax=con.createStatement();
                     ResultSet rsMax=stateMax.executeQuery("select NEXT_OPERATION_SEQ_NUM from WIP_OPERATIONS where OPERATION_SEQ_NUM = '"+aMFGRCExpTransCode[i][4]+"' and WIP_ENTITY_ID ="+entityId+" and organization_id="+organizationID+" ");
					 if (rsMax.next() && (rsMax.getString(1)==null || rsMax.getString(1).equals("")) )
					 {
          ...
          { 
            String sqlOver=" select OVERCOMPLETION_PRIMARY_QTY from wip_move_transactions where TO_INTRAOPERATION_STEP_TYPE=1 "+
                           "    and organization_id="+organizationID+" and wip_entity_id ="+entityId+"  "+   //20091123 liling performance isseue :add user index
		 ...
		 {		 
		   // %%%%%%%%%%%%%%%%%%% 寫入Run card Scrap Transaction %%%%%%%%%%%%%%%%%%%_起
		     String SqlScrapTrans="insert into APPS.YEW_RUNCARD_TRANSACTIONS( "+
						   ...
						   "            LASTUPDATE_DATE, ACTIONID, ACTIONNAME , QTY_AC_SCRAP, QTY_AC_TOMOVE ) "+  // 2007/04/03 增加寫入報廢數及良品數
				           "     values(?,YEW_RCARD_TRANSACTIONS_S.NEXTVAL,?,?,?,?,to_date('"+txnDate+"'||'"+sTime+"','yyyymmddhh24miss'),?,?,?,?,?,?,?,?,?,?,?,?,SYSDATE, '"+actionID+"', '"+actionName+"', '"+aMFGRCExpTransCode[i][11]+"', '"+aMFGRCExpTransCode[i][2]+"') ";  						
          // out.println("<br>SqlScrapTrans"+SqlScrapTrans+"<BR>");
		   PreparedStatement scrapTransStmt=con.prepareStatement(SqlScrapTrans); 
           scrapTransStmt.setInt(1,Integer.parseInt(aMFGRCExpTransCode[i][0])); // RUNCAD_ID          
	       scrapTransStmt.setString(2,aMFGRCExpTransCode[i][1]);                // RUNCARD_NO
           scrapTransStmt.setInt(3,Integer.parseInt(organizationID));           // ORGANIZATION_ID
	   ...
	   // #########################  流程卡完工後,呼叫寫入MMT及Material Transaction Lot Interface作庫存異動_起	
		 ...
		 { //out.println("Step1. 寫入MMT Interface<BR>");	
		  ...
		  } else if  (woType.equals("1"))	 //1:proging 工令
		         {
				   opSupplierLot = opSupplierLot;  //20160201 liling proging工令以wafer lot 寫入
				 }
		    else {
		           opSupplierLot = aMFGRCExpTransCode[i][1];	 // 一般自製工令,以流程卡作為批號
		         }  
		 ...
		 // out.println("Stepa.寫入MMT 之TRANSACTION_INTERFACE_ID="+groupId+"<BR>");
		  ...
		  String sqlInsMMT = "insert into MTL_TRANSACTIONS_INTERFACE( "+
							 ...
							 " ?,?,?,?,?,?,to_date('"+txnDate+"'||'"+sTime+"','yyyymmddhh24miss'), "+                                    // TRANSACTION_DATE,
		  ...
		  // out.println("<br>1Step supplierLotNo = "+supplierLotNo+"<BR>");	
		  if (opSupplierLot==null || opSupplierLot.equals("null") || opSupplierLot.equals("") || opSupplierLot=="N/A" || opSupplierLot.equals("N/A")) opSupplierLot = aMFGRCExpTransCode[i][1];   //20151224 liling wafer lot ='N/A'則帶入流程卡號
		 // out.println("<br>2Step supplierLotNo = "+supplierLotNo+"<BR>");	
		...
		//  out.println("Step2. 寫入MMT Lot Interface<BR>");
		    String sqlInsMMTLot = "  insert into MTL_TRANSACTION_LOTS_INTERFACE( "+
								  ...
								  "  LAST_UPDATE_DATE, LAST_UPDATED_BY, CREATION_DATE, CREATED_BY, ATTRIBUTE1, ATTRIBUTE2,LOT_EXPIRATION_DATE ) "+ 
								  "  VALUES (MTL_MATERIAL_TRANSACTIONS_S.CURRVAL,?,?,?,SYSDATE,?,SYSDATE,?,?,?,SYSDATE+365) ";   //LOT_EXPIRATION_DATE 以系統日+365
		...
		// out.println("Stepb.寫入MMT LOT 之TRANSACTION_INTERFACE_ID="+groupId+"<BR>");
		    out.println("<BR>入庫批號:"+opSupplierLot+"<BR>");  
...
// #########################  流程卡完工後,呼叫寫入MMT及Material Transaction Lot Interface作庫存異動_迄
    ...
    if ((woType=="3" || woType.equals("3") || oeOrderNo.length()>1) && (woPassFlag=="Y" || woPassFlag.equals("Y")))  //interface需成功寫入才寫Reservaton
    {
	 ...
	 { 			
		...
		if (rsRevType.next())
		 {   rsInterfaceId=rsRevType.getString(1);
             resvType = 2;  
		...
		rsRevType.close();
		stateRevType.close(); 		 
        out.print("<br>session1="+resvType);
		...
		// out.print("<br>SOURCE_TYPE_ID");	
			...
			rsInStmt.setInt(14,resvType);	       //RESERVATION_ACTION_CODE -- 1=Insert, 2=Update
	       ...
	       { 
              ...
              String rcSql=" update APPS.YEW_RUNCARD_ALL set LAST_UPDATED_BY=?, LAST_UPDATE_DATE=?, STATUSID=?, STATUS=?, "+
						  ...
						  // " PREVIOUS_OP_SEQ_NUM=?, STANDARD_OP_ID=?, STANDARD_OP_DESC=?, OPERATION_SEQ_ID=?, OPERATION_SEQ_NUM=?, NEXT_OP_SEQ_NUM=?, "+
						   " CLOSED_DATE=?, COMPLETION_QTY=?, QTY_IN_COMPLETE=?, QTY_IN_INPUT=? , RES_EMPLOYEE_OP='' "+   //20090428 liling add RES_EMPLOYEE_OP清為空白,移站時才會抓系統日"+
		                   " where WO_NO= '"+woNo+"' and RUNCAD_ID='"+aMFGRCExpTransCode[i][0]+"' "; 	
              PreparedStatement rcStmt=con.prepareStatement(rcSql);
	          //out.print("rcSql="+rcSql);           
	          rcStmt.setInt(1,Integer.parseInt(userMfgUserID));
			  ...
			  rcStmt.setFloat(9,Float.parseFloat(aMFGRCExpTransCode[i][2])+rcScrapQty);  //處理數		    
              rcStmt.executeUpdate();   
              rcStmt.close(); 
...
//=================  insert into WIP_JOB_SCHEDULE_INTERFACE  , begin =================
    try
    {
     //======取不足欄位
     //out.print("Step1");
     String sqla = " select DEPT_NO,JOB_TYPE,INVENTORY_ITEM_ID,INV_ITEM,ITEM_DESC,WO_QTY,SCHEDULE_STRART_DATE STARTDATE,WIP_ENTITY_ID, "+
	 			   "        SCHEDULE_END_DATE ENDDATE, CREATE_BY,ORGANIZATION_ID,COMPLETION_SUBINVENTORY, ROUTING_REFERENCE_ID "+
				   "   from YEW_WORKORDER_ALL	where WO_STATUS = 'R' and WO_NO = '"+woNo+"' ";
	// out.print("sqla="+sqla);
	...
	PreparedStatement instmt=null;
	 ...
	 if (newWoQty==0)
	 {
     	String sqlfa = "select sum(QUANTITY_ISSUED) qty_issued from WIP_REQUIREMENT_OPERATIONS where WIP_ENTITY_ID = "+entityId+" and organization_id="+organizationID+" ";	 			 
		...
		Statement statefa=con.createStatement();
		 ...
		 if (qtyIssued==null || qtyIssued.equals("") || qtyIssued=="0" || qtyIssued.equals("0"))
		  {
		    statusType=4 ;                       //compelete     //2007/04/05 liling update  7->4
  ...
  //=================  update runcard status  , begin ================= 		 
	...
	{ //out.print("statusType="+statusType);
		   String woSqla="update APPS.YEW_WORKORDER_ALL set WO_QTY=?, LAST_UPDATE_DATE=?, LAST_UPDATED_BY=? ";
             if( statusType==7 )
             { woSqla=woSqla+"  , WO_STATUS=?, STATUSID=?,  STATUS=? " ;} else{woSqla=woSqla+" ";} 
