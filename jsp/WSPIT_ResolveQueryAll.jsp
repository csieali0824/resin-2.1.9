...
{      
   ...
   if (document.MYFORM.ACTION.value=="008") //如果action是resolve,則必須檢查reason欄位是否有輸入資料
   {
      if (ch.length!=null)
      {
		   ...
		   {          
			   ...
			   {
					 if (document.MYFORM.elements["REASON-"+ch[i].value].value=="" || document.MYFORM.elements["REASON-"+ch[i].value].value==null)
					 { 
					   alert("Before you want to RESOLVE , please do not let the REASON be Null !!");   
					   return(false);
					 }	            
	      ...
	      if (ch.checked==true)  	      
			   {
					 if (document.MYFORM.elements["REASON-"+ch.value].value=="" || document.MYFORM.elements["REASON-"+ch.value].value==null)
					 { 
					   alert("Before you want to RESOLVE , please do not let the REASON be Null !!");   
					   return(false);
					 }	            
...
{   
  location.href="WSPIT_Resolve.jsp?TICKETNO="+docno;  
}
</script>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<title>PIT RESOLVE Form</title>
</head>
<body>
<FORM ACTION="WSPIT_ResolveQueryAll.jsp" METHOD="post" NAME="MYFORM">
...
<strong>產品問題追蹤-對策改善</strong></font>
&nbsp;&nbsp;&nbsp;&nbsp;
...
<table width="100%" border="0">
	...
	<table width="100%" border="1">
	  ...
	  try
      {     	         
	      String sql="SELECT w.ACTIONID,a.ACTIONNAME FROM WSWORKFLOW w,WSWFACTION a WHERE w.ACTIONID=a.ACTIONID and FORMID='PIT' AND TYPENO='001' AND FROMSTATUSID='102'";		  	  
          rs=statement.executeQuery(sql);			  	 	    			
	      comboBoxBean.setRs(rs);	  
	...
	<TR bgcolor="#CCFFFF"><TH width="2%"><input name="checkselect" type=checkbox onClick="this.value=check(this.form.CH)" title="選取或取消選取"></TH><TH width="2%">&nbsp;&nbsp;</TH>
	  ...
	  <TH width="23%"><font size="2">Reason of Fault </font></TH>
	  <TH width="23%"><font size="2">Correct Action </font></TH>
	  </TR>
<% 		 		 		
...
{   
	...
	String sqlWhere=" where p.SFUNCTION=f.CODE and p.T_VERSION=v.T_VERSION and STATUSID='102'"; //把狀態為SUBMITTED的資料取出
...
%>		
	<TR><TD><INPUT TYPE=checkbox NAME='CH' VALUE="<%=ticketNo%>"></TD><TD><A href='javascript:openForm("<%=ticketNo%>")'><IMG border="0" src="../image/docicon.gif"></A></TD>
	  ...
	  <TD><font size="2">
	    <textarea name="REASON-<%=ticketNo%>"  cols="20" rows="3" ></textarea>
	  </font></TD>
	  <TD><font size="2"><textarea name="CORRECTACTION-<%=ticketNo%>" cols="20" rows="3" ></textarea></font></TD>	  
	  </TR>
<%	 
...
%>  
 ...
 <!-- 表單參數 -->  
	...
	<input name="FROMSTATUS" type="HIDDEN" value="102">		
