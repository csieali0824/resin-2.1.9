...
function setsubmit(URL)
{
	...
	if (document.SUBFORM.COMMENT.value==null || document.SUBFORM.COMMENT.value=="")
	{
		document.SUBFORM.COMMENT.style.borderColor="#FF6600";
		document.SUBFORM.COMMENT.style.borderStyle="solid";
		document.SUBFORM.COMMENT.style.borderWidth="thin";
		return false;
	}
...
String SEQNO = "QPCN"+(formkey.substring(formkey.indexOf("#")+1,formkey.lastIndexOf("+"))).replace("sid","");
String QNO="";
String CUSTNO="";
...
try
{
	if (action.equals("s"))
	{
		...
		if (rs.next())
		{
			...
			pstmtDt.setString(1,COMMENT);
			pstmtDt.setString(2,REPLIER);
			pstmtDt.setString(3,SEQNO);
			pstmtDt.setString(4,"SALES");
			pstmtDt.setString(5,EMAILLIST);
		...
		else
		{
			...
			//out.println(sql);
			PreparedStatement pstmtDt=con.prepareStatement(sql);  
			pstmtDt.setString(1,"SALES");
			pstmtDt.setString(2,COMMENT);
			pstmtDt.setString(3,"");
		...
		pstmtDt1.setString(1,"SALES");
		pstmtDt1.setString(2,COMMENT);
		...
		subject = SEQNO + " Notice - Sales : "+REPLIER+" has something requirement"+remarks1;
		detail= "Dear All:<p>Please be noted that "+SEQNO+" has required by "+REPLIER+" if you would like to know more details please click line route to "+url;
...
%>
<body bgcolor="#DCF1E0">
<form name="SUBFORM"  METHOD="post" ACTION="TSCQRAProductNoticeSalesReply.jsp">
		...
		<tr>
			<td>
				<table width="100%" border="0" cellpadding="0" cellspacing="1"n bgcolor="#FFFFFF">
					...
					<tr>
						<td><font class="style3"><br>Comment:</font><br>
						<textarea cols="70" rows="12" name="COMMENT" class="style2"><%=COMMENT%></textarea>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td><br><br><input type="button" name="save" class="style2" value="Submit" onClick="setsubmit('../jsp/TSCQRAProductNoticeSalesReply.jsp?formkey=<%=java.net.URLEncoder.encode(formkey)%>&action=s');"></td>
					</tr>
				</table>
			</td>
		</tr>	
...
%>
	<Script language="JavaScript">
		var SALES_NOTICE = "";
		if (document.SUBFORM.COMMENT.value.length>0) SALES_NOTICE += document.SUBFORM.COMMENT.value;
		SALES_NOTICE += ("<br><font style='font-weight:bold'>Replied on </font><font style='color:blue;font-weight:bold'>"+document.SUBFORM.REPLIYDATE.value+"</font><font style='font-weight:bold'> by "+document.SUBFORM.REPLIER.value+"</font>");
		window.opener.document.getElementById("DIV_SALES_"+document.SUBFORM.NO.value).innerHTML = SALES_NOTICE;
		this.window.close(); 
	</Script>
<%
}
