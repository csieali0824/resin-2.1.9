...
<%@ page import="java.sql.*,bean.DateBean,bean.QryAllChkBoxEditBean" %>
...
<html>
<head>
<title><jsp:getProperty name="rPH" property="pgRoot"/><jsp:getProperty name="rPH" property="pgList"/></title>
</head>

<body>
<% String searchString=request.getParameter("SEARCHSTRING"); %>
<form name="MYFORM" action="MMRootDel.jsp" method="post" onSubmit='return NeedConfirm("<jsp:getProperty name='rPH' property='pgDelete'/>")'>
<strong><font color="#0080C0" size="5"><jsp:getProperty name="rPH" property="pgRoot"/><jsp:getProperty name="rPH" property="pgList"/></font></strong>
<br><br>
...
<font face="Arial" size="3"><jsp:getProperty name="rPH" property="pgRoot"/></font><input name="SEARCHSTRING" type="text" value="<%if (searchString!=null) out.println(searchString);%>">
<input name="search" type="button" onClick='setSubmit("MMModuleList.jsp")' value='<jsp:getProperty name="rPH" property="pgSearch"/>'>
<%
try {	
	String sSql = "SELECT RROOT,RDESC,RSEQ FROM ORADDMAN.MENUROOT WHERE RROOT IS NOT NULL ";
	if (searchString!=null) sSql = sSql + " AND (UPPER(RROOT) LIKE UPPER('%"+searchString+"%') OR UPPER(RDESC) Like UPPER('%"+searchString+"%') )";
	sSql = sSql + " ORDER BY RSEQ,RROOT";
	Statement st=con.createStatement(); 
	ResultSet rs=st.executeQuery(sSql);
	String a[] = {"RROOT","RDESC","RSEQ"};
	qryAllChkBoxEditBean.setPageURL("../jsp/MMRootEdit.jsp");
