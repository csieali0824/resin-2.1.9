	 ...
	 try 
	      ...
	      while(rs.next())
	    {
		   ...
		   sgendatebeg=sgendate.substring(0,6)+"01";  	   
            ...
            int diff_m = 0;
            int  iweek = 0;
			int  currentday = dateBean.getDay();
			int dd=0; 
			int ddd=0; 
			...
			//out.println(nMinDay+"111"); 
			diff_y = nYear - yy;  // 年度與目前差異數			
			diff_m =nMonth - mm;  //月份與目前月份差異數       
			//out.println(diff_m+"222"); 
			if(diff_y==0)
			{    
                  dateBean.setAdjMonth(diff_m);
				 // out.println(dateBean.setAdjMonth(diff_m)); 
		          dd=dateBean.getMaxDay();
				  //out.println(dd+"333"); 
		          dateBean.setAdjMonth(0-diff_m);
				  //out.println(dateBean.setAdjMonth(0-diff_m) ); 
			 }
			 else 
			 {   dateBean.setAdjYear(diff_y);
			      //out.println(dateBean.setAdjYear(diff_y)); 
                  dateBean.setAdjMonth(diff_m);
				 // out.println(dateBean.setAdjMonth(diff_m)); 
		          dd=dateBean.getMaxDay();
				  //out.println(dd+"444"); 
			...
			if(nMinDay<=0)
              { 			      
			    dateBean.setAdjMonth(diff_m-1);			
		        ddd=dateBean.getMaxDay();
				//out.println(ddd+"555"); 
		        dateBean.setAdjMonth(0-(diff_m-1));
			  }			
