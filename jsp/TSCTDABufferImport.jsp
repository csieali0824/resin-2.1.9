...
String SHIPTOID = request.getParameter("SHIPTOID"); //add by Peggy 20210302
...
int icnt=0,ErrCnt=0,sRow=2,rec_cnt=0,insert_cnt=0,v_found=0;
String seqno="",seqkey="";
...
String strLineType="",strItemName ="",strOrderType="",strOrderTypeID="",strCustName="",strCustPO="",strRFQType="",strItemDesc="",strLineOrderType="",strLineCustPOLine="";
...
String strEndCustID ="",strEndCustID1 ="",strShippingMethodName="",strShipTO="",SHIPTO="",v_def_shipto="",v_def_fob="",fobPoint="",strShiptolocationid=""; //add by Peggy 20140825
...
if (InsertFlag.equals("Y"))
{
	try
	{
		sql = " SELECT a.customer_no,a.customer_id,"+
			  ...
			  " (select count(1) from oraddman.tsc_rfq_upload_temp c where  c.create_flag='N' and c.salesareano=a.salesareano and c.customer_no= a.customer_no and c.customer_po=a.customer_po and c.upload_by=a.upload_by and nvl(c.ship_to_org_id,0)=nvl(a.ship_to_org_id,0)) rowcnt"+
			  ",e.customer_number END_CUSTOMER_ID,a.END_CUSTOMER"+  //add by Peggy 20140825
			  ",nvl(a.ship_to_org_id,0) ship_to_org_id,po_line_no"+ //add by Peggy 20190815
			  ...
			  " and a.customer_po = ?"+
			  " and a.upload_by =?"+
			  " and nvl(a.ship_to_org_id,0) =?"+ //add by Peggy 20210302
		...
		while(rs.next())
		{
			...
			if (icnt ==0)
			{
				...
				SHIPTO = rs.getString("ship_to_org_id");  //add by Peggy 20190815
				fobPoint =  rs.getString("fob");          //add by Peggy 20220128
			}
			...
			aa[icnt][19]=(rs.getString("po_line_no")==null?"":rs.getString("po_line_no"));
			aa[icnt][20]="&nbsp;"; 
			...
			aa[icnt][29]="&nbsp;";  //add by Peggy 20190225
			icnt++; 
		}
		rs.close();
		st.close();
		...
		session.setAttribute("SHIPTO",SHIPTO); //add by Peggy 20190815
		session.setAttribute("FOBPOINT",fobPoint); //add by Peggy 20220128
		session.setAttribute("PROGRAMNAME","D4-015");
		urlDir = "TSSalesDRQ_Create.jsp?CUSTOMERID="+java.net.URLEncoder.encode(CustomerID)+
				 ...
				 "&FOBPOINT="+java.net.URLEncoder.encode(fobPoint)+  //add by Peggy 20220128
				 "&PROGRAMNAME=D4-015";
...
if (DelFlag.equals("Y"))
{
	String sqlx="update oraddman.tsc_rfq_upload_temp SET CREATE_FLAG='D',LAST_UPDATED_BY=?,LAST_UPDATE_DATE=sysdate WHERE CREATE_FLAG='N' AND CUSTOMER_ID='" + CustomerID +"' AND CUSTOMER_PO='"+ CustomerPO +"' AND UPLOAD_BY ='" + UPLOADBY+"' AND NVL(SHIP_TO_ORG_ID,0) ='" + SHIPTOID+"'";   
...
else if (DelAll.equals("Y"))
{
	...
	seqstmt.setString(4,SalesAreaNo); 	 
	seqstmt.executeUpdate(); 
	seqstmt.close(); 
}
%>
		...
		%>		
			<table align="center" width="100%" cellspacing="0" cellpadding="0" bordercolordark="#990000">
					...
					<TD width="90%">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td bgcolor="#CAE1CF" width="15%" align="center"><a href="../jsp/TSCTDABufferImport.jsp?ACTIONCODE=DETAIL" style="color:888888;font-size:13px;text-decoration:none;">Pending Detail</a></td>
					...
					<td colspan="2">
						<table  bordercolordark="#000033" cellspacing="0"  cellpadding="0" width="100%" align="left" bordercolorlight="ffffff" border="1">
			  				...
			  				<tr>
								...
								<td width="85%" ><A HREF="../jsp/samplefiles/D4-015_SampleFile.xls"><font style="font-size:13px;">Download Sample File</font></A></td>
			  				</tr>
			  				<tr>
								<td style="font-size:13px;background-color:<%=bkcolor%>">User Guide：</td>
								<td width="85%" ><A HREF="../jsp/samplefiles/D4-015_FuncSpc.doc"><font style="font-size:13px;">Download User Guide</font></A></td>
			  				</tr>
						</table>
					</td>
				</tr>
				...
				<tr>
					...
					<input type="button" style="font-size:13px;font-family: Tahoma,Georgia;" name="submit1" value='Upload' onClick="setCreate('../jsp/TSCTDABufferImport.jsp?ACTIONCODE=UPLOAD')">
					</td>
				</tr>
			</table>
		<%
			...
			<table align="center" width="100%" cellspacing="0" cellpadding="0" bordercolordark="#990000">
				<tr>
					<TD width="90%">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td bgcolor="<%=bkcolor%>" width="15%" height="20" align="center" style="border-color:#CAE1CF;border:insert;font-size:13px"><a href="../jsp/TSCTDABufferImport.jsp?ACTIONCODE=DETAIL" style="font-size:13px;text-decoration:none;">Pending Detail</a></td>
								<td bgcolor="#CAE1CF" width="15%" align="center"><a href="../jsp/TSCTDABufferImport.jsp" style="color:888888;font-size:13px;text-decoration:none;">Excel Upload</a></td>
								<td width="70%" style="border-color:ffffff;">&nbsp;</td>
							</tr>
						</table>
					</TD>
...
if (ACTIONCODE.equals("UPLOAD"))
{
	try
	{
		...
		else
		{
			...
			sql = " select a.FOB_CODE, a.FOB "+
		          " from OE_FOBS_ACTIVE_V a order by a.fob_code";
			Statement statementhx=con.createStatement( ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
			ResultSet rshx = statementhx.executeQuery(sql);					 
					
			String uploadFilePath="\\resin-2.1.9\\webapps\\oradds\\jsp\\upload_exl\\D4015_"+strDateTime+"-"+uploadFile_name;
			...
			for (int i = sRow; i < sht.getRows(); i++) 
			{
				...
				strCustNo = strCustNo.trim();
				if (strCustNo==null) strCustNo="";
				if (strCustNo.equals("")) continue;
				...
				//CUSTOMER PO line num,add by Peggy 20210803
				jxl.Cell cellLineCustPOLine = sht.getCell(3, i);          
				strLineCustPOLine = (cellLineCustPOLine.getContents()).trim();
				strLineCustPOLine= strLineCustPOLine.replace("'","");
				if (strLineCustPOLine==null) strLineCustPOLine="";
				
				//22D/30D,add by Peggy 20210803
				jxl.Cell cellItemName = sht.getCell(4, i);          
				...
				try
				{
					jxl.Cell cellRequestDate = sht.getCell(9, i);
				...
				jxl.Cell cellShippingMethod = sht.getCell(10, i);          
				strShippingMethodName = (cellShippingMethod.getContents()).trim();
				if (strShippingMethodName==null) strShippingMethodName="";
				
				//REMARK
				jxl.Cell cellRemark = sht.getCell(11, i);          
				strLineRemark = (cellRemark.getContents()).trim();
				
				//END CUSTOMER ID,add by Peggy 20140825
				jxl.Cell cellEndCustID = sht.getCell(12, i);          
				strEndCustID = (cellEndCustID.getContents()).trim();
				if (strEndCustID==null) strEndCustID="";		
							
				//FOB,add by Peggy 20210302
				jxl.Cell cellFOB = sht.getCell(13, i);          
				strFOB= (cellFOB.getContents()).trim();
				if (strFOB==null) strFOB="";	
												
				//SHIP TO LOCATION ID,add by Peggy 20220128
				jxl.Cell cellShipto = sht.getCell(14, i);          
				strShiptolocationid= (cellShipto.getContents()).trim();
				if (strShiptolocationid==null) strShiptolocationid="";
																
				strErr ="";strCustID="";strCustName="";strItemID="";rec_cnt=0;strFactory="";strOrderType="";strOrderTypeID="";v_def_fob="";v_def_shipto="";//初始化
				...
				{ 
					...
					{						  
						...
						if (strItemName != null && !strItemName.equals(""))  //add by Peggy 20210803
						{
						...
						while(rs.next())
						{
							...
							//add by Peggy 20190425
							if (strLineOrderType==null || strLineOrderType.equals(""))
							{
								strLineOrderType=rs.getString("ORDER_TYPE");
								//if (strLineOrderType.equals("1141") && !strCurr.equals("USD"))
								//{
				...
				if (!strCustID.equals(""))
				{
					...
					if (!strShiptolocationid.equals("")) //add by Peggy 20220128
					{
						sql += " and a.LOCATION='"+strShiptolocationid+"'";
					}
					else if (strCustNo.equals("14413")) //DELTA,add by Peggy 20190815
					{
						//if (strLineCustPO.length()>3 && strLineCustPO.substring(0,4).equals("CCPL") && strLineOrderType.equals("1214"))
						if (((strLineCustPO.length()>=9 && strLineCustPO.substring(0,9).equals("CCPG01518")) || (strLineCustPO.length()>=10 && strLineCustPO.substring(0,10).equals("CCPF2300TB"))) && strLineOrderType.equals("1214"))
						{
							sql +=" and a.site_use_id = 50156";
						}
						//else if (strLineCustPO.length()>3 && strLineCustPO.substring(0,4).equals("CCPA") && strLineOrderType.equals("1214"))
						//else if (strLineCustPO.length()>8 && strLineCustPO.substring(0,9).equals("CCPG01511") && strLineOrderType.equals("1214"))
						else if (((strLineCustPO.length()>=9 && strLineCustPO.substring(0,9).equals("CCPG01511")) || (strLineCustPO.length()>=10 && (strLineCustPO.substring(0,10).equals("CCPM121002") || strLineCustPO.substring(0,10).equals("CCPF2301TB")))) && strLineOrderType.equals("1214"))  //add new po:CCPM121002 by Peggy 20210129
						{
							sql +=" and a.site_use_id = 61410";
						}						
						else if (strLineCustPO.substring(0,3).equals("CCP"))
						{
							sql +=" and a.site_use_id = 50156";
						}
						else if (strLineCustPO.substring(0,3).equals("TPH"))
						{
							//sql +=" and a.site_use_id = 55398";
							sql +=" and a.site_use_id =65370";  //modify by Peggy 20201104 from vivian
						}
						else if (strLineCustPO.substring(0,3).equals("FLC")) //add by Peggy 20230817
						{
					...
					else if (strCustNo.equals("1671")) //DELTA,add by Peggy 20200203
					{
						sql +=" and a.site_use_id = 4520";
					}	
					else if (strCustNo.equals("1067")) //DELTA,add by Peggy 20200203
					{
						sql +=" and a.site_use_id = 1328";
					}	
					else if (strCustNo.equals("24051")) //DELTA,add by Peggy 20210302
					{	
						if (strLineCustPO.substring(0,3).equals("CIF") && strOrderType.equals("1141"))
						{
					...
					while (rsb.next())
					{
						if (strShippingMethodName.equals("") && strShippingMethod.equals("") && rsb.getString("ship_via") != null)
						{
							if (rsh.isBeforeFirst() ==false) rsh.beforeFirst();
							while (rsh.next())
							{
								if (rsh.getString("LOOKUP_CODE").equals(rsb.getString("ship_via")))
								{
									strShippingMethod =rsb.getString("ship_via");
									strShippingMethodName = rsh.getString("MEANING");
									break;
								}
							}							
						}
						if (v_def_fob.equals("") && rsb.getString("FOB_POINT")!= null)
						{	
							v_def_fob  = rsb.getString("FOB_POINT");
						}
						strCurr=rsb.getString("CURRENCY_CODE");
						if (strCurr==null) strCurr="";
						...
						//add by Peggy 20210302
						if (rsb.getString("SITE_USE_CODE").equals("SHIP_TO"))
						{	
							v_def_shipto=rsb.getString("SITE_USE_ID");
						}
					}
					rsb.close();
					statementb.close();
				...
				//出貨方式
				if (strShippingMethodName == null || strShippingMethodName.equals(""))
				{
				...
				else
				{
					...
					while (rsh.next())
					{
						if (rsh.getString("MEANING").equals(strShippingMethodName))
						{
					...
					{
						strErr +=("運輸方式未定義("+(strShippingMethodName==null?"":strShippingMethodName)+")<br>");
					}	
				}			
				
				//FOB
				if (strFOB == null || strFOB.equals("")) strFOB=v_def_fob;//add by Peggy 20210302
				...
				{
					v_found=0;
					if (rshx.isBeforeFirst() ==false) rshx.beforeFirst();
					while (rshx.next())
					{
						if (rshx.getString("FOB").equals(strFOB))
						{
							v_found=1;
							break;
						}
					...
					if (v_found==0)
					{
						strErr +=("FOB錯誤("+(strFOB==null?"":strFOB)+")<br>");
					}				
				}
...
<%					
				...
				else
				{
					...
					if (strCustNo.equals("16869"))
					{
						for (int m =0 ; m < aa.length  ;m++ )
						{
							//if (aa[m][3]!=null && strLineCustPO.substring(0,3).equals(aa[m][3].substring(0,3)))
							if (aa[m][0] !=null && aa[m][0].equals("16869") && aa[m][3]!=null &&  aa[m][3].length()>=10 && (strLineCustPO.length()>=10 && strLineCustPO.substring(0,10).equals(aa[m][3].substring(0,10))))  //MODIFY BY PEGGY 20170726,PHIHONG改成前十碼
							{
								strCustPO=aa[m][3];
							}
						}
					...
					if (!strShiptolocationid.equals(""))
					{
						strShipTO=v_def_shipto; //add by Peggy 20210302
					}				
					else if (strCustNo.equals("14413")) //DELTA,add by Peggy 20190815
					{
						//if (strCustPO.length()>3 && strCustPO.substring(0,4).equals("CCPL") && strOrderTypeID.equals("1342"))
						if (((strCustPO.length()>=9 && strCustPO.substring(0,9).equals("CCPG01518")) || (strCustPO.length()>=10 && strCustPO.substring(0,10).equals("CCPF2300TB"))) && strOrderTypeID.equals("1342"))
						{
							strShipTO="50156";
						}
						//else if (strCustPO.length()>3 && strCustPO.substring(0,4).equals("CCPA") && strOrderTypeID.equals("1342"))
						else if (((strCustPO.length()>=9 && strCustPO.substring(0,9).equals("CCPG01511")) || (strCustPO.length()>=10 && (strCustPO.substring(0,10).equals("CCPM121002") || strCustPO.substring(0,10).equals("CCPF2300TB")))) && strOrderTypeID.equals("1342")) //add new po:CCPM121002 by Peggy 20210129
						{
							strShipTO="61410";
						}					
						else if (strCustPO.substring(0,3).equals("CCP"))
						{
							strShipTO="50156";
						}
						else if (strCustPO.substring(0,3).equals("TPH"))
						{
							//strShipTO="55398";
							strShipTO="65370";  //modify by Peggy 20201104 from vivian
						}
						else if (strLineCustPO.substring(0,3).equals("FLC")) //add by Peggy 20230817
						{
							sql +=" and a.site_use_id = 65370";
					...
					{
						if (strCustPO.substring(0,3).equals("CIF") && strOrderType.equals("1141"))
						{
							strShipTO="56522";
						}
						else
				...
				aa[icnt][23]=strShipTO; //add by Peggy 20190815		
				aa[icnt][24]=strLineCustPOLine; //add by Peggy 20210803	
...
<%				
		...
		else
		{
			for (int i =0 ; i < aa.length ; i++)
			{
				...
				pstmt.setString(25,aa[i][23]);           //SHIP TO ORG ID,add by Peggy 20190815
				pstmt.setString(26,aa[i][24]);           //PO_LINE_NO,add by Peggy 20190815
				pstmt.executeQuery();
				pstmt.close();
				insert_cnt++;
			}
	...
	%>
		<script language="javascript">
			location.href("TSCTDABufferImport.jsp?ACTIONCODE=DETAIL");
		</script>
...
else if (ACTIONCODE.equals("DETAIL"))
{
	...
	{
		int i =0;
		String customerpo="",customerno="",upload_by="",shipto="";
		sql = " SELECT a.salesareano, a.upload_date, a.upload_by, a.customer_no,a.customer_id,"+
			  ...
			  ",nvl(a.ship_to_org_id,0) ship_to_org_id"+
			  //",rownum"+
			  ",ROW_NUMBER() OVER (ORDER BY  a.customer_no, a.customer_po,a.upload_by,a.line_no,b.description) ROW_NUM"+ //modify by Peggy 20230130
			  " FROM oraddman.tsc_rfq_upload_temp a,inv.mtl_system_items_b b,ORADDMAN.TSAREA_ORDERCLS d"+
			  ",ar_customers e"+
			  " where a.create_flag=?"+
			  " and a.salesareano=?"+
...
%>
	<table align="center" border="1" width="90%" cellspacing="0" cellpadding="1" bordercolorlight="#DFD9DD" bordercolordark="#4A598A">
		<tr style="background-color:<%=bkcolor%>;font-size:10px" >
			<td width="2%"  align="center"><input type="button" name="btnall" value="Delete All" title="刪除全部資料" style="font-size:11px;font-family: Tahoma,Georgia;" onClick="delData('../jsp/TSCTDABufferImport.jsp?DELALL=Y')"></td>
			...
			<td width="8%"  align="center">End Customer</td>
			<td width="4%"  align="center">Ship to</td>
			<td width="8%" align="center">Upload By</td>
		</tr>
	<%
			}			
	%>
	...
	<%
			...
			if (!customerpo.equals(rs.getString("customer_po")) || !customerno.equals(rs.getString("customer_no")) || !upload_by.equals(rs.getString("upload_by")) || !shipto.equals(rs.getString("ship_to_org_id"))) //modif by Peggy 20210302
			{
				i++;
	%>
				<td rowspan="<%=rs.getString("rowcnt")%>" style="color:#000000;font-size:11px"><input type="button" name="btn<%=i%>" value="Delete" style="font-size:11px;font-family: Tahoma,Georgia;" title="刪除資料" onClick="delData('../jsp/TSCTDABufferImport.jsp?DELFLAG=Y&CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERPO=<%=rs.getString("customer_po")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>')"></td>
				<td id="tda<%=i%>" rowspan="<%=rs.getString("rowcnt")%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-size:11px" onClick="javascript:location.href='TSCTDABufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面"><%="("+rs.getString("customer_no")+")"+rs.getString("customer_name")%></td>
				<td id="tdc<%=i%>" rowspan="<%=rs.getString("rowcnt")%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-size:11px" onClick="javascript:location.href='TSCTDABufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面" align="center"><%=rs.getString("rfq_type")%></td>								
	<%
	...
	%>
				...
				<td id="tdh<%=i%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-size:11px" onClick="javascript:location.href='TSCTDABufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面" align="center"><%=rs.getString("request_date")%></td>								
				<td id="tdi<%=i%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-size:11px" onClick="javascript:location.href='TSCTDABufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面" align="center"><%=rs.getString("order_type")%></td>								
				<td id="tdj<%=i%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-size:11px" onClick="javascript:location.href='TSCTDABufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面" align="center"><%=(rs.getString("end_customer_id")==null?"&nbsp;":rs.getString("end_customer_id"))%></td>								
				<td id="tdk<%=i%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-size:11px" onClick="javascript:location.href='TSCTDABufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面" align="center"><%=(rs.getString("end_customer")==null?"&nbsp;":rs.getString("end_customer"))%></td>								
				<td id="tdm<%=i%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-size:11px" onClick="javascript:location.href='TSCTDABufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面" align="center"><%=(rs.getString("ship_to_org_id").equals("0")?"&nbsp;":rs.getString("ship_to_org_id"))%></td>								
				<td id="tdl<%=i%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-size:11px" onClick="javascript:location.href='TSCTDABufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&SHIPTOID=<%=rs.getString("ship_to_org_id")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面" align="center"><%=rs.getString("upload_by")%></td>								
			</tr>
	<%
		}
		st.close();
