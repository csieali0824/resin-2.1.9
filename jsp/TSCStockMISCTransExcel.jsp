...
<body>
<FORM ACTION="../jsp/TSCStockMISCTransExcel.jsp" METHOD="post" name="MYFORM">
<%
...
{ 	
	...
	/*//英文內文水平垂直置中     
	WritableCellFormat ACenter = new WritableCellFormat(new WritableFont(WritableFont.createFont("Arial"), fontsize1, WritableFont.NO_BOLD,false,UnderlineStyle.NO_UNDERLINE ,jxl.format.Colour.BLACK));   
	...
	//英文內文水平垂直靠左
	WritableCellFormat ALeft = new WritableCellFormat(new WritableFont(WritableFont.createFont("Arial"), fontsize1, WritableFont.NO_BOLD,false,UnderlineStyle.NO_UNDERLINE ,jxl.format.Colour.BLACK));   
	ALeft.setAlignment(jxl.format.Alignment.LEFT);
	ALeft.setVerticalAlignment(jxl.format.VerticalAlignment.CENTRE);
	ALeft.setWrap(true);

	//英文內文水平垂直靠右
	WritableCellFormat ARight = new WritableCellFormat(new WritableFont(WritableFont.createFont("Arial"), fontsize1, WritableFont.NO_BOLD,false,UnderlineStyle.NO_UNDERLINE ,jxl.format.Colour.BLACK));   
	...
	while (rsx.next())
	{
		...
		if (icnt==0)
		{
			...
			//logo
			jxl.write.WritableImage image = new jxl.write.WritableImage(col,row,col+2.5,row+2, new File("..//resin-2.1.9//webapps/oradds/jsp/images/logo.png")); 
			ws.addImage(image); 
	
			ws.mergeCells(col, row, col+8, row);     
			...
			ws.addCell(new jxl.write.Label(col+7, row,"日期:民國 "+rsx.getInt("iyear")+" 年 "+ rsx.getInt("imonth")+" 月 "+rsx.getInt("iday")+" 日" , ACenterB));
			row++;//列+1
			...
			ws.setColumnView(col,15);col++;	
			ws.addCell(new jxl.write.Label(col, row ,"轉出品名規格" , ACenterBL));
			...
			ws.setColumnView(col,12);col++;	
			ws.addCell(new jxl.write.Label(col, row ,"轉入品名規格" , ACenterBL));
			ws.setColumnView(col,25);col++;	
			...
			ws.addCell(new jxl.write.Label(col, row ,"移轉原因" , ACenterBL));
			ws.setColumnView(col,20);col++;	
		...
		col++;
    	ws.addCell(new jxl.write.Label(col, row , rsx.getString("NEW_ITEM_DESC") , ALeftL));
		col++;
	...
	while (rsx.next())
	{
		...
		{		
			ws.mergeCells(col+7, row, col+8, row);  
			ws.addCell(new jxl.write.Label(col+7, row,"申請人:"+rsx.getString("created_by")+"\n"+rsx.getString("creation_date"), ARightB)); 
		}	
	}
	rsx.close();
	statementx.close();
