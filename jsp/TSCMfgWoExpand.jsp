...
<!--20231116 liling 增加工令類型8 Forcast -->
<!--20240816 liling 增加package=ThinDPAK,D/C不帶G-->
...
<script language="JavaScript" type="text/JavaScript">
	...
	{ //alert();
//20231115
		if (document.DISPLAYREPAIR.WOTYPE.value ==3 || document.DISPLAYREPAIR.WOTYPE.value==5 || document.DISPLAYREPAIR.WOTYPE.value==7  || document.DISPLAYREPAIR.WOTYPE.value==8)
		{
			//檢查DC_YYWW
...
// 判斷是否展開有取得BOM ROUTING SEQUENCE ID_起
		...
		{
			//抓取流程卡前置碼作業
			if( woType=="2" || woType.equals("2") || woType=="3" || woType.equals("3") || woType=="5" || woType.equals("5") || woType=="8" || woType.equals("8") )   //只有前後段需要前置(會有一工令,對應多張流程卡)
			{
	 /*
	 ...
	 */
				String sqli = " select distinct RUNCARD_PREFIX, CUSTLOT_PREFIX from YEW_RUNCARD_PREFIX "+
						" where DEPT_NO= '"+deptNo+"'  and MARKET_TYPE = '"+marketType+"' "+
						//   "   and WO_TYPE = replace("+woType+",'5','3','8','3')  "+  //20151027 liling update sample 比照後段,20231116 add type8 比照後段
						"    and WO_TYPE = decode('"+woType+"','5','3','8','3','"+woType+"')    "+   //20231116 add type8 比照後段
						"   and PACKAGE_NO =  substr('"+invItem+"',1,4) ";	//20220430

				//out.print("sqli="+sqli+"<br>");
				Statement statei=con.createStatement();
				ResultSet rsi=statei.executeQuery(sqli);
				if (rsi.next())
				{
	...
	<%
		...
		//	if (woType.equals("3"))
		if (woType.equals("3") || woType.equals("5") ||  woType.equals("8"))	 //20151026 liling yew issue
		{
...
//  }//制一部  20120510 M1~M3 皆需要判斷 PACKAGE
		...
		try
		{
			...
			if( woType=="3" || woType.equals("3")  || woType=="5" || woType.equals("5") || woType=="7" || woType.equals("7")  || woType=="8" || woType.equals("8") )   //20130325 只有後段需要dc  //20130905 加入樣品工單入dc	 //20140827 加入實驗工單入dc
			{
				...
				if (dateCodeFlag=="N" || dateCodeFlag.equals("N"))    //沒有按下重算單批數量才抓單批數量
				{
					...
					String c_year="",c_week="",c_eng_year="",c_mon="",c_eng_mon="",a_eng_mon="" ,c_onsemi="";
					...
					String sql = "select tsc_get_calendar_week(sysdate,'YEAR') AS C_YEAR,tsc_get_calendar_week(sysdate,'WEEK') AS C_WEEK,CHR(tsc_get_calendar_week(sysdate,'YEAR')-2010+65) C_ENG_YEAR , "+
							"       LTRIM(TO_CHAR(SYSDATE,'MM'),'0') AS C_MON, SUBSTR(TO_CHAR(SYSDATE,'MON'),1,1) AS C_ENG_MON ,CHR(TO_CHAR(SYSDATE,'MM')+64) A_ENG_MON  ,tsc_get_calendar_week(sysdate,'ONSEMI') AS C_ONSEMI FROM DUAL"; //modify by Peggy 20160302	,20171020 liling add c_eng_mon		, 20251121 JB add C_ONSEMI
					...
					if (rs.next())
					{
						...
						c_onsemi = rs.getString("C_ONSEMI");	//20251121 JB add
					}
					rs.close();
					state.close();
				...
				*/
					...
					else if (!packageCode.startsWith("ABS") && !packageCode.equals("MBS")  && !packageCode.startsWith("SOD123")  && !packageCode.startsWith("SOD-123") && !packageCode.startsWith("SOD-128")  && !packageCode.startsWith("DO-218AB") && !packageCode.startsWith("Thin SMA")  && !tscPackage.startsWith("ThinDPAK") && tscPacking.length() ==3 && tscPacking.substring(2,3).equals("G"))
					{
						dateCode  ="G"+dateCode;
						dc_yyww = c_year.substring(2,4)+("0"+c_week).substring(("0"+c_week).length()-2);  //add by Peggy 20220704
					}
	...
	<table cellSpacing="1" bordercolordark="#B5B89A" cellPadding="1" width="97%" align="center" bordercolorlight="#FFFFFF" border="0">
		<tr bgcolor="#CCCC99">
			...
			<%
				...
				if(woType=="3" || woType.equals("3") || woType=="5" || woType.equals("5") || woType=="7" || woType.equals("7")  || woType=="8" || woType.equals("8") )  //20140827 ADD TYPE=7 for ENG type dc
				{
			%>
	...
	{
		if(woType=="3" || woType.equals("3") || woType=="5" || woType.equals("5")  || woType=="8" || woType.equals("8"))	//20231115
		{
	%>
	<table cellSpacing="1" bordercolordark="#B5B89A" cellPadding="1" width="97%" align="center" bordercolorlight="#FFFFFF" border="0">
		<tr bgcolor="#CCCC99">
