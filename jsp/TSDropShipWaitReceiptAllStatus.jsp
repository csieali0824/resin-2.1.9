...
<title>Query All status-Drop Ship MO Non-Create PO</title>
...
{   
  location.href="../jsp/TSDropShipWaitReceiptAllStatus.jsp?STATUSID="+statusID+"&PAGEURL="+pageURL+"&SEARCHSTRING="+document.MYFORM.SEARCHSTRING.value+"&FROMYEAR="+document.MYFORM.FROMYEAR.value+"&FROMMONTH="+document.MYFORM.FROMMONTH.value+"&FROMDAY="+document.MYFORM.FROMDAY.value+"&TOYEAR="+document.MYFORM.TOYEAR.value+"&TOMONTH="+document.MYFORM.TOMONTH.value+"&TODAY="+document.MYFORM.TODAY.value ;
}
 ...
 try
  {   
   ...
   {	 
	 ...
	 {	  	    
		lotRs=lotStat.executeQuery(" select count(oola.LINE_ID) from OE_ORDER_HEADERS_ALL ooha, OE_ORDER_LINES_ALL oola, OE_DROP_SHIP_SOURCES odss, MTL_SYSTEM_ITEMS MSI, ORADDMAN.TSWFSTATUS TWF "+
		                           " where ooha.HEADER_ID = oola.HEADER_ID and ooha.HEADER_ID = odss.HEADER_ID "+
								   "   and oola.LINE_ID = odss.LINE_ID and odss.REQUISITION_HEADER_ID is not null "+
								   "   and odss.PO_HEADER_ID is null and oola.FLOW_STATUS_CODE ='AWAITING_RECEIPT' "+
								   "   and MSI.ORGANIZATION_ID = oola.SHIP_FROM_ORG_ID "+
								   "   and MSI.INVENTORY_ITEM_ID = oola.INVENTORY_ITEM_ID "+
								   "   and oola.FLOW_STATUS_CODE = TWF.STATUSNAME  "+
								   "   and to_char(ooha.ORDER_NUMBER)='"+searchString+"'");	
		lotRs.next();
	    if (lotRs.getInt(1)>0) // 若有存在批號的話
	    {		   
	       rs=statement.executeQuery(" select count(oola.LINE_ID) from OE_ORDER_HEADERS_ALL ooha, OE_ORDER_LINES_ALL oola, OE_DROP_SHIP_SOURCES odss, MTL_SYSTEM_ITEMS MSI, ORADDMAN.TSWFSTATUS TWF "+
		                             "  where ooha.HEADER_ID = oola.HEADER_ID and ooha.HEADER_ID = odss.HEADER_ID "+
								     "    and oola.LINE_ID = odss.LINE_ID and odss.REQUISITION_HEADER_ID is not null "+
								     "    and odss.PO_HEADER_ID is null and oola.FLOW_STATUS_CODE ='AWAITING_RECEIPT' "+
								     "    and MSI.ORGANIZATION_ID = oola.SHIP_FROM_ORG_ID "+
								     "    and MSI.INVENTORY_ITEM_ID = oola.INVENTORY_ITEM_ID "+
								     "    and oola.FLOW_STATUS_CODE = TWF.STATUSNAME  "+
		                             "    and trim(TWF.STATUSID)='"+statusID+"' "+
		                             "    and to_char(ooha.ORDER_NUMBER)='"+searchString+"' "+
		                             "    and substr(to_char(ooha.CREATION_DATE,'YYYYMMDDHH24MISS'),0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"'");	
	    } else {	  
           rs=statement.executeQuery("  select count(oola.LINE_ID) from OE_ORDER_HEADERS_ALL ooha, OE_ORDER_LINES_ALL oola, OE_DROP_SHIP_SOURCES odss, MTL_SYSTEM_ITEMS MSI, ORADDMAN.TSWFSTATUS TWF "+
		                             "  where ooha.HEADER_ID = oola.HEADER_ID and ooha.HEADER_ID = odss.HEADER_ID "+
								     "    and oola.LINE_ID = odss.LINE_ID and odss.REQUISITION_HEADER_ID is not null "+
								     ...
								     "    and MSI.INVENTORY_ITEM_ID = oola.INVENTORY_ITEM_ID "+
								     "   and oola.FLOW_STATUS_CODE = TWF.STATUSNAME  "+
		                             "    and trim(TWF.STATUSID)='"+statusID+"' and (to_char(ooha.ORDER_NUMBER) like '"+searchString+"%' ) and substr(to_char(ooha.CREATION_DATE,'YYYYMMDDHH24MISS'),0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"'");	 	 
        } //end of lotRs if
	          ...
	          rs=statement.executeQuery(" select count(oola.LINE_ID) from OE_ORDER_HEADERS_ALL ooha, OE_ORDER_LINES_ALL oola, OE_DROP_SHIP_SOURCES odss, MTL_SYSTEM_ITEMS MSI, ORADDMAN.TSWFSTATUS TWF "+
								        ...
								        "    and MSI.INVENTORY_ITEM_ID = oola.INVENTORY_ITEM_ID "+
								        "    and oola.FLOW_STATUS_CODE = TWF.STATUSNAME  "+
			                            "    and trim(TWF.STATUSID)='"+statusID+"' and substr(to_char(ooha.CREATION_DATE,'YYYYMMDDHH24MISS'),0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"'");	
	        }	 
   } 		   
...
<FORM NAME="MYFORM" onsubmit='return submitCheck("確認取消","確認特採項目")' ACTION="../jsp/TSDropShipWaitReceiptMBatchProcess.jsp?FORMID=DS&FROMSTATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>" METHOD="POST"> 
<strong><font color="#0080C0" size="5">三角銷售訂單未轉採購單處理</font></strong><FONT COLOR=RED SIZE=4>&nbsp;&nbsp;單據狀態:<%=statusName%>(<%=statusDesc%>)</FONT><FONT COLOR=BLACK SIZE=2>(總共<%=maxrow%>&nbsp;筆記錄)</FONT>
...
<A HREF="../jsp/TSDropShipQueryAllStatus.jsp?STATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=FIRST&FROMYEAR=<%=fromYear%>&FROMMONTH=<%=fromMonth%>&FROMDAY=<%=fromDay%>&TOYEAR=<%=toYear%>&TOMONTH=<%=toMonth%>&TODAY=<%=toDay%>"><font size="2"><strong><font color="#FF0080">第一頁</font></strong></font></A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSDropShipWaitReceiptAllStatus.jsp?STATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=LAST&FROMYEAR=<%=fromYear%>&FROMMONTH=<%=fromMonth%>&FROMDAY=<%=fromDay%>&TOYEAR=<%=toYear%>&TOMONTH=<%=toMonth%>&TODAY=<%=toDay%>"><font size="2"><strong><font color="#FF0080">最終頁</font></strong></font></A><font color="#FF0080"><strong><font size="2">&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSDropShipWaitReceiptAllStatus.jsp?STATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=300&FROMYEAR=<%=fromYear%>&FROMMONTH=<%=fromMonth%>&FROMDAY=<%=fromDay%>&TOYEAR=<%=toYear%>&TOMONTH=<%=toMonth%>&TODAY=<%=toDay%>">下一頁</A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSDropShipWaitReceiptAllStatus.jsp?STATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=-300&FROMYEAR=<%=fromYear%>&FROMMONTH=<%=fromMonth%>&FROMDAY=<%=fromDay%>&TOYEAR=<%=toYear%>&TOMONTH=<%=toMonth%>&TODAY=<%=toDay%>">前一頁</A>&nbsp;&nbsp;(第<%=currentPageNumber%>&nbsp;頁/共<%=totalPageNumber%>&nbsp;頁)</font></strong></font>
&nbsp;&nbsp;&nbsp;&nbsp;發票日期
:FROM
<%
     try
  ...
  {   
   ...
   {      	 	 
       if (searchString!=null && !searchString.equals("")) //如果有搜尋特定單號則另下SQL
	   {
		  ...
		  lotRs=lotStat.executeQuery(" select count(*) from OE_ORDER_HEADERS_ALL ooha, OE_ORDER_LINES_ALL oola, OE_DROP_SHIP_SOURCES odss, MTL_SYSTEM_ITEMS MSI, ORADDMAN.TSWFSTATUS TWF "+
		                             " where ooha.HEADER_ID = oola.HEADER_ID and ooha.HEADER_ID = odss.HEADER_ID "+
									 "   and oola.LINE_ID = odss.LINE_ID and odss.REQUISITION_HEADER_ID is not null "+
									 "   and odss.PO_HEADER_ID is null and oola.FLOW_STATUS_CODE ='AWAITING_RECEIPT' "+
									 "   and MSI.ORGANIZATION_ID = oola.SHIP_FROM_ORG_ID "+
									 "   and MSI.INVENTORY_ITEM_ID = oola.INVENTORY_ITEM_ID "+								 
									 "   and oola.FLOW_STATUS_CODE = TWF.STATUSNAME "+
									 "   and trim(TWF.STATUSID)='"+statusID+"' and to_char(ooha.ORDER_NUMBER) ='"+searchString+"'");	
 		  lotRs.next();
	      if (lotRs.getInt(1)>0) //若有存在批號的話
	      { 
		    rs=statement.executeQuery("select oola.LINE_ID, odss.REQUISITION_HEADER_ID, odss.REQUISITION_LINE_ID, ooha.ORDER_NUMBER as 訂單號, ooha.HEADER_ID as 訂單識別碼, oola.LINE_ID as 項次識別碼, oola.ORDERED_ITEM as 訂單料號, oola.INVENTORY_ITEM_ID as 料號識別碼, MSI.SEGMENT1 as 台半料號, MSI.DESCRIPTION as 台半品名, oola.ORDERED_QUANTITY as 訂單數量, oola.ORDER_QUANTITY_UOM as 訂單單位, oola.SCHEDULE_SHIP_DATE as 預計出貨日, oola.SHIPPING_METHOD_CODE as 出貨方式, oola.UNIT_SELLING_PRICE as 售價, oola.UNIT_LIST_PRICE as 單價 "+
			                          " from OE_ORDER_HEADERS_ALL ooha, OE_ORDER_LINES_ALL oola, OE_DROP_SHIP_SOURCES odss, MTL_SYSTEM_ITEMS MSI, ORADDMAN.TSWFSTATUS TWF "+
			                          " where ooha.HEADER_ID = oola.HEADER_ID and ooha.HEADER_ID = odss.HEADER_ID "+
									  "   and oola.LINE_ID = odss.LINE_ID and odss.REQUISITION_HEADER_ID is not null "+
									  "   and odss.PO_HEADER_ID is null and oola.FLOW_STATUS_CODE ='AWAITING_RECEIPT' "+
									  "   and MSI.ORGANIZATION_ID = oola.SHIP_FROM_ORG_ID "+
									  "   and MSI.INVENTORY_ITEM_ID = oola.INVENTORY_ITEM_ID "+
									  "   and oola.FLOW_STATUS_CODE = TWF.STATUSNAME  "+
									  "   and ooha.HEADER_ID = oola.HEADER_ID and trim(TWF.STATUSID)='"+statusID+"' and to_char(ooha.ORDER_NUMBER) ='"+searchString+"' and substr(to_char(ooha.CREATION_DATE,'YYYYMMDDHH24MISS'),0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' order by ooha.ORDER_NUMBER, OOLA.SCHEDULE_SHIP_DATE ASC"); 
		  } else {			    
              rs=statement.executeQuery(" select oola.LINE_ID, odss.REQUISITION_HEADER_ID, odss.REQUISITION_LINE_ID, ooha.ORDER_NUMBER as 訂單號, ooha.HEADER_ID as 訂單識別碼, oola.LINE_ID as 項次識別碼, oola.ORDERED_ITEM as 訂單料號, oola.INVENTORY_ITEM_ID as 料號識別碼, MSI.SEGMENT1 as 台半料號, MSI.DESCRIPTION as 台半品名, oola.ORDERED_QUANTITY as 訂單數量, oola.ORDER_QUANTITY_UOM as 訂單單位, oola.SCHEDULE_SHIP_DATE as 預計出貨日, oola.SHIPPING_METHOD_CODE as 出貨方式, oola.UNIT_SELLING_PRICE as 售價, oola.UNIT_LIST_PRICE as 單價 "+
			                            "   from OE_ORDER_HEADERS_ALL ooha, OE_ORDER_LINES_ALL oola, OE_DROP_SHIP_SOURCES odss, MTL_SYSTEM_ITEMS MSI, ORADDMAN.TSWFSTATUS TWF "+
			                            "  where ooha.HEADER_ID = oola.HEADER_ID and ooha.HEADER_ID = odss.HEADER_ID "+
	           ...
	           rs=statement.executeQuery("select oola.LINE_ID, odss.REQUISITION_HEADER_ID, odss.REQUISITION_LINE_ID, ooha.ORDER_NUMBER as 訂單號, ooha.HEADER_ID as 訂單識別碼, oola.LINE_ID as 項次識別碼, oola.ORDERED_ITEM as 訂單料號, oola.INVENTORY_ITEM_ID as 料號識別碼, MSI.SEGMENT1 as 台半料號, MSI.DESCRIPTION as 台半品名, oola.ORDERED_QUANTITY as 訂單數量, oola.ORDER_QUANTITY_UOM as 訂單單位, oola.SCHEDULE_SHIP_DATE as 預計出貨日, oola.SHIPPING_METHOD_CODE as 出貨方式, oola.UNIT_SELLING_PRICE as 售價, oola.UNIT_LIST_PRICE as 單價 "+
									     ...
									     "   and MSI.INVENTORY_ITEM_ID = oola.INVENTORY_ITEM_ID "+
									     "   and oola.FLOW_STATUS_CODE = TWF.STATUSNAME  "+
									     "   and ooha.HEADER_ID =  oola.HEADER_ID and trim(TWF.STATUSID)='"+statusID+"' and substr(to_char(ooha.CREATION_DATE,'YYYYMMDDHH24MISS'),0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' order by ooha.ORDER_NUMBER, OOLA.SCHEDULE_SHIP_DATE ASC"); 			   			  
	          }	  	
   ...
   sKeyArray[0]="LINE_ID";
   sKeyArray[1]="REQUISITION_HEADER_ID";
   sKeyArray[2]="REQUISITION_LINE_ID";
