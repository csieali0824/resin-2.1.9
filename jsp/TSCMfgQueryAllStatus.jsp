...
{   
	location.href="../jsp/TSCMfgQueryAllStatus.jsp?STATUSID="+statusID+"&PAGEURL="+pageURL+"&SEARCHSTRING="+document.MYFORM.SEARCHSTRING.value+"&FROMYEAR="+document.MYFORM.FROMYEAR.value+"&FROMMONTH="+document.MYFORM.FROMMONTH.value+"&FROMDAY="+document.MYFORM.FROMDAY.value+"&TOYEAR="+document.MYFORM.TOYEAR.value+"&TOMONTH="+document.MYFORM.TOMONTH.value+"&TODAY="+document.MYFORM.TODAY.value+"&MARKETTYPE="+document.MYFORM.MARKETTYPE.value+"&WOTYPE="+document.MYFORM.WOTYPE.value  ;
}
...
function setSubmit(url)
{
	...
	if(len == undefined) 
	{
		if (document.MYFORM.CH.checked == true)
		{
			wo_no = document.getElementById("1_1").value;
			//alert(wo_no);
			url = url + "&WO_NO="+wo_no;
			check_flag =true;
		}
	...
	if (!check_flag)
	{
		 alert("Please select WIP number!!");
		 return false;
	}
	else
	{
...
if (statusID.equals("040") || statusID.equals("041")) //  若狀態是 009 訂單生成中,才顯示明細供使用者設定相關參數,否則,使用者無法作任何Submit...(防止User自行於網址列輸入LineNO) //加入狀態041 Update by SHIN 20090721
{
...
{   
	...
	//取得資料總筆數
	if (UserRoles.indexOf("admin")>=0 || UserRoles.indexOf("YEW_WIP_ADMIN")>=0 || UserRoles.indexOf("MPC_User")>=0) //若為admin則可看到全部 //add MPC_User by SHIN 20090721
	{	 
		if (searchString!=null && !searchString.equals("")) //如果有搜尋特定單號則另下SQL
		{	    
			lotRs=lotStat.executeQuery(" select COUNT(A.WO_NO) from YEW_WORKORDER_ALL A where A.WO_NO='"+searchString+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' " );	
			lotRs.next();
			if (lotRs.getInt(1)>0) //若有存在的話
			{
                // 20110707 Marvie Update : Performance Issue
				//rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where trim(A.STATUSID)='"+statusID+"' and A.WO_NO ='"+searchString+"' and substr(a.CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' ");	
				rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where A.STATUSID ='"+statusID+"' and A.WO_NO ='"+searchString+"' and substr(a.CREATION_DATE,1,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' ");	
			} else {
                // 20110707 Marvie Update : Performance Issue
				//rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where trim(A.STATUSID)='"+statusID+"' and (a.OE_ORDER_NO like '"+searchString+"%' or a.WAFER_IQC_NO like '"+searchString+"%' or a.WAFER_LOT_NO like '"+searchString+"') and substr(a.CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' ");	 	 
				rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where A.STATUSID ='"+statusID+"' and (a.OE_ORDER_NO like '"+searchString+"%' or a.WAFER_IQC_NO like '"+searchString+"%' or a.WAFER_LOT_NO like '"+searchString+"') and substr(a.CREATION_DATE,1,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' ");	 	 
			} //end of lotRs if
		} else {
            // 20110707 Marvie Update : Performance Issue
			//rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where trim(A.STATUSID)='"+statusID+"' and substr(a.CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' ");
			sql="select count(A.STATUSID) from YEW_WORKORDER_ALL A where A.STATUSID='"+statusID+"'";
            if (!(queryDateFrom.equals("20000101") && queryDateTo.equals("30001231")))
			  sql = sql + " and a.CREATION_DATE between '"+queryDateFrom+"' and '"+queryDateTo+"235959'";
			if (!marketType.equals("%"))
			  sql = sql + " and market_type = '"+marketType+"'";
			if (!woType.equals("%"))
			  sql = sql + " and workorder_type = '"+woType+"'";
//out.println("<br>sql="+sql+"<br>");
			rs=statement.executeQuery(sql);
		}
	} else {
		if (UserRoles.indexOf("YEW_MFG_PC")>=0 || UserRoles.indexOf("YEW_WIP_USER")>=0) //若為各製造部PC或統計則可看到各製造部自己開的工令去作展開
		{
			lotRs=lotStat.executeQuery("select COUNT(A.WO_NO) from YEW_WORKORDER_ALL A where A.WO_NO='"+searchString+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"' " );	
			lotRs.next();
			if (lotRs.getInt(1)>0) //若有存在的話
			{		   
                // 20110707 Marvie Update : Performance Issue
				//rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where trim(A.STATUSID)='"+statusID+"' and A.WO_NO ='"+searchString+"' and substr(a.CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and MARKET_TYPE like '"+marketType+"' and WORKORDER_TYPE like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"' ");	
				rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where A.STATUSID='"+statusID+"' and A.WO_NO ='"+searchString+"' and substr(a.CREATION_DATE,1,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and MARKET_TYPE like '"+marketType+"' and WORKORDER_TYPE like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"' ");	
			} else {
                // 20110707 Marvie Update : Performance Issue
				//rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where trim(A.STATUSID)='"+statusID+"' and (a.OE_ORDER_NO like '"+searchString+"%' or a.WAFER_IQC_NO like '"+searchString+"%' or a.WAFER_LOT_NO like '"+searchString+"') and substr(a.CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and MARKET_TYPE like '"+marketType+"' and WORKORDER_TYPE like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"'  ");	 	 
				rs=statement.executeQuery("select count(A.WO_NO) from YEW_WORKORDER_ALL A where A.STATUSID='"+statusID+"' and (a.OE_ORDER_NO like '"+searchString+"%' or a.WAFER_IQC_NO like '"+searchString+"%' or a.WAFER_LOT_NO like '"+searchString+"') and substr(a.CREATION_DATE,1,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and MARKET_TYPE like '"+marketType+"' and WORKORDER_TYPE like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"'  ");	 	 
			} //end of lotRs if  
		} // End of if (UserRoles.indexOf("YEW_MFG_PC")>=0) //若為各製造部PC則可看到各製造部的工令去作展開 

	}  // end of else (UserRoles.indexOf("admin")>=0) //若為admin則可看到全部
...
<strong><font color="#0080C0" size="5">工令單據處理</font></strong> <FONT COLOR=RED SIZE=4>&nbsp;&nbsp;工令狀態:<%=statusName%>(<%=statusDesc%>)</FONT><FONT COLOR=BLACK SIZE=2>(總共<%=maxrow%>&nbsp;筆記錄)</FONT>
<table width="100%" border="0">
<tr>
<td width="16%"> <input name="button" type=button onClick="this.value=check(this.form.CH)" value='選擇全部'>
&nbsp;&nbsp;</td>
<td width="84%"><strong><font color="#400040" size="2">請輸入工令號,訂單號,檢驗批單號:</font></strong>
...
<A HREF="../jsp/TSCMfgQueryAllStatus.jsp?STATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=FIRST&FROMYEAR=<%=fromYear%>&FROMMONTH=<%=fromMonth%>&FROMDAY=<%=fromDay%>&TOYEAR=<%=toYear%>&TOMONTH=<%=toMonth%>&TODAY=<%=toDay%>"><font size="2"><strong><font color="#FF0080">第一頁</font></strong></font></A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSSalesDRQQueryAllStatus.jsp?STATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=LAST&FROMYEAR=<%=fromYear%>&FROMMONTH=<%=fromMonth%>&FROMDAY=<%=fromDay%>&TOYEAR=<%=toYear%>&TOMONTH=<%=toMonth%>&TODAY=<%=toDay%>"><font size="2"><strong><font color="#FF0080">最終頁</font></strong></font></A><font color="#FF0080"><strong><font size="2">&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSSalesDRQQueryAllStatus.jsp?STATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=300&FROMYEAR=<%=fromYear%>&FROMMONTH=<%=fromMonth%>&FROMDAY=<%=fromDay%>&TOYEAR=<%=toYear%>&TOMONTH=<%=toMonth%>&TODAY=<%=toDay%>">下一頁</A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSSalesDRQQueryAllStatus.jsp?STATUSID=<%=statusID%>&PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=-300&FROMYEAR=<%=fromYear%>&FROMMONTH=<%=fromMonth%>&FROMDAY=<%=fromDay%>&TOYEAR=<%=toYear%>&TOMONTH=<%=toMonth%>&TODAY=<%=toDay%>">前一頁</A>&nbsp;&nbsp;(第<%=currentPageNumber%>&nbsp;頁/共<%=totalPageNumber%>&nbsp;頁)</font></strong></font>
&nbsp;&nbsp;&nbsp;&nbsp;開單日期
:FROM
<%
//try
...
try
{
	...
	ResultSet rs=null;
	String sql=null;  
	if (UserRoles.indexOf("admin")>=0 || UserRoles.indexOf("YEW_WIP_ADMIN")>=0 || UserRoles.indexOf("MPC_User")>=0) //若角色為admin則可看到全部詢問單 //add MPC_User by SHIN 20090721
	{    	 
		if (searchString!=null && !searchString.equals("")) //如果有搜尋特定單號則另下SQL
		{   	
			lotRs=lotStat.executeQuery("select count(a.WO_NO) from APPS.YEW_WORKORDER_ALL A where a.WO_NO='"+searchString+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' ");	
			lotRs.next();
			if (lotRs.getInt(1)>0) //若有存在批號的話
			{
                // 20110708 Marvie Update : Performance Issue
				//rs=statement.executeQuery(" select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別,  "+
				//"      INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位,substr(SCHEDULE_STRART_DATE,0,8) 預計投入日,substr(SCHEDULE_END_DATE,0,8) 預計完工日,substr(CREATION_DATE,0,8) 工令設立  "+
				//" from YEW_WORKORDER_ALL where trim(STATUSID)='"+statusID+"' and WO_NO ='"+searchString+"' and substr(CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"'  order by WO_NO, CREATION_DATE ASC"); 
				rs=statement.executeQuery(" select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','5','樣品','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別,  "+
				"      INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位,substr(SCHEDULE_STRART_DATE,1,8) 預計投入日,substr(SCHEDULE_END_DATE,1,8) 預計完工日,substr(CREATION_DATE,1,8) 工令設立  "+
				" from YEW_WORKORDER_ALL where STATUSID='"+statusID+"' and WO_NO ='"+searchString+"' and substr(CREATION_DATE,1,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"'  order by WO_NO"); 
			} else {
                // 20110708 Marvie Update : Performance Issue
				//rs=statement.executeQuery("select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別, "+
				//"  INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位, substr(SCHEDULE_STRART_DATE,0,8) 預計投入日,substr(SCHEDULE_END_DATE,0,8) 預計完工日,substr(CREATION_DATE,0,8) 工令設立 "+
				//" from YEW_WORKORDER_ALL where trim(STATUSID)='"+statusID+"' and (OE_ORDER_NO like '"+searchString+"%' or WAFER_IQC_NO like '"+searchString+"%' or WAFER_LOT_NO like '"+searchString+"') and substr(CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"'  order by WO_NO, CREATION_DATE ASC");	
				rs=statement.executeQuery("select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','5','樣品','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別, "+
				"  INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位, substr(SCHEDULE_STRART_DATE,1,8) 預計投入日,substr(SCHEDULE_END_DATE,1,8) 預計完工日,substr(CREATION_DATE,1,8) 工令設立 "+
				" from YEW_WORKORDER_ALL where STATUSID='"+statusID+"' and (OE_ORDER_NO like '"+searchString+"%' or WAFER_IQC_NO like '"+searchString+"%' or WAFER_LOT_NO like '"+searchString+"') and substr(CREATION_DATE,1,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"'  order by WO_NO");	
			}
		} else {
            // 20110708 Marvie Update : Performance Issue
			//rs=statement.executeQuery("select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別, "+
			//"  INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位, substr(SCHEDULE_STRART_DATE,0,8) 預計投入日,substr(SCHEDULE_END_DATE,0,8) 預計完工日,substr(CREATION_DATE,0,8) 工令設立 "+
			//" from YEW_WORKORDER_ALL where trim(STATUSID)='"+statusID+"' and (OE_ORDER_NO like '"+searchString+"%' or WAFER_IQC_NO like '"+searchString+"%' or WAFER_LOT_NO like '"+searchString+"') and substr(CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"'  order by WO_NO, CREATION_DATE ASC");	 
			sql="select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別,"+
			          " decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,"+
					  " decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','5','樣品','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別,"+
			          " INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位, substr(SCHEDULE_STRART_DATE,1,8) 預計投入日,"+
					  " substr(SCHEDULE_END_DATE,1,8) 預計完工日, substr(CREATION_DATE,1,8) 工令設立"+
			     " from YEW_WORKORDER_ALL"+
				 " where STATUSID='"+statusID+"'";
            if (!(queryDateFrom.equals("20000101") && queryDateTo.equals("30001231")))
			  sql = sql + " and CREATION_DATE between '"+queryDateFrom+"' and '"+queryDateTo+"235959'";
			if (!marketType.equals("%"))
			  sql = sql + " and market_type = '"+marketType+"'";
			if (!woType.equals("%"))
			  sql = sql + " and workorder_type = '"+woType+"'";
		    sql = sql + " order by WO_NO";
//out.println("<br>sql="+sql+"<br>");
			rs=statement.executeQuery(sql);
		}
	} else if (UserRoles.indexOf("YEW_MFG_PC")>=0 || UserRoles.indexOf("YEW_WIP_USER")>=0) //若為各製造部門PC或統計,則依各製造部門人員看得到自己的工令
	{   // 若為各製造部門PC,則依各製造部門人員看得到自己的工令
		if (searchString!=null && !searchString.equals("")) //如果有搜尋特定單號則另下SQL
		{   	
			lotRs=lotStat.executeQuery("select count(a.WO_NO) from APPS.YEW_WORKORDER_ALL A where a.WO_NO='"+searchString+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"' ");	
			lotRs.next();
			if (lotRs.getInt(1)>0) //若有存在的話
			{
                // 20110708 Marvie Update : Performance Issue
				//rs=statement.executeQuery(" select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別,  "+
				//"      INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位,substr(SCHEDULE_STRART_DATE,0,8) 預計投入日,substr(SCHEDULE_END_DATE,0,8) 預計完工日,substr(CREATION_DATE,0,8) 工令設立  "+
				//" from YEW_WORKORDER_ALL where trim(STATUSID)='"+statusID+"' and WO_NO ='"+searchString+"' and substr(CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"' order by WO_NO, CREATION_DATE ASC"); 
				rs=statement.executeQuery(" select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','5','樣品','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別,  "+
				"      INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位,substr(SCHEDULE_STRART_DATE,1,8) 預計投入日,substr(SCHEDULE_END_DATE,1,8) 預計完工日,substr(CREATION_DATE,1,8) 工令設立  "+
				" from YEW_WORKORDER_ALL where STATUSID='"+statusID+"' and WO_NO ='"+searchString+"' and substr(CREATION_DATE,1,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"' order by WO_NO"); 
			} else {
                // 20110708 Marvie Update : Performance Issue
				//rs=statement.executeQuery("select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別, "+
				//"  INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位, substr(SCHEDULE_STRART_DATE,0,8) 預計投入日,substr(SCHEDULE_END_DATE,0,8) 預計完工日,substr(CREATION_DATE,0,8) 工令設立 "+
				//" from YEW_WORKORDER_ALL where trim(STATUSID)='"+statusID+"' and (OE_ORDER_NO like '"+searchString+"%' or WAFER_IQC_NO like '"+searchString+"%' or WAFER_LOT_NO like '"+searchString+"') and substr(CREATION_DATE,0,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"' order by WO_NO, CREATION_DATE ASC");	
				rs=statement.executeQuery("select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','5','樣品','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別, "+
				"  INV_ITEM 料號,ITEM_DESC 品名,WO_QTY 數量,WO_UOM 單位, substr(SCHEDULE_STRART_DATE,1,8) 預計投入日,substr(SCHEDULE_END_DATE,1,8) 預計完工日,substr(CREATION_DATE,1,8) 工令設立 "+
				" from YEW_WORKORDER_ALL where STATUSID='"+statusID+"' and (OE_ORDER_NO like '"+searchString+"%' or WAFER_IQC_NO like '"+searchString+"%' or WAFER_LOT_NO like '"+searchString+"') and substr(CREATION_DATE,1,8) between '"+queryDateFrom+"' and '"+queryDateTo+"' and market_type like '"+marketType+"' and workorder_type like '"+woType+"' and DEPT_NO = '"+userMfgDeptNo+"' order by WO_NO");	
			}
		} else {
            // 20110708 Marvie Update : Performance Issue
			//rs=statement.executeQuery("select WO_NO, decode(DEPT_NO,'1','製一','2','製二','3','製三',DEPT_NO) as 製造部別, decode(MARKET_TYPE,'1','內銷','2','外銷') as 內外銷別,decode(WORKORDER_TYPE,'1','切割','2','前段','3','後段','4','重工','7','工程實驗或其它',WORKORDER_TYPE) as 工令類別, "+
	...
	} // End of else 若為各製造部門PC,則依各製造部門人員看得到自己的工令
	...
	//qryAllChkBoxEditBean.setSearchKey("WO_NO");
...
{       
	...
	if (statusID.equals("048") || (statusID.equals("048")) || statusID.equals("040")) //add status id=040 by Peggy 20131216
	{ 
		String sqlAct = null;
		String whereAct = null;

		if (statusID.equals("048") || statusID.equals("048"))
		{ 
		...
		else if (statusID.equals("048")) {
			sqlAct = "select DISTINCT x1.ACTIONID,x2.ACTIONNAME from ORADDMAN.TSWORKFLOW x1,ORADDMAN.TSWFACTION x2 ";
			whereAct = "WHERE FROMSTATUSID='"+statusID+"' AND x1.ACTIONID=x2.ACTIONID and x1.ACTIONID !='013' and x1.LOCALE='"+locale+"'";         										   
		}
		else if (statusID.equals("040")) //add status id=040 by Peggy 20131216
		{
			sqlAct = "select x1.ACTIONID,x2.ACTIONNAME from ORADDMAN.TSWORKFLOW x1,ORADDMAN.TSWFACTION x2 ";
			whereAct =" WHERE FORMID='WO' AND FROMSTATUSID='"+statusID+"' AND x1.ACTIONID=x2.ACTIONID and x1.ACTIONID not in ('020','021','022') and  x1.LOCALE='"+locale+"'";
		}
		/*								  
		...
		*/
		...
		{	
			if (statusID.equals("040"))
			{
				out.println("<INPUT TYPE='button' NAME='button1' value='Submit' onclick='setSubmit("+'"'+"../jsp/TSCMfgWoPassMProcess.jsp?FROMSTATUSID="+statusID+'"'+")'>");
			}
			else
			{      
