...
{
   warray=new Array(document.MYFORM.APRNCECODE.value,document.MYFORM.PRODAPPEAR.value,document.MYFORM.APPEARDESC.value);   
   for (i=0;i<3;i++)
   {     
      if (warray[i]=="" || warray[i]==null || warray[i]=="--" )
     { 
      alert("Before you want to add , please do not let the any fields of Product Appearance data be Null !!");   
      return(false);
      } 
...
<title>WINS System-產品外觀資料新增</title>
</head>
<body>
<FORM ACTION="WSMRProdAprnceInput.jsp" METHOD="post" NAME="MYFORM">
  ...
  <strong>  產品編碼系統-產品外觀代碼新增 </strong></font> &nbsp; &nbsp; &nbsp; 
  &nbsp;<BR>
  <A HREF="../WinsMainMenu.jsp">首頁</A> &nbsp;&nbsp;<A HREF="../jsp/WSModelEncodingSub.jsp">產品編碼管理作業</A> &nbsp;&nbsp;<A HREF="../jsp/WSMRProdAprnceInquiry.jsp">產品外觀代碼資料查詢</A> 
<%   
...
String aprnceCode=request.getParameter("APRNCECODE"),prodAppear=request.getParameter("PRODAPPEAR"),appearDesc=request.getParameter("APPEARDESC");
...
String [] allMonth={aprnceCode,prodAppear,appearDesc};
...
<table width="60%" border="1" cellspacing="0" cellpadding="0">
  <tr>      
      <td width="20%" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>產品外觀代碼</strong></font></div></td>
	  <td width="20%" colspan="1" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>產品外觀說明</strong></font></div></td> 
      <td width="42%" colspan="1" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>備註</strong></font></div></td> 	  
      ...
      <td width="18%" rowspan="3">
        <font color="#333399" face="Arial Black" size="2"><strong> <div align="center" >
         <INPUT TYPE="button"  value="Add" onClick='setSubmit("../jsp/WSMRProdAprnceInput.jsp")' >
	    </div></strong></font>
      </td>   
    </tr>
  <tr>
    <td>    
    <input type="text" name="APRNCECODE"  size="15" maxlength="1" <%if (allMonth[0]!=null) out.println("value="); else out.println("value=");%>>
    </td>
    <td><input type="text" name="PRODAPPEAR"  size="15" maxlength="15"   <%if (allMonth[1]!=null) out.println("value="); else out.println("value=");%> >
    </td>
    <td><input type="text" name="APPEARDESC"  size="25" maxlength="25"   <%if (allMonth[2]!=null) out.println("value="); else out.println("value=");%> >
    </td>    
    </tr>    
  <tr bgcolor="#003366">
    <td colspan="3"><div align="center"><strong>
     <%
	  try
      {
        ...
        String oneDArray[]= {"增項","產品外觀代碼","產品外觀說明","備註"}; 		 	     			  
    	array2DimensionInputBean.setArrayString(oneDArray);
	     ...
	     if (aprnceCode!=null && !aprnceCode.equals("") && prodAppear!=null && !prodAppear.equals("") && bringLast==null) //bringLast是用來識別是否帶出上一次輸入之最新版本資料
		 {  //out.println("step1");             		    
		   if (a!=null) 
		   { //out.println("step2");
			 ...
			 { //out.println("step3");
			  ...
			  { //out.println("step4");
			    b[i][j]=a[i][j];	
                if (a[k][0].equals(aprnceCode)) { dupFLAG = "TRUE"; }				
			  }
			  k++;
			 }
			 //b[k][0]=chooseItem;
             ...
             String sqlIMDUP = "select * from MRPROD_APRNCE where APPEAR_CODE='"+aprnceCode+"' ";
             //out.println(sqlIMDUP);     
             ResultSet rsIMDUP=stateIMDUP.executeQuery(sqlIMDUP);             
             if (rsIMDUP.next() )
             { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input :</font><font color='#FFFFFF' face='Arial Black'> "+aprnceCode+"</font><font color='#FF0000' face='Arial Black'> on Database history"+"</font>"); }
             else if (dupFLAG=="TRUE" || dupFLAG.equals("TRUE")) { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input :</font><font color='#FFFFFF' face='Arial Black'> "+aprnceCode+"</font><font color='#FF0000' face='Arial Black'> on Your list"+"</font>"); }
             else
             {
              b[k][0]=aprnceCode;b[k][1]=prodAppear;b[k][2]=appearDesc;
             ...
             {
              String c[][]={{aprnceCode,prodAppear,appearDesc}};						             			 
      ...
      {	//out.println("Step1");
	    String a[][]=array2DimensionInputBean.getArray2DContent();//取得目前陣列內容 		    		                       		    		  	   
		 ...
		 { //out.println("Step2");
		        div1=a.length;
				div2=a[0].length;
	        	array2DimensionInputBean.setFieldName("ADDITEMS");	//out.println("Step3");		 			 	   			 
       ...
       %>
<BR>
<INPUT name="button2" TYPE="button" onClick='setSubmit3("../jsp/WSMRProdAprnceInput.jsp")'  value="DELETE" >
------------------------------------------------------------------------------------------------------------
<INPUT TYPE="button"  value="SAVE" onClick='setSubmit2("../jsp/WSMRProdAprnceInsert.jsp",<%=div1%>,<%=div2%>)' >
---------------------
<!-- 表單參數 -->  
