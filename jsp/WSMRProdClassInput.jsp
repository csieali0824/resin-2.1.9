...
{
   warray=new Array(document.MYFORM.PCLSCODE.value,document.MYFORM.PRODCLASS.value,document.MYFORM.CLASSDESC.value);   
   for (i=0;i<3;i++)
   {     
...
{    
   ...
   {     
      for (j=1;j<dim2;j++)
	  {
		 ...
		 { 
           alert("Before you want to save , please do not let the any filed of  detail be Null !!");   
           return(false);
		 }  
	  } //enf for of jj             
   } //end of for null check
...
<title>WINS System-產品別資料新增</title>
</head>
<body>
<FORM ACTION="WSMRProdAprnceInput.jsp" METHOD="post" NAME="MYFORM">
  ...
  <strong> 產品編碼系統-產品類別代碼新增 </strong></font> &nbsp; &nbsp; &nbsp; &nbsp;<BR>
  <A HREF="../WinsMainMenu.jsp">首頁</A> &nbsp;&nbsp;<A HREF="../jsp/WSModelEncodingSub.jsp">產品編碼管理作業</A> 
  &nbsp;&nbsp;<A HREF="../jsp/WSMRProdClassInquiry.jsp?ENTRY=TRUE">產品類別代碼資料查詢</A> 
  <%   
...
String pclsCode=request.getParameter("PCLSCODE"),prodClass=request.getParameter("PRODCLASS"),classDesc=request.getParameter("CLASSDESC");
...
String [] allMonth={pclsCode,prodClass,classDesc};
...
<table width="100%" border="0">
  ...
  <table width="81%" border="1" cellspacing="0" cellpadding="0">
    <tr> 
      <td width="25%" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>產品類別代碼</strong></font></div></td>
      <td width="31%" colspan="1" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>產品別說明</strong></font></div></td>
      <td width="44%" colspan="1" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>備註</strong></font></div></td>
      <td width="18%" rowspan="3">
        <font color="#333399" face="Arial Black" size="2"><strong> <div align="center" >
        <INPUT name="button3" TYPE="button" onClick='setSubmit("../jsp/WSMRProdClassInput.jsp")'  value="Add" >
	    </div></strong></font>
    </tr>
    <tr> 
      <td> <input type="text" name="PCLSCODE"  size="15" maxlength="1" <%if (allMonth[0]!=null) out.println("value="); else out.println("value=");%>></td>
      <td><input type="text" name="PRODCLASS"  size="15" maxlength="15"   <%if (allMonth[1]!=null) out.println("value="); else out.println("value=");%> > 
      </td>
      <td><input type="text" name="CLASSDESC"  size="25" maxlength="25"   <%if (allMonth[2]!=null) out.println("value="); else out.println("value=");%> > 
      </td>
    </tr>
    <tr bgcolor="#003366"> 
	  ...
	  try
      {
        ...
        String oneDArray[]= {"增項","產品別代碼","產品別說明","備註"}; 		 	     			  
    	array2DimensionInputBean.setArrayString(oneDArray);
	     ...
	     if (pclsCode!=null && !pclsCode.equals("") && prodClass!=null && !prodClass.equals("") && bringLast==null) //bringLast是用來識別是否帶出上一次輸入之最新版本資料
		 {  //out.println("step1");             		    
		   if (a!=null) 
		   { //out.println("step2");
			 ...
			 { //out.println("step3");
			  ...
			  { //out.println("step4");
			    b[i][j]=a[i][j];	
                if (a[k][0].equals(pclsCode)) { dupFLAG = "TRUE"; }				
			  }
			  k++;
			 }
			 //b[k][0]=chooseItem;
             ...
             String sqlIMDUP = "select * from MRPRODCLS where PCLS_CODE='"+pclsCode+"' ";
             //out.println(sqlIMDUP);     
             ResultSet rsIMDUP=stateIMDUP.executeQuery(sqlIMDUP);             
             if (rsIMDUP.next() )
             { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input :</font><font color='#FFFFFF' face='Arial Black'> "+pclsCode+"</font><font color='#FF0000' face='Arial Black'> on Database history"+"</font>"); }
             else if (dupFLAG=="TRUE" || dupFLAG.equals("TRUE")) { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input :</font><font color='#FFFFFF' face='Arial Black'> "+pclsCode+"</font><font color='#FF0000' face='Arial Black'> on Your list"+"</font>"); }
             else
             {
              b[k][0]=pclsCode;b[k][1]=prodClass;b[k][2]=classDesc;
             ...
             {
              String c[][]={{pclsCode,prodClass,classDesc}};						             			 
...
<% 
     int div1=0,div2=0;      //做為運算共有多少個row和column輸入欄位的變數
      ...
      {	
       ...
       %>
<BR>
<INPUT name="button2" TYPE="button" onClick='setSubmit3("../jsp/WSMRProdClassInput.jsp")'  value="DELETE" >
------------------------------------------------------------------------------------------------------------
<INPUT TYPE="button"  value="SAVE" onClick='setSubmit2("../jsp/WSMRProdClassInsert.jsp",<%=div1%>,<%=div2%>)' >
---------------------
<!-- 表單參數 -->  
