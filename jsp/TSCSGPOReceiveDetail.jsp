...
<title>SG Receiving Detail</title>
<!--=============以下區段為安全認證機制==========-->
	...
	if (confirm("您確定要刪除Line No:"+objLine+"所有資料嗎?"))
	{
		for (var i = objLine ; i <= document.MYFORMD.LINECNT.value ; i++)
		{
			if ( i < document.MYFORMD.LINECNT.value)
			{
				...
				document.MYFORMD.elements["CARTON_NO_"+document.MYFORMD.ID.value+"_"+i].value = document.MYFORMD.elements["CARTON_NO_"+document.MYFORMD.ID.value+"_"+(i+1)].value;
				document.MYFORMD.elements["NW_"+document.MYFORMD.ID.value+"_"+i].value = document.MYFORMD.elements["NW_"+document.MYFORMD.ID.value+"_"+(i+1)].value;
				document.MYFORMD.elements["GW_"+document.MYFORMD.ID.value+"_"+i].value = document.MYFORMD.elements["GW_"+document.MYFORMD.ID.value+"_"+(i+1)].value;
			}
			else
			{
				...
				document.MYFORMD.elements["CRATON_NO_"+document.MYFORMD.ID.value+"_"+i].value = "";
				document.MYFORMD.elements["NW_"+document.MYFORMD.ID.value+"_"+i].value = "";
				document.MYFORMD.elements["GW_"+document.MYFORMD.ID.value+"_"+i].value = "";
			}
		}
		ComputeQty(0);
	}
...
{
	var v_date = "",rec_year="",rec_month="",rec_day="";v_lot="",v_dc="",v_qty="",v_dc_yyww="";
	...
	for (var i =1 ; i <= document.MYFORMD.LINECNT.value ; i++)
	{
		...
		v_dc_yyww =document.MYFORMD.elements["DC_YYWW_"+document.MYFORMD.ID.value+"_"+i].value;
		...
		if (v_lot !="" || v_date !="" || v_dc !="" || v_qty !="")
		{
			...
			{
				alert("請輸入DC YYWW!!");
				document.MYFORMD.elements["DC_YYWW_"+document.MYFORMD.ID.value+"_"+i].focus();
				return false;
			}	
			else if (v_dc_yyww.length!=4)
			{
				alert("DC YYWW長度固定為四碼數字!!");
				document.MYFORMD.elements["DC_YYWW_"+document.MYFORMD.ID.value+"_"+i].focus();
				return false;			
			}		
...
%>
<body>  
<FORM ACTION="../jsp/TSCSGPOReceiveDetail.jsp" METHOD="post" NAME="MYFORMD">
<%
if (!ACODE.equals("SAVE"))
{
	...
	%>
			...
			<table align="center" width='100%' border='1' bordercolorlight='#333366' bordercolordark='#ffffff' cellPadding='1' cellspacing='0'>
				<tr style="background-color:#006666;color:#FFFFFF;">
					...
					<td width="7%" align="center">Data Code</td>
					<td width="5%" align="center">DC YYWW</td>
					<td width="7%" align="center">收貨數量(K)</td>
					<td width="11%" align="center">收料日期</td>
					<td width="11%" align="center">備註</td>
					<td width="12%" align="center">未按FIFO收貨原因</td>
					<td width="5%" align="center">箱數</td>
					<td width="9%" align="center">出貨類型</td>
					<td width="5%" align="center">淨重</td>
					<td width="5%" align="center">毛重</td>
				</tr>
	<%
				if (ORIGCNT.equals(""))
				{
						...
						for( int i=0 ; i< LotArray.length ; i++ ) 
						{
							...
							%>
								<tr>
									...
									<td align="center"><input type="TEXT" NAME="LOT_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][2]==null){ out.println("");}else{out.println(LotArray[i][2]);}%>"  style="font-family: Tahoma,Georgia;" size="12" onBlur="setdate(<%=ID%>,<%=lineno%>)"></td>
									<td align="center"><input type="TEXT" NAME="DC_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][3]==null){ out.println("");}else{out.println(LotArray[i][3]);}%>"   style="font-family: Tahoma,Georgia;" size="8" onChange="checkDC(this.value,'DC_<%=ID%>_<%=lineno%>')"></td>
									<td align="center"><input type="TEXT" NAME="DC_YYWW_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][11]==null){ out.println("");}else{out.println(LotArray[i][11]);}%>"   style="font-family: Tahoma,Georgia;" size="4" onKeypress="return (event.keyCode >= 48 && event.keyCode <=57)"></td>
									<td align="center"><input type="TEXT" NAME="QTY_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][4]==null){ out.println("");}else{out.println(LotArray[i][4]);}%>"  style="font-family: Tahoma,Georgia;text-align:RIGHT" size="8" onChange="ComputeQty(<%=lineno%>);" onKeypress="return ((event.keyCode >= 48 && event.keyCode <=57) || event.keyCode == 46)"></td>
									<td><input type="TEXT" name="RECEIVE_DATE_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][1]==null){ out.println("");}else{out.println(LotArray[i][1]);}%>"  style="font-family: Tahoma,Georgia;" size="8" onKeyPress="return (event.keyCode >= 48 && event.keyCode <=57)"><A href='javascript:void(0)' onclick='gfPop.fPopCalendar(document.MYFORMD.RECEIVE_DATE_<%=ID%>_<%=lineno%>);return false;'><img name='popcal' border='0' src='../image/calbtn.gif'></A></td>
									<td align="center"><input type="TEXT" name="REMARKS_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][5]==null){ out.println("");}else{out.println(LotArray[i][5]);}%>"  style="font-family: Tahoma,Georgia;" size="12"></td>
									<td align="center"><input type="TEXT" name="NO_FIFO_REASON_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][6]==null){ out.println("");}else{out.println(LotArray[i][6]);}%>"  style="font-family: Tahoma,Georgia;" size="15"></td>
									<td align="center"><input type="TEXT" name="CARTON_NO_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][7]==null){ out.println("");}else{out.println(LotArray[i][7]);}%>"  style="font-family: Tahoma,Georgia;" size="7"></td>
									<td align="center"><input type="TEXT" name="DELIVERY_TYPE_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][8]==null){ out.println("");}else{out.println(LotArray[i][8]);}%>"  style="font-family: Tahoma,Georgia;" size="7"></td>
									<td align="center"><input type="TEXT" name="NW_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][9]==null){ out.println("");}else{out.println(LotArray[i][9]);}%>"  style="font-family: Tahoma,Georgia;" size="4"></td>
									<td align="center"><input type="TEXT" name="GW_<%=ID%>_<%=lineno%>" value="<% if (LotArray[i][10]==null){ out.println("");}else{out.println(LotArray[i][10]);}%>"  style="font-family: Tahoma,Georgia;" size="4"></td>
								</tr>							
							<%
	...
	%>
						<tr>
							<td align="center"><input type="button" name="DEL<%=lineno%>" value="Delete" style="font-family: Tahoma,Georgia;"  onClick='setDelete(<%=lineno%>)'></td>
							<td align="center"><%=lineno%></td>
							<td align="center"><input type="TEXT" NAME="LOT_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("LOT_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("LOT_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;" size="12" onBlur="setdate(<%=ID%>,<%=lineno%>)"></td>
							<td align="center"><input type="TEXT" NAME="DC_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("DC_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("DC_"+ID+"_"+lineno));}%>"   style="font-family: Tahoma,Georgia;" size="8" onChange="checkDC(this.value,'DC_<%=ID%>_<%=lineno%>')"></td>
							<td align="center"><input type="TEXT" NAME="DC_YYWW_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("DC_YYWW_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("DC_YYWW_"+ID+"_"+lineno));}%>"   style="font-family: Tahoma,Georgia;" size="4" onKeypress="return (event.keyCode >= 48 && event.keyCode <=57)"></td>
							<td align="center"><input type="TEXT" NAME="QTY_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("QTY_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("QTY_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;text-align:RIGHT" size="8"  onChange="ComputeQty(<%=lineno%>);"  onKeypress="return ((event.keyCode >= 48 && event.keyCode <=57) || event.keyCode == 46)"></td>
							<td><input type="TEXT" NAME="RECEIVE_DATE_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("RECEIVE_DATE_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("RECEIVE_DATE_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;" size="8" onKeypress="return (event.keyCode >= 48 && event.keyCode <=57)"><A href='javascript:void(0)' onclick='gfPop.fPopCalendar(document.MYFORMD.RECEIVE_DATE_<%=ID%>_<%=lineno%>);return false;'><img name='popcal' border='0' src='../image/calbtn.gif'></A></td>
							<td align="center"><input type="TEXT" NAME="REMARKS_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("REMARKS_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("REMARKS_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;" size="12"></td>
							<td align="center"><input type="TEXT" NAME="NO_FIFO_REASON_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("NO_FIFO_REASON_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("NO_FIFO_REASON_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;" size="15"></td>
							<td align="center"><input type="TEXT" NAME="CARTON_NO_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("CARTON_NO_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("CARTON_NO_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;" size="7"></td>
							<td align="center"><input type="TEXT" NAME="DELIVERY_TYPE_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("DELIVERY_TYPE_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("DELIVERY_TYPE_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;" size="7"></td>
							<td align="center"><input type="TEXT" NAME="NW_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("NW_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("NW_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;" size="4"></td>
							<td align="center"><input type="TEXT" NAME="GW_<%=ID%>_<%=lineno%>" value="<% if (request.getParameter("GW_"+ID+"_"+lineno)==null){ out.println("");}else{out.println(request.getParameter("GW_"+ID+"_"+lineno));}%>"  style="font-family: Tahoma,Georgia;" size="4"></td>
						</tr>	
	<%
	...
	%>		
				<tr>
					<td  colspan="4" style="border-left-style:none;border-right-style:none;"><input type="button" name="Addline" value="Add Line" style="font-family: Tahoma,Georgia;" onClick='setAddLine("../jsp/TSCSGPOReceiveDetail.jsp?ACODE=ADDLINE")'><input type="text" NAME="TXTLINE" value="5" style="font-family: Tahoma,Georgia;text-align:RIGHT" size="5"></td>
					...
					<td colspan="7" style="border-left-style:none">&nbsp;</td>
				</tr>
			</table>
			<table width="100%">
				<tr>
					<td align="center">
						<input type="button" name="save1" value="SAVE" onClick='setSubmit("../jsp/TSCSGPOReceiveDetail.jsp?ACODE=SAVE")' style="font-family: Tahoma,Georgia;">
	...
	<%
			...
			{
				DateCodeList ="";
				sql = " select DISTINCT date_code from tsc.TSC_DATE_CODE where YEAR >= to_char(add_months(sysdate,-12),'yyyy')";
				//out.println(sql);
...
{
	String lot [][] = new String [Integer.parseInt(LINECNT)][12];
	...
	for (int i =0 ; i < Integer.parseInt(LINECNT) ; i++)
	{
		...
		if (request.getParameter("RECEIVE_DATE_"+ID+"_"+(i+1))!=null && request.getParameter("LOT_"+ID+"_"+(i+1))!=null)
		{
			...
			lot[i][7] = request.getParameter("CARTON_NO_"+ID+"_"+(i+1));
			lot[i][8] = request.getParameter("DELIVERY_TYPE_"+ID+"_"+(i+1));
			lot[i][9] = request.getParameter("NW_"+ID+"_"+(i+1));
			lot[i][10] = request.getParameter("GW_"+ID+"_"+(i+1));
			lot[i][11] = request.getParameter("DC_YYWW_"+ID+"_"+(i+1));
			lot_cnt ++;
		}
	}
	lot_tot_row = lot_cnt;
	...
	String lot_tot [][] = new String [lot_tot_row][12];
	...
	for (int i = 0 ; i < lot.length ; i++)
	{
		if (lot[i][0]!=null && !lot[i][0].equals("") && lot[i][2] != null && !lot[i][2].equals("") && iLine <=lot_tot.length)
		{
			...
			lot_tot[iLine][7] = lot[i][7];
			lot_tot[iLine][8] = lot[i][8];
			lot_tot[iLine][9] = lot[i][9];
			lot_tot[iLine][10] = lot[i][10];
			lot_tot[iLine][11] = lot[i][11];
			iLine ++;
		}
	} 
	if (LotArray!=null)
	{
		for( int i=0 ; i< LotArray.length ; i++ ) 
		{
			if (LotArray[i][0]!=null && !LotArray[i][0].equals("") && LotArray[i][2] != null && !LotArray[i][2].equals("") && !LotArray[i][0].equals(ID) && iLine <= (LotArray.length - Integer.parseInt(ORIGCNT)))
			{
				...
				lot_tot[iLine][7] = LotArray[i][7];
				lot_tot[iLine][8] = LotArray[i][8];
				lot_tot[iLine][9] = LotArray[i][9];
				lot_tot[iLine][10] = LotArray[i][10];
				lot_tot[iLine][11] = LotArray[i][11];
				iLine ++;
			}
		}
	}
