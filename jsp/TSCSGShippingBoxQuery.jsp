<%@ page contentType="text/html; charset=big5" language="java" import="java.sql.*,java.math.BigDecimal,java.text.*" %>
...
<FORM ACTION="../jsp/TSCSGShippingBoxQuery.jsp" METHOD="post" NAME="MYFORM">
	<strong><font style="font-size:18px;color:#000099">SG Carton Info </font></strong>
	<BR>
	<%
		...
		String Q_VENDOR = request.getParameter("Q_VENDOR");
		if (Q_VENDOR==null) Q_VENDOR="";
	...
	<table cellSpacing="0" bordercolordark="#5C7671"  cellPadding="1" width="100%" align="center" borderColorLight="#CFDAD8" border='1'>
		<tr>
			<td width="6%" style="background-color:#D3E6F3;color:#006666">Supplier:</td>
			<td width="14%">
				<%
					PreparedStatement statement1 = con.prepareStatement("SELECT DISTINCT b.vendor_id,b.vendor_name FROM tsc_item_packing_master a,ap.ap_suppliers b where a.int_type=? AND a.vendor_id = b.vendor_id group by b.vendor_id,b.vendor_name order by b.vendor_name");
					statement1.setString(1,"SG");
					ResultSet rs1=statement1.executeQuery();
					comboBoxBean.setRs(rs1);
					comboBoxBean.setSelection(Q_VENDOR);
					comboBoxBean.setFieldName("Q_VENDOR");
					out.println(comboBoxBean.getRsString());
					rs1.close();
					statement1.close();
				%>
			</td>
			<td width="8%" style="background-color:#D3E6F3;color:#006666">TSC Prod Group:</td>
			<td width="8%">
				<select NAME="TSC_PROD_GROUP" style="font-size:11px;font-family:Tahoma,Georgia;">
					<OPTION VALUE=-- <%if (TSC_PROD_GROUP.equals("")) out.println("selected");%>>--</OPTION>
					<OPTION VALUE="PRD-Subcon" <%if (TSC_PROD_GROUP.equals("PRD-Subcon")) out.println("selected");%>>PRD-Subcon</OPTION>
					<OPTION VALUE="SSD" <%if (TSC_PROD_GROUP.equals("SSD")) out.println("selected");%>>SSD</OPTION>
					<OPTION VALUE="PMD" <%if (TSC_PROD_GROUP.equals("PMD")) out.println("selected");%>>PMD</OPTION>
				</select>
			</td>
			<td width="7%" style="background-color:#D3E6F3;color:#006666">TSC Package</td>
			<td width="10%">
				<%
					statement1 = con.prepareStatement("SELECT DISTINCT TSC_PACKAGE,TSC_PACKAGE TSC_PACKAGE1 FROM tsc_item_packing_master a where a.int_type=? group by TSC_PACKAGE order by TSC_PACKAGE");
					statement1.setString(1,"SG");
			...
			<td width="7%" style="background-color:#D3E6F3;color:#006666">Status</td>
			<td width="10%">
				<select NAME="STATUS1" style="font-family: Tahoma,Georgia; font-size: 11px ">
					<OPTION VALUE=-- <%if (STATUS1.equals("")) out.println("selected");%>>--</OPTION>
					<OPTION VALUE="Y" <%if (STATUS1.equals("Y")) out.println("selected");%>>Active</OPTION>
					<OPTION VALUE="N" <%if (STATUS1.equals("N")) out.println("selected");%>>Inactive</OPTION>
				</select>
			</td>
			<td width="12%">
				<INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>'  style="font-family: Tahoma,Georgia;" onClick='setSubmit("../jsp/TSCSGShippingBoxQuery.jsp?ACTIONCODE=Q")' >
				&nbsp;&nbsp;&nbsp;
				<INPUT TYPE='button' id='AddNew' value='New add' onClick='setAddNew("../jsp/TSCSGShippingBoxSetup.jsp")'>
			</td>
		</tr>
	</table>
	<%
		if (ACTIONCODE.equals("Q"))
		{
			try
			{
				sql = " select a.rowid,b.vendor_name, a.tsc_prod_group, a.tsc_package, a.packing_code, a.def_flag,"+
						...
						",a.reel_qty"+
						"  from tsc_item_packing_master a,ap_suppliers b where a.INT_TYPE='SG' and a.vendor_id = b.vendor_id(+)";
				if (!Q_VENDOR.equals("") && !Q_VENDOR.equals("--"))
				{
					sql += " and a.VENDOR_ID='"+ Q_VENDOR+"'";
				}
				...
				sql += " order by a.vendor_id,a.tsc_prod_group, a.tsc_package,SUBSTR(a.packing_code,1,2),INNER_QTY,CARTON_QTY, length(a.packing_code)";
				//out.println(sql);
	...
	%>
	<hr>
	<div align="left" style="color:#0000FF">Qty Uom :PCS</div>
	<table cellspacing="0" bordercolordark="#FFFFFF" cellpadding="1" width="100%" align="center" bordercolorlight="#CCCC99" border="1">
		<tr bgcolor="#C8E3E8">
			<td align="center" width="10%">&nbsp;</td>
			<td align="center" width="3%">Seq No </td>
			<td align="center" width="8%">TSC Prod Group</td>
			<td align="center" width="10%">Supplier</td>
			<td align="center" width="7%">TSC Package</td>
			<td align="center" width="5%">Packing Code</td>
			<td align="center" width="5%">Reel Qty</td>
			<td align="center" width="5%">Inner Qty</td>
			<td align="center" width="5%">Carton Qty</td>
			<td align="center" width="7%">Carton Size</td>
			<td align="center" width="5%">Net Weight</td>
			<td align="center" width="5%">Gross Weight</td>
			<td align="center" width="8%">Item Desc </td>
			<td align="center" width="5%">Status</td>
			<td align="center" width="6%">Last Updated by </td>
			<td align="center" width="6%">Last Update Date </td>
		</tr>
		<%
				}
				out.println("<tr>");
				out.println("<td align='center'>");
				out.println("<input type='button' id='UPD"+i+"' value='Update' onclick='setModify("+'"'+"../jsp/TSCSGShippingBoxSetup.jsp?ACTIONMODE=MODIFY&ID="+java.net.URLEncoder.encode(rs.getString("ROWID"))+'"'+")'>");
				out.println("<input type='button' id='CANCEL"+i+"' value='"+(rs.getString("STATUS").equals("Y")?"Inactive":"Active")+"' onclick='setInactive("+'"'+"../jsp/TSCSGShippingBoxQuery.jsp?ACTIONCODE=CANCEL&ID="+java.net.URLEncoder.encode(rs.getString("ROWID"))+"&STATUS="+(rs.getString("STATUS").equals("Y")?"N":"Y")+ '"'+","+'"'+(rs.getString("STATUS").equals("Y")?"N":"Y")+'"'+")'>");
				out.println("</td>");
				out.println("<td>"+i+"</td>");
				...
				out.println("<td align='center'>"+(rs.getString("STATUS").equals("Y")?"<font color='blue'>Active</font>":"<font color='red'>Inactive</font>")+"</td>");
	...
	<%
				...
				{
					out.println("<div align='center' style='color:#ff0000'>No Data Found!!</div>");
				}
			}
			catch (Exception e)
			{
