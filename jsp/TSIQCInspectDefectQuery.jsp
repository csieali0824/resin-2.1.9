...
{   
  location.href="../jsp/TSIQCInspectDefectQuery.jsp?PAGEURL="+pageURL+"&SEARCHSTRING="+document.MYFORM.SEARCHSTRING.value;
}
...
<%   
  ...
  String defectID=request.getParameter("DEFECT_ID");
 ...
 try
   ...
   */
   ...
   ResultSet  rs=null;
      ...
      String sql = "select count(*) from ORADDMAN.TSCIQC_DEFECT";
	  String sWhere = " where ( DEFECT_NAME='"+searchString+"' ) ";
	  ...
	  if (examineItemID!=null && !examineItemID.equals("--")) sWhere = sWhere + "and EXAMINE_ITEM_ID = '"+examineItemID+"' ";
     ...
     if (searchString!=null && !searchString.equals("")) //如果有搜尋檢驗類別則另下SQL
	 {
	    ...
	    {	
	       ...
	       rs=statement.executeQuery(sql + " and (DEFECT_NAME='"+searchString+"' or DEFECT_DESC ='"+searchString+"') ");	
	    } else {
                    rs=statement.executeQuery(sql+" and (DEFECT_NAME='"+searchString+"' or DEFECT_DESC ='"+searchString+"') ");	 	 
                  } //end of lotRs if		  
     } else {
...
<strong><font color="#0080C0" size="5">IQC品管檢驗測試缺點項目查詢</font></strong>(總共<%=maxrow%>&nbsp;筆記錄)</FONT><BR>
  ...
  <tr bgcolor="#B1D3A9">     
	  <td width="46%"><font color="#FF0066"><strong>檢驗類別代碼: 
        ...
        <%	    	     		 		 
		  ...
		  */
		  out.println("<select NAME='CLASS_ID' onChange='setSubmit("+'"'+"../jsp/TSIQCInspectDefectQuery.jsp?PAGEURL="+pageURL+'"'+")'>");
	...
	<td width="54%"><font color="#FF0066"><strong>檢驗項目代碼: 
        ...
        <%	    	     		 		 
		 ...
		 /* 
          ...
          out.println(comboBoxBean.getRsString());
		 */
		  out.println("<select NAME='ITEM_ID' onChange='setSubmit("+'"'+"../jsp/TSIQCInspectDefectQuery.jsp?PAGEURL="+pageURL+"&CLASS_ID="+classID+'"'+")'>");
          out.println("<OPTION VALUE=-->--");     
          while (rsC.next())
             {            
              String s1=(String)rsC.getString(1); 
 ...
 <td width="46%"><font color="#FF0066"><strong>檢驗測試項目代碼: 
        ...
        <%	    	     		 		 
         ...
         { 	   		 
		  ...
		  /*
          comboBoxBean.setRs(rsC);
		  if (itemID!=null && !itemID.equals("--")) comboBoxBean.setSelection(examineItemID);		  		  		  
	      comboBoxBean.setFieldName("EXAMINE_ITEM_ID");	   
          out.println(comboBoxBean.getRsString());
		  */
		  out.println("<select NAME='EXAMINE_ITEM_ID' onChange='setSubmit("+'"'+"../jsp/TSIQCInspectDefectQuery.jsp?PAGEURL="+pageURL+"&CLASS_ID="+classID+"&ITEM_ID="+itemID+'"'+")'>");
          out.println("<OPTION VALUE=-->--");     
          while (rsC.next())
             {            
              String s1=(String)rsC.getString(1); 
   ...
   <td width="46%"><font color="#FF0066"><strong>測試缺點項目代碼: 
        </strong></font><font> 
        <%	    	     		 		 
	     try
         { 	   		 
		  ...
		  sSqlC = "select Unique DEFECT_ID as x ,DEFECT_CODE||'('||DEFECT_NAME||')' from ORADDMAN.TSCIQC_DEFECT ";		  
		  sWhereC= "where DEFECT_ID is not null ";	
		  //sSqlC = sSqlC+sWhereC;	
		  if (classID !=null && !classID.equals("--")) sWhereC = sWhereC + "and CLASS_ID ='"+classID+"' ";
		  if (itemID !=null && !itemID.equals("--")) sWhereC = sWhereC + "and ITEM_ID ='"+itemID+"' ";
		  if (examineItemID!=null && !examineItemID.equals("--")) sWhereC = sWhereC + "and EXAMINE_ITEM_ID = '"+examineItemID+"' ";
		  sSqlC = sSqlC + sWhereC + sOrderC;	  
		  //out.println(sSqlC);		      
          Statement statementC=con.createStatement();
          ResultSet rsC=statementC.executeQuery(sSqlC);
          comboBoxBean.setRs(rsC);
		  if (defectID!=null && !defectID.equals("--")) comboBoxBean.setSelection(defectID);		  		  		  
	      comboBoxBean.setFieldName("DEFECT_ID");	   
          out.println(comboBoxBean.getRsString());
...
<tr bgcolor="#B1D3A9">   
 <td width="54%"><strong><font color="#FFFFFF">測試缺點項目名稱,測試缺點項目說明:</font></strong>
...
<A HREF="../jsp/TSIQCInspectDefectQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=FIRST"><font><strong><font color="#FF0080">第一頁</font></strong></font></A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectDefectQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=LAST"><font><strong><font color="#FF0080">最後一頁</font></strong></font></A><font color="#FF0080"><strong><font>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectDefectQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=100">下一頁</A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectDefectQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=-100">上一頁</A>&nbsp;&nbsp;(第<%=currentPageNumber%>&nbsp;頁/共<%=totalPageNumber%>&nbsp;頁)</font></strong></font>
<%   
  try
  {   
   Statement lotStat=con.createStatement();
   ...
   ResultSet rs=null;
   String sql=null;  
          String sqlS = "select count(*) from ORADDMAN.TSCIQC_DEFECT ";
		  String sWhereC = "where DEFECT_ID IS NOT NULL ";
		  if (classID!=null && !classID.equals("--")) sWhereC =sWhereC +" and CLASS_ID = '"+classID+"'  ";
		  if (itemID!=null && !itemID.equals("--")) sWhereC = sWhereC + "and ITEM_ID = '"+itemID+"' ";
		  if (examineItemID!=null && !examineItemID.equals("--")) sWhereC = sWhereC + "and EXAMINE_ITEM_ID = '"+examineItemID+"' ";
		  sqlS = sqlS + sWhereC;
	   ...
	   {
	      String sWhereS = sWhereC + "and ( DEFECT_NAME='"+searchString+"' ) ";
	      lotRs=lotStat.executeQuery(sqlS+" and CLASS_ID IS NOT NULL and ITEM_ID is not null and EXAMINE_ITEM_ID is NOT NULL and ( DEFECT_NAME='"+searchString+"' ) ");	 // 如果存在檢驗測試缺點項目代碼
 		  lotRs.next();
		  String sql1 = "select DEFECT_ID,DEFECT_CODE as 缺點項目代碼,DEFECT_NAME as 缺點項目名稱,DEFECT_DESC as 缺點項目描述,DEFECT_REMARK as 缺點項目備註,CREATION_DATE as 建檔日期,CREATED_BY as 建檔人員,LAST_UPDATE_DATE as 更新日期,LAST_UPDATED_BY as 更新人員 from ORADDMAN.TSCIQC_DEFECT ";
		  sql1 = sql1 + sWhereS;
	      if (lotRs.getInt(1)>0) //如果存在業務員工號
	      {	
		    //out.println("sqlStep1="+sql1+" and ( EXAMINE_ITEM_NAME='"+searchString+"' ) order by EXAMINE_ITEM_ID");    
		    rs=statement.executeQuery(sql1+" and ( DEFECT_NAME='"+searchString+"' ) order by DEFECT_ID"); 
		  } else {	
		    //out.println("sqlStep2="+sql1+" and ( EXAMINE_ITEM_NAME='"+searchString+"' ) order by EXAMINE_ITEM_ID"); 	   
            rs=statement.executeQuery(sql1+" and ( DEFECT_NAME='"+searchString+"' or DEFECT_DESC='"+searchString+"') order by DEFECT_ID");	 	 
          }			
       } else {	 
	               String sql2 = "select DEFECT_ID,DEFECT_CODE as 缺點項目代碼,DEFECT_NAME as 缺點項目名稱,DEFECT_DESC as 缺點項目描述,DEFECT_REMARK as 缺點項目備註,CREATION_DATE as 建檔日期,CREATED_BY as 建檔人員,LAST_UPDATE_DATE as 更新日期,LAST_UPDATED_BY as 更新人員 from ORADDMAN.TSCIQC_DEFECT ";
		           sql2 = sql2 + sWhereC;
				   //out.println("sqlStep3="+sql2+" order by EXAMINE_ITEM_ID");
	               rs=statement.executeQuery(sql2+" order by DEFECT_ID");
	             }	  		        
   ...
   qryAllChkBoxEditBean.setSearchKey("DEFECT_ID");
