...
<html>
<head>
<title>Query All IQC Class Information</title>
...
{   
  location.href="../jsp/TSIQCInspectClassQuery.jsp?PAGEURL="+pageURL+"&SEARCHSTRING="+document.MYFORM.SEARCHSTRING.value;
}
 ...
 try
   ...
   */
   ...
   ResultSet  rs=null;
	 ...
	 {	   
	    lotRs=lotStat.executeQuery("select count(*) from ORADDMAN.TSCIQC_CLASS where CLASS_ID IS NOT NULL and ( CLASS_NAME='"+searchString+"' )");	
		lotRs.next();
	    if (lotRs.getInt(1)>0) //若有存在批號的話
	    {		   
	       rs=statement.executeQuery("select count(*) from ORADDMAN.TSCIQC_CLASS where CLASS_ID IS NOT NULL and (CLASS_NAME='"+searchString+"' or CLASS_DESC ='"+searchString+"') ");	
	    } else {
                    rs=statement.executeQuery("select count(*) from ORADDMAN.TSCIQC_CLASS where CLASS_ID IS NOT NULL and (CLASS_NAME='"+searchString+"' or CLASS_DESC ='"+searchString+"') ");	 	 
                  } //end of lotRs if		  
     } else {
	             rs=statement.executeQuery("select count(*) from ORADDMAN.TSCIQC_CLASS where CLASS_ID IS NOT NULL ");
	           }	 
...
<strong><font color="#0080C0" size="5">IQC品管檢驗類別查詢</font></strong>(總共<%=maxrow%>&nbsp;筆記錄)</FONT><BR>
<table width="100%" border="0" cellpadding="0" cellspacing="1" bordercolorlight="#999999" bordercolordark="#FFFFFF">
  <tr bgcolor="#B1D3A9">     
	  <td width="42%"><font color="#FF0066"><strong>檢驗類別代碼: 
        </strong></font><font> 
        <%	    	     		 		 
	     try
         { 	   		 
		  ...
		  if (classID!=null && !classID.equals("--")) comboBoxBean.setSelection(classID);		  		  		  
	      comboBoxBean.setFieldName("CLASS_ID");	   
    ...
    <td width="58%"><strong><font color="#FFFFFF">檢驗類別名稱,類別說明:</font></strong>
...
<A HREF="../jsp/TSIQCInspectClassQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=FIRST"><font><strong><font color="#FF0080">第一頁</font></strong></font></A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectClassQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=LAST"><font><strong><font color="#FF0080">最後一頁</font></strong></font></A><font color="#FF0080"><strong><font>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectClassQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=100">下一頁</A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectClassQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=-100">上一頁</A>&nbsp;&nbsp;(第<%=currentPageNumber%>&nbsp;頁/共<%=totalPageNumber%>&nbsp;頁)</font></strong></font>
<%   
  try
  {   
   Statement lotStat=con.createStatement();
   ...
   String sql=null;  
	   ...
	   {
	      lotRs=lotStat.executeQuery("select count(*) from ORADDMAN.TSCIQC_CLASS where CLASS_ID IS NOT NULL and ( CLASS_NAME='"+searchString+"' ) ");	 // 如果存在檢驗類別代碼
 		  lotRs.next();
	      if (lotRs.getInt(1)>0) //如果存在業務員工號
	      {
		    rs=statement.executeQuery("select CLASS_ID,CLASS_CODE as 檢驗類別代碼,CLASS_NAME as 檢驗類別名稱,CLASS_DESC as 檢驗類別描述,CLS_REMARK as 檢驗類別備註,CREATION_DATE as 建檔日期,CREATED_BY as 建檔人員,LAST_UPDATE_DATE as 更新日期,LAST_UPDATED_BY as 更新人員 from ORADDMAN.TSCIQC_CLASS where CLASS_ID IS NOT NULL and ( CLASS_NAME='"+searchString+"' ) order by CLASS_ID"); 
		  } else {		   
            rs=statement.executeQuery("select CLASS_ID,CLASS_CODE as 檢驗類別代碼,CLASS_NAME as 檢驗類別名稱,CLASS_DESC as 檢驗類別描述,CLS_REMARK as 檢驗類別備註,CREATION_DATE as 建檔日期,CREATED_BY as 建檔人員,LAST_UPDATE_DATE as 更新日期,LAST_UPDATED_BY as 更新人員 from ORADDMAN.TSCIQC_CLASS where CLASS_ID IS NOT NULL and ( CLASS_NAME='"+searchString+"' or CLASS_DESC='"+searchString+"') order by CLASS_ID");	 	 
          }			
       } else {	 
	             if (classID!=null && !classID.equals("--"))
	             {
				    rs=statement.executeQuery("select CLASS_ID,CLASS_CODE as 檢驗類別代碼,CLASS_NAME as 檢驗類別名稱,CLASS_DESC as 檢驗類別描述,CLS_REMARK as 檢驗類別備註,CREATION_DATE as 建檔日期,CREATED_BY as 建檔人員,LAST_UPDATE_DATE as 更新日期,LAST_UPDATED_BY as 更新人員 from ORADDMAN.TSCIQC_CLASS where CLASS_ID ='"+classID+"' order by CLASS_ID");
				 } else 
				       {
					     rs=statement.executeQuery("select CLASS_ID,CLASS_CODE as 檢驗類別代碼,CLASS_NAME as 檢驗類別名稱,CLASS_DESC as 檢驗類別描述,CLS_REMARK as 檢驗類別備註,CREATION_DATE as 建檔日期,CREATED_BY as 建檔人員,LAST_UPDATE_DATE as 更新日期,LAST_UPDATED_BY as 更新人員 from ORADDMAN.TSCIQC_CLASS where CLASS_ID IS NOT NULL order by CLASS_ID");
				       }	
	          }	  		        
   ...
   qryAllChkBoxEditBean.setSearchKey("CLASS_ID");
