<!--20160401 by Peggy,增加指定嘜頭欄位-->
<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*,java.net.*,java.io.*,java.text.*,java.lang.*,oracle.sql.*,oracle.jdbc.driver.*"%>
...
<FORM ACTION="../jsp/TSLabelQuery.jsp" METHOD="post" NAME="MYFORM">
<br>
...
<BR>
  ...
  <table cellSpacing='0' cellPadding='1' width='100%' align='center' borderColorLight="#CFDAD8"  bordercolordark="#5C7671" border='1'>
     <tr>
		...
		<td width="8%" bgcolor="#D3E6F3"  style="font-size:11px;font-weight:bold;color:#006666">業務模組:</td>   
		<td width="12%">
		<%
		try
		{
	   ...
	   <td width="17%" align="center">
		    <INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>' onClick='setSubmit("../jsp/TSLabelQuery.jsp")' > 
			&nbsp;&nbsp;&nbsp;
		    <INPUT TYPE="button" align="middle"  value='新增'  style="font-family:ARIAL" onClick='setAdd("../jsp/TSLabelAdd.jsp?STATUS=NEW")' > 
	    &nbsp;&nbsp;&nbsp;</td>
   </tr>
</table>  
<hr>
...
{       	 
	...
	sql = " select  b.label_group_code"+
		  ...
		  ",d.LABEL_TYPE_NAME "+
		  ",a.SHIPPING_MARK"+
		  ",a.CHECK_FUNCTION"+ //add by Peggy 20240507
		  ",b.label_form_code||'-'|| b.label_form_version label_form"; //add by Peggy 20210122
	...
	while (rs1.next())
	{
		sql += ",(select nvl(isactive_flag,'N') from oraddman.ts_label_group_assignments x where x.label_group_code=b.label_group_code and NVL(x.isactive_flag,'N') ='Y' and x.module_code='"+rs1.getString("module_code")+"' ) AS "+rs1.getString("module_code")+"";
		area_cnt++;
	}		  
	sql +=" from oraddman.ts_label_all a,oraddman.ts_label_groups b,oraddman.ts_label_types d"+
	...
	{
		sql += " and exists (select 1 from oraddman.ts_label_group_assignments c where c.module_code = '" + MODULE_CODE +"' and c.label_group_code=b.label_group_code and NVL(c.isactive_flag,'N')='Y')";
	}
	if (STATUS.equals("Y"))
	{
	...
	sql += " order by b.label_group_code,b.label_group_name,decode(d.LABEL_TYPE_SIZE,'REEL',1,'BOX',2,'CARTON',3,4),a.label_type_code,a.label_code";
	//out.println(sql);
	Statement statement=con.createStatement(); 
	ResultSet rs=statement.executeQuery(sql);
	while (rs.next()) 
	{ 	
		...
		<table width="100%" border="1" cellpadding="1" cellspacing="0" borderColorLight="#CFDAD8"  bordercolordark="#5C7671">
			<tr bgcolor="#D3E6F3"> 
				...
				<td width="4%" rowspan="2" style="font-size:11px;color:#006666" align="center">群組代碼</td>
				<td width="10%" rowspan="2" style="font-size:11px;color:#006666" align="center">群組名稱</td>
				<td width="6%" rowspan="2" style="font-size:11px;color:#006666" align="center">文件代碼</td>
				...
				<td width="5%" rowspan="2" style="font-size:11px;color:#006666" align="center">標籤代碼</td>
				<td width="7%" rowspan="2" style="font-size:11px;color:#006666" align="center">標籤名稱</td>
				<td width="7%" rowspan="2" style="font-size:11px;color:#006666" align="center">指定嘜頭</td>
				<td width="7%" rowspan="2" style="font-size:11px;color:#006666" align="center">Check Function</td>				
				<td width="16%" colspan="5" style="font-size:11px;color:#006666" align="center">業務模組</td>
				...
				<td width="8%" rowspan="2" style="font-size:11px;color:#006666" align="center">最後異動日</td>            
				<td width="7%" rowspan="2" style="font-size:11px;color:#006666" align="center">最後異動者</td>            
			</tr>
			<tr bgcolor="#D3E6F3"> 
		<%
			...
			{
		%>
				<td style="font-size:11px;color:#006666" align="center"><%=rs1.getString("module_code")%></td>
		<%
			}
			rs1.close();   
			statement1.close();		
    	...
    	%>
			...
			<td align="center">
			<img border="0" src="images/updateicon_enabled.gif" height="20" title="修改資料" onClick="setUpdate('../jsp/TSLabelAdd.jsp?STATUS=UPD&LABEL_CODE=<%=rs.getString("label_code")%>')">&nbsp;&nbsp;&nbsp;<img border="0" src="images/deletion.gif" height="14" title="刪除資料" onClick="setDelete('../jsp/TSLabelAdd.jsp?LABEL_CODE=<%=rs.getString("label_code")%>&ACTYPE=DELETE')">
			</td>
			...
			<td><%=(rs.getString("SHIPPING_MARK")==null?"&nbsp;":rs.getString("SHIPPING_MARK"))%></td>
			<td><%=(rs.getString("CHECK_FUNCTION")==null?"&nbsp;":rs.getString("CHECK_FUNCTION"))%></td>			
			<% 
			for (int i = 1 ; i <=area_cnt ; i++)
			{
			%>
				<td align="center"><%=(rs.getString(16+(i-1))==null?"&nbsp;":rs.getString(16+(i-1)))%></td>
			<%
			}
			%>
