...
<head>
    ...
    <%-- include 放在 head 內 --%>
    <jsp:include page="dateCodeUpdate.jsp"/>
    <jsp:include page="dateCodeConfirm.jsp"/>
...
<%--            </button>--%>
    ...
    %>
            <div class="alert alert-danger alert-dismissible fade show custom-alert" role="alert">
                <%=dateCodeRule.errList2String(errList)%>
...
<script>
    ...
    if (<%=dateCodeList.size() > 0%>) {
        query(<%=dateCodeRule.convertToJson(dateCodeList)%>);
    }
    ...
    function generateColumns(headers) {
        return headers.map(header => ({
            ...
            title: header === 'dateCode' ? 'Date Code':
                   ...
                   header === 'markingDesc' ? 'Marking Desc':
