...
<%@ page import="bean.DateBean,bean.SendMailBean,bean.WorkingDateBean,CodeUtil" %>
<html>
<head>
...
<%
    ...
    String sSqlMailUser = "select DISTINCT USERMAIL,USERNAME from ORADDMAN.WSUSER, ORADDMAN.WSGROUPUSERROLE where USERNAME=GROUPUSERNAME and ROLENAME in ('admin','sysaudit') and LOCKFLAG='N' and USERNAME = 'jingker' ";
	...
	while(rsMail.next())
	{
	   try 
          {		     
		   ...
		   urAddress = serverHostName+":8080/oradds/jsp/TSCInvI7SubInventoryNBReport.jsp?DATEBEGIN="+strFirstDayWeek+"&DATEEND="+strLastDayWeek;		
		   urAddress1 = serverHostName+":8080/oradds/jsp/TSCInvI8SubInventoryNBReport.jsp?DATEBEGIN="+strFirstDayWeek+"&DATEEND="+strLastDayWeek;
		   urAddress2 = serverHostName+":8080/oradds/jsp/TSCInvI9SubInventoryNBReport.jsp?DATEBEGIN="+strFirstDayWeek+"&DATEEND="+strLastDayWeek;
        ...
        //   sendMailBean.setSubject(CodeUtil.unicodeToBig5("來自ORACLE系統的郵件:每週料件分類設定異常檢核表-("+strFirstDayWeek+"~"+strLastDayWeek+")")); 
		   sendMailBean.setSubject(CodeUtil.unicodeToBig5("Oracle ERP Alert E-Mail-Weekly Organization Subinventory Balance Error List("+strFirstDayWeek+"~"+strLastDayWeek+")"));         
		   sendMailBean.setUrlName("Dear "+UserID+",\n"+CodeUtil.unicodeToBig5("   請點擊來自ORACLE系統的郵件:每週I7倉庫存異常檢核表-("+strFirstDayWeek+"~"+strLastDayWeek+")"));     		   
		   //System.out.println("UserID="+UserID);
	       sendMailBean.setUrlAddr(urAddress);		   
		   //System.out.println("userMail="+userMail);
		   sendMailBean.setUrlName1("\n"+CodeUtil.unicodeToBig5("   每週I8倉庫存異常檢核表-("+strFirstDayWeek+"~"+strLastDayWeek+")")+""); 
		   sendMailBean.setUrlAddr1(urAddress1);
		   sendMailBean.setUrlName2("\n"+CodeUtil.unicodeToBig5("   每週I9倉庫存異常檢核表-("+strFirstDayWeek+"~"+strLastDayWeek+")")+""); 
