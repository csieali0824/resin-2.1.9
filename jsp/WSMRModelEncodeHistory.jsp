...
<%@ page import="bean.QryAllChkBoxEditBean,bean.ComboBoxAllBean,bean.ComboBoxBean,bean.DateBean,bean.ArrayComboBoxBean,bean.ArrayListCheckBoxBean"%>
...
function mrModelNoHistory(appNo,pp)
{    
  subWin=window.open("WSModelNoHistoryInquiry.jsp?APPNO="+appNo+"&MODELNO="+pp);  
}
</script>
<meta http-equiv="Content-Type" content="text/html; charset=big5">
</head>
<body>
<font color="#3366FF" size="+2" face="Arial"><font size="+3" face="Arial Black"><font color="#3366FF" size="+2" face="Arial"><font size="+3" face="Arial Black"><font face="Courier, MS Sans Serif"><font color="#54A7A7" size="+2" face="Arial Black">DBTEL</font></font></font></font></font></font><font color="#000000" size="+2" face="Times New Roman"><strong>產品編碼系統資料查詢</strong></font>
...
<table width="100%" border="1" cellpadding="0" cellspacing="0">
  <tr bgcolor="#FFFFFF">    
    <td width="33%" bordercolor="#FFFFFF" nowrap><font color="#0000AA"><strong>產品類別 :</strong></font>
        <% 		 		 		 
	     try
         { 
          //String adminRole = "";
            
		  String sSql = "select PROD_CLASS, PCLS_CODE||'('||PROD_CLASS||')' from MRPRODCLS where PROD_CLASS IS NOT NULL ";		      
          Statement statement=con.createStatement();
          ResultSet rs=statement.executeQuery(sSql);
		  comboBoxAllBean.setRs(rs);		  
		  comboBoxAllBean.setSelection(prodCls);
	      comboBoxAllBean.setFieldName("PRODCLS");	   
          out.println(comboBoxAllBean.getRsString());
		  /*
          out.println("<select NAME='PRODCLS' onChange='setSubmit("+'"'+"../jsp/WSMRModelEncodeHistory.jsp"+'"'+")'>");
    ...
    <td width="33%" bordercolor="#FFFFFF" nowrap><font color="#0000AA"><strong>申請日 起 :</strong></font>
        <%
    ...
    <td width="35%" bordercolor="#FFFFFF" nowrap><font color="#0000AA"><strong>申請日 迄:</strong></font>
        <%
  ...
  <tr>
   <td width="33%" bordercolor="#FFFFFF" nowrap><font color="#0000AA"><strong>產品編號 : <input name="MODELNO" type="text" value="<%=modelNo%>" size="20" maxlength="15"></strong></font></td>
   <td width="33%" bordercolor="#FFFFFF" nowrap><font color="#0000AA"><strong>PM管理人員 :
      <% 		 		 		 
	     try
         { 
          //String adminRole = "";
            
		  String sSql = "select WEBID, WEBID||'('||USER_NAME||')' from MRPM_USER where WEBID IS NOT NULL ";		      
          Statement statement=con.createStatement();
          ResultSet rs=statement.executeQuery(sSql);
		  comboBoxAllBean.setRs(rs);		  
		  comboBoxAllBean.setSelection(PMUser);
	      comboBoxAllBean.setFieldName("PMUSER");	   
          out.println(comboBoxAllBean.getRsString());
		  /*
          out.println("<select NAME='PRODCLS' onChange='setSubmit("+'"'+"../jsp/WSMRModelEncodeHistory.jsp"+'"'+")'>");
          out.println("<OPTION VALUE=-->--");     
       ...
       %></strong></font></td>
   <td width="33%" bordercolor="#FFFFFF" nowrap><input name="submit1" type="submit" value="QUERY" onClick='return setSubmit("../jsp/WSMRModelEncodeHistory.jsp")'></td>
  </tr>
</table>
<table width="100%" border="1" cellspacing="0" cellpadding="0">
        <tr bgcolor="#FFFFCC">                    
		  <td width="2%"  height="19" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong></strong></font></td>
          <td width="3%"  height="19" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>明細</strong></font></td>    
          <td width="12%"  height="19" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>產品申請單號</strong></font></td>
          <td width="11%" bgcolor="#000099"  nowrap><font color="#FFFFFF" size="2"><strong>產品編號</strong></font></td>
          <td width="6%" bgcolor="#000099"  nowrap><font color="#FFFFFF" size="2"><strong>產品類別</strong></font></td> 
		  <td width="6%" bgcolor="#000099"  nowrap><font color="#FFFFFF" size="2"><strong>申請日期</strong></font></td>		  		         
		  <td width="6%" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>申請人員</strong></font></td>		  
		  <td width="8%" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>外部型號</strong></font></td>
		  <td width="8%" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>專案啟動日期</strong></font></td>
          <td width="8%" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>專案截止日期</strong></font></td>    
          <td width="7%" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>研發單位</strong></font></td>          
          <td width="7%" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>產品規格附件</strong></font></td>
          <td width="7%" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>產品外觀附件</strong></font></td>
          <td width="16%" bgcolor="#000099" nowrap><font color="#FFFFFF" size="2"><strong>備註</strong></font></td>       
        </tr> 
          <%  
            try
            {  
              ...
              String sqlTC =  "select APPNO, MODELNO, PRODCLS, APPLY_DATE, APPLY_USER, SALESCODE, PJTTNON_DATE,PJTTNOFF_DATE, RDESIGN_DPT, "+                              
			                  "MVERSION, REMARK, ENUSER  "+
			                  "from MRMODELAPP ";
			  String sWhereTC = "where PID in('MO','MZ') ";
                                //"and TTYPE in('S') and TTDTE >= 20040524 ";     // ?????????
	 
              if (prodCls == null || prodCls.equals("--")) { sWhereTC = sWhereTC + "and PRODCLS != '0' "; } // {sWhereTC = sWhereTC + "and b.TXRPCENTERNO != '0'  "; }
			  else { sWhereTC = sWhereTC + "and PRODCLS ='"+prodCls+"'  "; }
              if (modelNo == null || modelNo.equals("")) { sWhereTC = sWhereTC + "and MODELNO !='0' "; } // {sWhereTC = sWhereTC + "and b.TXRPCENTERNO != '0'  "; }
			  else { sWhereTC = sWhereTC + "and MODELNO like '%"+modelNo+"%'  "; }
              if (appNo == null || appNo.equals("")) { sWhereTC = sWhereTC + "and APPNO != '0' "; } // {sWhereTC = sWhereTC + "and b.TXRPCENTERNO != '0'  "; }
			  else { sWhereTC = sWhereTC + "and APPNO = '"+appNo+"'  "; }  
              if (PMUser == null || PMUser.equals("--") || PMUser.equals("") ) { sWhereTC = sWhereTC + "and APPLY_USER !='0' "; } // {sWhereTC = sWhereTC + "and b.TXRPCENTERNO != '0'  "; }
			  ...
			  String sOrderTC = "order by APPNO, APPLY_DATE, MVERSION ";
    ...
    <%
      ...
      if (request.getParameterNames().hasMoreElements()) {
        ...
        <tr bgcolor="#FFFFFF">                  
          ...
          <td bgcolor="#FFFFFF">
		      <div align="center"><font size="-1" color="#FFFFFF">
              <a href="../jsp/WSModelNoAppDisplayPage.jsp?CALLMODE=<%=callMode%>&MRAPPNO=<%=rsTC.getString("APPNO")%>">                       
		              <div align="center"><img src="../image/docicon.gif" width="14" height="15" border="0"></div></a> 
              </font></div></td>   
          <td height="20"><font size="2" color="#000099">                      
                 <% 
                      if (rsTC.getString("APPNO")!=null ) { out.println(rsTC.getString("APPNO")); } 
                      else { out.println("&nbsp;"); }
                 %></font></td>		 
          <td><font size="2" color="#000000">
                 <a href='javaScript:mrModelNoHistory("<%=rsTC.getString("APPNO")%>","<%=rsTC.getString("MODELNO")%>")' >
                 <%    //out.println(rsTC.getString("MODELNO")+ "   "); 
                       int modelLength = rsTC.getString("MODELNO").length();
                       if (modelLength<=8)
                       { out.println("<font face='Arial'>"+rsTC.getString("MODELNO").substring(0,3)+"</font>"+"<font face='Arial' size='+2' color='#FF0000'><em>"+rsTC.getString("MODELNO").substring(3,7)+"</em></font>"+"<font face='Arial'>"+rsTC.getString("MODELNO").substring(7,8)+"</font>"); }
                       else if (modelLength==9){ out.println("<font face='Arial'>"+rsTC.getString("MODELNO").substring(0,3)+"</font>"+"<font size='+2' face='Arial' color='#FF0000'><em>"+rsTC.getString("MODELNO").substring(3,7)+"</em></font>"+"<font face='Arial'>"+rsTC.getString("MODELNO").substring(7,9)+"</font>"); } 
                       else if (modelLength==10){ out.println("<font face='Arial'>"+rsTC.getString("MODELNO").substring(0,3)+"</font>"+"<font size='+2' face='Arial' color='#FF0000'><em>"+rsTC.getString("MODELNO").substring(3,7)+"</em></font>"+"<font face='Arial'>"+rsTC.getString("MODELNO").substring(7,10)+"</font>");  }                                                                        
                       //if (rsTC.getString("MODELNO")!=null) { out.println(rsTC.getString("MODELNO").substring(0,4)+"<font size='+2'><em>"+rsTC.getString("MODELNO").substring(4,8)+"</em></font>"+rsTC.getString("MODELNO").substring(8,10)); } 
                      // else { out.println("&nbsp;"); }
                       //=rsTC.getString("TCOM") 
                 %></a></font></td>
          <td><font size="2" color="#000000">
                  <%
                       if (rsTC.getString("PRODCLS")!=null) { out.println(rsTC.getString("PRODCLS")); } 
                       else { out.println("&nbsp;"); }
          ...
          <td><font size="2" color="#000000">
                 <%
                     ...
                     //  else { out.println("&nbsp;"); }
                     String sqlBlob = "select APPNO, SPECFILE_NAME,IMAGEVIEW_NAME from MRMODEL_DOCMNT where APPNO='"+rsTC.getString("APPNO")+"' ";		      
                     Statement stateBlob=con.createStatement();
                     ResultSet rsBlob=stateBlob.executeQuery(sqlBlob);
                     if (rsBlob.next())
                     { 
                       if (rsBlob.getString("SPECFILE_NAME")==null || rsBlob.getString("SPECFILE_NAME").equals("")) { out.println("&nbsp;"); }   
                       else { out.println("<font color='#003399' face='Arial'>"+rsBlob.getString("SPECFILE_NAME")+"</font>"); } 
                     }
                     else { out.println("&nbsp;"); }
                     rsBlob.close();
                     stateBlob.close();  
          ...
          <td><font size="2" color="#000000">
                 <%
                      String sqlBlob2 = "select APPNO, SPECFILE_NAME,IMAGEVIEW_NAME from MRMODEL_DOCMNT where APPNO='"+rsTC.getString("APPNO")+"' ";		      
                      Statement stateBlob2=con.createStatement();
                      ResultSet rsBlob2=stateBlob2.executeQuery(sqlBlob2);
                      if (rsBlob2.next())
                      { 
                        if (rsBlob2.getString("IMAGEVIEW_NAME")==null || rsBlob2.getString("IMAGEVIEW_NAME").equals("")) { out.println("&nbsp;"); }    
                        else { out.println("<font color='#003399' face='Arial'>"+rsBlob2.getString("IMAGEVIEW_NAME")+"</font>"); } 
                      }
                      else { out.println("&nbsp;"); }
                      rsBlob2.close();
                      stateBlob2.close();
