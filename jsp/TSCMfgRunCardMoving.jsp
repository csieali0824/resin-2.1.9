<!-- 20090426 liling 將for 人工打入異動日期,以原'作業員'RES_EMPLOYEE_OP 欄位做為異動日期-->
<!-- 20110902 liling update AR_CUSTOMERS_V ->AR_CUSTOMERS for R12 issue  -->
<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*"%>
<html>
<head>
<title>MFG System Run Card Moving Page</title>
<!--=============以下區段為安全認證機制==========-->
...
<jsp:useBean id="arrMFGRCMovingBean" scope="session" class="bean.Array2DimensionInputBean"/>
<jsp:useBean id="arrMFGMovResBean" scope="session" class="bean.Array2DimensionInputBean"/>
...
function setSubmit(URL,ms1,ms2)
{
	var linkURL = "#ACTION";
   	if (document.DISPLAYREPAIR.ACTIONID.value==null || document.DISPLAYREPAIR.ACTIONID.value=="--")
   	{
...
function setSubmitQtyToMove(URL, xINDEX, xInQueueQty, xScrapQty, xInInputQty, xWKClass, xWKHour, xWKMachine, xWKEmployee, xOverValue, xRtOverValue, xPrevQty, xRunCardQty, woType, destStdDesc, prevOpSeqNum, prevOpDescFlag, xMACHINEWKHour)
{  
	//xQUEUEQTY ,本次移站數量
	...
	if (xRESOURCEMACHINEQTY != "*")
  	{
		...
		if (eval(xRESOURCEMACHINEQTY)>=80 )   //Liling 2007/08/30 add 因yew老是超打工時,故提出警示需求
		{
			alert("請注意此批機器工時已大於80小時 !!!");
			eval(formRESOURCEMACHINEQTY);
			return false;
		} 
	}
	...
	if (xRESOURCEQTY != "*")
  	{
		...
		if (eval(xRESOURCEQTY)>=80 )   //Liling 2007/08/30 add 因yew老是超打工時,故提出警示需求
		{
			alert("請注意此批"+fieldname+"工時已大於80小時 !!!");
			eval(formRESOURCEQTY);
			return false;
		} 
	}
	...
	txt4=xRESEMPLOYEE;	 //檢查移站日期是否為數字 20090426
	...
	if (txt4.length < 8 || txt4.length > 8 )  //檢查日期長度 20090426
   	{
    	alert("移站日期錯誤!!!");
	...
	else
	{
 	...
 	var calProcess = Math.round((eval(xQUEUEQTY)+eval(xSCRAPQTY))*1000)/1000;
  	// 若不是第一站,則限制 良品數 + 報廢數 不得大於 處理數
  	if (calProcess!= xINPUTQTY )
  	{
  	...
  	document.DISPLAYREPAIR.action=URL+"&QUEUEQTY"+xINDEX+"="+xQUEUEQTY+"&INPUTQTY"+xINDEX+"="+xINPUTQTY+"&WKCLASS"+xINDEX+"="+xWKCLASS+"&RESOURCEQTY"+xINDEX+"="+xRESOURCEQTY+"&RESMACHINE"+xINDEX+"="+xRESMACHINE+"&RESEMPLOYEE"+xINDEX+"="+xRESEMPLOYEE+"&RUNCARDID="+xINDEX+"&RESOURCEQTY_MACHINE"+xINDEX+"="+xRESOURCEMACHINEQTY+linkURL;  
  	document.DISPLAYREPAIR.submit();    
}

function setTabNext(tabNextIndex, URL, xINDEX, xInQueueQty, xScrapQty, xInInputQty, xWKClass, xWKHour, xWKMachine, xWKEmployee , xOverValue, xRtOverValue, xPrevQty, xRunCardQty, woType, destStdDesc, prevOpSeqNum, prevOpDescFlag , xMACHINEWKHour)
{ 
	formQUEUEQTY = "document.DISPLAYREPAIR.QUEUEQTY"+xINDEX+".focus()";
  	formQUEUEQTY_Write = "document.DISPLAYREPAIR.QUEUEQTY"+xINDEX+".value";
  	xQUEUEQTY = eval(formQUEUEQTY_Write);  // 把值取得給java script 變數(良品數)
	...
	catch(err)
	{
  	...
  	var calInQueueQty = Math.round((eval(xQUEUEQTY)+eval(xSCRAPQTY))*1000)/1000; // 若是第一站,則 處理數 = 良品數 + 報廢數
   	...
   	if (event.keyCode==13) // event.keycode = 13 --> Enter 鍵
   	{
		if (tabNextIndex=="1")
	  	{	     
			...
			else 
			{
				...
				else if (xQUEUEQTY>xInQueueQty) // 若 良品數 > 處理數,則自動計算報廢數
				{
		...
		{
			setSubmitQtyToMove(URL, xINDEX, xInQueueQty, xScrapQty, xInInputQty, xWKClass, xWKHour, xWKMachine, xWKEmployee, xOverValue, xRtOverValue, xPrevQty , xRunCardQty, woType, destStdDesc, prevOpSeqNum, prevOpDescFlag, xMACHINEWKHour);
		}
   	} 
...
function setOSPSubmit(URL,ms1)
{
	...
	if (document.DISPLAYREPAIR.ACTIONID.value=="023")  //OSPROCESS表示為確認移站轉至OSP動作
  	{
    	flag=confirm(ms1);      
        if (flag==false) return(false);
		else 
...
function subWinInspLabelInfo(oeOrderNo,custNo,custName,custLabelTmp,runCardNo)
{  
	subWin=window.open("../jsp/subwindow/TSMfgInspCustLabelInfo.jsp?MONO="+oeOrderNo+"&CUSTNO="+custNo+"&CUSTNAME="+custName+"&CUSTLABELTMP="+custLabelTmp+"&RUNCARDNO="+runCardNo,"subwin","width=640,height=480,status=yes,scrollbars=yes,menubar=yes");  
}
</script>
...
String overValue="",prevValue=""; //超額完工數  , 前站移站*超出率
...
<FORM NAME="DISPLAYREPAIR" onsubmit='return submitCheck("取消確認","是否送出","請選擇執行動作")' ACTION="../jsp/TSCMfgRCMovingMProcess.jsp?WO_NO=<%=woNo%>&RUNCARD_NO=<%=runCardNo%>" METHOD="post">
<!--=============以下區段為取得工令設立基本資料==========-->
...
boolean ospPriceRevise = false;    // 20100320 Marvie Add : OSP DEXI price revise
try
{
	String sqlJudgeOSP = "";
   	if (jobType==null || jobType.equals("1"))
   	{
		...
		//			 "   and d.disable_date is null "+   //by fenggp 排出已失效的德錫外報站
	...
	else if (jobType.equals("2"))
    {
		sqlJudgeOSP =" select d.DESCRIPTION , d.COST_CODE_TYPE "+
					 ...
					 " and a.NEXT_OP_SEQ_NUM = b.OPERATION_SEQ_NUM "+
					 " and b.RESOURCE_SEQ_NUM != 10 "+   // 對重工工單,10為固定人工資源
					 " and d.ORGANIZATION_ID = '"+organizationId+"' "; 
	...
	{ 			
		...
		ospCheckFlag = true;
		// 20100320 Marvie Add : OSP DEXI price revise
		Statement stateRevise=con.createStatement();
		ResultSet rsRevise=stateRevise.executeQuery("select global_attribute1 from po_system_parameters_all where org_id = 325");
		if (rsRevise.next())
		{ 
			if (rsRevise.getString(1) != null && (rsRevise.getString(1) == "Y" || rsRevise.getString(1).equals("Y"))) ospPriceRevise = true;
		}
		if (ospPriceRevise)
		{
%>
...
<%
	...
	else if (stdOpDesc!=null && stdOpDesc.indexOf("包裝")>=0)
	{
%>
            <table cellSpacing="1" bordercolordark="#B5B89A" cellPadding="1" width="97%" align="center" bordercolorlight="#FFFFFF"  border="0">
             <tr bgcolor="#CCCC99"> 
	          <td width="5%" nowrap>
	           <font color="#000066">
	           後段包裝站客戶標籤資訊<input type='button' name='SUBLOTCH' value='...' onClick='subWinFrontLotFind("<%=entityId%>","<%=primaryItemID%>","<%=woQty%>")'>             
	           </font>
	          </td>
              <td colspan="3">	  
	             <!--%<div align="center"><img src="/oradds/jsp/subwindow/TSMfgCustLabelBarCodeDraw.jsp?DATA=<%=runCardNo%>&CODE=<%="3"%>&WIDTH=0.5&HEIGHT=25" align="middle"/></div>%-->
	          </td>
	         </tr>
            </table>
<%
...
<%
	...
	out.println("<td>");	  
	out.println("<font color='#990000'>流程卡號</font></td>");
	...
	String keydownlink= "";
	int destStdDesc = stdOpDesc.indexOf("目檢");
	...
	while (rs.next())
	{
		...
		{ 
			if (stdOpDesc!=null && stdOpDesc.indexOf("目檢")>=0)
	        {  
		    	defToMoveQty = runcardQty; // 目檢站,預設為流程卡數
			} 
			else 
			{ 
		...
		out.println("<INPUT TYPE='button' value='Set' onClick='setSubmitQtyToMove("+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+","+'"'+rs.getString("RES_MACHINE_WKHOUR_OP")+'"'+")'>");   
		out.println("</TD>");     // 按鈕 Set		
		out.print("<TD nowrap>"); // 移站數量	(良品數)	
		keydownlink = "setTabNext('1',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+","+'"'+rs.getString("RES_MACHINE_WKHOUR_OP")+'"'+")";
		if (runCardID==rs.getString("RUNCAD_ID") || runCardID.equals(rs.getString("RUNCAD_ID"))) // 若是處理項次,則予此次給定comments
		{ 
			//out.print("<input name='QUEUEQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='"+queueQty+"' onfocus=this.select() size=5 onKeyDown=setTabNext('1',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
		...
		{ 
			...
			{
				//out.print("<input name='QUEUEQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='' onfocus=this.select() size=5 onKeyDown=setTabNext('1',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>");  
				out.print("<input name='QUEUEQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='' onfocus=this.select() size=5 onKeyDown="+keydownlink+">");
			}
			else
			{
				//out.print("<input name='QUEUEQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='"+defToMoveQty+"' onfocus=this.select() size=5 onKeyDown=setTabNext('1',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
		...
		keydownlink = "setTabNext('10',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+","+'"'+rs.getString("RES_MACHINE_WKHOUR_OP")+'"'+")";
		if (runCardID==rs.getString("RUNCAD_ID") || runCardID.equals(rs.getString("RUNCAD_ID"))) // 若是處理項次,則予此次給定comments
		{ 
			//out.print("<input name='SCRAPQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='"+scrapQty+"'  onfocus=this.select() size=5 onKeyDown=setTabNext('10',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
		...
		{ 
			...
			{
				//out.print("<input name='SCRAPQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='"+rs.getString("QTY_IN_SCRAP")+"' onfocus=this.select() size=5 onKeyDown=setTabNext('10',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
			...
			else 
			{
				...
				{ // 若 良品數+報廢數 != 處理數, 則畫面顯示之報廢數 = 0
					//out.print("<input name='SCRAPQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='0' onfocus=this.select() size=5 onKeyDown=setTabNext('10',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>");
		...
		keydownlink = "setTabNext('2',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+","+'"'+rs.getString("RES_MACHINE_WKHOUR_OP")+'"'+")";
		if (runCardID==rs.getString("RUNCAD_ID") || runCardID.equals(rs.getString("RUNCAD_ID"))) // 若是處理項次,則予此次給定comments
		{ 
			//out.print("<input name='INPUTQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='"+inputQty+"' size=5 readonly class='koko' onKeyDown=setTabNext('2',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
		...
		{ 
			...
			{
				//out.print("<input name='INPUTQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='"+rs.getString("QTY_IN_QUEUE")+"' size=5 readonly class='koko' onKeyDown=setTabNext('2',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>");  
		...
		{
			out.println("<TD nowrap>");
			keydownlink = "setTabNext('4',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+","+'"'+rs.getString("RES_MACHINE_WKHOUR_OP")+'"'+")";
			if (runCardID==rs.getString("RUNCAD_ID") || runCardID.equals(rs.getString("RUNCAD_ID"))) // 若是處理項次,則予此次給定comments
			{ 
		...
		{
			out.println("<TD nowrap>");
			keydownlink = "setTabNext('5',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+","+'"'+rs.getString("RES_MACHINE_WKHOUR_OP")+'"'+")";
			if (runCardID==rs.getString("RUNCAD_ID") || runCardID.equals(rs.getString("RUNCAD_ID"))) // 若是處理項次,則予此次給定comments
			{ 
				//out.print("<input name='RESOURCEQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='"+resourceQty+"' size=5 onKeyDown=setTabNext('4',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
			...
			{ 
				...
				{
					//out.print("<input name='RESOURCEQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='' size=5 onKeyDown=setTabNext('4',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>");  
					out.print("<input name='RESOURCEQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='' size=5 onKeyDown="+keydownlink+">");
				}
				else
				{
					//out.print("<input name='RESOURCEQTY"+rs.getString("RUNCAD_ID")+"' type='text' value='"+rs.getString("RES_WKHOUR_OP")+"' size=5 onKeyDown=setTabNext('4',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
		...
		//異動日期_起  20090426 update by liling
		out.println("<TD nowrap>");
		keydownlink ="setTabNext('6',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+","+'"'+rs.getString("RES_MACHINE_WKHOUR_OP")+'"'+")";
		if (runCardID==rs.getString("RUNCAD_ID") || runCardID.equals(rs.getString("RUNCAD_ID"))) // 若是處理項次,則予此次給定comments
		{ 
			//out.print("<input name='RESEMPLOYEE"+rs.getString("RUNCAD_ID")+"' type='text' value='"+resEmployee+"' size=6 onKeyDown=setTabNext('5',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
		...
		{ 
			...
			{
				//out.print("<input name='RESEMPLOYEE"+rs.getString("RUNCAD_ID")+"' type='text' value='"+systemDate+"' size=6 onKeyDown=setTabNext('5',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>");  
				out.print("<input name='RESEMPLOYEE"+rs.getString("RUNCAD_ID")+"' type='text' value='"+systemDate+"' size=7 onKeyDown="+keydownlink+">");
			}
			else
			{
				//out.print("<input name='RESEMPLOYEE"+rs.getString("RUNCAD_ID")+"' type='text' value='"+rs.getString("RES_EMPLOYEE_OP")+"' size=6 onKeyDown=setTabNext('5',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
		...
		// 機台編號_起
		out.println("<TD nowrap>");
		keydownlink = "setTabNext('7',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+","+'"'+rs.getString("RES_MACHINE_WKHOUR_OP")+'"'+")";
		if (runCardID==rs.getString("RUNCAD_ID") || runCardID.equals(rs.getString("RUNCAD_ID"))) // 若是處理項次,則予此次給定comments
		{ 
			//out.print("<input name='RESMACHINE"+rs.getString("RUNCAD_ID")+"' type='text' value='"+resMachine+"' size=5 onKeyDown=setTabNext('6',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
		...
		{ 
			...
			{
				//out.print("<input name='RESMACHINE"+rs.getString("RUNCAD_ID")+"' type='text' value='' size=5 onKeyDown=setTabNext('6',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>");  
				out.print("<input name='RESMACHINE"+rs.getString("RUNCAD_ID")+"' type='text' value='' size=5 onKeyDown="+keydownlink+">");
			}
			else
			{
				//out.print("<input name='RESMACHINE"+rs.getString("RUNCAD_ID")+"' type='text' value='"+rs.getString("RES_MACHINE_OP")+"' size=5 onKeyDown=setTabNext('6',"+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARDID="+rs.getString("RUNCAD_ID")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("RUNCAD_ID")+'"'+","+'"'+rs.getString("QTY_IN_QUEUE")+'"'+","+'"'+rs.getString("QTY_IN_SCRAP")+'"'+","+'"'+rs.getString("QTY_IN_INPUT")+'"'+","+'"'+rs.getString("RES_WKCLASS_OP")+'"'+","+'"'+rs.getString("RES_WKHOUR_OP")+'"'+","+'"'+rs.getString("RES_MACHINE_OP")+'"'+","+'"'+rs.getString("RES_EMPLOYEE_OP")+'"'+","+'"'+overValue+'"'+","+'"'+rtOverValue+'"'+","+'"'+prevQty+'"'+","+'"'+runcardQty+'"'+","+'"'+woType+'"'+","+'"'+destStdDesc+'"'+","+'"'+prevOpSeqNo+'"'+","+'"'+prevOpDescFlag+'"'+")>"); 
        	...
        	try
           	{
            	String sqlOPRes = "";
              	if (jobType==null || jobType.equals("1"))
              	{
					...
					//		  "        a.PRINCIPLE_FLAG, a.ASSIGNED_UNITS, b.UNIT_OF_MEASURE, b.AUTOCHARGE_TYPE "+
                    //         " from BOM_OPERATION_RESOURCES a, BOM_RESOURCES b, BOM_DEPARTMENT_RESOURCES c, BOM_OPERATION_SEQUENCES d "+
                    //         " where a.RESOURCE_ID = b.RESOURCE_ID and b.RESOURCE_ID = c.RESOURCE_ID "+
					//          "   and c.DEPARTMENT_ID = d.DEPARTMENT_ID "+
					//          "   and a.OPERATION_SEQUENCE_ID = d.OPERATION_SEQUENCE_ID "+
				...
				{ 	
					...
					arrMFGMovResBean.setArray2DString(r);
              	}
              	rsOPRes.close();
              	stateOPRes.close();	   
          	} //end of try
	...
	{ 
		String sql = "update APPS.YEW_RUNCARD_ALL set QTY_IN_TOMOVE=?, PREVIOUS_OP_SEQ_ID=?, NEXT_OP_SEQ_ID=?,RES_EMPLOYEE_OP=? ,RES_WKHOUR_OP=?,RES_MACHINE_WKHOUR_OP=? ,RES_WKCLASS_OP =?,LAST_UPDATED_BY = ?, LAST_UPDATE_DATE=to_char(SYSDATE,'yyyymmddhh24miss')"+
		             " where WO_NO='"+woNo+"' and RUNCAD_ID='"+runCardID+"' ";
	    PreparedStatement pstmt=con.prepareStatement(sql);  
        ...
        pstmt.setString(4,resEmployee);  // 異動日期
		pstmt.setString(5,resourceQty);  //人工工時,add by Peggy 20120313
		pstmt.setString(6,resourceQty_machine);  //機器工時,add by Peggy 20120313
		pstmt.setString(7,wkClass);  //班別,add by Peggy 20120313
		pstmt.setInt(8,Integer.parseInt(userMfgUserID));     //add by peggy 20120315  
		pstmt.executeUpdate(); 
        pstmt.close();
...
{
	out.println("Exception erra:"+e.getMessage());
}
...
try
{
	...
	{  //  本站為最後一站,則可執行Complete
		...
		{ //不為最後一站,則繼續可執行Transfer
	        sqlAction = "select DISTINCT x1.ACTIONID,x2.ACTIONNAME from ORADDMAN.TSWORKFLOW x1,ORADDMAN.TSWFACTION x2 WHERE FORMID='WO' AND FROMSTATUSID='"+frStatID+"' AND x1.ACTIONID=x2.ACTIONID and  x1.LOCALE='"+locale+"' and x2.ACTIONID = '006' "; 	   
		}	           
	}
	// 20100320 Marvie Add : OSP DEXI price revise
	if (ospPriceRevise)  sqlAction = sqlAction + " and 1=2";
    Statement statement=con.createStatement();
    ResultSet rs=statement.executeQuery(sqlAction);
	out.println("<select NAME='ACTIONID' onChange='setSubmit1("+'"'+"../jsp/TSCMfgRunCardMoving.jsp?WO_NO="+woNo+"&RUNCARD_NO="+runCardNo+"&EXPAND="+expand+'"'+")'>");				  				  
	out.println("<OPTION VALUE=-->--");     
	while (rs.next())
	{            
	...
	rs.next();
   	if (rs.getInt(1)>0 && !ospPriceRevise)
	{
		if (!ospCheckFlag) // 若下一站不為委外加工站,則呼叫正常移站處理頁面_起
		{
%>
			<INPUT TYPE='button' NAME='submit2' value='Submit' onClick='setSubmit("../jsp/TSCMfgRCMovingMProcess.jsp?WO_NO=<%=woNo%>&RUNCARD_NO=<%=runCardNo%>","確認流程卡移站?","請選擇其中一筆流程卡移站!!!")'>
<%
		} 
		else 
		{
