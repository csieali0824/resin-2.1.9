...
function setSubmit(URL)
{
   ...
   {     
     ...
     { 
      alert("Before you want to add , please do not let the Monthly EXW price of forecast data be Null !!");   
      return(false);
      } 
   ...
   {     
     ...
     { 
	  c=txt.charAt(j);
	    ...
	    {
            alert("Monthly EXW price should be numerical!!");   
            return(false);
		 }
      } 
   } //end of for  null check
...
{    
   ...
   {     
      for (j=1;j<dim2;j++)
	  {
		 ...
		 { 
           alert("Before you want to save , please do not let the Monthly EXW price  detail be Null !!");   
           return(false);
		 }  
	  } //enf for of jj             
   } //end of for null check
      ...
      for (k=1;k<dim2;k++)
	  {
         ...
         { 
	       c=txt.charAt(j);
	        if ("0123456789.".indexOf(c,0)<0) 
	       {
             alert("Monthly EXW price of forecast detail should be numerical!!");   
             return(false);
		    }
         }
	   } //enf for of k
...
<title>Monthly Exworks Price Input Form</title>
</head>
<body>
...
<strong>Ship-out Price of Forecast Input (Detail)</strong></font> 
...
{    
  ...
  if ( bringLast!=null   && bringLast.equals("Y"))  //若要帶出前一版本資料則執行以下動作
  {
    ...
    ResultSet blrs=null;
				...
				{			   				  				
				  blrs=blstat.executeQuery("select FPEXWPRI,FPMVER FROM PSALES_FORE_EXWPRICE where FPCOMP='"+comp+"' and FPTYPE='"+type+"' and FPREG='"+region+"' and FPCOUN='"+country+"' and FPPRJCD='"+chooseItem+"' and FPMONTH='"+dateBean.getMonthString()+"' and FPYEAR='"+dateBean.getYearString()+"' and FPCURR='"+curr+"' order by FPMVER DESC");                  				  
				  if (blrs!=null)
				  {				    					 
					 if ( blrs.next())
					 {
				       allMonth[jj]=blrs.getString("FPEXWPRI");					  					    						
					 } else {
					    allMonth[jj]="0";
					 }				  				    
				  } //end if of blrs!=null				  
     ...
     <% 				 		 
      ...
      <td width="24%"><font size="2"> <font color="#333399" face="Arial Black"><strong>Country: 
       ...
       %> 
        ...
        <INPUT TYPE="button"  value="Add" onClick='setSubmit("../jsp/WSExwPriceInput.jsp")' >
	  </font></td>     
  </tr>
</table>
<table width="100%" border="1" cellspacing="0" cellpadding="0">
  <tr>      
      <td width="10%" bgcolor="#FFFFCC"><font size="2">Model</font></td>
    ...
    <td> <font size="2">
      <%	    	     		 		 
         ...
         {   
		  ...
		  //sSqlC = "select PROJECTCODE as x, PROJECTCODE from PIMASTER ";		  
		  //sWhereC= "where PROJECTCODE IS NOT NULL and PROJECTCODE not in ("+aString+") order by x";		             		 
		  sSqlC = "select trim(INTER_MODEL) as x,trim(INTER_MODEL) from PSALES_PROD_CENTER ";		  
		  sWhereC= "where INTER_MODEL IS NOT NULL and trim(INTER_MODEL) not in ("+aString+") order by x";	
		  sSqlC = sSqlC+sWhereC;		  
  ...
  <tr>  
    <td colspan="14"><a href='javaScript:setSubmit("../jsp/WSExwPriceInput.jsp?BRINGLAST=Y")'>Last version of this model</a><BR><HR SIZE=2></td>
    </tr>
  <tr bgcolor="#FFCC33">
    <td><div align="right">      <strong>
	  ...
	  try
      {
	    String oneDArray[]= {"","MODEL",ymh1,ymh2,ymh3,ymh4,ymh5,ymh6,ymh7,ymh8,ymh9,ymh10,ymh11,ymh12}; 	 	     			  
...
<% 
     int div1=0,div2=0;      //做為運算共有多少個row和column輸入欄位的變數
      ...
      {	
       ...
       %>
<BR>
<INPUT name="button2" TYPE="button" onClick='setSubmit("../jsp/WSExwPriceInput.jsp")'  value="DELETE" >
------------------------------------------------------------------------------------------------------------
<INPUT TYPE="button"  value="SAVE" onClick='setSubmit2("../jsp/WSExwPriceInsert.jsp",<%=div1%>,<%=div2%>)' >
---------------------
<!-- 表單參數 -->  
