...
<body>
<FORM ACTION="../jsp/TSYewLabelGroupExcel.jsp" METHOD="post" name="MYFORM">
<%
...
{ 	
	...
	sql = " select case a.label_kind when 'TSC' then '台半標準標籤' when 'CUST' then '客戶標籤' when 'SHIPPING MARK' then '嘜頭標籤' else a.label_kind end as label_kind"+
	      ",a.label_group_code,a.label_group_name, a.label_group_desc,a.label_kind, "+
	...
	{ 	
		if (reccnt==0)
		{
			...
			ws.addCell(new jxl.write.Label(col, row, "標籤類別" , ACenterBLB));
			...
			//文件代碼
			ws.mergeCells(col, row, col, row); 
			ws.addCell(new jxl.write.Label(col, row, "文件代碼" , ACenterBLB));
			ws.setColumnView(col,15);	
			col++;	

			//文件版次
			ws.mergeCells(col, row, col, row); 
			ws.addCell(new jxl.write.Label(col, row, "文件版次" , ACenterBLB));
		...
		col=0;
		ws.addCell(new jxl.write.Label(col, row, rs.getString("label_kind") , ACenterL));
		col++;	
		...
		col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("label_form_code")==null?"":rs.getString("label_form_code")), ACenterL));
		col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("label_form_version")==null?"":rs.getString("label_form_version")), ACenterL));
		col++;	
