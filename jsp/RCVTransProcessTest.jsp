...
<body>
<FORM ACTION="RCVTransProcessTest.jsp" METHOD="post" NAME="MYFORM">
<%
...
String DayFr=dateBean.getYearMonthDay().substring(6,8);
  ...
  if (shipDate==null || shipDate.equals("")) shipDate = "20060917"; // 測試用
...
if (shipDate!=null)
{
 ...
 try
 {
  ...
  // else if (shipDate > systemDate )
   ...
   { out.println("COUNT RATE="+countRate);
			  ...
			  String sql1a=  " select INSPLOT_NO as SELECTFLAG, INSPLOT_NO ,RECEIPT_QTY ,'3077' as CREATE_BY, "+
			                        " PO_HEADER_ID, PO_LINE_ID, PO_LINE_LOCATION_ID, "+
			                        " EMPLOYEE_ID, ORGANIZATION_ID  "+
                            " from ORADDMAN.TSCIQC_LOTINSPECT_DETAIL "+
                            " where INSPLOT_NO = '"+inspLotNo+"' and to_char(LINE_NO) = '"+lineNo+"' ";
							// "and STATUSID = '021'"; 
              ...
              {   out.println("Step2="+shippingDate+"<BR>");
				 ...
				 cs3.setFloat(6,Float.parseFloat(rs.getString("RECEIPT_QTY")));  
			     ...
			     cs3.setString(15,"100");   // SUB_INVENTORY
 ...
 %>
 			   <input type="hidden" size="15" name="TSINVOICENO" value="<%=tsInvoiceNo%>" readonly>
			   <input type="hidden" size="15" name="SHIPDATE" value="<%=shipDate%>" readonly>
			   <input type="hidden" size="15" name="CREATEBY" value="<%=createBy%>" readonly>
			   <input type="hidden" size="15" name="POHEADERID" value="<%=rs.getInt("PO_HEADER_ID")%>" readonly>
			   <input type="hidden" size="15" name="POLINEID" value="<%=rs.getInt("PO_LINE_ID")%>" readonly>
			   <input type="hidden" size="15" name="POLOCATIONLINEID" value="<%=rs.getInt("PO_LOCATION_LINE_ID")%>" readonly>
			   <input type="hidden" size="15" name="EMPLOYEEID" value="<%=rs.getInt("EMPLOYEE_ID")%>" readonly>
			   <input type="hidden" size="15" name="ORGANIZATIONID" value="<%=rs.getInt("ORGANIZATION_ID")%>" readonly>
			   <input type="hidden" size="15" name="RCVQTY" value="<%=rs.getInt("RECEIPT_QTY")%>" readonly>  
			   <input type="hidden" size="15" name="ERRORH" value="<%=errorMessageHeader%>" readonly> 	
			   <input type="hidden" size="15" name="ERRORL" value="<%=errorMessageLine%>" readonly> 
			  
			   <BR>			      
			   
  <%            
              out.println("Error Message="+errorMessageHeader);
			  out.println("Error Message Line="+errorMessageLine);
			  } // End of while ()
