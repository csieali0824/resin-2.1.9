...
<FORM ACTION="../jsp/WSShipStatisticAuto.jsp" METHOD="post" NAME="MYFORM" >
  <table width="100%" border="0">
    ...
    <tr> 
      <td align="center"><strong><font color="#0000FF" size="+2" face="Arial">Shipping 
	   ...
	   //dateBean.setAdjDate(-1);//*****因為資料庫資料均為前一日故-1*******
  ...
  <table width="100%" border="1" cellspacing="0" cellpadding="0">
    ...
    <tr bordercolor="#000000" > 
      ...
      <td width="5%" bgcolor="#CCFFFF"><strong><font size="2" face="新細明體">本月</font></strong></td>
      <td width="4%" bgcolor="#CCFFFF"><div align="center"><strong><font size="2" face="新細明體">當日</font></strong></div></td>
  ...
  %>
    ...
    {  
	  ...
	  // String sqlT =  "SELECT abs(SUM(TQTY)) FROM ITH WHERE TRIM(TPROD)='"+fmItem+"' and trim(TTYPE) in ('B','RM') and TTDTE between "+dateBean.getYear()+dateBean.getMonth()+"01 and "+dateBean.getYearMonthDay()+"and TWHS=52";  // fix by kerwin
	  //out.println(sqlTT);
      ResultSet docrs=docstatement.executeQuery(sqlTT);
	   ...
	   {	 
	     if (docrs.getString(1)!=null) 
		 {		 
		   thisMonthShipAmt=docrs.getInt(1);//當月累積出貨量
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous1MonthShipAmt=docrs.getInt(1); //取得前月出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjMonth(1);
	   docrs.close(); 	   	   	   	   
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous2MonthShipAmt=docrs.getInt(1); //取得前2個月出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjMonth(2);
	   docrs.close(); 
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous3MonthShipAmt=docrs.getInt(1); //取得前3個月出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjMonth(3);
	   docrs.close();				  					   
    ...
    {  
      ...
      ResultSet docrs=docstatement.executeQuery("SELECT abs(SUM(TQTY)) FROM ITH WHERE TRIM(TPROD)='"+fmItem+"' and trim(TTYPE) in ('B','RM') and TTDTE="+dateBean.getYearMonthDay()+"and TWHS=52");
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous1DayShipAmt=docrs.getInt(1); //取得前1日出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjDate(1);
	   docrs.close(); 	   	   	   	   
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous2DayShipAmt=docrs.getInt(1); //取得前2日出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjDate(2);
	   docrs.close(); 
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous3DayShipAmt=docrs.getInt(1); //取得前3日出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjDate(3);
	   docrs.close();
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous4DayShipAmt=docrs.getInt(1); //取得前4日出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjDate(4);
	   docrs.close();		  					   					   
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous5DayShipAmt=docrs.getInt(1); //取得前5日出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjDate(5);
	   docrs.close();
       ...
       {	 
	     if (docrs.getString(1)!=null)
		 {
		  previous6DayShipAmt=docrs.getInt(1); //取得前6日出貨量		  
		 }		  
	   } //end of docrs while
	   dateBean.setAdjDate(6);
	   docrs.close();
