...
<!--%/20040109/將Excel Veiw 夾在檔頭%-->
 ...
 <font color="#000066" face="Times New Roman" size="+2"><strong>Database Lock Information Query</strong></font>  
<BR>
  <!--%/20040109/將Excel Veiw 夾在檔頭%-->
<%
...
{
   sSql = "select S.SID SESSION_ID,S.USERNAME, "+
		   ...
		   "O.OWNER||'.'||O.OBJECT_NAME||' ('||O.OBJECT_TYPE||')' as OBJECT_INF,S.TYPE LOCK_TYPE,L.ID1 LOCK_ID1,L.ID2 LOCK_ID2 "+
           "from V$LOCK L,SYS.DBA_OBJECTS O,V$SESSION S  ";
   sSqlCNT = "select count(S.SID) as CaseCount from V$LOCK L,SYS.DBA_OBJECTS O,V$SESSION S ";
   sWhere =  "where L.SID = S.SID AND L.ID1 = O.OBJECT_ID  ";                  
   sOrder = "order by S.SID ";
...
{ 
 ...
 //out.println(sSql);  
   ...
   ResultSet rs2=statement2.executeQuery("select count(S.SID) as CaseCountORG from V$LOCK L,SYS.DBA_OBJECTS O,V$SESSION S ");
   if (rs2.next())
   {
 ...
 <A HREF="/oradds/ORAddsMainMenu.jsp">回首頁</A>
  ...
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#FFFFFF">
    <tr bgcolor="#CCFF99" > 
	  ...
	  <td width="3%" height="22" nowrap><div align="center"><font color="#000000" size="-1">&nbsp;</font></div></td>
      <td width="7%" height="22" nowrap><div align="center"><font color="#000000" size="-1">Owner</font></div></td>
      ...
      <td width="7%" nowrap><div align="center"><font color="#000000" size="-1">Mode Held</font></div></td>
      <td width="21%" nowrap><div align="center"><font color="#000000" size="-1">Object Information</font></div></td> 
	  <td width="21%" nowrap><div align="center"><font color="#000000" size="-1">Lock Type</font></div></td> 
	  <td width="21%" nowrap><div align="center"><font color="#000000" size="-1">Lock ID</font></div></td> 
	  <td width="21%" nowrap><div align="center"><font color="#000000" size="-1">Lock ID2</font></div></td>                   
    </tr>
    ...
    <tr bgcolor="<%=colorStr%>"> 
	  ...
	  <td><div align="center"><a href="../jsp/OraSysSynonmsResultPage.jsp?SYNONMSNAME=<%=rsTC.getString("SESSION_ID")%>"><img src="../image/docicon.gif" width="14" height="15" border="0"></a></div></td>    
	  <td><font size="-1"><%=rsTC.getString("USERNAME") %></font></td>
      <td width="19%" nowrap><font size="-1"><%=rsTC.getString("MODE_HELD")%></font></td>
	  <td width="40%" nowrap><font size="-1"><%=rsTC.getString("COMMENTS")%></font></td>
      <td width="7%"><font size="-1"><%=rsTC.getString("MODE_REQUESTED")%></font></td>
      <td width="21%" nowrap><font size="-1"><%=rsTC.getString("OBJECT_INF")%></font></td> 
	  <td width="21%" nowrap><font size="-1"><%=rsTC.getString("LOCK_TYPE")%></font></td>
	  <td width="21%" nowrap><font size="-1"><%=rsTC.getString("LOCK_ID1")%></font></td>
	  <td width="21%" nowrap><font size="-1"><%=rsTC.getString("LOCK_ID2")%></font></td>           
    </tr>
    <%
  rs1__index++;
  rs_hasDataTC = rsTC.next();
...
%>
    <tr bgcolor="#CCFF99"> 
      <td height="23" colspan="11" ><font color="#000000">查詢筆數</font> 
        <% 
	      if (CaseCount==0) 
