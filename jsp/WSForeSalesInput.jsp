   ...
   {     
      for (k=1;k<dim2;k++)
	  {
         ...
         { 
	       ...
	       {
             alert("Monthly forecast detail should be numerical!!");   
             return(false);
		    }
         }
	   } //enf for of k
...
<title>Monthly Forecast Input Form</title>
</head>
<body>
<FORM ACTION="WSForeSalesInput.jsp" METHOD="post" NAME="MYFORM">
...
<strong> Sales Forecast Input (Detail)</strong></font> 
...
int commitmentMonth=thisDateBean.monthDifferentFromDate(dateBean.getYear(),dateBean.getMonth()); //計算設定之日期與現在日期差幾個月?
if (commitmentMonth>0) 
{
  commitmentMonth=0;
} else {
  commitmentMonth=Math.abs(commitmentMonth);
}
...
{    
  ...
  if ( bringLast!=null   && bringLast.equals("Y"))  //若要帶出前一版本資料則執行以下動作
  {
    ...
    ResultSet blrs=null;
				...
				{			   				  				
				  blrs=blstat.executeQuery("select FMQTY,FMVER FROM PSALES_FORE_MONTH where  FMTYPE='"+type+"' and FMCOUN='"+country+"' and FMPRJCD='"+chooseItem+"' and FMCOLOR='"+color+"' and FMMONTH='"+dateBean.getMonthString()+"' and FMYEAR='"+dateBean.getYearString()+"' order by FMVER DESC");                  				  
				  if (blrs!=null)
				  {				    					 
					 if ( blrs.next())
					 {
				       allMonth[jj]=blrs.getString("FMQTY");					  					    						
					 } else {
					    allMonth[jj]="0";
					 }				  				    
				  } //end if of blrs!=null				  
...
<table width="100%" border="0">
    <tr bgcolor="#D0FFFF">
      <td width="31%" bordercolor="#FFFFFF"><font color="#330099" size="2" face="Arial Black"><strong>Business
            Unit:
     <% 				 		 
	     try
         {		  		  		 	    		  		 		 		  
          ...
          if (rs.next())  out.println(rs.getString("MCDESC"));		
          rs.close();      
		  statement.close();		 
         } //end of try
         catch (Exception e)
      ...
      <td width="24%"><font size="2"> <font color="#333399" face="Arial Black"><strong>Country: 
       ...
       %> 
        ...
        <INPUT TYPE="button"  value="Add" onClick='setSubmit("../jsp/WSForeSalesInput.jsp",document.MYFORM.DIM1.value)' >
	  </font></td>     
  </tr>
</table>
<table width="100%" border="1" cellspacing="0" cellpadding="0">
  <tr>
    <td width="12%" bgcolor="#FFFFCC"><font size="2">MODEL</font></td>      
      <td width="12%" bgcolor="#FFFFCC"> <font size="2">COLOR </font></td>
	  <td width="4%" colspan="1" bgcolor="#FFFFCC"><div align="center"><font size="2">
          <%
	  ...
	  <td width="4%" bgcolor="#FFFFCC"><div align="center"><font  size="2">
          <%dateBean.setAdjMonth(1);
  ...
  <tr>
    <td><font size="2">
      <%	    	     		 		 
         ...
         {   
		  ...
		  sSqlC = "select trim(PROJECTCODE) as x,trim(PROJECTCODE) from PIMASTER ";	
		  sWhereC= " order by x";	
    ...
    <td><font size="2">
      <% 		
         ...
         {   
		  String sSql = "";		 
		  sSql = "select COLORCODE,COLORDESC||'('||COLORCODE||')' from PICOLOR_MASTER"; 		  		 		 		  
		
          Statement statement=con.createStatement();
          ResultSet rs=statement.executeQuery(sSql);
          comboBoxBean.setRs(rs);		  
		  comboBoxBean.setSelection(color);
  ...
  <tr>  
    <td colspan="15"><a href='javaScript:setSubmit("../jsp/WSForeSalesInput.jsp?BRINGLAST=Y")'>Last version</a><BR><HR SIZE=2></td>
    </tr>
  <tr bgcolor="#FFCC33">
    <td colspan="2" bgcolor="#FFCC99"><div align="right">      <strong>
        <%
	  try
      {
	    String oneDArray[]= {"","MODEL@COLOR",ymh1,ymh2,ymh3,ymh4,ymh5,ymh6,ymh7,ymh8,ymh9,ymh10,ymh11,ymh12}; 	 	     			  
    	forePriCostInputBean.setArrayString(oneDArray);
		 ...
		 {               		    
		   if (a!=null) 
		   {
			 ...
			 String c[][]={{chooseItem+"@"+color,month1,month2,month3,month4,month5,month6,month7,month8,month9,month10,month11,month12}};			             			 
		    ...
		    //--------------------------------------------------------------------
			 ...
			 for (int ti=0;ti<temp.length;ti++)
			 {
			    for (int tj=1;tj<=commitmentMonth;tj++)  //因為只有n個月為commitment
				{
				  ...
				  String fmPrjcd=null,fmColor=null;
				  fmPrjcd=temp[ti][0].substring(0,temp[ti][0].indexOf("@"));
				  fmColor=temp[ti][0].substring(temp[ti][0].indexOf("@")+1);
				  chkrs=chkstat.executeQuery("select FMQTY,FMVER FROM PSALES_FORE_MONTH where FMCOMP='"+comp+"' and FMTYPE='"+type+"' and FMREG='"+region+"' and FMCOUN='"+country+"' and FMPRJCD='"+fmPrjcd+"' and FMCOLOR='"+fmColor+"' and FMYEAR='"+tpy+"' and FMMONTH='"+tpm+"' order by FMVER DESC");                  				  
				  if (chkrs!=null)
				  {				    					 
					 if ( chkrs.next())
					 {					   
				       temp[ti][tj]=chkrs.getString("FMQTY");
					  } else {
					    temp[ti][tj]="0";						
					  }
				  } else {
...
-----MONTHLY FORECAST DETAIL-------------------------------------------------------------<font color="#0000FF"><strong>(Unit/K
pcs)</strong></font>------------------<BR>
<% 
     int div1=0,div2=0;      //做為運算共有多少個row和column輸入欄位的變數
      ...
      {	
       ...
       %>
<BR>
<INPUT name="button2" TYPE="button" onClick='NeedConfirm("../jsp/WSForeSalesInput.jsp")'  value="DELETE" >
------------------------------------------------------------------------------------------------------------
<INPUT TYPE="button"  value="SAVE" onClick='setSubmit2("../jsp/WSForeSalesInsert.jsp",<%=div1%>,<%=div2%>)' >
---------------------
<!-- 表單參數 -->  
	...
	<input name="DIM1" type="HIDDEN" value="<%=div1%>" > 
</FORM>   
</body>
<!--=============以下區段為釋放連結池==========-->
...
<script language="JavaScript" type="text/JavaScript">
function setSubmit(URL,dimLength)
{
   if (document.MYFORM.CHOOSEITEM.value=="--" || document.MYFORM.CHOOSEITEM.value==null)
   { 
           alert("Before you want to add , please do not let the MODEL be Null !!");   
           return(false);
   }  

   if (document.MYFORM.COLOR.value=="--" || document.MYFORM.COLOR.value==null)
   { 
           alert("Before you want to add , please do not let the COLOR be Null !!");   
           return(false);
   }  
   ...
   {     
     ...
     { 
      alert("Before you want to add , please do not let the Monthly forecast data be Null !!");   
      return(false);
      } 
   } //end of for  null check
  ...
  header=document.MYFORM.CHOOSEITEM.value+"@"+document.MYFORM.COLOR.value;    
  if (dimLength>0)
  {    
    if (dimLength==1)
	{
	  if (document.MYFORM.ADDITEMS.value == header)
	       { 		   
              alert("The one you choosed is exist!!");   
              return(false);
	       }  	   	
    } else {
        for (i=0;i<dimLength;i++)
        {      	       
	       if (document.MYFORM.ADDITEMS[i].value == header)
	       { 		   
              alert("The one you choosed is exist!!");   
              return(false);
	       }  	             
        } //end of for EXIST check
	 }	
  }	 
