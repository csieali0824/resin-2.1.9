...
<%@ page import="bean.ComboBoxBean,bean.DateBean,bean.WorkingDateBean,bean.ArrayComboBoxBean,bean.MiscellaneousBean,bean.SendMailBean,CodeUtil"%>
...
<strong>台北/山東訂單對應檢核表</strong></font>
<%
...
{
   sSql = " select OOHA.ORDER_NUMBER,OOHA.ATTRIBUTE3,OOLA.LINE_NUMBER||'.'||OOLA.SHIPMENT_NUMBER LINE_NUMBER,MSI.SEGMENT1 PRODNO, "+
       	  "        DECODE(OOLA.ORDER_QUANTITY_UOM,'PCE',(OOLA.ORDERED_QUANTITY/1000),OOLA.ORDERED_QUANTITY) ORDERED_QUANTITY,  "+
	      "        OOLA.LAST_UPDATE_DATE,OOHA.BOOKED_DATE,OOLA.LINE_ID "+
  		  "   from OE_ORDER_HEADERS_ALL OOHA,  OE_ORDER_LINES_ALL OOLA,MTL_SYSTEM_ITEMS MSI ";         			
   sSqlCNT = " select count(ooha.ORDER_NUMBER) as CaseCount "+
             "  from OE_ORDER_HEADERS_ALL OOHA,  OE_ORDER_LINES_ALL OOLA,MTL_SYSTEM_ITEMS MSI ";
   sWhere =  " where OOHA.ORG_ID = 41 and MSI.ORGANIZATION_ID = OOLA.SHIP_FROM_ORG_ID and OOHA.HEADER_ID = OOLA.HEADER_ID "+
  			 "   and OOLA.INVENTORY_ITEM_ID = MSI.INVENTORY_ITEM_ID AND OOLA.PACKING_INSTRUCTIONS='Y' "+
  			 "   and ( OOHA.ORDER_NUMBER LIKE '1131%' OR OOHA.ORDER_NUMBER LIKE '1141%' OR OOHA.ORDER_NUMBER LIKE '1213%') "+
  			 "   and OOLA.BOOKED_FLAG = 'Y' and OOLA.CANCELLED_FLAG <> 'Y' and OOLA.LINE_CATEGORY_CODE <>'RETURN' ";		
   sWhereSDRQ = "  ";
   havingGrp = " ";               
   havingGrpSDRQ = "  ";
   sOrder = " order by OOHA.BOOKED_DATE,OOHA.ORDER_NUMBER ";
...
{   
  ...
  // if (prodManufactory==null || prodManufactory.equals("--") || prodManufactory.equals("")) { sWhere=sWhere+"and oola.PACKING_INSTRUCTIONS =' ' ";}
  // else { sWhere=sWhere+" and oola.PACKING_INSTRUCTIONS ='"+prodManufactory+"'"; }
   
   // out.print("aaaaaaaaaDayFr="+DayFr+",DayTo="+DayTo+", sWhere="+sWhere+"......");
   //if ((!(DayFr=="--")&&(DayFr=="00")) && DayTo=="--") sWhere=sWhere+"  AND to_char(trunc(ooha.booked_date),'yyyymmdd') >= '"+dateSetBegin+"' ";
   if (DayFr!="--" && DayTo!="--") sWhere=sWhere+" AND to_char(trunc(ooha.booked_date),'yyyymmdd') BETWEEN '"+dateSetBegin+"' AND '"+dateSetEnd+"' ";  
   //  out.print("<br>sWhere="+sWhere);      
    if (salesOrderNo==null || salesOrderNo.equals("") ) { sWhere=sWhere+" "; }
	else {  sWhere=sWhere+" and ooha.ORDER_NUMBER = '"+salesOrderNo+"' "; }
  ...
  //out.println("sSql="+sSql);    
   ...
   String sqlOrgCnt = " select count(ooha.ORDER_NUMBER) as CaseCountORG "+
            		  "  from OE_ORDER_HEADERS_ALL OOHA,  OE_ORDER_LINES_ALL OOLA,MTL_SYSTEM_ITEMS MSI   ";
...
%> 
  <table cellSpacing='0' bordercolordark='#D8DEA9'  cellPadding='1' width='100%' align='center' borderColorLight='#ffffff' border='1'>     	 	 	  
     <tr>	    
	   <td nowrap colspan="2"><font color="#006666"><strong>TPE BOOKED DATE</strong></font>
        <%
		//  String CurrYear = null;	     		 
	    // try
        // {       
        //  String a[]={"2007","2008","2009","2010","2011","2012","2013","2014"};
		...
		{     
			int  j =0; 
			String a[]= new String[Integer.parseInt(dateBean.getYearString())-2007+1];
			for (int i = Integer.parseInt(dateBean.getYearString()) ; i >=2007 ; i--)
			{
				a[j++] = ""+i; 
			}
			arrayComboBoxBean.setArrayString(a);
     ...
     <td colspan="1" nowrap><font color="#006666"><strong>銷售訂單號</strong></font></td> 
	 <td colspan="1">
		  <input type name="SALESORDERNO" value="<%=salesOrderNo%>">
	 </td> 
	 <td >
		    <INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>' onClick='setSubmit("../jsp/TSCMOCpyYEWAuditReport.jsp")' > 			 
	</td>
   </tr>
  </table>  
<%
...
%>
  ...
  <table width='100%' align='center' border="1" cellpadding="1" cellspacing="0" bordercolorlight="#999999" bordercolordark="#FFFFFF">
    <tr>
	  <td height="22" nowrap colspan="8" bgcolor="#6699CC"><div align="center"><font color="D8DEA9">台北銷售訂單資訊</font></div></td>
	  <td height="22" nowrap colspan="7" bgcolor="#6699CC"><div align="center"><font color="D8DEA9">山東銷售訂單資訊</font></div></td>
	</tr>
    <tr bgcolor="#D8DEA9"> 
	  ...
	  <td width="3%" nowrap><div align="center"><font color="#006666">項次</font></div></td>
      <td width="8%" nowrap><div align="center"><font color="#006666">台半品號</font></div></td>  
	  <td width="8%" nowrap><div align="center"><font color="#006666">數量k</font></div></td> 
	  <td width="10%" nowrap><div align="center"><font color="#006666">明細異動日</font></div></td>
	  <td width="10%" nowrap><div align="center"><font color="#006666">BOOKED_DATE</font></div></td>
	  <td width="8%" nowrap><div align="center"><font color="#006666">YEW訂單號</font></div></td> 	  
	  <td width="3%" nowrap><div align="center"><font color="#006666">版本</font></div></td> 
	  <td width="8%" nowrap><div align="center"><font color="#006666">台半品號</font></div></td>                 	  
      ...
      <td width="10%" nowrap><div align="center"><font color="#006666">建檔日</font></div></td>
    </tr>
    ...
    <tr bgcolor="<%=colorStr%>"> 
	  ...
	  <td width="7%" nowrap><div align="center"><font size="2" color="#006666"><%=rsTC.getString("ATTRIBUTE3")%></font></div></td>
      ...
      <td width="15%" nowrap><font size="2" color="#006666"><%   out.println(rsTC.getString("PRODNO"));	 %></font></td> 
	  <td width="6%" nowrap><div align="right"><font color="#006666"><%=rsTC.getString("ORDERED_QUANTITY")%></font></div></td>
	  <td width="11%" nowrap><div align="center"><font color="#006666"><%=rsTC.getString("LAST_UPDATE_DATE")%></font></div></td>
	  <td width="6%" nowrap><div align="center"><font color="#006666"><%=rsTC.getString("BOOKED_DATE")%></font></div></td>
	  <% 
	                  String orderNo="",ver ="",itemNo ="",itemDesc ="",qty="",importDate ="",orderStatus="";
	                  float qtyF = 0; 
	                  String sSqlSQL = " select OOHA.ORDER_NUMBER,OOHA.ATTRIBUTE3,OOLA.LINE_NUMBER||'.'||OOLA.SHIPMENT_NUMBER LINE_NUMBER,MSI.SEGMENT1 PRODNO,MSI.DESCRIPTION, "+
       								   "        DECODE(OOLA.ORDER_QUANTITY_UOM,'PCE',(OOLA.ORDERED_QUANTITY/1000),OOLA.ORDERED_QUANTITY) ORDERED_QUANTITY, "+
	   								   "        OOHA.FLOW_STATUS_CODE STATUS,OOHA.CREATION_DATE ";
					  String sFrom = " from OE_ORDER_HEADERS_ALL OOHA,  OE_ORDER_LINES_ALL OOLA,MTL_SYSTEM_ITEMS MSI ";
					  String sWhereSQL = " where OOHA.ORG_ID = 325 and MSI.ORGANIZATION_ID = OOLA.SHIP_FROM_ORG_ID "+
  										 "   and OOHA.HEADER_ID = OOLA.HEADER_ID and OOLA.INVENTORY_ITEM_ID = MSI.INVENTORY_ITEM_ID "+
  										 "   and ( OOHA.ORDER_NUMBER LIKE '1131%' OR OOHA.ORDER_NUMBER LIKE '1141%' OR OOHA.ORDER_NUMBER LIKE '1213%') "+
  										 "   and OOLA.CANCELLED_FLAG <> 'Y' and OOLA.LINE_CATEGORY_CODE <>'RETURN' "+
                                         "   and ooha.order_number= '"+rsTC.getString("ORDER_NUMBER")+"' and oola.orig_sys_line_ref like '%-"+rsTC.getString("LINE_ID")+"' ";
										// "  and a.QTY = "+rsTC.getString("ORDERED_QUANTITY")+" ";
	                //  out.println(sSqlSQL);
  
						 sSqlSQL = sSqlSQL + sFrom + sWhereSQL;
					     Statement stmentSQL=con.createStatement();
//out.println("<BR>YEW="+sSqlSQL);
                         ResultSet rsSQL=stmentSQL.executeQuery(sSqlSQL);
					     if (rsSQL.next())
					     { 
					               orderNo = rsSQL.getString("ORDER_NUMBER");
						           ver = rsSQL.getString("ATTRIBUTE3");
						           itemNo = rsSQL.getString("PRODNO");
						           itemDesc = rsSQL.getString("DESCRIPTION");
						          // qtyF = rsSQL.getFloat("ORDERED_QUANTITY");
                                   qty=rsSQL.getString("ORDERED_QUANTITY");
								   orderStatus = rsSQL.getString("STATUS");
						           importDate = rsSQL.getString("CREATION_DATE");
					     }
					     rsSQL.close();
					     stmentSQL.close(); 
					 
						//	  miscellaneousBean.setRoundDigit(2); 取小數二位
	  ...
	  <td  nowrap <% if (ver==null || ver.equals("")) out.println("bgcolor='#FF3333'"); %>>
	           <div align="center"><font color="#006666">
			   <% 
	  ...
	  <td  nowrap <% if (itemNo==null || itemNo.equals("")) out.println("bgcolor='#FF3333'"); %>>
	           <div align="center"><font color="#006666">
			   <% 
	  ...
	  <td  nowrap <% if (itemDesc==null || itemDesc.equals("")) out.println("bgcolor='#FF3333'"); %>>
	           <div align="center"><font color="#006666">
			   <% 			      
	  ...
	  <td  nowrap  <% if (qty==null || qty.equals("")) out.println("bgcolor='#FF3333'"); %>>
	           <div align="right"><font color="#006666">
			   <% 			      
				  if (qty==null || qty.equals("")) out.println(qty);
				  else out.println(qty);
			   %></font></div>
	  </td>
	  <td  nowrap <% if (orderStatus==null || orderStatus.equals("")) out.println("bgcolor='#FF3333'"); %>>
	           <div align="center"><font color="#006666">
			   <% 			     
				  if (orderStatus==null || orderStatus.equals("")) out.println("&nbsp;");
				  else out.println(orderStatus);
			   %>
			   </font></div>
	  </td>   
	 <td  nowrap <% if (importDate==null || importDate.equals("")) out.println("bgcolor='#FF3333'"); %>>
	           <div align="center"><font color="#006666">
			   <% 			     
...
%>
    <tr bgcolor="#D8DEA9"> 
      <td height="23" colspan="15" ><font color="#006666"><jsp:getProperty name="rPH" property="pgMRItemQty"/></font> 
        <% 
	      if (CaseCount==0) 
...
<%
 ...
 if (checkFlag==true)	
 {
  if (mailAct.equals("Y"))
  {
    String sSqlMailUser = "select DISTINCT USERMAIL,USERNAME,WEBID from ORADDMAN.WSUSER, ORADDMAN.WSGROUPUSERROLE where USERNAME=GROUPUSERNAME and ROLENAME in ('admin','audit') and LOCKFLAG='N' and USERNAME in ('liling') ";
