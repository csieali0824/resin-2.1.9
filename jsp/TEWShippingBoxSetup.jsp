<!-- 20150407 by Peggy,tsc_item_packing_master增加org_id,tew固定寫入49-->
...
{
	subWin=window.open("../jsp/subwindow/TEWPOSupplierFind.jsp?SEARCHSTR="+Supplier+"&TYPE=BOX","subwin","width=740,height=480,scrollbars=yes,menubar=no");  
}
</script>
<STYLE TYPE='text/css'>  
...
<title>TEW編箱資料維護</title>
</head>
<body>
<FORM NAME="MYFORMD" ACTION="../jsp/TEWShippingBoxSetup.jsp" METHOD="POST"> 
<%
...
if (TSCPROD==null) TSCPROD="PRD-Subcon";
...
if (ACODE == null) ACODE="";
String REELQTY = request.getParameter("REELQTY");  //add by Peggy 20180130
...
{  
	sql = " select b.vendor_name, a.tsc_prod_group, a.tsc_package, a.packing_code, a.def_flag,"+
			  ...
			  ",a.reel_qty"+ //add by Peggy 20180130
			  ",SUBSTR(a.CARTON_SIZE,1,INSTR(a.CARTON_SIZE,'*')-1) CARTON_L"+ //add by Peggy 20180130
			  ",SUBSTR(a.CARTON_SIZE,INSTR(a.CARTON_SIZE,'*')+1,INSTR(a.CARTON_SIZE,'*',INSTR(a.CARTON_SIZE,'*')+1)-INSTR(a.CARTON_SIZE,'*')-1) CARTON_W"+ //add by Peggy 20180130
			  ",SUBSTR(a.CARTON_SIZE,(LENGTH(a.CARTON_SIZE)-INSTR(a.CARTON_SIZE,'*',-1))*-1) CARTON_H"+	 //add by Peggy 20180130		  
	...
	if (rs.next())
	{
		...
		REELQTY =rs.getString("REEL_QTY");  //add by Peggy 20180130
		if (REELQTY==null) REELQTY="";		
		...
		if (CARTONSIZE==null) CARTONSIZE="";
		CARTON_L=rs.getString("CARTON_L");  //add by Peggy 20180130
		if (CARTON_L==null) CARTON_L="";	
		CARTON_W=rs.getString("CARTON_W");  //add by Peggy 20180130
		if (CARTON_W==null) CARTON_W="";			
		CARTON_H=rs.getString("CARTON_H");  //add by Peggy 20180130
		if (CARTON_H==null) CARTON_H="";			
		NW=rs.getString("NW");
		if (NW==null) NW="0";
		GW=rs.getString("GW");
...
<table align="center" width="70%">
	<tr>
		<td align="center">
			<strong><font style="font-size:20px;color:#006666">TEW 編箱資料<%=(ACTIONMODE.equals("MODIFY")?"維護":"新增")%></font></strong>
		</td>
	</tr>
	<tr>
		<td align="right">&nbsp;</td>
		...
		<td>
			<table width="100%"  align="CENTER" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#99CC99">
				<tr>
					...
					try
					{
						sql = " select distinct c.vendor_id,c.VENDOR_NAME"+
							  " from  ap.ap_suppliers c "+
							  " where (exists (select 1 from PO_APPROVED_SUPPLIER_LIST a"+
							  "                where a.ASL_STATUS_ID =?"+
							  "                and nvl(a.DISABLE_FLAG,'N') =?"+
							  "                and a.OWNING_ORGANIZATION_ID=?"+
							  "                and EXISTS (SELECT 1 from inv.mtl_system_items_b msi where msi.organization_id=a.owning_organization_id and TSC_OM_CATEGORY(msi.INVENTORY_ITEM_ID,msi.ORGANIZATION_ID,'TSC_PROD_GROUP')=? and msi.inventory_item_id=a.item_id)"+
							  "                and a.vendor_id=c.vendor_id)"+
							  " or exists (select 1 from tsc_item_packing_master b where b.INT_TYPE=? AND b.VENDOR_ID =c.VENDOR_ID)"+
							  " or c.attribute5 is not null) ";
						if (ACTIONMODE.equals(UPDCODE)) sql += " and  c.vendor_id='"+VENDOR+"'";
						//out.println(sql);						  
						PreparedStatement statement1 = con.prepareStatement(sql);
						statement1.setString(1,"2");
						statement1.setString(2,"N");
						statement1.setString(3,"49");
						statement1.setString(4,"PRD-Subcon");
						statement1.setString(5,"TEW");
					...
					%>
					<input type="hidden" name="VENDOR" value="<%=VENDOR%>"><input type="hidden" name="INT_TYPE" value="<%=INT_TYPE%>"></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td nowrap><!--<input type="text" name="CARTONSIZE" style="text-align:left;font-family: Tahoma,Georgia;font-size:12px" value="<%=CARTONSIZE%>">-->
				...
				<tr>
					<td width="35%" height="25" bgcolor="#C9E2D0" nowrap>台半品名：</td>
	...
	<tr>
		<td>
			<table width="100%" border="0">
  				<tr align=center>
    				<td width="16%"> <input name="btnSubmit" type=button onClick='setSubmit("../jsp/TEWShippingBoxSetup.jsp?ACODE=SAVE&ID=<%=java.net.URLEncoder.encode(ID)%>")' value="Submit" style="font-family:'Tahoma,Georgia';font-size:12px">&nbsp;&nbsp;&nbsp;
