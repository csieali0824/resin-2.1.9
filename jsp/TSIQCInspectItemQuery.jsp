...
{   
  location.href="../jsp/TSIQCInspectItemQuery.jsp?PAGEURL="+pageURL+"&SEARCHSTRING="+document.MYFORM.SEARCHSTRING.value;
}
 ...
 try
   ...
   */
   ...
   ResultSet  rs=null;
	 ...
	 {	   
	    lotRs=lotStat.executeQuery("select count(*) from ORADDMAN.TSCIQC_ITEM where CLASS_ID = '"+classID+"' and ( ITEM_NAME='"+searchString+"' )");	
		lotRs.next();
	    if (lotRs.getInt(1)>0) //若有存在批號的話
	    {		   
	       rs=statement.executeQuery("select count(*) from ORADDMAN.TSCIQC_ITEM where CLASS_ID = '"+classID+"' and (ITEM_NAME='"+searchString+"' or ITEM_DESC ='"+searchString+"') ");	
	    } else {
                    rs=statement.executeQuery("select count(*) from ORADDMAN.TSCIQC_ITEM where CLASS_ID = '"+classID+"' and (ITEM_NAME='"+searchString+"' or ITEM_DESC ='"+searchString+"') ");	 	 
                  } //end of lotRs if		  
     } else {
...
<strong><font color="#0080C0" size="5">IQC品管檢驗項目查詢</font></strong>(總共<%=maxrow%>&nbsp;筆記錄)</FONT><BR>
&nbsp;<A HREF="/oradds/jsp/TSIQCInspectItemInput.jsp">檢驗項目資料新增</A>
<table width="100%" border="0" cellpadding="0" cellspacing="1" bordercolorlight="#999999" bordercolordark="#FFFFFF">
  <tr bgcolor="#B1D3A9">     
	  <td width="25%"><font color="#FF0066"><strong>檢驗類別代碼: 
        </strong></font><font> 
        <%	    	     		 		 
	     try
         { 	   		 
		  ...
		  */
		  out.println("<select NAME='CLASS_ID' onChange='setSubmit("+'"'+"../jsp/TSIQCInspectItemQuery.jsp?PAGEURL="+pageURL+'"'+")'>");
	...
	<td width="30%"><font color="#FF0066"><strong>檢驗項目代碼: 
        </strong></font><font> 
        <%	    	     		 		 
	     try
         { 	   		 
...
<tr bgcolor="#B1D3A9">	
 <td width="45%"><strong><font color="#FFFFFF">檢驗類別名稱,類別說明:</font></strong>
...
<A HREF="../jsp/TSIQCInspectItemQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=FIRST"><font><strong><font color="#FF0080">第一頁</font></strong></font></A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectItemQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=LAST"><font><strong><font color="#FF0080">最後一頁</font></strong></font></A><font color="#FF0080"><strong><font>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectItemQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=100">下一頁</A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/TSIQCInspectItemQuery.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=-100">上一頁</A>&nbsp;&nbsp;(第<%=currentPageNumber%>&nbsp;頁/共<%=totalPageNumber%>&nbsp;頁)</font></strong></font>
<%   
  try
  {   
   Statement lotStat=con.createStatement();
   ...
   String sql=null;  
	   ...
	   {
	      lotRs=lotStat.executeQuery("select count(*) from ORADDMAN.TSCIQC_ITEM where CLASS_ID IS NOT NULL and ( ITEM_NAME='"+searchString+"' ) ");	 // 如果存在檢驗類別代碼
 		  lotRs.next();
	      if (lotRs.getInt(1)>0) //如果存在業務員工號
	      {
		    rs=statement.executeQuery("select ITEM_ID,ITEM_CODE as 檢驗項目代碼,ITEM_NAME as 檢驗項目名稱,ITEM_DESC as 檢驗項目描述,ITM_REMARK as 檢驗項目備註,CREATION_DATE as 建檔日期,CREATED_BY as 建檔人員,LAST_UPDATE_DATE as 更新日期,LAST_UPDATED_BY as 更新人員 from ORADDMAN.TSCIQC_ITEM where CLASS_ID = '"+classID+"' and ( ITEM_NAME='"+searchString+"' ) order by ITEM_ID"); 
		  } else {		   
            rs=statement.executeQuery("select ITEM_ID,ITEM_CODE as 檢驗項目代碼,ITEM_NAME as 檢驗項目名稱,ITEM_DESC as 檢驗項目描述,ITM_REMARK as 檢驗項目備註,CREATION_DATE as 建檔日期,CREATED_BY as 建檔人員,LAST_UPDATE_DATE as 更新日期,LAST_UPDATED_BY as 更新人員 from ORADDMAN.TSCIQC_ITEM where CLASS_ID = '"+classID+"' and ( ITEM_NAME='"+searchString+"' or ITEM_DESC='"+searchString+"') order by ITEM_ID");	 	 
          }			
       } else {	 
