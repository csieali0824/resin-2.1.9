	...
	while(rsMail.next())
	{
	   try 
          {		     
		   ...
		   urAddress = serverHostName+":8080/oradds/jsp/TSCWipJobCanceledNotIssueReport.jsp?DATEBEGIN="+strFirstDayWeek+"&DATEEND="+strLastDayWeek;		
        ...
        //   sendMailBean.setSubject(CodeUtil.unicodeToBig5("來自ORACLE系統的郵件:每週料件分類設定異常檢核表-("+strFirstDayWeek+"~"+strLastDayWeek+")")); 
		   sendMailBean.setSubject(CodeUtil.unicodeToBig5("Oracle ERP Alert E-Mail-Weekly WIP Job Canceled Not Issue List("+strFirstDayWeek+"~"+strLastDayWeek+")"));         
		   sendMailBean.setUrlName("Dear "+UserID+",\n"+CodeUtil.unicodeToBig5("   請點擊來自ORACLE系統的郵件:每週WIP工單取消未發料檢核表-("+strFirstDayWeek+"~"+strLastDayWeek+")"));     
