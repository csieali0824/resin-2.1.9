...
{   
  location.href="../jsp/INVIssStatus.jsp?STATUSID="+statusID+"&PAGEURL="+pageURL+"&SEARCHSTRING="+document.MYFORM.SEARCHSTRING.value  
}
...
{  //out.println("Step1"); 
   ...
   //取得資料總筆數  
	...
	{	//out.println("Step3");
	   if (statusID.equals("004")) //為領料單倉管確認中,可看到其所管轄倉庫之所有領用申請案件
  	   { //out.println("Step4");
	     if (searchString!=null) //如果有搜尋特定單號則另下SQL
	     {
...
<strong><font color="#0080C0" size="5">領料案件處理</font></strong> <FONT COLOR=RED SIZE=4>&nbsp;&nbsp;狀態:<%=statusName%>(<%=statusDesc%>)</FONT><FONT COLOR=BLACK SIZE=2>(總共<%=maxrow%>筆記錄)</FONT>
...
<%    
  ...
  {   
   ...
   String sql=null;
	...
	{
	   if (statusID.equals("004")) //為領料單倉管確認中,可看到其所管轄倉庫之所有領料申請案件
	   { 
	     if (searchString!=null) //如果有搜尋特定單號則另下SQL
	     {
