<!--20140822 Peggy ,新增ERP END CUSTOMER ID欄位-->
...
<!--20160313 Peggy,add sample order direct ship to cust flag-->
...
String strQty="",strCustItem="",strSellingPrice="",strCRD="",strShippingMethod="",strFOB="",strEndCust="",strErr="",strSSD="",strMarketGroup="",strPOLINENO="",strQUOTE="";
String strEndCustID ="",strEndCustID1=""; //add by Peggy 20140822
String rowColor="#ffffff",highlightColor="#EEDDCC";
String tsc_package="",tsc_family=""; //add by Peggy 20150721
String strEndCustPartNo="",strDelivertoid="";  //add by Peggy 20190225
...
if (InsertFlag.equals("Y"))
{
	sql = " SELECT a.customer_no,a.customer_id,"+
		  ...
		  ",a.PO_LINE_NO,a.QUOTE_NUMBER"+
		  ",e.customer_number END_CUSTOMER_ID,a.END_CUSTOMER,END_CUSTOMER_PARTNO"+  //add by Peggy 20140825
		  ...
		  " and b.description not like '%Disable%'"+ 
		  " and nvl(b.customer_order_flag,'N')='Y'"+  
		  " and nvl(b.customer_order_enabled_flag,'N')='Y'"+ 
		  " and tsc_item_pcn_flag(43,b.inventory_item_id,trunc(sysdate))='N'";  	  
	...
	while(rs.next())
	{
		if (icnt ==0)
		{
			aa=new String[Integer.parseInt(rs.getString("rowcnt"))][oneDArray.length];
		...
		aa[icnt][29]=(rs.getString("end_customer_partno")==null?"&nbsp;":rs.getString("end_customer_partno"));    //20190225 end customer partno by Peggy
		icnt++; 
	}
	rs.close();
	st.close();
	...
	session.setAttribute("PROGRAMNAME","D4-003");
	urlDir = "TSSalesDRQ_Create.jsp?CUSTOMERID="+java.net.URLEncoder.encode(CustomerID)+
			 ...
			 "&ORIGCURR="+java.net.URLEncoder.encode(strCurr)+
			...
			<table width="100%" cellspacing="0" cellpadding="0" bordercolordark="#009933">
				<tr>
					<td height="50" align="center">
						<font color="#003399" size="+2" face="Arial Black">TSCE </font>
		...
		%>		
			<table align="center" width="100%" cellspacing="0" cellpadding="0" bordercolordark="#990000">
					...
					<TD width="85%">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td bgcolor="#CAE1CF" width="15%" align="center"><a href="../jsp/TSCEBufferImport.jsp?ACTIONCODE=DETAIL" style="color:888888;font-family:Arial;font-size:13px;text-decoration:none;">Pending Detail</a></td>
					...
					<td colspan="2">
						<table  bordercolordark="#000033" cellspacing="0"  cellpadding="0" width="100%" align="left" bordercolorlight="ffffff" border="1">
			  				...
			  				<tr>
								...
								<td width="85%" ><A HREF="../jsp/samplefiles/D4-003_SampleFile.xls"><font style="font-size:13px;font-family:arial">Download Sample File</font></A></td>
			  				</tr>
			  				<tr>
								<td style="color:#ffffff;font-size:13px;font-family:ARIAL;background-color:#69A2B4">User Guide：</td>
								<td width="85%" ><A HREF="../jsp/samplefiles/D4-003_User_Guide.doc"><font style="font-size:13px;font-family:arial">Download User Guide</font></A></td>
			  				</tr>
						</table>
					</td>
				</tr>
				<tr>
					...
					<input type="button" style="font-size:13px;font-family:arial" name="submit1" value='Upload' onClick="setCreate('../jsp/TSCEBufferImport.jsp?ACTIONCODE=UPLOAD')">
					</td>
				</tr>
			</table>
		<%
			...
			<table align="center" width="100%" cellspacing="0" cellpadding="0" bordercolordark="#990000">
				<tr>
					<TD width="85%">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td bgcolor="#69A2B4" width="15%" height="20" align="center" style="border-color:#CAE1CF;border:insert;color:#ffffff;font-family:Arial;font-size:13px"><a href="../jsp/TSCEBufferImport.jsp?ACTIONCODE=DETAIL" style="color:FFFFFF;font-family:Arial;font-size:13px;text-decoration:none;">Pending Detail</a></td>
								<td bgcolor="#CAE1CF" width="15%" align="center"><a href="../jsp/TSCEBufferImport.jsp" style="color:888888;font-family:Arial;font-size:13px;text-decoration:none;">Excel Upload</a></td>
								<td width="70%" style="border-color:ffffff;">&nbsp;</td>
							</tr>
						</table>
					</TD>
...
if (ACTIONCODE.equals("UPLOAD"))
{
	try
	{
		...
		else
		{
			...
			String uploadFilePath="\\resin-2.1.9\\webapps\\oradds\\jsp\\upload_exl\\D4003_"+strDateTime+"-"+uploadFile_name;
			...
			for (int i = sRow; i < sht.getRows(); i++) 
			{
				...
				//out.println("strCustItem="+strCustItem);
				...
				//CRD
				jxl.Cell cellCRD = sht.getCell(7, i);          
				strCRD = (cellCRD.getContents()).trim();
				if (strCRD == null) strCRD= "";

				//PO LINE NO
				jxl.Cell cellPOLINENO = sht.getCell(8, i);          
				strPOLINENO = (cellPOLINENO.getContents()).trim();
				if (strPOLINENO == null) strPOLINENO= "";
				
				//QUOTE NUMBER
				jxl.Cell cellQUOTE = sht.getCell(9, i);          
				strQUOTE = (cellQUOTE.getContents()).trim();
				...
				//SHIPPING METHOD,add by Peggy 20140425
				...
				//END CUSTOMER ID,add by Peggy 20140822
				jxl.Cell cellEndCustID = sht.getCell(12, i);          
				strEndCustID = (cellEndCustID.getContents()).trim();
				if (strEndCustID==null) strEndCustID="";				
				
				strErr ="";strCustID="";strCustName="";strItemID="";rec_cnt=0;strFactory="";strOrderType="";strOrderTypeID="";strFOB="";strSSD="";strMarketGroup="";strDelivertoid="";//初始化
				...
				else
				{
					...
					ResultSet rsa=statementa.executeQuery("select CUSTOMER_ID,CUSTOMER_NAME,ATTRIBUTE2 market_group,CUSTOMER_NUMBER from ar_CUSTOMERS where status = 'A'  and CUSTOMER_NUMBER ='"+strCustNo+"'");
					if(rsa.next())
					{
						strCustID = rsa.getString("CUSTOMER_ID"); 
						strCustName=rsa.getString("CUSTOMER_NAME"); 
						strMarketGroup=rsa.getString("market_group");
						...
						while (rsb.next())
						{
							...
							if (rsb.getString("SITE_USE_CODE").equals("DELIVER_TO"))
							{
								strDelivertoid = rsb.getString("SITE_USE_ID");
							}
						}
						rsb.close();
						statementb.close();
				...
				//品名	
				if ((strItemDesc  == null || strItemDesc.equals("")) && (strCustItem == null && strCustItem.equals("")))
				{
					strItemName = "&nbsp;";
					strItemDesc = "&nbsp;";
					strCustItem = "&nbsp;";
					strErr += "台半料號(品名)及客戶品名不可同時空白<br>";
				}
				else
				{ 
					...
					{						  
						sql = " select  DISTINCT a.item,a.ITEM_DESCRIPTION,a.INVENTORY_ITEM_ID,"+
							  ...
							  " ,d.attribute1 end_customer_partno"+ //add by Peggy 20190225
							  " from oe_items_v a"+
							  ",inv.mtl_system_items_b msi "+
							  " ,APPS.MTL_ITEM_CATEGORIES_V c "+
							  " ,MTL_CUSTOMER_ITEM_XREFS d"+  //add by Peggy 20190225
						...
						while(rs.next())
						{
							...
							strEndCustPartNo = rs.getString("end_customer_partno");  //add by Peggy 20190225
							...
							if (strMarketGroup.equals("AU") ||  (!strCustNo.equals("1521") && rs.getString("TSC_PACKAGE").equals("SMA"))) //modify by Peggy 20140425
							{
								strFactory ="002";
								strOrderType ="1156";
							}
					...
					else
					{
						if (strItemDesc != null && !strItemDesc.equals(""))
						{
							sql = " SELECT DISTINCT msi.description,msi.segment1, msi.inventory_item_id,msi.primary_uom_code,NVL (msi.attribute3, 'N/A') attribute3,"+
								  ...
								  " AND msi.inventory_item_status_code <> 'Inactive'"+
                      			  " AND (msi.description =  '"+strItemDesc+"' or msi.segment1='"+strItemDesc+"')";	
							Statement statement=con.createStatement();
							//out.println(sql);
							ResultSet rs = statement.executeQuery(sql);
							while(rs.next())
							{
								...
								if (strMarketGroup.equals("AU") && rs.getString("TSC_PACKAGE").equals("SMA"))
								{
									strFactory ="002";
									strOrderType ="1156";
								}
					...
					else if (strItemDesc.indexOf("-ON")>0 && (strEndCustPartNo==null || strEndCustPartNo.equals("")))
					{
						strErr += "EBV客戶品號不可空白<br>";
					}
					...
					csf.setString(9,strCustID);  //add by Peggy 20160513
					csf.setString(10,strFOB);    //add by Peggy 20190319
					csf.setString(11,strDelivertoid);        //add by Peggy 20190319
					csf.execute();
					strShippingMethod = csf.getString(6);                
					csf.close();	
										
					CallableStatement csg = con.prepareCall("{call tsc_edi_pkg.GET_SSD(?,?,?,?,?,?,?,sysdate,?,?)}");
					csg.setString(1,SalesAreaNo);
					csg.setString(2,strFactory);      
					csg.setString(3,strCRD);                   
					csg.setString(4,strShippingMethod);    
					csg.setString(5,strOrderType);   
					csg.registerOutParameter(6, Types.VARCHAR);  
					csg.setString(7,strCustID);   
					csg.setString(8,strFOB);          //add by Peggy 20210207
					csg.setString(9,strDelivertoid);  //add by Peggy 20210208
					csg.execute();
					strSSD = (csg.getString(6)==null?"":csg.getString(6));                
					csg.close();						
				}
				...
				else
				{
					...
					if ((strOrderType.equals("1141") ||strOrderType.equals("1131")) && strFOB.startsWith("FOB"))
					{
						strFOB ="FOB TAIWAN";
					}					
				}
				...
				//PO LINE NO 
				if (strPOLINENO == null || strPOLINENO.equals(""))
				{
					strPOLINENO = "";
				}
				
				//QUOTE
				if (strQUOTE == null || strQUOTE.equals(""))
				{
					strQUOTE = "";
				}
...
%>
						<table cellspacing="0" bordercolordark="#ffffff" cellpadding="1" width="100%" align="center" bordercolorlight="#ffffff" border="1">
							<tr bgcolor="#96AEBC" style="color:#ffffff;font-size:11px;font-family:ARIAL">
								...
								<td width='4%'>CRD</td>
								<td width='4%'>Shippine Method</td>
								<td width='3%'>SSD</td>
								<td width='3%'>PO Line No</td>
								<td width='4%'>Quote Number</td>
...
%>
					<tr bgcolor="#CCFFAC" style="color:#000000;font-size:11px;font-family:ARIAL">
						...
						<td><%=strSSD%></td>
						<td><%=(strPOLINENO==null ||strPOLINENO.equals("")?"&nbsp;":strPOLINENO) %></td>
						<td><%=(strQUOTE==null || strQUOTE.equals("")?"&nbsp;":strQUOTE)%></td>
						<td><%=(strLineRemark==null || strLineRemark.equals("")?"&nbsp;":strLineRemark)%></td>
...
<%					
				...
				aa[icnt][21]=strPOLINENO;
				aa[icnt][22]=strQUOTE.replace("&nbsp;","");
				aa[icnt][23]=strEndCustID1;
				aa[icnt][24]=strEndCust;
				aa[icnt][25]=strEndCustPartNo; //add by Peggy 20190225
...
<%				
			out.println("<table width='80%' align='center'><tr><td align='center'><font style='color:#ff0000;font-family:標楷體;font-size:16px'>上傳動作失敗，請洽系統管理員，謝謝!</font></td></tr></table>");		
		}
		else
		{
			for (int i =0 ; i < aa.length ; i++)
			{
				...
				pstmt.setString(23,aa[i][21]);           //客戶訂單項次
				pstmt.setString(24,aa[i][22]);           //QUOTE NUMBER			
				pstmt.setString(25,aa[i][23]);           //END CUSTOMER ID,add by Peggy 20140822	
				pstmt.setString(26,aa[i][24]);           //END CUSTOMER,add by Peggy 20140822	
				pstmt.setString(27,aa[i][25]);           //END CUSTOMER PARTNO,add by Peggy 20190225	
				pstmt.executeQuery();
				pstmt.close();
				insert_cnt++;
			}
	...
	if (insert_cnt >0) 
	{
		//response.sendRedirect("TSCEBufferImport.jsp?ACTIONCODE=DETAIL");
	%>
		<script language="javascript">
			location.href("TSCEBufferImport.jsp?ACTIONCODE=DETAIL");
		</script>
...
{
%>
	<table align="center" border="1" width="98%" cellspacing="0" cellpadding="1" bordercolorlight="#DFD9DD" bordercolordark="#4A598A">
		<tr style="background-color:#69A2B4;font-family:arial;color:#ffffff;font-size:11px" >
			<td width="2%"  align="center">&nbsp;</td>
			<td width="18%" align="center">Customer Name</td>
			<td width="6%"  align="center">RFQ Type</td>
			<td width="8%" align="center">Customer PO</td>
			<td width="12%" align="center">Item Desc</td>
			<td width="8%"  align="center">Customer Item</td>
			...
			<td width="5%"  align="center">End Cust ID</td>
	...
	try
	{
		...
		sql = " SELECT a.salesareano, a.upload_date, a.upload_by, a.customer_no,a.customer_id,"+
			  ...
			  " (select count(1) from oraddman.tsc_rfq_upload_temp c where  c.create_flag='N' and c.salesareano=a.salesareano and c.customer_no= a.customer_no and c.customer_po=a.customer_po and c.upload_by=a.upload_by) rowcnt"+
			  ",e.customer_number end_customer_id,a.end_customer,a.end_customer_partno"+  //add by Peggy 20140825
			  " FROM oraddman.tsc_rfq_upload_temp a"+
			  ",inv.mtl_system_items_b b"+
			  ",ORADDMAN.TSAREA_ORDERCLS d"+
			  ",ar_customers e"+
			  " where a.create_flag=?"+
			  " and a.salesareano=?"+
	...
	<%
			if (!customerpo.equals(rs.getString("customer_po")) || !customerno.equals(rs.getString("customer_no")) || !upload_by.equals(rs.getString("upload_by")))
			{
				i++;
	%>
				<td rowspan="<%=rs.getString("rowcnt")%>" style="color:#000000;font-family:Arial;font-size:11px"><input type="button" name="btn<%=i%>" value="Delete" style="font-family:arial;font-size:11px" title="刪除資料" onClick="delData('../jsp/TSCEBufferImport.jsp?DELFLAG=Y&CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERPO=<%=rs.getString("customer_po")%>&UPLOADBY=<%=rs.getString("upload_by")%>')"></td>
				<td id="tda<%=i%>" rowspan="<%=rs.getString("rowcnt")%>" onMouseOver="focuscolor(<%=i%>)" onMouseOut="unfocuscolor(<%=i%>)" style="color:#000000;font-family:Arial;font-size:11px" onClick="javascript:location.href='TSCEBufferImport.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERPO=<%=rs.getString("customer_po")%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面"><%="("+rs.getString("customer_no")+")"+rs.getString("customer_name")%></td>
	...
	{
		out.println("<tr><td colspan='9'>Exception:"+e.getMessage()+sql+"</td></tr>");
	}	 
	%>
		</table>
<%
