...
{ 
	...
	var SHIPPING_MARK = document.MYFORM.SHIPPING_MARK.value; //add by Peggy 20160401
	var CHECK_FUNCTION = document.MYFORM.CHECK_FUNCTION.value; //add by Peggy 20240507
	document.MYFORM.btnSubmit.disabled =true;
	document.MYFORM.btnCancel.disabled =true;
	document.MYFORM.action=URL+"&LABEL_CODE="+LABEL_CODE+"&LABEL_GROUP_CODE="+LABEL_GROUP_CODE+"&LABEL_NAME="+LABEL_NAME+"&LABEL_TYPE_CODE="+LABEL_TYPE_CODE+"&EFFECTIVE_FROM="+EFFECTIVE_FROM+"&EFFECTIVE_TO="+EFFECTIVE_TO+"&SHIPPING_MARK="+SHIPPING_MARK+"&CHECK_FUNCTION="+CHECK_FUNCTION;
...
function subWindowFind(itemid)
{
	if (itemid=="1")
	{
		subWin=window.open("../jsp/subwindow/TSLabelGroupInfo.jsp","subwin","width=740,height=480,scrollbars=yes,menubar=no");  
	}
	else if (itemid=="2")
	{
		subWin=window.open("../jsp/subwindow/TSLabelTypeInfo.jsp","subwin","width=740,height=480,scrollbars=yes,menubar=no");  
	}
}
...
<title>TS Label Maintain</title>
</head>
<body>
<FORM NAME="MYFORM" METHOD="POST" ACTION="TSLabelProcess.jsp" ENCTYPE="multipart/form-data" > 
<%
...
String SHIPPING_MARK= request.getParameter("SHIPPING_MARK");  //add by Peggy 20160401
if (SHIPPING_MARK==null) SHIPPING_MARK="";
String CHECK_FUNCTION= request.getParameter("CHECK_FUNCTION");  //add by Peggy 20240507
if (CHECK_FUNCTION==null) CHECK_FUNCTION="";
...
{  
	if (STATUS.equals("UPD"))
	{
		sql = " select a.label_code, a.label_name, a.label_file, a.label_group_code,a.label_type_code,"+
			  ...
			  ",a.SHIPPING_MARK"+ //add by Peggy 20160401
			  ",a.CHECK_FUNCTION"+ //add by Peggy 20240507
			  " from oraddman.ts_label_all a,oraddman.ts_label_groups b,oraddman.ts_label_types c "+
		...
		if (rs.next())
		{
			...
			if (SHIPPING_MARK==null) SHIPPING_MARK="";
			CHECK_FUNCTION = rs.getString("CHECK_FUNCTION");
			if (CHECK_FUNCTION==null) CHECK_FUNCTION="";
		}
		else
		{
	...
	<tr>
		<td>
			<table width="100%"  align="CENTER" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#99CC99">
				...
				<tr>
					<td width="20%" height="25" bgcolor="#C9E2D0" nowrap><span style="font-size:12px;color:#006666">標籤檔名</span>：</td>
					<td nowrap><INPUT TYPE="TEXT" name="LABEL_CODE" value="<%=LABEL_CODE%>" size="30" style="font-family: Tahoma,Georgia;" readonly></td>
				</tr>					
				<tr>
				...
				<tr>
					<td width="20%" height="25" bgcolor="#C9E2D0" nowrap><span style="font-size:12px;color:#006666">指定嘜頭</span>：</td>
					<td nowrap><INPUT TYPE="TEXT" name="SHIPPING_MARK" value="<%=SHIPPING_MARK%>" size="60" style="font-family: Tahoma,Georgia;">(請輸入指定嘜頭,一個以上嘜頭者,請用半形逗號區隔,若無指定,此欄位保持空白)</td>
				</tr>
				<tr>
					<td width="20%" height="25" bgcolor="#C9E2D0" nowrap><span style="font-size:12px;color:#006666">Check Function</span>：</td>
					<td nowrap><INPUT TYPE="TEXT" name="CHECK_FUNCTION" value="<%=CHECK_FUNCTION%>" size="60" style="font-family: Tahoma,Georgia;"></td>
				</tr>					
				<tr>
	...
	<tr>
		<td>
			<table width="100%" border="0">
  				<tr align=center>
    				<td width="16%"> <input type="button"  name="btnSubmit" onClick='setSubmit("../jsp/TSLabelProcess.jsp?STATUS=<%=STATUS%>")' value="存檔" style="font-family:'Tahoma,Georgia';font-size:12px">&nbsp;&nbsp;&nbsp;
     								<input type="button" name="btnCancel" onClick='setCancel("../jsp/TSLabelQuery.jsp")' value="回上頁" style="font-family:'Tahoma,Georgia';font-size:12px">
					</td>    
  				</tr>
			</table>
		</td>
