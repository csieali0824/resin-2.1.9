...
function setSubmit(URL)
{
   ...
   {     
     ...
     { 
      alert("Before you want to add , please do not let the Monthly sales quota of forecast  be Null !!");   
      return(false);
      } 
   ...
   {     
     ...
     { 
	  c=txt.charAt(j);
	    ...
	    {
            alert("Monthly sales quota of forecast  should be numerical!!");   
            return(false);
		 }
      } 
   } //end of for  null check
...
<!--	  <td width="7.5%" colspan="1" bgcolor="#FF6666"><div align="center"><font size="2">  -->
	  <%
	    for (int i=0;i<12;i++)   
		 {
   	       if (i==0)
 		     { out.println("<td width='7.5%' colspan='1' bgcolor='#FF6666'><div align='center'><font  size='2'>"); }
		   else	 
...
//		   if (bringLast==null || chooseItem.equals("--"))
          ...
          Statement statementC=ifxDbtelcon.createStatement();
		    ...
		    //--------------------------------------------------------------------
			 ...
			 for (int ti=0;ti<temp.length;ti++)
			 {
			    for (int tj=1;tj<=commitmentMonth;tj++)  //因為只有n個月為commitment
				{
				  ...
				  chkrs=chkstat.executeQuery("select FSQUOTA,FPMVER FROM PSALES_HIERCHY_SQUOTA where FSHIERCHY='"+dept+"'  and FSALES='"+temp[ti][0]+"' and FPYEAR='"+tpy+"' and FPMONTH='"+tpm+"' and FPCURR='"+curr+"' order by FPMVER DESC");                  				  
				  if (chkrs!=null)
				  {				    					 
					 if ( chkrs.next())
					 {					   
