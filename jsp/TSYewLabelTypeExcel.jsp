...
<body>
<FORM ACTION="../jsp/TSYewTSLabelTypeExcel.jsp" METHOD="post" name="MYFORM">
<%
...
{ 	
	...
	sql = " SELECT a.LABEL_TYPE_CODE,a.LABEL_TYPE_NAME,a.description,to_char(a.LAST_UPDATE_DATE,'yyyy-mm-dd') LAST_UPDATE_DATE,a.LAST_UPDATED_BY,to_char(a.EFFECTIVE_FROM_DATE,'yyyy-mm-dd') EFFECTIVE_FROM_DATE,to_char(a.EFFECTIVE_TO_DATE,'yyyy-mm-dd') EFFECTIVE_TO_DATE,a.PRINT_NUM,a.LABEL_TYPE ,a.LABEL_SIZE"+
	...
	{ 	
		if (reccnt==0)
		{
			...
			//標籤尺寸
			ws.addCell(new jxl.write.Label(col, row, "標籤尺寸(mm)" , ACenterBLB));
			ws.setColumnView(col,15);	
			col++;

			//標籤容器
			ws.addCell(new jxl.write.Label(col, row, "材積種類" , ACenterBLB));
			ws.setColumnView(col,15);	
			col++;
		...
		col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("LABEL_SIZE")==null?"":rs.getString("LABEL_SIZE")), ALeftL));
		col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("LABEL_TYPE")==null?"":rs.getString("LABEL_TYPE")), ALeftL));
		col++;	
