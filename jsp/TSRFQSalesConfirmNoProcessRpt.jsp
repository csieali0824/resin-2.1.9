...
String organizationCode=request.getParameter("ORGANIZATION_CODE");
  ...
  String hourSet=request.getParameter("HOURSET");   
  ...
  if (hourSet == null || hourSet.equals("")) hourSet = "72";
...
<FORM ACTION="../jsp/TSRFQSalesConfirmNoProcessRpt.jsp" METHOD="post" NAME="MYFORM">
...
<strong><jsp:getProperty name="rPH" property="pgRFQConfirmed"/><jsp:getProperty name="rPH" property="pgNonProcess"/><jsp:getProperty name="rPH" property="pgDetailRpt"/></strong></font>
<!--%/20040109/將Excel Veiw 夾在檔頭%-->
<%
 
  sWhereGP = " and LSTATUSID not in ('013') and LSTATUS != 'CANCELED' "; // 不看取消單據
...
/*  檢查使用是否有查詢其它地區詢問單的權限 -- 依登入時的使用者群組 */
...
if ((dateSetBegin==null || dateSetBegin.equals("")) && (dateSetEnd==null || dateSetEnd.equals("")))
{
   ...
   sWhere =  "where c.DNDOCNO = d.DNDOCNO "+
           //  "  and substr(c.CREATION_DATE,0,8) >= '20060801' "+ //8月/1日起實施
			 "  and d.ASSIGN_MANUFACT = e.MANUFACTORY_NO(+) "+
			 // "or  d.DOCNO || '-' || d.ASSIGN_LNO in ("+distDnDocNo+") ) "+
			 "  and b.LOCALE='"+locale+"' "+
			 "  and b.SALES_AREA_NO=c.TSAREANO "+
			 "  and d.DNDOCNO = g.DNDOCNO and d.LINE_NO = g.LINE_NO and g.ORISTATUSID = '007' "+	// 找第一筆回覆給業務的時間(RESPONDING)	
			 "  and ( ROUND(( to_date(to_char(SYSDATE,'YYYYMMDDHH24MISS'),'YYYYMMDDHH24MISS') - to_date(g.CDATETIME,'YYYYMMDDHH24MISS') ) * 24,2) > "+hourSet+" "+ //找目前系統時間大於企劃回覆處理72小時的單據
			 "  and d.LSTATUSID <= '008' ) ";  
...
{   
  ...
  */ 
   ...
   // if (salesAreaNo==null || salesAreaNo.equals("") || salesAreaNo.equals("--")) {sWhere=sWhere+" ";}
    if (salesAreaNo!=null && !salesAreaNo.equals("--")) { sWhere=sWhere+" and c.TSAREANO ='"+salesAreaNo+"'";	}
	...
	if (createdBy!=null && !createdBy.equals("")) { sWhere=sWhere+" and c.CREATED_BY ='"+createdBy+"'"; }
...
%> 
  <table cellSpacing='1' bordercolordark='#D8DEA9'  cellPadding='1' width='100%' align='center' borderColorLight='#ffffff' border='1' bordercolor="#CCCCCC">     	 	 
	 <tr>
	    <td width="13%" colspan="1" nowrap><font color="#006666"><strong>
	      <jsp:getProperty name="rPH" property="pgSalesArea"/></strong></font>         
       </td> 
		<td width="40%">
		   <div align="left">
		...
		<td width="21%" colspan="1" nowrap><font color="#006666"><strong>
		  <jsp:getProperty name="rPH" property="pgProdFactory"/> </strong></font>         
       </td> 
		<td width="26%">
		   <div align="left">
		   <font color="#006666"><strong> </strong></font>
     ...
     <tr>	    
	   <td nowrap colspan="2"><font color="#006666"><strong><jsp:getProperty name="rPH" property="pgCreateFormDate"/><jsp:getProperty name="rPH" property="pgDateFr"/></strong></font>
        <%
	...
	<td colspan="2">
		    <INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>' onClick='setSubmit("../jsp/TSRFQSalesConfirmNoProcessRpt.jsp")' > 			 
	</td>
   </tr>
  </table>  
<%
			  ...
			  //out.println("wipEntID="+wipEntID);out.println("rsTC.getString(INVENTORY_ITEM_ID)="+rsTC.getString("INVENTORY_ITEM_ID"));
			       ...
			       { 
					  ...
					  %><jsp:getProperty name="rPH" property="pgCreateFormUser"/><%
					           String sqlCrUser = "select upper(USERNAME) from WSUSER where WEBID = '"+rsTC.getString("CREATED_BY")+"' "; // 取開單人員							   
				               Statement stateCrUser=con.createStatement();
							   //out.println(sqlM);
                               ResultSet rsCrUser=stateCrUser.executeQuery(sqlCrUser);
							   if (rsCrUser.next())
							   { 
							     out.println(" : "+rsCrUser.getString(1));// 取大寫名稱
							   }
							   rsCrUser.close();
							   stateCrUser.close();			    
					   ...
					   out.println(" : "+rsTC.getString("CREATION_DATE"));
					   out.println("</font></td>"); 
					   out.println("</tr>");
			              ...
			              {			    
				            ...
				            // if ( dnDocNo ==rsTC.getString("DNDOCNO") || dnDocNo.equals(rsTC.getString("DNDOCNO")) )
				             //{ //out.println("wipEntID="+wipEntID);out.println("rsTC.getString(INVENTORY_ITEM_ID)="+rsTC.getString("INVENTORY_ITEM_ID"));
					           ...
					           %><jsp:getProperty name="rPH" property="pgTotal"/><%
					           out.println("<font color='#CC3366'>"+rsTC.getString("MAXLINE")+"</font>");
					           %><jsp:getProperty name="rPH" property="pgItemQty"/><jsp:getProperty name="rPH" property="pgRFQConfirmed"/><jsp:getProperty name="rPH" property="pgNonProcess"/><%
					           out.println("</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"); 
							   %>
					           ...
					           %><jsp:getProperty name="rPH" property="pgCreateFormUser"/><%
							     ...
							     rsCrUser.close();
							     stateCrUser.close();	
					             //out.println(" : <font color='#CC3366'>"+rsTC.getString("CREATED_BY")+"</font>");					           
					             out.println("</font>");  
								 out.println("<font color='#006666'>");
					           %><jsp:getProperty name="rPH" property="pgCreateFormDate"/><%
				               ...
				               String sqlM = "select DISTINCT d.LINE_NO,d.LSTATUS,d.ITEM_DESCRIPTION, d.QUANTITY,d.UOM, "+
										     ...
										     "from ORADDMAN.TSSALES_AREA b, "+
										          "ORADDMAN.TSDELIVERY_NOTICE_DETAIL d, ORADDMAN.TSPROD_MANUFACTORY e, "+
												  "ORADDMAN.TSDELIVERY_DETAIL_HISTORY f ";										     
							  String whereM = "where d.LSTATUSID != '013' and d.LSTATUS != 'CANCELED' and (d.DNDOCNO = '"+rsTC.getString("DNDOCNO")+"' ) "+ // 不取單據狀態為Canceled
							                  "and d.DNDOCNO = f.DNDOCNO and d.LINE_NO = f.LINE_NO "+
											  "and d.ASSIGN_MANUFACT = e.MANUFACTORY_NO(+) "+										      
										      "and substr(d.DNDOCNO,3,3)=b.SALES_AREA_NO and b.LOCALE='"+locale+"' "+
											  "and f.ORISTATUSID ='007' "+	
											  "and ( ROUND((to_date(to_char(SYSDATE,'YYYYMMDDHH24MISS'),'YYYYMMDDHH24MISS') - to_date(f.CDATETIME,'YYYYMMDDHH24MISS') ) * 24,2) > 72 "+ //找企劃回覆但業務未處理大於72小時的單據	
			                                  " and d.LSTATUSID <= '008' ) "; 			
							...
							*/   
				               ...
				               { 
								...
								out.println("</font></td><td nowrap><font size='1' color='#FFFFFF'>");
								%><jsp:getProperty name="rPH" property="pgPlanRspPeriod"/><%								
				           ...
				           //  }  // End of if (dnDocNo ==rsTC.getString("DNDOCNO") || dnDocNo.equals(rsTC.getString("DNDOCNO"))) 
							  /*
					                    ...
					                    out.println("</font>"); 
										 %><a href="../jsp/TSRFQFactResponseMOCreateRpt.jsp?SPANNING=TRUE&ORGANIZATION_ID=<%=organizationId%>&DNDOCNO=<%=rsTC.getString("DNDOCNO")%>&DATESETBEGIN=<%=dateSetBegin%>&DATESETEND=<%=dateSetEnd%>#<%=rsTC.getString("DNDOCNO")%>"><img src="../image/PLUS.gif" width="14" height="15" border="0"></a>
										 <% 
										 out.println("<font size='-1' color='#006666'>");
					                     %><jsp:getProperty name="rPH" property="pgCreateFormUser"/><%
...
<input name="HOURSET" type="hidden" value="<%=hourSet%>">
</FORM>
