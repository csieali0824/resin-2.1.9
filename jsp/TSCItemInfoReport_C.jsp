<!-- 20151123 Peggy,add TSC_PROD_FAMILY column-->
<!-- 20160114 Peggy,call function TSC_GET_ITEM_SPQ_MOQ get spq,moq,sample_spq  value-->
<!-- 20160421 Peggy,add no wafer lead time-->
<!-- 20161222 Peggy,add 工廠別,官網AECQ,DESCRIPTION,STATUS,PCN/PDN,CARTON SIZE,CARTON WEIGHT,Product Group 8-->
<!-- 20170510 Peggy,Product Group 8同Forecast 及 BI 都使用 Function TSC_INV_Prod_Group_7-->
<!-- 20170911 Peggy,新增reel,box,carton qty column-->
<!-- 20170913 Peggy,檔案日漸長大,改存至資料夾,讓user自行下載-->
<!-- 20171017 Peggy,新增22D creation date-->
<!-- 20180202 Peggy,新增Last Order Date-->
<!-- 20180223 Peggy,新增TSC PROD CATEGORY-->
<!-- 20180620 Peggy,新增HTS Code FOR TSCA issue-->
<!-- 20180718 Peggy,column rename "TSC PART NO" to "TSC Ordering Code",add new column "part id" not include packing code-->
<!-- 20180810 Peggy,增加欄位COO(A01/SSD/PMD值=TAIWAN,TEW外購/YEW值=CHINA),HTS Code欄位改名TARIC code-->
<!-- 20180928 Peggy,新增Packaging Description-->
<!-- 20190328 Peggy,新增MSL-->
...
String strAECQ="",strDesc="",strCartonSize="",strGW="",strwebstatus="",strgroup="",obsoletedate="",strPath="",strPartName="",item_desc="";
...
{ 	
	...
	strPath ="\\resin-2.1.9\\webapps\\oradds\\TSCItemListCeline\\"+FileName;
    ...
    sql = "select a.*"+
		  ...
		  //",case when a.attribute3 in ('005') and a.tw_vendor_flag='N' then round(decode( pp_ssd.TP_PRICE_UOM,'KPC',pp_ssd.TP_PRICE/1000,pp_ssd.TP_PRICE),5) when a.attribute3 in ('008') and a.tw_vendor_flag='N' then round(decode( pp_prod.TP_PRICE_UOM,'KPC',pp_prod.TP_PRICE/1000,pp_prod.TP_PRICE),5) when a.attribute3 in ('002') then round(decode( pp_cny.TP_PRICE_UOM,'KPC',pp_cny.TP_PRICE/1000,pp_cny.TP_PRICE)/6.17,5) else decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) end AS TSCE_PRICE"+
          ...
          ",'PCE' UOM"+
		  //",nvl(tsbpt.bottom_price,round(case when a.tsc_prod_group in ('PMD') or (a.tsc_prod_group in ('SSD') and TSC_PROD_CATEGORY='MOSFET') then tsbp.sales_bottom_price "+
		  //"      when substr(a.description,1,4) in ('P6KE','SMBJ','SMCJ') then tsmp.mpp_price"+
		  //"      when substr(a.description,1,5) in ('1.5KE') then tsmp.mpp_price"+
		  //"      when substr(a.SEGMENT1,11,1) in ('2','H') then case when a.attribute3 in ('005') and a.tw_vendor_flag='N' then round(decode( pp_ssd.TP_PRICE_UOM,'KPC',pp_ssd.TP_PRICE/1000,pp_ssd.TP_PRICE),5) when a.attribute3 in ('008') and a.tw_vendor_flag='N' then round(decode( pp_prod.TP_PRICE_UOM,'KPC',pp_prod.TP_PRICE/1000,pp_prod.TP_PRICE),5) when a.attribute3 in ('002') then decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) *(1+(7.02315-6.17)/6.17) else  decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) end * 1.495"+
		  //"      when substr(a.SEGMENT1,11,1) not in ('2','H') then  case when a.attribute3 in ('005') and a.tw_vendor_flag='N' then round(decode( pp_ssd.TP_PRICE_UOM,'KPC',pp_ssd.TP_PRICE/1000,pp_ssd.TP_PRICE),5) when a.attribute3 in ('008') and a.tw_vendor_flag='N' then round(decode( pp_prod.TP_PRICE_UOM,'KPC',pp_prod.TP_PRICE/1000,pp_prod.TP_PRICE),5)  when a.attribute3 in ('002') then decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) *(1+(7.02315-6.17)/6.17) else decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) end * 1.404"+
		  //"      else null end,5)) as bottom_price"+ //add by Peggy 20181112
		  //",round(case when a.tsc_prod_group in ('PMD') or (a.tsc_prod_group in ('SSD') and TSC_PROD_CATEGORY='MOSFET') then tsbp.regional_buy_price/1000 "+
		  //"      else case when substr(a.SEGMENT1,11,1) in ('2','H') then case when a.attribute3 in ('005') and a.tw_vendor_flag='N' then round(decode( pp_ssd.TP_PRICE_UOM,'KPC',pp_ssd.TP_PRICE/1000,pp_ssd.TP_PRICE),5) when a.attribute3 in ('008') and a.tw_vendor_flag='N' then round(decode( pp_prod.TP_PRICE_UOM,'KPC',pp_prod.TP_PRICE/1000,pp_prod.TP_PRICE),5) when a.attribute3 in ('002') then decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) *(1+(7.02315-6.17)/6.17) else decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) end * 1.3225"+
		  //"                when substr(a.SEGMENT1,11,1) not in ('2','H') then case when a.attribute3 in ('005') and a.tw_vendor_flag='N' then round(decode( pp_ssd.TP_PRICE_UOM,'KPC',pp_ssd.TP_PRICE/1000,pp_ssd.TP_PRICE),5) when a.attribute3 in ('008') and a.tw_vendor_flag='N' then round(decode( pp_prod.TP_PRICE_UOM,'KPC',pp_prod.TP_PRICE/1000,pp_prod.TP_PRICE),5) when a.attribute3 in ('002') then decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) *(1+(7.02315-6.17)/6.17) else decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) end * 1.242"+
		  //"                else null end"+
		  //"       end,5) as sales_head_price"+ //add by Peggy 20190321 		  
          ",qq.SPQ"+
          ",qq.MOQ"+
          ",qq.SAMPLE_SPQ"+
          ",tt.LEAD_TIME"+
		  ...
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.arrow_avnet_future_book_cost else tgpbe.arrow_avnet_future_book_cost end arrow_avnet_future_book_cost"+  //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.arrow_avnet_future_spq else tgpbe.arrow_avnet_future_spq end arrow_avnet_future_spq"+                    //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.arrow_avnet_future_moq else tgpbe.arrow_avnet_future_moq end arrow_avnet_future_moq"+                    //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.arrow_avnet_future_mpp else tgpbe.arrow_avnet_future_mpp end arrow_avnet_future_mpp"+                    //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.arrow_avnet_future_moq_mpp else tgpbe.arrow_avnet_future_moq_mpp end arrow_avnet_future_moq_mpp"+        //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.arrow_avnet_future_remarks else tgpbe.arrow_avnet_future_remarks end arrow_avnet_future_remarks"+        //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.catalog_price else tgpbe.catalog_price end catalog_price"+                                               //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.last_time_buy_small_reel else tgpbe.last_time_buy_small_reel end last_time_buy_small_reel"+              //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.catalog_remarks else tgpbe.catalog_remarks end catalog_remarks"+                                         //modify by Peggy 20220503
		  //"       , case when tgpbe.tsc_item_name is null then tgpbn.price_book_code else tgpbe.price_book_code end price_book_code"+                                         //modify by Peggy 20220503
		  ...
		  //"       ,oraddman.ts_global_price_book_new tgpbn"+ //add by Peggy 20211029
		  //"       ,oraddman.ts_global_price_book_eol tgpbe"+ //add by Peggy 20220503
	...
	sql +="       AND msii.ORGANIZATION_ID=43"+    //add by Peggy 20190215
		  ...
		  //"       ,(select tsc_partno,mpp_price from oraddman.tsc_sales_mpp_price"+
		  //"         union "+
		  //"         select fairchild_partno tsc_partno,mpp_price from oraddman.tsc_sales_mpp_price) tsmp"+   //add by Peggy 20181112
		  ...
		  //"       ,oraddman.tsc_recommend_stock trs"+ //add by Peggy 20230807
		  ...
		  //"       AND a.segment1=trs.item_name(+)"+ //add by Peggy 20230807
		  //"       AND case when substr(a.segment1,9,2)='QQ' and substr(a.segment1,11,1)='2' then substr(a.item_desc,1,length(a.item_desc)-1) else a.item_desc end=tsmp.TSC_PARTNO(+)"+//add by Peggy 20210319
	...
	{ 	
		...
		if (reccnt==0)
		{
			...
			////Franchise Lead time (Week)
			//ws.addCell(new jxl.write.Label(col, row, "Franchise Lead time (Week)" , ACenterBLB));
			//ws.setColumnView(col,15);	
			//col++;				
			...
			////Part No
			//ws.addCell(new jxl.write.Label(col, row, "Part No" , ACenterBLB));
			//ws.setColumnView(col,15);	
			//col++;	
			...
			*/
			...
			////Price book Code,add by Peggy 20220127
			//ws.addCell(new jxl.write.Label(col, row, "Price book Code" , ACenterBLB));
			//ws.setColumnView(col,15);	
			//col++;			
			
			////Arrow/Avnet/Future Book cost (USD/PC),add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "Arrow/Avnet/Future Book cost (USD/PC)" , ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;	
			
			//TSCA disti代理商SPQ/MOQ follow TSC,add by Peggy 20230706	
			//SPQ for Arrow/Avnet/Future (PC),add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "SPQ for Arrow/Avnet/Future (PC)", ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;
			
			//MOQ for Arrow/Avnet/Future (PC),add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "MOQ for Arrow/Avnet/Future (PC)" , ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;
			
			////MPP for Arrow/Avnet/Future (USD/PC),add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "MPP for Arrow/Avnet/Future (USD/PC)", ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;
			
			//MOQ_MPP for Arrow/Avnet/Future (PC),add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "MOQ_MPP for Arrow/Avnet/Future (PC)" , ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;
			
			////Arrow/Avnet/Future Remark,add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "Arrow/Avnet/Future Remark" , ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;																				
			
			////Catalog price (USD/PC),add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "Catalog price (USD/PC)" , ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;	
			
			////Last time buy for small reel,add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "Last time buy for small reel" , ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;	

			////Catalog Remark,add by Peggy 20211029
			//ws.addCell(new jxl.write.Label(col, row, "Catalog Remark" , ACenterBLB));
			//ws.setColumnView(col,10);	
			//col++;	
				
				
			////Catalog Remark,add by Peggy 20230626
			//ws.addCell(new jxl.write.Label(col, row, "Design registration program" , ACenterBLB));
			//ws.setColumnView(col,12);	
			//col++;	
			...
			////Recommended to Stock in Channel,ADD BY PEGGY 20230807
			//ws.addCell(new jxl.write.Label(col, row, "Recommended to Stock in Channel" , ACenterBLB));
			//ws.setColumnView(col,12);	
			//col++;
		...
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("PART_ID")==null?"":rs.getString("PART_ID")), ALeftL)); //add by Peggy 20210729
		col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("PACKAGE_CODE")!=null && rs.getString("PACKAGE_CODE").indexOf("QQ")>=0?"":rs.getString("PACKAGE_CODE")) , (rs.getString("prefeered_packing_code_flag")!=null && rs.getString("prefeered_packing_code_flag").equals("Y")?ALeftLR:ALeftL)));
		col++;	
		...
		col++;	
		//if (rs.getString("franchise_lead_time_week")==null) //add by Peggy 20231002
		//{
		//	ws.addCell(new jxl.write.Label(col, row, "" , ACenterL));
		//}
		//else
		//{		
		//	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("franchise_lead_time_week")).doubleValue(), ARightL));
		//}
		//col++;			
		...
		//col++;	
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("price_last_update_date")==null?"":rs.getString("price_last_update_date")) , ACenterL));
		//col++;	
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("item_creation_date")==null?"":rs.getString("item_creation_date")) , ACenterL));
		//col++;	
		...
		//col++;					
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("factory_code")==null?"":rs.getString("factory_code")), ACenterL));
		col++;	
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("attribute3")==null?"":rs.getString("attribute3")) , ACenterL));
		//col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("coo")==null?"":rs.getString("coo")), ACenterL)); //add by Peggy 20180810
		col++;			
		...
		//}
		//col++;	
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("tsc_partno")==null?"":rs.getString("tsc_partno")), ALeftL));  //add by Peggy 20230426
		//col++;			
		...
		//col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("PACKAGINGDESCRIPTION")==null?"":rs.getString("PACKAGINGDESCRIPTION")) , ACenterL));  //add by Peggy 20180928
		col++;	
		...
		col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("pcn_list")==null?"":rs.getString("pcn_list")) , ALeftL));
		col++;	
		ws.addCell(new jxl.write.Label(col, row, (rs.getString("Product_Group_8")==null?"":rs.getString("Product_Group_8")) , ALeftL));  //modify by Peggy 20170510
		col++;	
		...
		col++;
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("last_order_date")==null?"":rs.getString("last_order_date")) , ACenterL));  //add by Peggy 20180202
		//col++;	
		...
		col++;	
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("ORDER_ENABLED_FLAG")==null?"":rs.getString("ORDER_ENABLED_FLAG")) , ACenterL));  //add by Peggy 20190218
		//col++;	
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("TW_VENDOR_FLAG")==null?"":rs.getString("TW_VENDOR_FLAG")), ACenterL));  //add by Peggy 20200130
		//col++;	
		...
		col++;
		*/	
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("price_book_code")==null?"":rs.getString("price_book_code")) , ACenterL));  //add by Peggy 20220127
		//col++;		
		//if (rs.getString("arrow_avnet_future_book_cost")==null)
		//{
		//	ws.addCell(new jxl.write.Label(col, row, "" , ACenterL));
		//}
		//else
		//{		
		//	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("arrow_avnet_future_book_cost")).doubleValue(), ARightL));
		//}
		//col++;
		//if (rs.getString("arrow_avnet_future_spq")==null)
		//{
		//	ws.addCell(new jxl.write.Label(col, row, "" , ACenterL));
		//}
		//else
		//{		
		//	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("arrow_avnet_future_spq")).doubleValue(), ARightL));
		//}
		//col++;
		//if (rs.getString("arrow_avnet_future_moq")==null)
		//{
		//	ws.addCell(new jxl.write.Label(col, row, "" , ACenterL));
		//}
		//else
		//{		
		//	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("arrow_avnet_future_moq")).doubleValue(), ARightL));
		//}
		//col++;				
		//if (rs.getString("arrow_avnet_future_mpp")==null)
		//{
		//	ws.addCell(new jxl.write.Label(col, row, "" , ACenterL));
		//}
		//else
		//{		
		//	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("arrow_avnet_future_mpp")).doubleValue(), ARightL));
		//}
		//col++;	
		//if (rs.getString("arrow_avnet_future_moq_mpp")==null)
		//{
		//	ws.addCell(new jxl.write.Label(col, row, "" , ACenterL));
		//}
		//else
		//{		
		//	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("arrow_avnet_future_moq_mpp")).doubleValue(), ARightL));
		//}
		//col++;												
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("arrow_avnet_future_remarks")==null?"":rs.getString("arrow_avnet_future_remarks")) , ACenterL));  //add by Peggy 20211029
		//col++;		
		//if (rs.getString("catalog_price")==null)
		//{
		//	ws.addCell(new jxl.write.Label(col, row, "" , ACenterL));
		...
		//}
		//col++;			
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("last_time_buy_small_reel")==null?"":rs.getString("last_time_buy_small_reel")) , ACenterL));  //add by Peggy 20211029
		//col++;		
		//ws.addCell(new jxl.write.Label(col, row, (rs.getString("catalog_remarks")==null?"":rs.getString("catalog_remarks")) , ACenterL));  //add by Peggy 20211029
		//col++;	
		//if (rs.getString("part_id")!=null && (rs.getString("part_id").equals("TESDH5V0U04P2Q1") ||
  		//    rs.getString("part_id").equals("TESDH3V3B02P4Q2") ||
		//	rs.getString("part_id").equals("TESDH5V0B05P1Q1") ||
		//	rs.getString("part_id").equals("TESDH5V0B03P1Q0") || 	
		//	rs.getString("part_id").equals("TESDL3V3B23P1Q0") || 
		//	rs.getString("part_id").equals("TESDL5V0B12P1Q0") ||
		//	rs.getString("part_id").equals("TESDH3V3B03P1Q0") ||
		//	rs.getString("part_id").equals("TESDL5V0B45P1M3") ||
		//	rs.getString("part_id").equals("TESDL3V3U1051M5") ||
		//	rs.getString("part_id").equals("TESDL5V0B20P1M5") ||
		//	rs.getString("part_id").equals("TQL820CA14V33") ||
		//	rs.getString("part_id").equals("TQL820CA14V50") ||
		//	rs.getString("part_id").equals("TS19605CA20H") ||
		//	rs.getString("part_id").equals("TSG65N068CE") ||
		//	rs.getString("part_id").equals("TSG65N110CE") ||
		//	rs.getString("part_id").equals("TSG65N195CE") ||
		//	rs.getString("part_id").equals("TSG65N190CR") ||
		//	rs.getString("part_id").equals("TQL821CSV33") ||
		//	rs.getString("part_id").equals("TQL821CSV50") ||
		//	rs.getString("part_id").equals("TQL850CSV33") ||
		//	rs.getString("part_id").equals("TQL850CSV50") ||
		//	rs.getString("part_id").equals("TQL851CSV33") ||
		//	rs.getString("part_id").equals("TQL851CSV50"))
		//   )
		//{
		//	ws.addCell(new jxl.write.Label(col, row,"Y" , ACenterL));  //add by Peggy 20230626
		//}
		//else
		//{
		//	ws.addCell(new jxl.write.Label(col, row,"" , ACenterL));  //add by Peggy 20230626
		//}
		//col++;		
	...
	if (RTYPE.equals("AUTO") && reccnt>0)
	{
		...
		{
			remarks="";
			message.addRecipient(Message.RecipientType.TO, new javax.mail.internet.InternetAddress("celine.yu@mail.ts.com.tw"));
		}
		message.addRecipient(Message.RecipientType.BCC, new javax.mail.internet.InternetAddress("peggy.chen@ts.com.tw"));
		
		if (DATATYPE.equals("ALL"))
		{
			message.setSubject("TS Item Report(C)(Include Inactive)"+" - "+dateBean.getYearMonthDay()+(SYSDATE.equals("")?"":"(price date"+SYSDATE+")")+remarks);
		}
		else if (DATATYPE.equals("INACTIVE"))
		{
			message.setSubject("TS Item Report(C)(Inactive)"+" - "+dateBean.getYearMonthDay()+(SYSDATE.equals("")?"":"(price date"+SYSDATE+")")+remarks);
		}
		else
		{
			message.setSubject("TS Item Report - "+dateBean.getYearMonthDay()+(SYSDATE.equals("")?"":"(price date"+SYSDATE+")")+remarks);
		}
...
{ 
	out.println("Exception:"+e.getMessage()+" cnt:"+reccnt+" Item:"+item_desc); 
} 	
%>
</FORM>
</body>
