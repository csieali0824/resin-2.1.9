...
{
   sSql = "SELECT ooha.ORDER_NUMBER, oola.header_id,oola.line_id,oola.LINE_NUMBER||'.'||oola.SHIPMENT_NUMBER LINE_NUMBER,  tic.PRODNO ,   "+
          "       msib.SEGMENT1,msib.DESCRIPTION,DECODE(oola.ITEM_IDENTIFIER_TYPE,'Customer',oola.ORDERED_ITEM,NULL) Customer_ITEM,   "+
		  "       DECODE(oola.ORDER_QUANTITY_UOM,'PCE',(oola.ORDERED_QUANTITY/1000),oola.ORDERED_QUANTITY) ORDERED_QUANTITY,   "+		
		  "       DECODE(oola.ORDER_QUANTITY_UOM,'KPC','KPCS','PCE','KPCS',oola.ORDER_QUANTITY_UOM) ORDER_QUANTITY_UOM ,  "+  
		  "       TRUNC(oola.SCHEDULE_SHIP_DATE,'dd') SCHEDULE_SHIP_DATE,  oola.last_update_date , "+
		  "       oola.PACKING_INSTRUCTIONS,oola.ATTRIBUTE8,MIC.CATEGORY_ID, ooha.ATTRIBUTE3, oola.ATTRIBUTE8,tiol.LASTUPDATE_DATE  "+
		  "  from oe_order_headers_all ooha, oe_order_lines_all oola,mtl_system_items_b msib , "+
		  "       TSC_ITEM_CONVERT tic ,MTL_ITEM_CATEGORIES MIC, tsc_import_oldsystem_log_line tiol ";         			
   sSqlCNT = " select count(DISTINCT ooha.ORDER_NUMBER) as CaseCount "+
             "  from oe_order_headers_all ooha, oe_order_lines_all oola,mtl_system_items_b msib , "+
		     "       TSC_ITEM_CONVERT tic ,MTL_ITEM_CATEGORIES MIC, tsc_import_oldsystem_log_line tiol ";
   sWhere =  "where MIC.INVENTORY_ITEM_ID = msib.INVENTORY_ITEM_ID "+
			 "  AND MIC.ORGANIZATION_ID = oola.SHIP_FROM_ORG_ID   "+			 
			 "  AND MIC.CATEGORY_SET_ID = 6 AND ooha.ORDER_NUMBER = tiol.ORDER_NUMBER  "+ //AND ooha.ATTRIBUTE3 = tiol.VER "+
			 "  AND oola.LINE_ID = tiol.LINE_ID AND msib.INVENTORY_ITEM_ID = tic.INVENTORY_ITEM_ID (+)   "+
			 "  AND ooha.header_id = oola.header_id AND oola.SHIP_FROM_ORG_ID = msib.ORGANIZATION_ID "+
			 "  AND oola.INVENTORY_ITEM_ID = msib.INVENTORY_ITEM_ID  AND  oola.PACKING_INSTRUCTIONS = 'Y' "+
			 "  AND ( ooha.order_number LIKE '1151%' OR ooha.order_number LIKE '1152%' OR ooha.order_number LIKE '1191%' OR ooha.order_number LIKE '1213%')  "+
			// "  AND TO_DATE(SUBSTR(oola.attribute8,0,11)) BETWEEN TO_DATE('2006/04/10','yyyy/mm/dd') AND TO_DATE('2006/06/23','yyyy/mm/dd') "+			 
			// "  AND oola.BOOKED_FLAG = 'Y' AND oola.CANCELLED_FLAG <> 'Y'   AND oola.line_category_code <>'RETURN'    ";		
			 "  AND oola.BOOKED_FLAG = 'Y'  AND oola.line_category_code <>'RETURN'    ";		//20081208 liling
   sWhereSDRQ = "  ";
   havingGrp = " ";               
   havingGrpSDRQ = "  ";
   sOrder = "order by tiol.LASTUPDATE_DATE,ooha.ORDER_NUMBER,ooha.ATTRIBUTE3 ";
...
{   
   ...
   // out.print("aaaaaaaaaDayFr="+DayFr+",DayTo="+DayTo);
   if ((!(DayFr=="--")&&(DayFr=="00")) && DayTo=="--") sWhere=sWhere+"  AND TO_CHAR(tiol.LASTUPDATE_DATE,'YYYYMMDD') >= '"+dateSetBegin+"' ";
   if (DayFr!="--" && DayTo!="--") sWhere=sWhere+" AND TO_CHAR(tiol.LASTUPDATE_DATE,'YYYYMMDD') BETWEEN '"+dateSetBegin+"' AND '"+dateSetEnd+"' ";  
 //  out.print("sWhere="+sWhere);  
...
%> 
  <table cellSpacing='0' bordercolordark='#D8DEA9'  cellPadding='1' width='100%' align='center' borderColorLight='#ffffff' border='1'>     	 	 
	...
	<td colspan="2">
		    <INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>' onClick='setSubmit("../jsp/TSCMOImportCNSQLAuditReport.jsp")' > 			 
	</td>
   </tr>
  </table>  
<%
...
%>
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#FFFFFF">
    ...
    <tr bgcolor="<%=colorStr%>"> 
	  ...
	  <td width="6%" nowrap><div align="center"><font color="#006666"><%=rsTC.getString("ORDERED_QUANTITY")%></font></div></td>
	  <td width="11%" nowrap><div align="center"><font color="#006666"><%=rsTC.getString("LAST_UPDATE_DATE")%></font></div></td>
	  <td width="6%" nowrap><div align="center"><font color="#006666"><%=rsTC.getString("ATTRIBUTE8")%></font></div></td>
	  <td width="11%" nowrap><div align="center"><font color="#006666"><%=rsTC.getString("LASTUPDATE_DATE")%></font></div></td>
	  <% 
	                  String orderNo="",ver ="",itemNo ="",itemDesc ="",qty="",importDate ="";
	                  float qtyF = 0; 
	                  String sSqlSQL = "select a.ORDER_NUMBER, a.VER, a.ITEM_SEGMENT1, a.DESCRIPTION, a.QTY, a.LASTUPDATE_DATE ";
					  String sFrom = " ";
					  String sWhereSQL = "where a.ORDER_NUMBER = "+rsTC.getString("ORDER_NUMBER")+" and a.VER = "+rsTC.getString("ATTRIBUTE3").trim()+" "+
									     "  and a.LINE_ID = '"+rsTC.getString("LINE_ID")+"' ";
										// "  and a.QTY = "+rsTC.getString("ORDERED_QUANTITY")+" ";
	                //  out.println(sSqlSQL);
					  if (prodManufactory.equals("T"))
					  {
					     sFrom = "from TSC_IMPORT_OLDSYSTEM_LOG_LINE a ";
...
//out.println(sSqlSQL);
					     ...
					     { 
						           ...
						           itemNo = rsSQL.getString("ITEM_SEGMENT1");
						           itemDesc = rsSQL.getString("DESCRIPTION");
						           qtyF = rsSQL.getFloat("QTY");
						           importDate = rsSQL.getString("LASTUPDATE_DATE");
					     }
					     rsSQL.close();
					     stmentSQL.close(); 
					  } else if (prodManufactory.equals("Y"))	
					         {
							      sFrom = "from TSC_IMPORT_OLD_YEW_DETAIL_TEMP a ";
						          sSqlSQL = sSqlSQL + sFrom + sWhereSQL;     
								  //out.println(sSqlSQL);
						          Statement stmentSQL=con.createStatement();
                                  ResultSet rsSQL=stmentSQL.executeQuery(sSqlSQL);
					              ...
					              { 
						           ...
						           qtyF = rsSQL.getFloat("QTY");
						           importDate = rsSQL.getString("IMPORTDATE");
					              }
					              rsSQL.close();
					              stmentSQL.close(); 
						     }
