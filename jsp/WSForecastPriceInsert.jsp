...
<html>
<head>
<title>Sales Forecast Price Data Insert</title>
...
{  
   ...
   {   
     ...
     { 	 
	   for (int subac=1;subac<a[ac].length;subac++)
	   {
		 ...
		 String p_in_array=request.getParameter("MONTH"+ac+"-"+subac);//取前一頁之輸入欄位
           sql="insert into PSALES_FORE_PRICE(FPCOMP,FPYEAR,FPMONTH,FPREG,FPCOUN,FPTYPE,FPPRJCD,FPPRI,FPMVER,FPLUSER,FPCURR) values(?,?,?,?,?,?,?,?,?,?,?)";
           ...
           pstmt.setString(8,p_in_array); 
		   a[ac][subac]=p_in_array; //將之塞入對應之陣列內容
  ...
  out.println("Input Price of Forecast Data Successfully!!<BR>");
  out.println("<A HREF=../jsp/WSForePriCostEntry.jsp?COMP="+comp+"&TYPE="+type+"&REGION="+region+"&COUNTRY="+country+">Price & Cost of Forecast data Entry</A>&nbsp;&nbsp;<A HREF=../jsp/WSForecastEntry.jsp?COMP="+comp+"&TYPE="+type+"&REGION="+region+"&COUNTRY="+country+">Sales Forecast data Entry</A>&nbsp;&nbsp;<A HREF=../jsp/WSForecastMenu.jsp>Back to SubMenu</A>&nbsp;&nbsp;<A HREF=/wins/WinsMainMenu.jsp>HOME</A>");
