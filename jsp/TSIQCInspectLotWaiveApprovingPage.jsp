...
<jsp:useBean id="arrIQC2DWaiveApprovedBean" scope="session" class="bean.Array2DimensionInputBean"/>
...
function submitCheck(ms1,ms2,ms3,ms4,inspResult)
{         
  flag=confirm("是否確認送出?");
  ...
  {   
	  ...
	  if (document.DISPLAYREPAIR.ACTIONID.value=="016" && result=="02") // 若最終檢驗結果與選擇執行動作不符,則不允許執行
	  {
	    alert("您選擇的執行動作與檢驗結果不相符\n            請再確認!!!");
	 ...
	 // 若未選擇任一Line 作動作,則警告
	  ...
	  {	      
		  flagReject=confirm("是否確認批退此合格檢驗批?");
	  ...
	  if (document.DISPLAYREPAIR.ACTIONID.value=="027")
	  {	      
		  flagAccept=confirm("是否確認核准合格檢驗批?");
...
<BR>
<FORM NAME="DISPLAYREPAIR" onsubmit='return submitCheck("取消確認","是否送出","請選擇執行動作","",<%=result%>)' ACTION="../jsp/TSIQCInspectLotMProcess.jsp?INSPLOT_NO=<%=inspLotNo%>" METHOD="post">
<!--=============以下區段為取得IQC檢驗批基本資料==========-->
...
<table cellSpacing="1" bordercolordark="#B5B89A" cellPadding="1" width="97%" align="center" bordercolorlight="#FFFFFF"  border="0">
    ...
    <%
      ...
      {   
    	...
    	arrIQC2DWaiveApprovedBean.setArrayString(oneDArray);
		// 先取 該詢問單筆數
	     int rowLength = 0;
	   ...
	   {//out.println("0"); 
		...
		out.print("<TD nowrap><INPUT TYPE='button' value='Set' onClick='setSubmitComment("+'"'+"../jsp/TSIQCInspectLotWaiveApprovingPage.jsp?INSPLOTNO="+inspLotNo+"&LINE_NO="+rs.getString("LINE_NO")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("LINE_NO")+'"'+")'>");
		...
		out.println("<TD nowrap>"+rs.getString("SUPPLIER_LOT_NO")+"</TD><TD nowrap>"+rs.getString("INV_ITEM_DESC")+"</TD><TD nowrap>"+rs.getString("RECEIPT_QTY")+"</TD><TD nowrap>"+rs.getString("UOM")+"</TD><TD nowrap>"+rs.getString("RECEIPT_DATE")+"</TD><TD nowrap>"+rs.getString("INSPECT_REMARK")+"</TD></TR>");
		 ...
		 arrIQC2DWaiveApprovedBean.setArray2DString(b);
		 k++;
	   }    	   	   	 
	   out.println("</TABLE>");
	   statement.close();
       ...
       catch (Exception e)
       {
        out.println("Exception:"+e.getMessage());
	     ...
	     String a[][]=arrIQC2DWaiveApprovedBean.getArray2DContent();//取得目前陣列內容 		    		                       		    		  	   
         if (a!=null) 
		 {		  
...
<!--=================================-->         
		<%
                   ...
                   {       
                     Statement statement=con.createStatement();
                     ResultSet rs=statement.executeQuery("select WAIVE_NO from ORADDMAN.TSCIQC_LOTINSPECT_HEADER where INSPLOT_NO = '"+inspLotNo+"' ");
                     if (rs.next())
					 {
					   waiveNo = rs.getString("WAIVE_NO"); // 把特採核准號給驗收人員
					 }
	                 statement.close();
                     rs.close();       
                   } //end of try
...
<table cellSpacing="1" bordercolordark="#B5B89A" cellPadding="1" width="97%" align="center" bordercolorlight="#FFFFFF" border="0">             
	...
	<tr bgcolor="#CCCC99"> 
      <td colspan="5">特採核准批號: 
        <INPUT TYPE="TEXT" NAME="WAIVENO" SIZE=60 maxlength="60" value="<%=waiveNo%>">		           
     </td>
    </tr>           
</table>
<BR>
   ...
   <a name='#ACTION'>
    <%
      ...
      {  
       ...
       Statement statement=con.createStatement();
       ResultSet rs=statement.executeQuery("select x1.ACTIONID, decode(x2.ACTIONNAME,'WAIVE','ACCEPT',x2.ACTIONNAME) from ORADDMAN.TSWORKFLOW x1,ORADDMAN.TSWFACTION x2 WHERE FORMID='QC' AND FROMSTATUSID='"+frStatID+"' AND x1.ACTIONID=x2.ACTIONID and  x1.LOCALE='"+locale+"' order by 2 desc");
       //comboBoxBean.setRs(rs);
	   //comboBoxBean.setFieldName("ACTIONID");	   
       //out.println(comboBoxBean.getRsString());
	   out.println("<select NAME='ACTIONID' onChange='setSubmit1("+'"'+"../jsp/TSIQCInspectLotWaiveApprovingPage.jsp?INSPLOTNO="+inspLotNo+'"'+")'>");				  				  
