...
<script language="JavaScript" type="text/JavaScript">
	...
	function focuscolor(column, id)
	{
		var color2 = document.form1.HIGHLIGHTOLOR.value;
		color2=color2.toLowerCase();

		if (column === 'customerName' || column === 'rtfType') {
			document.getElementById('customerName'+id).style.backgroundColor = color2;
			document.getElementById('rtfType'+id).style.backgroundColor = color2;
		}
	}
	function unfocuscolor(column, id)
	{
		var color1 = document.form1.ROWCOLOR.value;
		color1=color1.toLowerCase();

		if (column === 'customerName' || column === 'rtfType') {
			document.getElementById('customerName'+id).style.backgroundColor = color1;
			document.getElementById('rtfType'+id).style.backgroundColor = color1;
		}
	}
...
<%
	...
	String columns[] = new String[]{"customer_po_line_number", "description", "cust_item_name", "qty", "selling_price", "request_date",
	"order_type", "end_customer_id", "end_customer", "upload_by"};
	try
	{
...
<form name="form1"  METHOD="post" ENCTYPE="multipart/form-data" accept-charset="UTF-8">
				...
				<table align="center" width="100%" cellspacing="0" cellpadding="0" bordercolordark="#990000">
						...
						<TD width="90%">
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<tr>
									...
									<td bgcolor="#CAE1CF" width="15%" align="center"><a href="../jsp/TSCModelNTest.jsp?ACTIONCODE=DETAIL" style="color:888888;font-size:13px;text-decoration:none;">Pending Detail</a></td>
					...
					<tr>
						<td colspan="2" title="請按我，謝謝!" align="center">
							<input type="button" style="font-size:13px;font-family: Tahoma,Georgia;" name="submit1" value='Upload' onClick="setCreate('../jsp/TSCModelNTest.jsp?ACTIONCODE=UPLOAD')">
						</td>
					</tr>
				</table>
				<%
				...
				<table align="center" width="100%" cellspacing="0" cellpadding="0" bordercolordark="#990000">
					<tr>
						<TD width="90%">
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td bgcolor="<%=bkcolor%>" width="15%" height="20" align="center" style="border-color:#CAE1CF;border:insert;font-size:13px"><a href="../jsp/TSCModelNTest.jsp?ACTIONCODE=DETAIL" style="font-size:13px;text-decoration:none;">Pending Detail</a></td>
									<td bgcolor="#CAE1CF" width="15%" align="center"><a href="../jsp/TSCModelNTest.jsp" style="color:888888;font-size:13px;text-decoration:none;">Excel Upload</a></td>
									<td width="70%" style="border-color:ffffff;">&nbsp;</td>
								</tr>
							</table>
						</TD>
	...
	<script language="javascript">
		location.href("TSCModelNTest.jsp?ACTIONCODE=DETAIL");
	</script>
		...
		<tr id="trRow">
			<%
				if (!customerpo.equals(rs.getString("customer_po")) || !customerno.equals(rs.getString("customer_no")) || !upload_by.equals(rs.getString("upload_by")))
				{
					i++;
			%>
			<td rowspan="<%=rs.getString("rowcnt")%>" style="color:#000000;font-size:11px"><input type="button" name="btn<%=i%>" value="Delete" style="font-size:11px;font-family: Tahoma,Georgia;" title="刪除資料" onClick="delData('../jsp/TSCModelNTest.jsp?DELFLAG=Y&CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERPO=<%=rs.getString("customer_po")%>&UPLOADBY=<%=rs.getString("upload_by")%>')"></td>
			<td id="customerName<%=i%>" rowspan="<%=rs.getString("rowcnt")%>"
				onMouseOver="focuscolor('customerName', '<%=i%>')"
				onMouseOut="unfocuscolor('customerName', '<%=i%>')"
				style="color:#000000;font-size:11px"
				onClick="javascript:location.href='TSCModelNTest.jsp?CUSTOMERID=<%=rs.getString("customer_id")%>&CUSTOMERNO=<%=rs.getString("customer_no")%>&CUSTOMERNAME=<%=rs.getString("customer_name")%>&CUSTOMERPO=<%=rs.getString("customer_po")%>&ODRTYPE=<%=rs.getString("OTYPE_ID")%>&RFQTYPE=<%=rs.getString("rfq_type")%>&UPLOADBY=<%=rs.getString("upload_by")%>&INSERTFLAG=Y'" title="按下滑鼠左鍵,進入Detail畫面"><%="("+rs.getString("customer_no")+")"+rs.getString("customer_name")%>
			</td>
			<td id="rtfType<%=i%>" rowspan="<%=rs.getString("rowcnt")%>"
				onMouseOver="focuscolor('rtfType', '<%=i%>')"
				onMouseOut="unfocuscolor('rtfType', '<%=i%>')"
				style="color:#000000;font-size:11px"
			...
			<%
				String fieldValue = "";
				String align = "";
				List attrColumnList = Arrays.asList(new String[] {"cust_item_name", "end_customer_id", "end_customer","qty", "selling_price"});
				for (int x = 0; x < columns.length; x++) {
					String column = columns[x];
					if (attrColumnList.contains(column)) {
						if (column.equals("qty") || column.equals("selling_price")) {
							fieldValue = (new DecimalFormat("##,##0.######")).format(rs.getDouble(column));
							align = "right";
						} else {
							fieldValue = rs.getString(column) == null ? "&nbsp;" : rs.getString(column);
							align = column.equals("cust_item_name")? "left" : "center";
						}
					} else {
						fieldValue = rs.getString(column);
						align = (column.equals("customer_po_line_number") || column.equals("description"))? "left" : "center";
					}
			%>
					<td id="<%=column+i%>" onMouseOver="focuscolor('<%=column%>', '<%=i%>')"
						onMouseOut="unfocuscolor('<%=column%>', '<%=i%>')" style="color:#000000;font-size:11px"
						onClick="javascript:location.href='TSCModelNTest.jsp?
						CUSTOMERID=<%=rs.getString("customer_id")%>
						&CUSTOMERNO=<%=rs.getString("customer_no")%>
						&CUSTOMERNAME=<%=java.net.URLEncoder.encode(rs.getString("customer_name"))%>
						&CUSTOMERPO=<%=java.net.URLEncoder.encode(rs.getString("customer_po"))%>
						&ODRTYPE=<%=rs.getString("OTYPE_ID")%>
						&RFQTYPE=<%=rs.getString("rfq_type")%>
						&UPLOADBY=<%=rs.getString("upload_by")%>
						&INSERTFLAG=Y'"
						title="按下滑鼠左鍵,進入Detail畫面"
						align="<%=align%>"
					>
						<%=fieldValue %>
					</td>
