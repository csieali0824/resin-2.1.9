...
{    
  subWin=window.open("WSForecastRegionRptNoAuth.jsp?YEARFR="+yearfr+"&MONTHFR="+monthfr+"&REGION="+region,"subwin","width=800,height=650,scrollbars=yes,menubar=no");  
}
function wsCountryReportByModel(yearfr,monthfr,region,locale)
{   
  subWin=window.open("WSForecastCountryRptNoAuth.jsp?YEARFR="+yearfr+"&MONTHFR="+monthfr+"&REGION="+region+"&LOCALE="+locale,"subwin","width=800,height=650,scrollbars=yes,menubar=no");  
}
function wsColorReportByModel(yearfr,monthfr,region,locale)
{   
  subWin=window.open("WSForecastColorRptNoAuth.jsp?YEARFR="+yearfr+"&MONTHFR="+monthfr+"&REGION="+region+"&LOCALE="+locale,"subwin","width=800,height=650,scrollbars=yes,menubar=no");  
}
</script>
...
<FORM ACTION="../jsp/WSForecastInquiryNoAuth.jsp" METHOD="post" NAME="MYFORM">
<font color="#3366FF" size="+2" face="Arial"><font size="+3" face="Arial Black"><font face="Courier, MS Sans Serif"><font color="#000000" size="+2" face="Arial Black"><strong>DBTEL</strong></font></font></font></font><strong><font color="#000000" size="+2" face="Times New Roman"> 
Forecast</font></strong><font color="#000000" size="+2" face="Times New Roman"><strong> 
...
%>
  <BR><A HREF="/wins/WinsMainMenu.jsp">HOME</A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="../jsp/WSForecastMenu.jsp">Back to submenu</A> 
  &nbsp;
<%         
...
<table width="100%" border="0">
    <tr bgcolor="#D0FFFF"> 
      <td width="27%" height="23" bordercolor="#FFFFFF"> 
      ...
      <td width="29%"><font color="#333399" face="Arial Black"><strong>Region</strong></font> 
        <% 
		 String REGION="";		 		 
	     try
         {   
    ...
    <tr bgcolor="#D0FFFF"> 
      ...
      <td> <font color="#330099" face="Arial Black"><strong>Month</strong></font> 
       ...
       %> </td>
      <td width="34%"><font color="#333399" face="Arial Black"> <strong>Model</strong></font> 
        <% 
		 String MODEL="";		 		 
	     try
         {   
	 ...
	 <td width="10%"><input name="submit1" type="submit" value="Inquiry"></td>	 
  </tr>
</table>
<table width="100%" border="1" cellspacing="0" cellpadding="0">
  <tr>
      <td width="5%" bgcolor="#33CC99" nowrap><font size="2" color="#FFFFFF"><strong>Design<BR>
         &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;House</strong></font></td>
	  <td width="5%" bgcolor="#33CC99" nowrap><font size="2" color="#FFFFFF">
	    <strong>Platform</strong>
	  </font></td>
      <td width="5%" bgcolor="#FFFFCC" nowrap><font size="2">Region</font></td>
      <td width="5%" bgcolor="#FFFFCC" nowrap><font size="2">Country</font></td>
      ...
      <td colspan="1" bgcolor="#FFFFCC" width="3%"><div align="center"><font size="2"><% 
      ...
      <% //  *** Main Loop  Start   *** //      	              		
		    try
		   { 			                         			  									   
			String sql = "select  DISTINCT m.DESIGNHOUSE, m.PLATFORM, w.FMREG, w.FMCOUN , w.FMTYPE, w.FMPRJCD from WSADMIN.PSALES_FORE_MONTH w, PIMASTER m  ";
			String swhere = "where w.FMQTY>0 and FMVER in (select max(FMVER) from PSALES_FORE_MONTH where FMYEAR||FMMONTH between '"+yearMonthString[0][0]+yearMonthString[0][1]+"' and '"+yearMonthString[11][0]+yearMonthString[11][1]+"' and FMREG=w.FMREG and FMCOUN=w.FMCOUN and FMTYPE=w.FMTYPE and FMPRJCD=w.FMPRJCD) and w.FMPRJCD = m.PROJECTCODE and FMYEAR||FMMONTH between '"+yearMonthString[0][0]+yearMonthString[0][1]+"' and '"+yearMonthString[11][0]+yearMonthString[11][1]+"' ";                          			  									   			
   ...
   %>
  <tr>
     <td nowrap><font size="2" color="#000066"><strong>
        <%= rs.getString("DESIGNHOUSE")%>	    
	  </strong></font>
	 </td>
	 <td nowrap><font size="2" color="#000066"><strong>
	 ...
	 <% String regionGet = rs.getString("FMREG");		   
        String localeGet = rs.getString("FMCOUN");
	    String typeGet = rs.getString("FMTYPE");
	 %>
	 <a href='javaScript:wsRegionReportByModel("<%=YearFr%>","<%=MonthFr%>","<%=regionGet%>")' >
        <%
	       try
           {   		   
		     out.println(regionGet);		
          } //end of try
           catch (Exception e)
          {
           out.println("Exception:"+e.getMessage());		  
	...
	<td nowrap>
     <font size="2">
	  <a href='javaScript:wsCountryReportByModel("<%=YearFr%>","<%=MonthFr%>","<%=regionGet%>","<%=localeGet%>")' >
	   <%      		 		 
	     try
         {   
		  String sSqlL = "";
		  String sWhereL = "";	  
		  sSqlL = "select LOCALE|| '(' || LOCALE_ENG_NAME||')' as LOCALENAME from WSLOCALE ";		  
		  sWhereL = "where LOCALE = '"+rs.getString("FMCOUN")+"' ";		                 
		  sSqlL = sSqlL+sWhereL;		  
	...
	<td nowrap>
     <font size="2">
	 <a href='javaScript:wsColorReportByModel("<%=YearFr%>","<%=MonthFr%>","<%=regionGet%>","<%=localeGet%>")' >
     <%
...
<%
         ...
         {   
		  String sSqlE = "";
		  String sWhereE = "";		 
		  sSqlE = "select TYPE_DESC_GBL as TYPENAME from WSTYPE_CODE ";		  
