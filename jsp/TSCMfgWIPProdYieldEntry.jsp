...
if ( dateSetBegin==null || dateSetBegin.equals("") )
{
  ...
  {   // out.println("workingDateBean.getWeek()="+workingDateBean.getWeek());
     ...
     sSql = " select DISTINCT YWWA.PART_NO, YWWA.ACCT_PERIOD_ID from YEW_WIP_WIPIO_ALL YWWA, ORG_ACCT_PERIODS OAP ";	

   sSqlCNT = " select count(DISTINCT YWWA.PART_NO) CaseCount from YEW_WIP_WIPIO_ALL YWWA, ORG_ACCT_PERIODS OAP  ";
			
   sWhere =  "  where YWWA.ORGANIZATION_ID = OAP.ORGANIZATION_ID and YWWA.ACCT_PERIOD_ID = OAP.ACCT_PERIOD_ID and YWWA.ORGANIZATION_ID = "+marketType+" and to_char(OAP.PERIOD_START_DATE,'YYYYMM')= '"+dateBean.getYearString()+dateBean.getMonthString()+"' ";
...
{ 
  ...
  {   
     ...
     String sqlOrgCnt = " select count(DISTINCT YWWA.PART_NO) CaseCount from YEW_WIP_WIPIO_ALL YWWA, ORG_ACCT_PERIODS OAP ";
...
<table cellspacing="0" bordercolordark="#6699CC" cellpadding="1" width="100%" align="center" bordercolorlight="#ffffff" border="1">	
	...
	<td width="31%" colspan="2">
			...
			<INPUT TYPE="button" align="middle" value='EXCEL' onClick='setSubmit1(this.form.CLASSID.value,this.form.MARKETTYPE.value)' disabled>	</td>
   </tr>
  </table>  
<%  
...
%>
 <table cellspacing="0" bordercolordark="#6699CC" cellpadding="1" width="100%" align="center" bordercolorlight="#ffffff" border="1">
  ...
  <col width="277" style="mso-width-source:userset;mso-width-alt:8864;width:208pt">
  ...
  <tr height="26" style="height:19.5pt">
    <td height="26" class="xl22" colspan="9" style="height: 19.5pt; mso-ignore: colspan">產品別良率的計算公式:</td>    
  </tr>
  <tr height="22" style="height:16.5pt">
    <td height="22" class="xl23" style="height: 16.5pt" width="53">　</td>
    <td class="xl23" width="80">期初</td>
    <td class="xl23" width="162">接收</td>
    <td class="xl23" width="96">產出</td>
    <td class="xl23" width="89">耗損</td>
    <td class="xl23" width="117">期末</td>
    <td class="xl23" width="96">工時</td>
    <td class="xl23" width="143">良率</td>
    <td class="xl23" width="84">PPH</td>
  </tr>
  <tr height="22" style="height:16.5pt">
    <td height="22" class="xl23" style="height: 16.5pt" width="53" nowrap>　</td>
    <td class="xl23" width="80" nowrap>上月的結存</td>
    <td class="xl23" width="162" nowrap>本月末站的投入數</td>
    <td class="xl23" width="96" nowrap>轉入下站的接收數</td>
    <td class="xl23" width="89" nowrap>期初+接收-產出</td>
    <td class="xl23" width="117" nowrap>本月結存=下月的期初</td>
    <td class="xl23" width="96" nowrap>每站所使用的工時</td>
    <td class="xl23" width="143" nowrap>產出/(期初+接收)*100%</td>
    <td class="xl23" width="84" nowrap>產出/工時</td>
  </tr>
  <tr height="22" style="height:16.5pt">
    <td height="22" class="xl23" style="height: 16.5pt" width="53" nowrap>前段:</td>
    <td class="xl23" width="80" nowrap>每站加總</td>
    <td class="xl23" width="162" nowrap>焊接站的接收數</td>
    <td class="xl23" width="96" nowrap>委外品的入庫數</td>
    <td class="xl23" width="89" nowrap>每站加總</td>
    <td class="xl23" width="117" nowrap>每站加總</td>
    <td class="xl23" width="96" nowrap>每站加總</td>
    <td class="xl23" width="143" nowrap>前段各站良率積*100%</td>
    <td class="xl23" width="84" nowrap>產出/工時</td>
  </tr>
  <tr height="22" style="height:16.5pt">
    <td height="22" class="xl23" style="height: 16.5pt" width="53" nowrap>後段:</td>
    <td class="xl23" width="80" nowrap>每站加總</td>
    <td class="xl23" width="162" nowrap>後段首站接收數</td>
    <td class="xl23" width="96" nowrap>包裝站入庫數</td>
    <td class="xl23" width="89" nowrap>每站加總</td>
    <td class="xl23" width="117" nowrap>每站加總</td>
    <td class="xl23" width="96" nowrap>每站加總</td>
    <td class="xl23" width="143" nowrap>後段各站良率積*100%</td>
    <td class="xl23" width="84" nowrap>產出/工時</td>
  </tr>
  <tr height="22" style="height:16.5pt">
    <td height="22" class="xl23" style="height: 16.5pt" width="53" nowrap>全段:</td>
    <td class="xl23" width="80" nowrap>前段+後段</td>
    <td class="xl23" width="162" nowrap>全段(產出-期初+耗損+期末)</td>
    <td class="xl23" width="96" nowrap>包裝入庫數</td>
    <td class="xl23" width="89" nowrap>前段+後段=全段</td>
    <td class="xl23" width="117" nowrap>前段+後段=全段</td>
    <td class="xl23" width="96" nowrap>前段+後段=全段</td>
    <td class="xl23" width="143" nowrap>前段良率*後段良率*100%</td>
    <td class="xl23" width="84" nowrap>產出/工時</td>
  </tr>
</table>

其它特殊計算說明:<BR>
1.前段產出小計:當月如因製程末站無產出,則以最終有產出數站為該前段小計產出數。<BR>
2.後段產出小計:當月如因無製程包裝站產出,則以最終有產出數站為該後段小計產出數。<BR>
3.全段產出小計:當月如因後段皆無產出(包裝入庫數),則前段小計產出為全段產出數。<BR>
4.前、後段產出:前、後段產出=接收+期初-期末-損耗。<BR>
5.當月無異動數:當月前、後段皆無製程移轉之任何異動,則不顯示該成本代號產品良率資訊。<BR>
  <!--%每頁筆●顯示筆到筆總共有資料%-->
