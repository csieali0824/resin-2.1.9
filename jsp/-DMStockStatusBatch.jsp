...
/*					 
					//檢查是否有Running Change 料號
					sSqlChangeItem="SELECT citem FROM opitem,tmpcherry010 WHERE model='"+sModel+"' AND uitem=bChld";
					PreparedStatement ptDChangeItem = bpcscon.prepareStatement(sSqlChangeItem);
					rsDChangeItem=ptDChangeItem.executeQuery();
					while (rsDChangeItem.next()) {
						sChangeItem = rsDChangeItem.getString(1);
						
						//取得各替代料之庫存數
						sChangeStock="SELECT SUM(lopb+lrct-lissu+ladju) as CINV FROM ili WHERE lwhs='01' AND lprod='"+sChangeItem+"' ";
						ptment=bpcscon.prepareStatement(sChangeStock);
						rsChangeStock=ptment.executeQuery();
						if (rsChangeStock.next())  { nChangeInv = rsChangeStock.getInt("CINV");}
						else { nChangeInv=0;}
						
						//取得替代料之暫收數量
						sChangeIqc="SELECT SUM(hqins) AS CIQC FROM hpo WHERE pwhse='01' AND pprod='"+sChangeItem+"' ";
						ptment=bpcscon.prepareStatement(sChangeIqc);
						rsChangeIqc=ptment.executeQuery();
						if (rsChangeIqc.next()) { nChangeIqc =rsChangeIqc.getInt("CIQC");}
						else {nChangeIqc = 0;}
						
						//取得替代料之工單缺料數
						sChangeSO = "SELECT SUM(mqreq-mqiss) FROM  fma WHERE mid='MA'  AND  mwhs='01' AND mprod='"+sChangeItem+"' ";
						ptment=bpcscon.prepareStatement(sChangeSO);
						rsChangeSO =ptment.executeQuery();
						if (rsChangeSO.next()) { nChangeSoQty=rsChangeSO.getInt(1); }
						else { nChangeSoQty = 0;}
						
						//insert into running change table 
						sChangeIns="INSERT INTO matestockopt(pmodelno,pprodno,pchldno,poption,pstockqty,pinsqty,pwipqty) "+
											"VALUES (?,?,?,?,?,?,?)";
						ptdmment = dmcon.prepareStatement(sChangeIns);
						ptdmment.setString(1,sModel);
						ptdmment.setString(2,sProd);
						ptdmment.setString(3,sItem);
						ptdmment.setString(4,sChangeItem);
						ptdmment.setInt(5,nChangeInv+nInventory);
						ptdmment.setInt(6,nChangeIqc+nIqcRev);
						ptdmment.setInt(7,nChangeSoQty+nSoQty);
						ptdmment.executeUpdate();
						ptdmment.close();	
				
					   }// end of while  ChangeItem
					ptDChangeItem.close();
					rsDChangeItem.close();
*/
