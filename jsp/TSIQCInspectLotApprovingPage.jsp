...
<jsp:useBean id="arrIQC2DApprovingBean" scope="session" class="bean.Array2DimensionInputBean"/>
...
<table cellSpacing="1" bordercolordark="#B5B89A" cellPadding="1" width="97%" align="center" bordercolorlight="#FFFFFF"  border="0">
    ...
    <%
      ...
      {   
    	...
    	arrIQC2DApprovingBean.setArrayString(oneDArray);
		// 先取 該詢問單筆數
	     int rowLength = 0;
	   ...
	   {//out.println("0"); 
		...
		out.print("<TD nowrap><INPUT TYPE='button' value='Set' onClick='setSubmitComment("+'"'+"../jsp/TSIQCInspectLotApprovingPage.jsp?INSPLOTNO="+inspLotNo+"&LINE_NO="+rs.getString("LINE_NO")+"&EXPAND="+expand+'"'+","+'"'+rs.getString("LINE_NO")+'"'+")'>");
		...
		out.println("<TD nowrap>"+rs.getString("SUPPLIER_LOT_NO")+"</TD><TD nowrap>"+rs.getString("INV_ITEM_DESC")+"</TD><TD nowrap>"+rs.getString("RECEIPT_QTY")+"</TD><TD nowrap>"+rs.getString("UOM")+"</TD><TD nowrap>"+rs.getString("RECEIPT_DATE")+"</TD><TD nowrap>"+rs.getString("INSPECT_REMARK")+"</TD></TR>");
		 ...
		 arrIQC2DApprovingBean.setArray2DString(b);
		 k++;
	   }    	   	   	 
	   out.println("</TABLE>");
	   statement.close();
       ...
       catch (Exception e)
       {
        out.println("Exception:"+e.getMessage());
	     ...
	     String a[][]=arrIQC2DApprovingBean.getArray2DContent();//取得目前陣列內容 		    		                       		    		  	   
         if (a!=null) 
		 {		  
...
//       out.println("select x1.ACTIONID, decode(x2.ACTIONNAME,'021','REMOVE',x2.ACTIONNAME)from ORADDMAN.TSWORKFLOW x1,ORADDMAN.TSWFACTION x2 WHERE FORMID='QC' AND FROMSTATUSID='"+frStatID+"' AND x1.ACTIONID=x2.ACTIONID and  x1.LOCALE='"+locale+"' order by 2");
	   ...
	   out.println("<select NAME='ACTIONID' onChange='setSubmit1("+'"'+"../jsp/TSIQCInspectLotApprovingPage.jsp?INSPLOTNO="+inspLotNo+'"'+")'>");				  				  
...
//	   out.println("select COUNT (*) from ORADDMAN.TSWORKFLOW x1,ORADDMAN.TSWFACTION x2 WHERE FORMID='QC' AND FROMSTATUSID='"+frStatID+"' AND x1.ACTIONID=x2.ACTIONID and  x1.LOCALE='"+locale+"'"); 
