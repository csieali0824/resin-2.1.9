...
<!--=================================-->
<%@ page import="bean.DateBean,bean.ArrayCheckBoxBean" %>
</head>
...
<body>
 ...
 {       
  ...
  String sWhere = " where SEQTYPE='P' and SEQDATE='"+strDateTime+"' ";		 
  sqlS = sqlS+sWhere;
  ...
  itemcreateNo = "P" + strDateTime + itemcreateNo1;  //取得新的shippNo
  ...
  if (sNew=="Y")
  {
    ...
    pstmtN.setString(1,"P");
    pstmtN.setString(2,"入庫"); 
  ...
  {
    String sqlN="update INV_SEQ set SEQMAX = '"+iSeq+"' where SEQTYPE = 'P' and SEQDATE = '"+sDate+"' ";
    PreparedStatement pstmtN=con.prepareStatement(sqlN);  
  ...
  //////寫入INV_M_REC
     String commSql="insert into INV_M_REC(RecCreateNo,RecProjCode,RecStatId,RecStat,RecCreateUser,RecCreateDate,RecCenterNo) values(?,?,?,?,?,?,?)";
  ...
  out.println("入庫單新增 OK!! <BR><BR><BR>入庫單號:"+itemcreateNo+"<BR>User:"+userID+"<BR>Date:"+iDate+"<BR><BR><BR>");
  //out.println("<A HREF=../jsp/INVRecInput.jsp>入庫單新增<BR><BR></A><A HREF=/wins/WinsMainMenu.jsp>回首頁</A>");  
       ...
       %>
<BR>
<A HREF=../jsp/INVRecInput.jsp>入庫單新增</A>
<BR>
