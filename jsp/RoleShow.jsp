...
{   
  location.href="../jsp/RoleShow.jsp?PAGEURL="+pageURL+"&SEARCHSTRING="+document.MYFORM.SEARCHSTRING.value  
}
</script>
<body>
...
<strong><font color="#0080C0" size="5">查詢所有角色記錄</font></strong> 
  ...
  {   
   ...
   {  
     rs=statement.executeQuery("select count(*) from WsRole where ROLENAME like '"+searchString+"%'");
   } else {
     rs=statement.executeQuery("select count(*) from WsRole order by ROLENAME");
   }	 
...
<table width="75%" border="0">
  <tr>
    <td><A HREF="/wins/WinsMainMenu.jsp">回首頁</A> &nbsp;&nbsp;<A HREF="../jsp/RoleAdd1.jsp">新增角色</A></td>
      <td><strong><font color="#400040" size="2">角色名稱:</font></strong> 
...
<INPUT TYPE="submit" value="刪除">
  &nbsp;&nbsp; <A HREF="../jsp/RoleShow.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=FIRST"><font size="2"><strong><font color="#FF0080">第一頁</font></strong></font></A>&nbsp;&nbsp;<A HREF="../jsp/RoleShow.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=LAST"><font size="2"><strong><font color="#FF0080">最後一頁</font></strong></font></A><font color="#FF0080"><strong><font size="2">&nbsp;&nbsp;<A HREF="../jsp/RoleShow.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=30">下一頁</A>&nbsp;&nbsp;<A HREF="../jsp/RoleShow.jsp?PAGEURL=<%=pageURL%>&SEARCHSTRING=<%=searchString%>&SCROLLROW=-30">上一頁</A>(第頁<%=currentPageNumber%>/共<%=totalPageNumber%>頁)</font></strong></font> 
<% 
  try
  {  
   ...
   {   
     if (searchString!=null) //如果有搜尋特定單號則另下SQL
     { 
       rs=statement.executeQuery("select ROLENAME,ROLEDESC from WSROLE where ROLENAME like '%"+searchString+"%' or ROLEDESC like '%"+searchString+"%'");	   
     } else {
       rs=statement.executeQuery("select ROLENAME,ROLEDESC from WSROLE order by ROLENAME");
	 }	   
   } else {
    if (searchString!=null) //如果有搜尋特定單號則另下SQL
     { 	   
	   rs=statement.executeQuery("select ROLENAME,ROLEDESC from WSROLE where ROLENAME like '"+searchString+"%'");
     } else {
	   rs=statement.executeQuery("select ROLENAME,ROLEDESC from WSROLE order by ROLENAME");
	 }	   
   }	
  ...
  /* queryAllRepairBean.setPageURL("../jsp/BBB.jsp");//小圖示連結到修改的網頁  
   ...
   qryAllChkBoxEditBean.setPageURL("../jsp/RoleEdit.jsp");//小圖示連結到修改的網頁
