<!--20140826 by Peggy,新增ERP END CUSTOMER ID欄位-->
<!--20150326 by Peggy,修正上傳客戶類別-->
...
<!--20160113 by Peggy,修正資料筆數多時會有錯誤問題-->
          	...
          	if ( (files != null) || (!files.isEmpty()) )  
			{
				...
				String strRemark="",custParts="";  //add by Peggy 20150707
        	 	...
        	 	else if (CompInfo.toUpperCase().startsWith("TANTRON")) //add by Peggy 20151223
         		{
         			...
         			session.setAttribute("REMARK","Order Import from file");
         			//session.setAttribute("PREORDERTYPE","1022");
					session.setAttribute("PREORDERTYPE","1175"); //for bonnie Tantron只會有1156或1142的MO單號,modify by Peggy 20191022
         			session.setAttribute("ISMODELSELECTED","Y");
         		...
         		else
         		{
         			...
         			session.setAttribute("CUSTOMERNAME","元超國際股份有限公司");
         			session.setAttribute("CUSTOMERPO", BufferOrderNo);
         			session.setAttribute("CURR", BufferCurrency);
         			session.setAttribute("CUSTACTIVE","A");
         			session.setAttribute("REMARK","Order Import from file");
             			...
             			try 
						{
                			...
                			while (rsUOM.next()) 
							{
				       				...
				       				BufferPartDescNew[1][i].trim() !=null && BufferPartDescNew[1][i].trim() !=null)
								{
									...
									b[j][14]=custParts;      //客戶料號 add by Peggy 20120316
									b[j][15]=""+BufferUnitPrice[7][i];       //加入EXCEL檔的單價,modify by Peggy 20140626
									b[j][16]="";            //訂單類型 add by Peggy 20120316
