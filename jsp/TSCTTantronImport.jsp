<!--20140826 by Peggy,�s�WERP END CUSTOMER ID���-->
<!--20150326 by Peggy,�ץ��W�ǫȤ����O-->
...
<!--20191128 Peggy,TSCT-DA�PTSCT-Disty�X�֦�TSCT,RFQ�~�Ȱϲ���006,�Τ@�b005-->
<html>
<head>
        ...
        if ( (todo != null) && (todo.equalsIgnoreCase("upload")) )  
		{
          	...
          	if ( (files != null) || (!files.isEmpty()) )  
			{
        		...
        		Cell   cellorderno   = sheet.getCell(1,1);
		 		...
		 		Cell cellCustPart = null; //add by Peggy 20200312
				...
				String strRemark="",custParts="";  //add by Peggy 20150707
         		...
         		String oneDArray[]= {"","No.","Inventory Item","Item Description","Order Qty","UOM","Cust Request Date","Shipping Method","Request Date","End-Customer PO","Remark","BI Rregion","End Cust Ship to"};  //add by Peggy 20170222
         		arrayRFQDocumentInputBean.setArrayString(oneDArray);
	
         		// Set TSCT Order Default Value
         		if (CompInfo.toUpperCase().startsWith("BRIGHT"))
         		{
         			session.setAttribute("SPQCHECKED","N");
         			session.setAttribute("CUSTOMERID","1081");
				 	session.setAttribute("CUSTOMERNO","1038");
					session.setAttribute("CUSTOMERNAME","BRIGHT DIAMOND HOLDING LIMITED.");
					...
					//	session.setAttribute("PROCESSAREA","005(�b����~�ȳ�-�x�W��)");
					//}
					//else
					//{
						session.setAttribute("SALESAREANO","006");
						session.setAttribute("PROCESSAREA","006(�b����~�ȳ�-�x�W��(Disty)");
					//}
         		}
        	 	else if (CompInfo.toUpperCase().startsWith("GLOBAL"))
         		{
         			session.setAttribute("SPQCHECKED","N");
         			session.setAttribute("CUSTOMERID","1242");
         			session.setAttribute("CUSTOMERNO","1117");
         			session.setAttribute("CUSTOMERNAME","GLOBAL PACIFIC GROUP LTD.");
         			session.setAttribute("CUSTOMERPO", BufferOrderNo);
         			session.setAttribute("CURR", BufferCurrency);
         			session.setAttribute("CUSTACTIVE","A");
         			session.setAttribute("REMARK","Order Import from file");
         		...
         		{
         			session.setAttribute("SPQCHECKED","N");
         			session.setAttribute("CUSTOMERID","449294");
         			session.setAttribute("CUSTOMERNO","22492");
         			session.setAttribute("CUSTOMERNAME","Tantron International Corporation");
         			session.setAttribute("CUSTOMERPO", BufferOrderNo);
         			session.setAttribute("CURR", BufferCurrency);
         			session.setAttribute("CUSTACTIVE","A");
         			session.setAttribute("REMARK","Order Import from file");
         			//session.setAttribute("PREORDERTYPE","1022");
					session.setAttribute("PREORDERTYPE","1175"); //for bonnie Tantron�u�|��1156��1142��MO�渹,modify by Peggy 20191022
         			session.setAttribute("ISMODELSELECTED","Y");
         		...
         		else
         		{
         			...
         			session.setAttribute("CUSTOMERNAME","���W��ڪѥ��������q");
         			session.setAttribute("CUSTOMERPO", BufferOrderNo);
         			session.setAttribute("CURR", BufferCurrency);
         			session.setAttribute("CUSTACTIVE","A");
         			session.setAttribute("REMARK","Order Import from file");
				...
				String sql = "	SELECT distinct c.customer_id,c.ACCOUNT_NUMBER customer_number,c.CUSTOMER_SNAME CUSTOMER_NAME_PHONETIC"+
				...
				ResultSet rss = statements.executeQuery(sql);				
          		...
          		int itemCNTtotal = 0;
          		for ( int itemno=0 ; itemno < irow ; itemno++)
          		{
					if (icnt>=10)
					{
						irow = itemno-icnt;
					...
					cellCustPart = sheet.getCell(1,itemno+6);  //add by Peggy 20220831
					custParts=cellCustPart.getContents();
					if (custParts==null) custParts="";
					custParts=custParts.replace("'","");  
             		cellpartdesc1 = sheet.getCell(2,itemno+6);
             		String partdesc1   = cellpartdesc1.getContents();
             		String partdescCNT = partdesc1;

					if (partdesc1==null || partdesc1.equals(""))
					{
						icnt++;
					}
           			...
           			try
            		{
						// To Check the ITEM result records
               			String sqlCNTitem = "";
						if (custParts!=null && !custParts.equals(""))
						{
							sqlCNTitem =" select count(1)"+
                                        ...
                                        " and a.ITEM = '"+custParts+"'"+
										" and a.ITEM_DESCRIPTION=nvl('"+partdescCNT+"',a.ITEM_DESCRIPTION)";
						...
						else
						{
							sqlCNTitem = " select count(*) from APPS.MTL_SYSTEM_ITEMS a"+
										  ...
										  " AND NVL(a.CUSTOMER_ORDER_FLAG,'N')='Y'"+  //add by Peggy 20170616		
										  " AND NVL(a.CUSTOMER_ORDER_ENABLED_FLAG,'N')='Y'"+  //add by Peggy 20170616
									      " AND (length(a.SEGMENT1)=22 or DESCRIPTION='S1GLWH-02' or (length(a.SEGMENT1)=30 and (a.SEGMENT1 LIKE '%F00' and DESCRIPTION<>'S1GLWH-02')))"+  //add by Peggy 20230118
               			...
               			if(rsCNTitem.getInt(1) == 0) 
               			{
							if (custParts!=null && !custParts.equals(""))
							{						
								out.println("<font color='red'>Not found cust PN:"+custParts+" in ERP<br></font>");
								uploadFlag =null;						
							}
                 			itemCNTsub[itemno]=1;
               			}
				...
				//out.println("<BR>itemCNTtotal="+itemCNTtotal+" columns="+columns+"<BR>");
         		// 20110222 Marvie Add : set columns = 11
         		columns = 30;
				...
				int j = 0;  
         		for( int i=0 ; i< irow ; i++ )
         		{
            		//attention: The first parameter is column,the second parameter is row.  
            	 	...
            	 	String CheckNext = "N";
             		cellCustPart = sheet.getCell(1,i+6);  //add by Peggy 20200311
					custParts=cellCustPart.getContents().trim(); //add by Peggy 20200312
					if (custParts==null) custParts="";
					custParts=custParts.replace("'",""); //add by Peggy 20210911
					...
					out.println("row="+i+" item_desc="+cellpartdesc1.getContents());
					//End Customer ID
					strEndCust ="";
					...
					else
					{
             		...
             		if(QtyCheck.trim() != "" && strdate.trim() != "")
             		{
             			...
             			try 
						{
							...
							if (custParts!=null && !custParts.equals(""))
							{
								sqlUOM =" select msi.INVENTORY_ITEM_ID,msi.SEGMENT1,msi.DESCRIPTION,msi.PRIMARY_UOM_CODE ,NVL(msi.ATTRIBUTE3,'N/A') ATTRIBUTE3"+
										" from oe_items_v a"+
										...
										" and NVL(msi.CUSTOMER_ORDER_ENABLED_FLAG,'N')='Y'"+
										" and a.ITEM = '"+custParts+"'"+
										" and a.ITEM_DESCRIPTION=nvl('"+BufferPartDesc[1][i]+"',a.ITEM_DESCRIPTION)"+
										//" and (length(msi.SEGMENT1)=22 or msi.DESCRIPTION ='S1GLWH-02' or (length(msi.SEGMENT1)=30 and msi.SEGMENT1 LIKE '%F00' and msi.DESCRIPTION <>'S1GLWH-02'))"+  //add by Peggy 20230118
							...
							{					
								sqlUOM  = "select INVENTORY_ITEM_ID,SEGMENT1,DESCRIPTION,PRIMARY_UOM_CODE ,NVL(ATTRIBUTE3,'N/A') ATTRIBUTE3"+
								 ...
								 " AND NVL(a.CUSTOMER_ORDER_FLAG,'N')='Y'"+  //add by Peggy 20170616		
								 " AND NVL(a.CUSTOMER_ORDER_ENABLED_FLAG,'N')='Y'"+  //add by Peggy 20170616
							     " AND (length(a.SEGMENT1)=22 or a.DESCRIPTION ='S1GLWH-02' or (length(a.SEGMENT1)=30 and a.SEGMENT1 LIKE '%F00' and a.DESCRIPTION <>'S1GLWH-02'))"+  //add by Peggy 20230118
								 " AND tsc_item_pcn_flag(43,a.inventory_item_id,trunc(sysdate))='N'"; //add by Peggy 20230118								 
							}								 
                			...
                			while (rsUOM.next()) 
							{
								...
								itemFactory = rsUOM.getString("ATTRIBUTE3");	
                   				...
                   				if (BufferPartNumber[1][i].trim() !=null && BufferPartNumber[1][i].trim() !=null &&
				       				BufferPartDescNew[1][i].trim() !=null && BufferPartDescNew[1][i].trim() !=null)
								{
									...
									b[j][8]=BufferOrderNo + "(" + RemarkDesc + ")";
									if (itemCNTsub[i]==1) b[j][9]=strRemark;  //add by Peggy 20150707
									else b[j][9]="*DUPLICATED*("+RemarkDesc+")";
									b[j][10]="N";
									b[j][11]="0";
									b[j][12]="0";
									b[j][13]=itemFactory;   //�Ͳ��t�O add by Peggy 20120316
									b[j][14]=custParts;      //�Ȥ�Ƹ� add by Peggy 20120316
									b[j][15]=""+BufferUnitPrice[7][i];       //�[�JEXCEL�ɪ����,modify by Peggy 20140626
									b[j][16]="";            //�q������ add by Peggy 20120316
									...
									b[j][26]="";            //sample order direct ship,add by Peggy 20160401
									b[j][27]="";            //bi region,add by Peggy 2017022
                			...
                			else 
							{
                   				...
                   				b[j][7]=BufferRequestDate[2][i];                   
                   				// 20110222 Marvie Update : Add Field  SPQ MOQ
                   				//b[j][6]="*WRONG*("+RemarkDesc+")";
				   				b[j][8]=BufferOrderNo + "(" + RemarkDesc + ")";
                   				b[j][9]="*WRONG*("+RemarkDesc+")";
				   				b[j][10]="N";
                   				b[j][11]="0";
				   				b[j][12]="0";
								b[j][13]="";       //�Ͳ��t�O add by Peggy 20120303
								b[j][14]="";       //�Ȥ�Ƹ� add by Peggy 20120303
								b[j][15]="";       //��� add by Peggy 20120303
								b[j][16]="";       //�q������ add by Peggy 20120303
								b[j][17]="";       //LineType add by Peggy 20120303	
								b[j][18]="";       //FOB add by Peggy 201203029									
								b[j][19]="";       //CUST PO LINE NO,add by Peggy 20120601
								b[j][20]="";       //QUOTE NUMBER,add by Peggy 20120917
								b[j][21]="";       //END CUSTOMER ID,add by Peggy 20140826
								b[j][22]="";       //SHIPPING MARKS,add by Peggy 20130305
								b[j][23]="";       //REMARKS,add by Peggy 20130305
				...
				// 20110222 Marvie Add : Add Field  SPQ MOQ
				...
				for (int i=0;i<bb.length;i++) 
				{
					...
					cc[i][26]="D";  //add by Peggy 20160401
...
<form method="post" action="TSCTTantronImport.jsp?UPLOADFLAG=Y" name="upform" enctype="multipart/form-data">
...
<% //TSCEBufferImport.jsp?UPLOADFLAG=Y
	...
	{ 
		...
		//		   "&CUSTOMERIDTMP="+CustomerIdTmp+"&INSERT="+Insert+"&PROGRAMNAME=D4-006";
		String urlDir = "TSSalesDRQCreateImport.jsp?"+"CUSTOMERID="+java.net.URLEncoder.encode(CustomerId)+
		           "&SPQCHECKED="+java.net.URLEncoder.encode(SPQChecked)+
				   ...
				   //"&PROCESSAREA="+java.net.URLEncoder.encode(ProcessArea)+
				   "&CUSTOMERIDTMP="+java.net.URLEncoder.encode(CustomerIdTmp)+
				   "&INSERT="+java.net.URLEncoder.encode(Insert)+"&PROGRAMNAME=D4-006";		
  ...
  <table width="60%" border="0" cellspacing="1" cellpadding="1" align="center">
    <tr>
      <td align="left"><font size="-1" face="Verdana, Arial, Helvetica, sans-serif"><b>Select
        TSCT Tantron Order in Excel format to upload :</b></font></td>
    </tr>
    <tr>
