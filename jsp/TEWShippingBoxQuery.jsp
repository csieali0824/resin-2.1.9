...
<STYLE TYPE='text/css'>  
  ...
  INPUT     { font-family: Tahoma,Georgia; font-size: 12px }
</STYLE>
<title>TEW 編箱基本設定</title>
<!--=============以下區段為安全認證機制==========-->
...
<strong><font style="font-size:18px;color:#000099">TEW 編箱基本設定</font></strong>
<BR>
<%
...
<table cellSpacing="0" bordercolordark="#5C7671"  cellPadding="1" width="100%" align="center" borderColorLight="#CFDAD8" border='1'>
	<tr>
		<td width="7%" style="background-color:#D3E6F3;color:#006666">供應商:</td>
		<td width="20%">
		<%
	      PreparedStatement statement1 = con.prepareStatement("SELECT b.vendor_id,b.vendor_name FROM tsc_item_packing_master a,ap.ap_suppliers b where a.int_type=? AND a.vendor_id = b.vendor_id group by b.vendor_id,b.vendor_name order by b.vendor_name");
		...
		<td width="10%"><input type="text" name="Q_PACKING_CODE" value="<%=Q_PACKING_CODE%>" style="font-family: Tahoma,Georgia; font-size: 12px"></td>
		...
		<td width="22%">
			<INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>'  style="font-family: Tahoma,Georgia;" onClick='setSubmit("../jsp/TEWShippingBoxQuery.jsp?ACTIONCODE=Q")' >
			&nbsp;&nbsp;&nbsp;
			<INPUT TYPE='button' id='AddNew' value='新增' onClick='setAddNew("../jsp/TEWShippingBoxSetup.jsp")'>
		</td>  
	</tr>
</table> 
<%
if (ACTIONCODE.equals("Q"))
{
	try
	{
		sql = " select a.rowid,b.vendor_name, a.tsc_prod_group, a.tsc_package, a.packing_code, a.def_flag,"+
			  ...
			  "  from tsc_item_packing_master a,ap_suppliers b where a.INT_TYPE='TEW' and a.vendor_id = b.vendor_id";
...
%>
				...
				<table cellspacing="0" bordercolordark="#FFFFFF" cellpadding="1" width="100%" align="center" bordercolorlight="#CCCC99" border="1">
					<tr bgcolor="#C8E3E8">
						...
						<td align="center" width="13%">供應商</td>
						...
						<td align="center" width="10%">台半品名</td>
						<td align="center" width="5%">狀態</td>
						<td align="center" width="7%">最近異動者</td>
						<td align="center" width="9%">最近異動日</td>
					</tr>
<%
			}
			out.println("<tr>");
			out.println("<td align='center'>");
			out.println("<input type='button' id='UPD"+i+"' value='修改' onclick='setModify("+'"'+"../jsp/TEWShippingBoxSetup.jsp?ACTIONMODE=MODIFY&ID="+java.net.URLEncoder.encode(rs.getString("ROWID"))+'"'+")'>");
			out.println("<input type='button' id='CANCEL"+i+"' value='"+(rs.getString("STATUS").equals("Y")?"停用":"啟用")+"' onclick='setInactive("+'"'+"../jsp/TEWShippingBoxQuery.jsp?ACTIONCODE=CANCEL&ID="+java.net.URLEncoder.encode(rs.getString("ROWID"))+"&STATUS="+(rs.getString("STATUS").equals("Y")?"N":"Y")+ '"'+","+'"'+(rs.getString("STATUS").equals("Y")?"N":"Y")+'"'+")'>");
			out.println("</td>");
			out.println("<td>"+i+"</td>");
