...
<%@ page import="bean.QryAllChkBoxEditBean,bean.ComboBoxAllBean,bean.ComboBoxBean,bean.DateBean,bean.WorkingDateBean,bean.ArrayComboBoxBean"%>
...
<strong>Book Sales Order Process</strong></font>
<BR>
  <A href="/oradds/ORAddsMainMenu.jsp">回首頁</A><!--%/20040109/將Excel Veiw 夾在檔頭%-->
<%
 
  sWhereGP = " and OOHA.ORG_ID > 0 ";
...
if (dateStringBegin==null || dateStringBegin.equals(""))
{
   sSql = "select OOHA.HEADER_ID, OOHA.SHIP_FROM_ORG_ID, OOHA.ORDER_TYPE_ID, OOHA.ORDER_NUMBER, CUSTOMER_NUMBER||'('||CUSTOMER_NAME||')' as CUSTOMER, "+
                 "OOHA.SHIP_TO_ORG_ID, OOHA.INVOICE_TO_ORG_ID "+				 
		  "from oe_order_headers_all OOHA, RA_CUSTOMERS RC ";
   sSqlCNT = "SELECT count(OOHA.HEADER_ID) as CaseCount FROM oe_order_headers_all OOHA, RA_CUSTOMERS RC  ";
   sWhere =  "WHERE OOHA.ORG_ID = 41 and TO_CHAR(OOHA.SHIP_FROM_ORG_ID) ='"+organizationId+"' "+
             "AND OOHA.SOLD_TO_ORG_ID = RC.CUSTOMER_ID "+            
			 "AND flow_status_code = 'ENTERED' "+
			 "AND TO_CHAR(OOHA.ORDER_TYPE_ID) = '"+orderType+"' "+  
			 "AND to_char(OOHA.CREATION_DATE,'YYYYMMDD') <= '"+dateBean.getYearMonthDay()+"' ";     // 預設進入頁面即將小於今日            
   sOrder = "order by OOHA.ORDER_NUMBER ";
...
{ 
  ...
  //out.println(sSql);    
   ...
   String sqlOrgCnt = "select count(OOHA.HEADER_ID) as CaseCountORG from oe_order_headers_all OOHA, RA_CUSTOMERS RC ";
...
%>  
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#FFFFFF">
     <tr><td bgcolor="#FFFF99"><font face="Arial" color="#000066"><span class="style1">&nbsp;</span>Organization</font>
			   <%
			       try
                   {   
		             Statement statement=con.createStatement();
                     ResultSet rs=null;	
			         String sql = "select a.organization_id, a.organization_id || '('||a.organization_code|| ')' "+
			 ...
			 <td bgcolor="#FFFF99" colspan="2"><font face="Arial" color="#000066"><span class="style1">&nbsp;</span>Order Type</font>
			    <%
		         try
                 {   
		           Statement statement=con.createStatement();
     ...
     <tr bgcolor="#FFFF99">
	    <td width="42%" bgcolor="#FFFF99" bordercolor="#CCCCFF" nowrap><font color="#000066" face="Arial"><span class="style1">&nbsp;</span>Create Date From</font>
        <%
	...
	<td width="42%" bordercolor="#CCCCFF" nowrap><font color="#000066" face="Arial"><span class="style1">&nbsp;</span>Create Date To</font>
        <%
	...
	<td width="16%">
		    <INPUT TYPE="button" align="middle"  value='查詢' onClick='setSubmit("../jsp/TestOracleOrderProcessBookSelect.jsp")' >   
	</td>
   </tr>
  </table>  
  ...
  <table cellSpacing="0" bordercolordark="#FFCC99" cellPadding="1" width="100%" align="center" bordercolorlight="#FFFF99" border="1">
   <tbody>
    <tr bgcolor="#FFFF99"> 
	  ...
	  <td width="3%"  height="19" nowrap><div align="center"><font color="#330099" face="Arial" size="2"><strong>Book</strong></font></div></td>   
      <td width="9%" height="22" nowrap><div align="center"><font color="#330099" face="Arial" size="-1">Org.</font></div></td>          
      <td width="10%" nowrap><div align="center"><font color="#330099" face="Arial" size="-1">Header ID</font></div></td>
      <td width="16%" nowrap><div align="center"><font color="#330099" face="Arial" size="-1">Order No.</font></div></td>      
      <td width="10%" nowrap><div align="center"><font color="#330099" face="Arial" size="-1">Order Type</font></div></td>
      <td width="15%" nowrap><div align="center"><font color="#330099" face="Arial" size="-1">Customer</font></div></td>      
      <td width="16%" nowrap><div align="center"><font color="#330099" face="Arial" size="-1">Ship To</font></div></td>
	  <td width="19%" nowrap><div align="center"><font color="#330099" face="Arial" size="-1">Bill To</font></div></td>	         
    </tr>
    ...
    <tr bgcolor="<%=colorStr%>"> 
	  ...
	  <td height="20"><div align="center"><font size="2" color="#000000">
          <input type="checkbox" name="CHKFLAG" value="<%=rsTC.getString("ORDER_NUMBER")%>">
          </font></div>
	  </td>     
	  <td nowrap><font size="-1"><%=rsTC.getString("SHIP_FROM_ORG_ID") %></font></td>      
      <td width="10%" nowrap><font size="-1"><%=rsTC.getString("HEADER_ID")%></font></td>
      <td width="16%" nowrap><font size="-1"><%=rsTC.getString("ORDER_NUMBER")%></font></td>
      <td width="10%" nowrap><font size="-1"><%=rsTC.getString("ORDER_TYPE_ID")%></font></td>
      <td width="15%" nowrap><font size="-1"><%=rsTC.getString("CUSTOMER")%></font></td>      
      <td width="16%" nowrap><font size="-1"><%=rsTC.getString("SHIP_TO_ORG_ID")%></font></td>      
      <td width="19%" nowrap><font size="-1"><%=rsTC.getString("INVOICE_TO_ORG_ID")%></font></td>      
    </tr>
    <%
      rs1__index++;
      rs_hasDataTC = rsTC.next();
              qryAllChkBoxEditBean.setPageURL("../jsp/TestOracleOrderProcessBookSelect.jsp");
              qryAllChkBoxEditBean.setSearchKey("ORDER_NUMBER");
...
%>
    <tr bgcolor="#FFFF99"> 
      <td height="23" colspan="10" ><font color="#330099" size="-1">訂單未確認筆數</font> 
        <% 
	      if (CaseCount==0) 
		  { //若 	  
		  } 
...
<BR>
<div align="left"><input name="submit2" type="submit" value="Batch Process Order Booked" onClick='return setSubmit2("../jsp/TestOracleOrderProcessBook.jsp")'></div>
</FORM>
