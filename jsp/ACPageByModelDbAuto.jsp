...
<%
	...
	String sSqlUT = "select max(substr(mcarton_no,13,4)) as MONTHDAY,max(to_char(in_station_time,'HH24')) as Hour24 from MES_WIP_TRACKING where  group_name='AC_PACKING' and pallet_full_flag='Y'  and to_char(in_station_time,'YYYYMMDD') <= '"+DateString+"' ";            
	      ResultSet rsUT=stmentUT.executeQuery(sSqlUT);
		  if (rsUT.next())
		  { MonthDay = rsUT.getString("MONTHDAY"); 
		     Hour24 =  rsUT.getString("Hour24"); }
		   String Month = MonthDay.substring(0, 2);   // 改抓系統資料更新時間
		   String Day = MonthDay.substring(2, 4);       // 由MES 系統
    stmentUT.close();
	rsUT.close();
...
<table width="100%" border="0">
  <tr><td><strong><font size="+2">MES AC生產月報表</font></strong><BR></td>
      <td><div align="right"><strong><em>資料最後更新:<%=Month%>月<%=Day%>日<%=Hour24%>時</em></strong></div></td>
  </tr>
</table>  	  
<table width="100%" border="0">
  <tr>
