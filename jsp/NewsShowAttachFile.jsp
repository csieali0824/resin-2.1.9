...
{  
  ...
  ResultSet rs=stmt.executeQuery("select * from ORADDMAN.TSDNOTICE_DOC where DNDOCNO='"+dnDocNo+"' and TYPE ='NEWS' ");
  if (rs.next())
  {
   blob = (Blob)rs.getObject("SOURCE_FILE") ;  
   bbuff = blob.getBytes(1, (int)blob.length());
       
   int specFileNameLngth = rs.getString("FILE_NAME").length();
   String specFileExtName = rs.getString("FILE_NAME").substring(specFileNameLngth-3,specFileNameLngth);
