...
<%
 try
 {
	   ...
	   String sqliim="select DISTINCT IPROD  from iim where IID='IM' and IPROD like '0%' and ICLAS='MB'   ORDER BY IPROD ";   
       Statement iimstmt=bpcscon.createStatement();    
	   ResultSet rsiim=iimstmt.executeQuery(sqliim);   
	  // out.println(sqliim); 
	   while (rsiim.next())
	   { 
	     String iprod=null; 
	     ...
	     iprod=rsiim.getString("IPROD"); 
		   //out.println(iprod); 
		  String sqlith="select TPROD,TQTY,TTDTE from ith where ttype='R' and TPROD like '0%' and TPROD ='"+iprod+"' and TTDTE='"+DateString+"' "; 
		  Statement statement=bpcscon.createStatement();
	      ResultSet rs=statement.executeQuery(sqlith);
		   //out.println(sqlith); 
		   //out.println(prodnum); 
	       if(rs.next())
		   {		   
		    ...
		    sgendate= rs.getString("TTDTE");
	         ...
	         rs.close();	  
			 //out.println(prodnum); 
		    if(prodnum==null || prodnum.equals(""))
		     {
			   prodnum=rsiim.getString("IPROD"); 
		       genqty=0; 
			   sgendate=DateString; 
			   }		   
			...
			if(dayofweek==7)
			{weekend="Y";} 
			int currentDay=dateBean.getMaxDay(); 
			//out.println(currentDay); 
			if(nDay==dateBean.getMaxDay())
			{monthend="Y";}
