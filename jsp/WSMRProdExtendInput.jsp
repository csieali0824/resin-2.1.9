...
{
   warray=new Array(document.MYFORM.PRODEXTCODE.value,document.MYFORM.PRODEXTEND.value,document.MYFORM.EXTENDDESC.value);   
   for (i=0;i<3;i++)
   {     
...
<title>WINS System-產品機種延伸資料新增</title>
</head>
<body>
<FORM ACTION="WSMRProdAprnceInput.jsp" METHOD="post" NAME="MYFORM">
  ...
  <strong>  產品編碼系統-機種延伸代碼新增 </strong></font> &nbsp; &nbsp; &nbsp; 
  &nbsp;<BR>
  <A HREF="../WinsMainMenu.jsp">首頁</A> &nbsp;&nbsp;<A HREF="../jsp/WSModelEncodingSub.jsp">產品編碼管理作業</A> &nbsp;&nbsp;<A HREF="../jsp/WSMRProdAprnceInquiry.jsp">產品機種延伸代碼資料查詢</A> 
<%   
...
String prodExtCode=request.getParameter("PRODEXTCODE"),prodExtend=request.getParameter("PRODEXTEND"),extendDesc=request.getParameter("EXTENDDESC");
...
String [] allMonth={prodExtCode,prodExtend,extendDesc};
...
<table width="60%" border="1" cellspacing="0" cellpadding="0">
  <tr>      
      <td width="20%" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>機種延伸代碼</strong></font></div></td>
	  <td width="21%" colspan="1" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>機種延伸說明</strong></font></div></td> 
      ...
      <td width="17%" rowspan="3"><div align="center"><strong>        
	   
        <INPUT TYPE="button"  value="Add" onClick='setSubmit("../jsp/WSMRProdExtendInput.jsp")' >
	    </strong></div>
    </td>     
    </tr>
  <tr>
    <td>    
    <input type="text" name="PRODEXTCODE"  size="15" maxlength="1" <%if (allMonth[0]!=null) out.println("value="); else out.println("value=");%>>
    </td>
    <td><input type="text" name="PRODEXTEND"  size="15" maxlength="15"   <%if (allMonth[1]!=null) out.println("value="); else out.println("value=");%> >
    </td>
    <td><input type="text" name="EXTENDDESC"  size="25" maxlength="25"   <%if (allMonth[2]!=null) out.println("value="); else out.println("value=");%> >
    </td>    
    </tr>    
  <tr bgcolor="#003366">
    <td colspan="3"><div align="center"><strong>
     <%
	  try
      {
        ...
        String oneDArray[]= {"增項","機種延伸代碼","機種延伸說明","備註"}; 		 	     			  
    	array2DimensionInputBean.setArrayString(oneDArray);
	     ...
	     if (prodExtCode!=null && !prodExtCode.equals("") && prodExtend!=null && !prodExtend.equals("") && bringLast==null) //bringLast是用來識別是否帶出上一次輸入之最新版本資料
		 {  //out.println("step1");             		    
		   if (a!=null) 
		   { //out.println("step2");
			 ...
			 { //out.println("step3");
			  ...
			  { //out.println("step4");
			    b[i][j]=a[i][j];	
                if (a[k][0].equals(prodExtCode)) { dupFLAG = "TRUE"; }				
			  }
			  k++;
			 }
			 //b[k][0]=chooseItem;
             ...
             String sqlIMDUP = "select * from MRPROD_EXT where PRODEXT_CODE='"+prodExtCode+"' ";
             //out.println(sqlIMDUP);     
             ResultSet rsIMDUP=stateIMDUP.executeQuery(sqlIMDUP);             
             if (rsIMDUP.next() )
             { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input :</font><font color='#FFFFFF' face='Arial Black'> "+prodExtCode+"</font><font color='#FF0000' face='Arial Black'> on Database history"+"</font>"); }
             else if (dupFLAG=="TRUE" || dupFLAG.equals("TRUE")) { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input :</font><font color='#FFFFFF' face='Arial Black'> "+prodExtCode+"</font><font color='#FF0000' face='Arial Black'> on Your list"+"</font>"); }
             else
             {
              b[k][0]=prodExtCode;b[k][1]=prodExtend;b[k][2]=extendDesc;
             ...
             {
              String c[][]={{prodExtCode,prodExtend,extendDesc}};						             			 
...
<% 
     int div1=0,div2=0;     //做為運算共有多少個row和column輸入欄位的變數
      ...
      {	 
       ...
       %>
<BR>
<INPUT name="button2" TYPE="button" onClick='setSubmit3("../jsp/WSMRProdExtendInput.jsp")'  value="DELETE" >
------------------------------------------------------------------------------------------------------------
<INPUT TYPE="button"  value="SAVE" onClick='setSubmit2("../jsp/WSMRProdExtendInsert.jsp",<%=div1%>,<%=div2%>)' >
---------------------
<!-- 表單參數 -->  
