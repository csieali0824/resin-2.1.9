...
function subWindowCustInfoFind(custID,custName)
{ 
   if (event.keyCode==13)
   {    
    subWin=window.open("../jsp/subwindow/TSDRQCustomerInfoFind.jsp?CUSTOMERID="+custID+"&NAME="+custName,"subwin");  
   }	
}
function setCustInfoFind(custID,custName)
{      
    subWin=window.open("../jsp/subwindow/TSDRQCustomerInfoFind.jsp?CUSTOMERID="+custID+"&NAME="+custName,"subwin");  
}
</script>
...
{   
  ...
  if ((!(DayFr=="--")&&(DayFr=="00")) && DayTo=="--") sWhere=sWhere+" and substr(c.REQUIRE_DATE,0,8) >="+"'"+dateSetBegin+"'";
  if (DayFr!="--" && DayTo!="--") sWhere=sWhere+" and substr(c.REQUIRE_DATE,0,8) >= "+"'"+dateSetBegin+"'"+" AND substr(c.REQUIRE_DATE,0,8) <= "+"'"+dateSetEnd+"'";
  
  if (salesAreaNo==null || salesAreaNo.equals("") || salesAreaNo.equals("--")) {sWhere=sWhere+" ";}
  else { sWhere=sWhere+" and c.TSAREANO ='"+salesAreaNo+"'"; }
  
  if (customerId==null || customerId.equals("")) {sWhere=sWhere+" ";}
  else { sWhere=sWhere+" and c.TSCUSTOMERID ='"+customerId+"'"; }
  
  if (dnDocNoSet==null || dnDocNoSet.equals("")) {sWhere=sWhere+" ";}
  else { sWhere=sWhere+" and c.dnDocNo ='"+dnDocNoSet+"'"; }
  
  if (preOrderType==null || preOrderType.equals("") || preOrderType.equals("--")) {sWhere=sWhere+" ";}
  else { sWhere=sWhere+" and to_char(c.ORDER_TYPE_ID) ='"+preOrderType+"'"; }
  
  if (custPONo==null || custPONo.equals("")) {sWhere=sWhere+" ";}
  else { sWhere=sWhere+" and c.CUST_PO ='"+custPONo+"'"; }
  
  if (createdBy==null || createdBy.equals("")) {sWhere=sWhere+" ";}
  else { sWhere=sWhere+" and CREATED_BY ='"+createdBy+"'"; }
  
  if (salesPerson==null || salesPerson.equals("")) {sWhere=sWhere+" ";}
  else { sWhere=sWhere+" and SALESPERSON ='"+salesPerson+"'"; }
  ...
  else { sWhere=sWhere+" and d.LSTATUS ='"+status+"'"; }
...
%>  
  <table cellSpacing='0' bordercolordark='#CCCC99'  cellPadding='1' width='100%' align='center' borderColorLight='#ffffff' border='1'>
     <tr>
		...
		<font color="#006666" size="2"><strong><jsp:getProperty name="rPH" property="pgSalesArea"/></strong></font>         
        </td> 
		<td width="32%">
		   <%		 
	       try
           {   
				   ...
				   else { comboBoxAllBean.setSelection(salesAreaNo); }
       ...
       %>
		...
		<td width="26%">
		   <font color="#006666" size="2"><strong><jsp:getProperty name="rPH" property="pgPreOrderType"/> </strong></font>
		</td>   
		<td width="23%" colspan="1" nowrap><div align="left">   
		      <%
		         try
                 {   
		           Statement statement=con.createStatement();
                   ResultSet rs=null;	
			       String sqlOrgInf = "select OTYPE_ID, ORDER_NUM||'('||DESCRIPTION||')' as TRANSACTION_TYPE_CODE "+
	 ...
	 <tr>
	   <td><font color="#006666" size="2"><strong><jsp:getProperty name="rPH" property="pgQDocNo"/> </strong></font></td>
	   <td><input type="text" name="DNDOCNOSET" value="<%=dnDocNoSet%>"></td>
	   <td>
		   <div align="left">
		   <font color="#006666" size="2"><strong><jsp:getProperty name="rPH" property="pgCreateFormUser"/> </strong></font>
		   </div>
	   </td>
	   <td>	   
		   <input type="text" name="CREATEDBY" size="15">		   
		</td> 
	 </tr>
	 <tr>
	    <td nowrap colspan="1"><font color="#006666" size="2"><strong><jsp:getProperty name="rPH" property="pgCustPONo"/> </strong></font>
         
        </td> 
		<td>
		   <div align="left">
		   <font color="#006666" size="2"><strong> </strong></font><input type="text" name="CUSTPONO" value="<%=custPONo%>">
		   </div>
		</td> 
		<td>
		   <div align="left">
		   <font color="#006666" size="2"><strong><jsp:getProperty name="rPH" property="pgSalesMan"/> </strong></font>
		   </div>
		</td>
		<td>   
		   <input type="text" name="SALESPERSON" size="15">		   
		</td>   
	 </tr>
	 <tr>
	    <td nowrap colspan="1"><font color="#006666" size="2"><strong><jsp:getProperty name="rPH" property="pgProdFactory"/> </strong></font>
		...
		<td>
		   <div align="left">
		   <font color="#006666" size="2"><strong><jsp:getProperty name="rPH" property="pgRepStatus"/> </strong></font>
		   </div>
		</td>
		<td>   
		   <%
  ...
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#FFFFFF">
    <tr bgcolor="#99CC99"> 
      ...
      <td width="17%" nowrap><div align="center"><font color="#006666" size="2"><jsp:getProperty name="rPH" property="pgCustomerName"/></font></div></td>
    ...
    <tr bgcolor="<%=colorStr%>"> 
      ...
      <td width="48%" nowrap><font size="2" color="#006666">
					  ...
					  out.println("</td></tr>");
			              ...
			              {			    
				             ...
				             { //out.println("wipEntID="+wipEntID);out.println("rsTC.getString(INVENTORY_ITEM_ID)="+rsTC.getString("INVENTORY_ITEM_ID"));
				               ...
				               int iRow = 0; 
				               String sqlM = "select d.LINE_NO,d.LSTATUS,d.ITEM_DESCRIPTION, d.QUANTITY, "+
                                             "TO_CHAR(TO_DATE(d.REQUEST_DATE,'YYYYMMDDHH24MISS'),'YYYY/MM/DD'),"+
											 "d.PCCFMDATE,d.FTACPDATE, d.PCACPDATE, d.SASCODATE,"+
                                             "e.MANUFACTORY_NAME "+
										     "from ORADDMAN.TSSALES_AREA b, "+
							  ...
							  String whereM = "where (d.DNDOCNO = '"+rsTC.getString("DNDOCNO")+"' ) "+
							   ...
							   String orderM = "order by d.LINE_NO";				   
											   //", TO_CHAR(TO_DATE(a.UPDATEDATE||a.UPDATETIME,'YYYYMMDDHH24MISS'),'YYYY/MM/DD:HH24:MI:SS')";
						       if (prodManufactory==null || prodManufactory.equals("--")) {whereM=whereM+" ";}
                               else { whereM=whereM+" and d.ASSIGN_MANUFACT ='"+prodManufactory+"'"; }
  
                               if (status==null || status.equals("--")) {whereM=whereM+" ";}
                               else { whereM=whereM+" and d.LSTATUS ='"+status+"'"; }
				               ...
				               while (rsM.next())
				               {
								...
								{ 
								out.println("<tr bgcolor='#CC3366'><td width='1%' nowrap><font size='1' color='#FFFFFF'>");
								...
								out.println("<td nowrap><font size='-2' color='#006666'>"+rsM.getString(5)+"</font></td>");
			  ...
			  %></font></td>      
      <td width="17%" nowrap><font size="2" color='#006666'><%=rsTC.getString("CUSTOMER")%></font></td>  
