...
<%
	...
	String toPersonID="",orderTypeID="",orderTypeName="",oTypeDesc="",soldToOrg="",priceList="",shipToOrg="", orderNum="",plantCode="",coo="";
	...
	String rfq_Type="",SALES_GROUP_ID="",rfq_Type_Name="",SupplierNumber="";         //add by Peggy 20130318
	...
	try
	{
		...
		if (UserRoles.indexOf("admin")>=0  || (UserRoles.indexOf("SMCUser")>=0 && (UserName.equals("CCYANG") || UserName.equals("RITA_ZHOU")) && lineStatusID.equals("014")) ) //modif by Peggy 20141008
		{
			sqlDocs="select b.assign_manufact,tsc_get_item_coo(b.inventory_item_id) COO," +
					...
					"a.*,b.LSTATUSID,b.LSTATUS,TO_CHAR(to_date(b.LAST_UPDATE_DATE,'YYYYMMDDHH24MISS'),'YYYY/MM/DD ') as LUPDATE, "+
					...
					",c.a_value rfq_type_name"+ //add by Peggy 202111119
					",b.supplier_number"+ //add by Peggy 20220428
		...
		else if (UserRoles.indexOf("SalesPlanner")>=0 || UserRoles.indexOf("CInternal_Planner")>=0) // 企劃Assigning?RESPONDING
		{
			...
			{
				sqlDocs += " and (b.ASSIGN_MANUFACT = '"+userProdCenterNo+"' or substr(a.DNDOCNO,3,3) >='"+userActCenterNo+"' ) "; // 若是空的地區集,則以主要的業務區
			}
			else
			{
		...
		else //業務或工廠PC
		{
			sqlDocs="select b.assign_manufact,tsc_get_item_coo(b.inventory_item_id) COO," +
					...
					" WHERE a.DNDOCNO=b.DNDOCNO and a.DNDOCNO='"+dnDocNo+"'"+
					" and a.rfq_type=c.a_seq(+) ";
			//add by Peggy 20170525,原sql老是發生lock,改成以下where看是否能解決
			if (userProdCenterNo==null || userProdCenterNo.equals(""))
			{
				if  (UserRegionSet==null || UserRegionSet.equals(""))
				{
					sqlDocs += " and substr(a.DNDOCNO,3,3) ='"+userActCenterNo+"'";
				}
				else
				{
					sqlDocs += " and substr(a.DNDOCNO,3,3) in ("+UserRegionSet+")";
				}
			}
			else
			{
				sqlDocs += " and b.ASSIGN_MANUFACT = '"+userProdCenterNo+"' ";
			}
			//if (UserRegionSet==null || UserRegionSet.equals(""))
			//{
...
//		out.println("sqlDocs="+sqlDocs);
		...
		plantCode=docrs.getString("ASSIGN_MANUFACT");
		coo=docrs.getString("COO");
		...
		rfq_Type_Name=docrs.getString("RFQ_TYPE_NAME");          //add by Peggy 20211119
		if (rfq_Type_Name==null) rfq_Type_Name="";
		SupplierNumber=docrs.getString("SUPPLIER_NUMBER"); //add by Peggy 20220428,for conti/vitesco
		if (SupplierNumber==null) SupplierNumber="";
		...
		if (tsAreaNo!=null)
		{
			Statement docstatement1=con.createStatement();
			ResultSet docrs1=docstatement1.executeQuery("select * from ORADDMAN.TSSALES_AREA WHERE SALES_AREA_NO='"+tsAreaNo+"' ");
			docrs1.next();
			salesAreaName=docrs1.getString("SALES_AREA_NAME");
			showSSD=docrs1.getString("SSD_FLAG");  //add by Peggychen 20110621
			docrs1.close();
			docstatement1.close();
		}
		...
		{
			Statement docstatement1=con.createStatement();
			ResultSet docrs1=docstatement1.executeQuery("select * from ORADDMAN.TSRECPERSON WHERE USERID='"+reqPersonID+"' and TSSALEAREANO = '"+tsAreaNo+"' ");
			if (docrs1.next())
			{
				recSalesNo=docrs1.getString("RECPERSONNO");
				recSalesName=docrs1.getString("USERNAME");
			}
			docrs1.close();
			docstatement1.close();
		}
		...
		{
			Statement docstatement1=con.createStatement();
			ResultSet docrs1=docstatement1.executeQuery("select DISTINCT a.CUSTOMER_NUMBER,a.attribute2 marketgroup "+
			...
			if (docrs1.next())
			{
				customerNo=docrs1.getString("CUSTOMER_NUMBER");
				CUSTMARKETGROUP=docrs1.getString("marketgroup"); //add by Peggy 20111005
		...
		{
			Statement docstatement1=con.createStatement();
			ResultSet docrs1=docstatement1.executeQuery(
				"select * from ORADDMAN.TSAREA_ORDERCLS" +
				" WHERE OTYPE_ID= case when '"+plantCode+"' = '011' and '"+coo+"' = 'CN' \n" +  // todo test TW
						"then ( select DISTINCT z.OTYPE_ID FROM ORADDMAN.TSAREA_ORDERCLS z \n" +
						" where z.ACTIVE ='Y' \n" +
						" AND z.ORDER_NUM='"+orderNum+"') else '"+ orderTypeID +"' end " +
				"  and SAREA_NO='"+tsAreaNo+"' "
			);

			if (docrs1.next())
			{
				orderTypeID=docrs1.getString("OTYPE_ID");
				orderTypeName=docrs1.getString("OTYPE_NAME");
				oTypeDesc=docrs1.getString("DESCRIPTION");
			}
			else
			{
				orderTypeName="N/A";
		...
		{
			Statement docstatement1=con.createStatement();
			ResultSet docrs1=docstatement1.executeQuery(" SELECT 1 FROM ORADDMAN.tscust_special_setup WHERE sales_area_no ='"+tsAreaNo+"' AND customer_id='"+ tsCustomerID +"' AND active_flag='A'"+
					" UNION ALL"+
...
<strong><font size="3" color="#000066"><jsp:getProperty name="rPH" property="pgQDocNo"/>:</font><%=dnDocNo%></strong> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="#8000FF">
    ...
    String custPOStr = "";
	if (custPO!=null && custPO.equals("")) custPOStr=custPO.replace("'","");  // 把 ' 字眼取代成空字串(2006/12/12)
	...
	if (tsCustomerID!=null && !tsCustomerID.equals(""))  //add by Peggy 20160419
	{
		Statement seqStat1=con.createStatement();
		ResultSet seqRs1=seqStat1.executeQuery("select COUNT (1) from ORADDMAN.TSDELIVERY_NOTICE where DNDOCNO !='"+dnDocNo+"' and TSCUSTOMERID='"+tsCustomerID+"'");
		seqRs1.next();
		if (seqRs1.getInt(1)>0)
		{  
		 %>
...
%>
	...
	<table cellSpacing="1" bordercolordark="#66CC99"  cellPadding="1" width="97%" align="center" bordercolor="#66CC99" border="1">
		<tr>
			<td nowrap>
		...
		<%
			if (hStatusID.equals("001") || hStatusID.equals("008"))  //add by peggy 2012022,草稿才顯示出貨地址,立帳地址,付款條件,FOB
			{
				...
				{
					Statement Statx=con.createStatement();
					ResultSet rsx=  Statx.executeQuery(" select  a.SITE_USE_ID,a.SITE_USE_CODE, loc.COUNTRY, replace(loc.ADDRESS1,'''') as ADDRESS1,a.ATTRIBUTE1"+
					...
					while(rsx.next())
					{
						if (rsx.getString("SITE_USE_CODE").equals("BILL_TO"))
						{
							...
							SALES_GROUP_ID= rsx.getString("ATTRIBUTE1");
						}
		...
		<tr>
			...
			<td colspan="1"><jsp:getProperty name="rPH" property="pgRFQType"/>:
				<font color='#0000ff' face='Arial' size='3'><%=rfq_Type_Name%></font>
				<%
		...
		<tr>
			<td colspan="3">
				...
				<%
					try
					{
						out.println("<TABLE cellSpacing='1' bordercolordark='#66CC99'  cellPadding='1' width='100%' align='center' bordercolor='#66CC99' border='1'>");
				...
				<%
					...
					{ // 管理員
						sqlDTL ="select LINE_NO, ITEM_SEGMENT1, ITEM_DESCRIPTION, QUANTITY, UOM, REQUEST_DATE,"+
								...
								"  when '"+ rfq_Type_Name+"' not in ('EDI') and '"+orderTypeID+"' in ('1015','1342','1021','1022','1743','1707','1763') then nvl((select b.order_num from oraddman.tsarea_ordercls b where b.sarea_no ='"+tsAreaNo+"' and  TO_CHAR('"+orderTypeID+"') = b.OTYPE_ID),'N/A') "+
								"  else tsc_rfq_create_erp_odr_pkg.TSC_GET_ORDER_TYPE(a.inventory_item_id) end ORDER_TYPE"+//add by mars 20241016
								...
								"  WHERE  language ='US' AND view_application_id = 3   AND lookup_type = 'SHIP_METHOD' AND security_group_id = 0) b"+ //add by Peggy 20160825
...
//	   		out.println(sqlDTL);
				...
				<%
								...
								{
									if (rs.getString("SHIPPING_METHOD_NAME") == null)
									{
										out.println("<TD><font color='#000000'>&nbsp;</TD>");
									}
									else
									{
										out.println("<TD><font color='#000000'>"+rs.getString("SHIPPING_METHOD_NAME")+"</TD>");
									}
								}
								...
								out.println("<TD><a onMouseOver='this.T_ABOVE=false;this.T_WIDTH=80;this.T_OPACITY=80;return escape("+'"'+rs.getString("ORDERNO")+'"'+")'><font color='#000000'>"+rs.getString("SASCODATE")+"</a></TD>");
								out.println("<TD>"+rs.getString("CUST_PO_NUMBER")+"</TD>"); //add by Peggy 20160629
								out.println("</TR>");
							}
							out.println("</TABLE>");
							statement.close();
...
%>
	...
	<input name="SALES_GROUP_ID" type="hidden" value="<%=SALES_GROUP_ID%>">
	<input name="SUPPLIER_NUMBER" type="hidden" value="<%=SupplierNumber%>">
