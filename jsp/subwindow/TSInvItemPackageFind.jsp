<!--20090604 liling �W�[TSC_PROD_GROUP���P�_�Ϥ��UPROD��MOQ�ƶq���P -->
<!--20140814 by Peggy,TSCE AU�Ȥ�B��SMA�t�C���~�u��q�s�F�X�f-->
<!--20141024 by Peggy,use function:tsc_get_item_packing_code get tsc_packing_code-->
<!--20150416 by Peggy,�S�w�Ȥ���w�S�wline type-->
<!--20150721 by Peggy,tsc_edi_pkg.get_shipping_method�W�[order type-->
<!--20151008 by Peggy,mtl_system_items_b�[�JCUSTOMER_ORDER_FLAG=Y AND CUSTOMER_ORDER_ENABLED_FLAG=Y�P�_-->
...
<!--20160318 by Peggy,�Ȥ������w�q��,�۰ʱa�J-->
<!--20160513 by Peggy,for TSC_EDI_PKG.GET_SHIPPING_METHOD�s�Wcustomer_id�ӭק�-->
...
function sendToMainWindow(invItem,itemDesc,sPQP,MOQP,plantCode,sType,tscPackage,orderType,lineType,SPQRULE,lineNo,UOM,ShippingMethod,SSD,itemID,sellingprice,itemstatus,yewflag,tsceonhand,packing_ins)
{
		...
		if (window.opener.document.MYFORM.SALESAREANO.value=="001")
		{
			if (orderType=="1214")
			{
				if (plantCode=="008" || plantCode=="002")
				{
					...
					window.opener.document.MYFORM.elements["LINE_FOB_"+lineNo].value="EX WORKS";  //20220105�_�אּEX WORKS FROM EMILY
				}
	...
	else
	{
		...
		if (sType == "D1001" || sType == "D1009")
		{
						...
						if (sType == "D1001")
						{
							...
							if (window.opener.document.MYFORM.SALESAREANO.value=="001")
							{
								if ((plantCode=="008" || plantCode=="002")  && orderType=="1214")
								{
									...
									window.opener.document.MYFORM.LINEFOB.value="EX WORKS"; //20220105�_�אּEX WORKS FROM EMILY
								}
			...
			//add by JB 20241021 WAFER�Ƹ������aLineType 1503
		...
		if (searchString!="" && searchString!=null)
	   	{
			...
			String sql = "select tsc_get_item_coo(a.inventory_item_id) coo,"+ //add by Peggy 20240708
		             ...
		             "        TSC_INV_CATEGORY(INVENTORY_ITEM_ID,43,23) as TSC_PACKAGE, "+          //I1Ū����,�A��IM,modify by Peggy 20130223
		             "        TSC_INV_CATEGORY(INVENTORY_ITEM_ID,43,21) as TSC_FAMILY, "+	           //I1Ū����,�A��IM,modify by Peggy 20130223
				     ...
				     "        TSC_INV_CATEGORY(INVENTORY_ITEM_ID,43,1100000003) as TSC_PROD_GROUP, "+ //I1Ū����,�A��IM,modify by Peggy 20130223
			...
			String where="where ORGANIZATION_ID = '49'"+
					 ...
					 "  and STOCK_ENABLED_FLAG ='Y' and MTL_TRANSACTIONS_ENABLED_FLAG = 'Y' "+ // 2007/03/28 �קK��� Item �ݩʬ� N �ɭP��RFQ �L�k�ͦ�MO���D
					 "  and a.INVENTORY_ITEM_STATUS_CODE <> 'Inactive' "+
					 "  and DESCRIPTION not like '%Disable%' "+ //// �� TSC_Package �� TSC_Family ������, �B���]�t�w�Q�]�w�� Disable���ƶ�
			...
			else
			{
				if (!invItem.equals("")) {
					...
					where = where + " and upper(SEGMENT1) like '"+invItem.toUpperCase()+"%' ";
				} else if (!itemDesc.equals("")) {
					where = where + " and upper(DESCRIPTION) like '"+itemDesc.toUpperCase()+"%' ";
				} else {
...
//					" or upper(DESCRIPTION) like '"+searchString.toUpperCase()+"%') ";
			...
			rsCNT.close();
			stateCNT.close();
			if (queryCount==0) //�Y���쪺�d�߼� == 0 ,�Y�䤣��b��,�h�i��O�L�]�w��]��Category��(��s���L�]�˲��~),����,�N�ˮ֮ƥ�D�ɧY�i
	    	{
		        ...
		        "  from APPS.MTL_SYSTEM_ITEMS a , ORADDMAN.TSPROD_MANUFACTORY b ";
		  		...
		  		else
		  		{
					if (!invItem.equals("")) {
						...
						where = where + " and upper(DESCRIPTION) like '"+searchString.toUpperCase()+"%' ";
					} else {
						where = where + "and (upper(SEGMENT1) like '"+searchString.toUpperCase()+"%' "+
								" or upper(DESCRIPTION) like '"+searchString.toUpperCase()+"%') ";
					}
...
//					" or upper(DESCRIPTION) like '"+searchString.toUpperCase()+"%') ";
			...
			{ // ����w���˫~�q��,�h�H MOQ �����w�Ȧ^��
		 		out.println("<TD BGCOLOR=BLACK>"+"SPQP (KPC)"+"</TD>"); // �̫�@��a�J�q�ʳ̤p�]�˶q
			}
			else if (sampleOrdCh.equals("true"))
		    {
			    out.println("<TD BGCOLOR=BLACK>"+"SPQP (KPC)"+"</TD>"); // �̫�@��a�J�q�ʳ̤p�]�˶q
		   	}
			out.println("<TD BGCOLOR=BLACK>"+"MOQP (KPC)"+"</TD>"); // �̤p�]�˶q
        	out.println("</TR>");
        	...
        	while (rs.next())
        	{
				...
				else
				{
         		...
         		out.println("<input type='hidden' name='PLANTCODE' value='"+plantCode+"' >");       //20090515 liling �ɤW,�H�Q1����� windows.closed
				...
				else
				{
					...
					//							 " and a.TSC_PROD_GROUP = '"+tscProdGroup+"' ";  //PMD MOQ,SPQ��Ƥw�ɤJ,modify by Peggy 20140509
				...
				{ //i=�n��ܪ����� �C�hselect �@����ܧY�n�ק�
		 		...
		 		if (sampleOrdCh==null || sampleOrdCh.equals("false"))
				{ // ����w���˫~�q��,�h�H MOQ �����w�Ȧ^��
		   			out.println("<TD align='right'>"+sPQP+"</TD>");  // �̫�@��a�J�q�ʳ̤p�]�˶q
		   			out.println("<TD align='right'>"+sMOP+"</TD>");  // �̫�@��a�J�q�ʳ̤p�]�˶q
		 		}
				else if (sampleOrdCh.equals("true"))
				{ // �Y��w���˫~�q��  
		   			out.println("<TD align='right'>"+sPQP+"</TD>");  // �̫�@��a�J�q�ʳ̤p�]�˶q
	   	...
	   	if (queryCount==1 && ((itemDesc.equals(itemDesc1) && (!itemDesc.equals("") || !itemDesc1.equals(""))) || (invItem.equals(invItem1) && (!invItem.equals("") || !invItem1.equals(""))))) //�Y���쪺�d�߼� == 1
	   	{
