<!-- 20110316 liling 修正TSC_PROD_GROUP的判斷區分各PROD的MOQ數量不同 -- >
<!-- 20110330 Marvie : Fairchild MOQ -->
<!-- 20150715 Peggy,加SAMPLEORDER判斷-->
<!-- 20151122 Peggy,add TSC_PROD_FAMILY column-->
<!-- 20160114 Peggy,call function TSC_GET_ITEM_SPQ_MOQ get spq,moq,sample_spq  value-->
<!-- 20160303 Peggy,add shipto變數-->
<!-- 20160623 Peggy,修正抓到別筆remarks-->
<!-- 20180720 Peggy,新增SHIPTOORG參加for TSCA CUSTOMER DIGIKEY ISSUE-->
<html>
<head>
  	...
  	for( int i=0 ; i< q.length ; i++ ) 
	{
    	if (q[i][0] != null) 
		{
			...
			{   
				...
				if (queryCount==0) 
				{ 
					//add by Peggy 20130225,I1 read不到,再read IM
					...
					if (queryCount==0) 
					{
						//若取到的查詢數 == 0 ,若找不到半筆,則可能是無設定於包裝Category內(賣零散無包裝產品),那麼,就檢核料件主檔即可
        		...
        		while (rs.next()) 
				{
		  			...
		  			if (rsSPQP.next()) 
					{
						...
						//if ((double)(Math.round((Double.parseDouble(q[i][3])*1000)*100000)/100000)%baseK !=0 || ( Double.parseDouble(q[i][3]) < Double.parseDouble(sMOP[i])))  //增加MOQ檢查,modify by Peggy 20130201
						//{
						...
						if (matched_flag.equals("N")) //add by Peggy 20140310
						{
							...
							out.println("<TD><INPUT TYPE='TEXT'  NAME='REMARKS_"+i+"' SIZE='13' value='客戶需求"+(new DecimalFormat("######.###")).format(Double.valueOf(q[i][3]).doubleValue())+"K' style='font-size:11px;font-family: Tahoma,Georgia'></TD>");  //自動寫入備註
