...
<%
 ...
 String altRoutingDest=request.getParameter("ALTROUTINGDEST");
 		// out.print("primaryItem="+primaryItem+" "+organizationID);
...
<html>
<head>
<title>Page for choose Routing Information List</title>
</head>
<script language="JavaScript" type="text/JavaScript">
function sendToMainWindow(routingRefID,altRoutingDest)
{         
  window.opener.document.MYFORM.ROUTINGREFID.value=routingRefID; 
  window.opener.document.MYFORM.ALTROUTINGDEST.value=altRoutingDest;
  this.window.close();
}

</script>
<body>  
<FORM action="TSMfgRoutingFind.jsp" METHOD="post" NAME="SUBINVFORM">
  <font size="-1"> Routing 成品料號: 
  ...
  <%  
      ...
      { 
		...
		// out.print("="+sqlItemId);
		 ...
		 rsID.close();
		 stateID.close();
		// out.print("inv_item="+primaryItem + "  "+primaryItemID);
	   ...
	   { //out.println("0="+organizationID);
				   ...
				   {
				      sql ="select a.ROUTING_SEQUENCE_ID, a.ALTERNATE_ROUTING_DESIGNATOR,c.DESCRIPTION as ALTERNATE_DESC, b.DESCRIPTION as ASSEMBLY_ITEM_DESC "+		                  
						   "from BOM_OPERATIONAL_ROUTINGS a, MTL_SYSTEM_ITEMS b ,BOM_ALTERNATE_DESIGNATORS c ";						        		                      									  
					  where = "where a.ORGANIZATION_ID = b.ORGANIZATION_ID and a.ORGANIZATION_ID = c.ORGANIZATION_ID(+) "+
					          "  and a.ORGANIZATION_ID ='"+organizationID+"' "+
							  "  and a.ALTERNATE_ROUTING_DESIGNATOR = c.ALTERNATE_DESIGNATOR_CODE(+) "+							  
					  ...
					  String sqlCNT = "select count(DISTINCT a.ROUTING_SEQUENCE_ID) "+
					                    "from BOM_OPERATIONAL_ROUTINGS a, MTL_SYSTEM_ITEMS b, BOM_ALTERNATE_DESIGNATORS c "+ where; 
	   ...
	   {  	
        ...
        {			 
		 ...
		 {  
		  ...
		  if (altRoutingDest==null || altRoutingDest.equals("null") || altRoutingDest=="" || altRoutingDest.equals("")) altRoutingDest = "";
	    ...
	    if (queryCount==1) //若取到的查詢數 == 1, 則直接帶回原視窗
	    {
		    ...
		    <script LANGUAGE="JavaScript">	
			  //alert("TEST");			
				window.opener.document.MYFORM.ROUTINGREFID.value=document.SUBINVFORM.ROUTINGREFID.value;
                window.opener.document.MYFORM.ALTROUTINGDEST.value=document.SUBINVFORM.ALTROUTINGDEST.value; 					   	
				//this.window.close(); 			  	
            </script>
		   <%
