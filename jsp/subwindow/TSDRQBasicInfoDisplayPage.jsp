...
<!-- 20151221 by Peggy,read oe_items_v�`�o��table lock�B��������n��ܩҦ��Ȥ�~���G��mark-->
...
{      	
	...
	else if (UserRoles.indexOf("SalesPlanner")>=0 || UserRoles.indexOf("CInternal_Planner")>=0) // ����Assigning?RESPONDING
	{ 
		sqlDocs="select a.*,b.LSTATUSID,b.LSTATUS,TO_CHAR(to_date(b.LAST_UPDATE_DATE,'YYYYMMDDHH24MISS'),'YYYY/MM/DD ') as LUPDATE, "+
	             "TO_CHAR(to_date(b.LAST_UPDATE_DATE,'YYYYMMDDHH24MISS'),'HH24:MI:SS') as LUPTIME, "+
				 "TO_CHAR(to_date(b.CREATION_DATE,'YYYYMMDDHH24MISS'),'YYYY/MM/DD ') as CREATEDATE, "+
		...
		{
			sqlDocs = sqlDocs + " and (b.ASSIGN_MANUFACT = '"+userProdCenterNo+"' or substr(a.DNDOCNO,3,3) >='"+userActCenterNo+"' ) "; // �Y�O�Ū��a�϶�,�h�H�D�n���~�Ȱ�
		} 
		else 
		{
	...
	shipToOrg=docrs.getString("SHIP_TO_ORG");//?a�Xeou-��|?�Do?�g�DO
...
<%
     ...
     String custPOStr = "";
	 if (custPO!=null && custPO.equals("")) custPO.replace("'","");  // �� ' �r�����N���Ŧr��(2006/12/12)
...
<table cellSpacing="0" bordercolordark="#66CC99"  cellPadding="1" width="97%" align="center" borderColorLight="#ffffff" border="1">
  ...
  <%
	if (hStatusID.equals("001") || hStatusID.equals("008"))  //add by peggy 2012022,��Z�~��ܥX�f�a�},�߱b�a�},�I�ڱ���,FOB
  	{
	   	...
	   	<% 
			...
			if (UserRoles.indexOf("admin")>=0  || (UserRoles.indexOf("SMCUser")>=0 && (UserName.equals("CCYANG") || UserName.equals("RITA_ZHOU")) && lineStatusID.equals("014")) ) //modif by Peggy 20141008 
       		...
       		{ // �����H��(���~�P)
		    	...
		    	if (UserRegionSet==null || UserRegionSet.equals(""))
				{
			   		sqlDTL += " and ( ASSIGN_MANUFACT = '"+userProdCenterNo+"' or substr(DNDOCNO,3,3) >='"+userActCenterNo+"' ) "; // �Y�O�Ū��a�϶�,�h�H�D�n���~�Ȱ�
		    	} 
				else 
				{
			...
			{  // �u�t�ͺީη~��
	   		...
	   		while (rs.next())
	   		{
				...
				//			  	"  ";
			 	...
			 	//while (rsCI.next())
				//{
			    //	detailLot = detailLot +"<tr><td>"+rsCI.getString("ITEM_ID")+"</td><td><div align=right>"+rsCI.getString("CUST_ITEM")+"</div></td><td><div align=right>"+rsCI.getString("ITEM_DESCRIPTION")+"</div></td><td><div align=right>"+rsCI.getString("ITEM_IDENTIFIER_TYPE")+"</div></td></tr>";  // �զX�e�q����q��Ϊ��帹
				//}
			 	//rsCI.close();
			 	//stateCI.close();
