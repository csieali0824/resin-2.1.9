...
function setSubmit1()
{
	...
	document.SUBFORM.action="TEWPOReceiveListFind.jsp?ACODE=SAVE";
...
<body >  
<FORM METHOD="post" ACTION="TEWPOReceiveListFind.jsp" NAME="SUBFORM">
<%
	...
	<tr>
		<td>
			<%     
				try
				{ 
					sql = " SELECT * FROM (SELECT x.ITEM_DESC,x.PO_HEADER_ID,x.PO_NO,x.vendor_site_code,x.VENDOR_SITE_ID,NVL(x.cust_partno,'N/A') cust_partno,x.unit_price,x.box_code,x.CURRENCY_CODE,MIN(need_by_date) NEED_BY_DATE,sum(x.UNRECEIVE_QUANTITY) UNRECEIVE_QUANTITY,"+
                          " SUM(RECEIVED_QUANTITY) RECEIVED_QUANTITY,   tew_rcv_pkg.GET_PO_ALLOT_QTY( x.PO_NO,x.item_id,NVL(x.cust_partno,'N/A'),0) allot_qty "+
                          ...
                          " DECODE( C.UNIT_MEAS_LOOKUP_CODE,'KPC',(C.QUANTITY-NVL(C.QUANTITY_CANCELLED,0)-NVL(C.QUANTITY_RECEIVED,0)-NVL(F.RECEIVED_QUANTITY,0))*1000,C.QUANTITY-NVL(C.QUANTITY_CANCELLED,0)-NVL(C.QUANTITY_RECEIVED,0)-NVL(F.RECEIVED_QUANTITY,0)) UNRECEIVE_QUANTITY,"+
                          " NVL(F.RECEIVED_QUANTITY,0)*1000 RECEIVED_QUANTITY,"+
                          " C.note_to_receiver,"+
                          ...
                          " ,(SELECT x.PO_LINE_LOCATION_ID,SUM(x.RECEIVED_QUANTITY-NVL(x.SHIPPED_QUANTITY,0)) RECEIVED_QUANTITY "+
						  ...
						  " AND exists (select 1 FROM po.po_agents pa WHERE pa.ATTRIBUTE1 ='TEW' and pa.AGENT_ID= a.AGENT_ID)"+
                          ...
                          " AND C.QUANTITY-NVL(C.QUANTITY_CANCELLED,0)-NVL(C.QUANTITY_RECEIVED,0)  >0) x"+
                          " group by x.ITEM_DESC,x.PO_HEADER_ID,x.PO_NO,x.vendor_site_code,x.VENDOR_SITE_ID,x.cust_partno,x.box_code,x.unit_price,x.CURRENCY_CODE,x.item_id) k "+
                          " ORDER BY k.vendor_site_code,(RECEIVED_QUANTITY- allot_qty) desc,k.PO_NO ";
					//out.println(sql);
					PreparedStatement statement = con.prepareStatement(sql);
					statement.setString(1,"41");
					statement.setString(2,(SHIPORGID.equals("566")?SHIPORGID:"49"));
					...
					while (rs.next())
					{
						...
						if (vline==1)
						{
							...
							out.println("<Td width='10%'>需求日期</Td>");
							out.println("<Td width='11%'>客戶品號</Td>");
							out.println("<Td width='10%'>採購未交數量</Td>");
							out.println("<Td width='10%'>已收未出數量</Td>");
							out.println("<Td width='10%'>已分配量</Td>");
					...
					{
						out.println("<font color='red'>查無採購單資料,請確認料號(22D)是否有下採購單,謝謝!</font>");
					} 
				}
				catch (Exception e)
				{
