<!-- 20141211 by Peggy,EDI�Ȥ᪺RFQ�]���n�D��JCUSTOMER PO LINE NUMBER-->
<!-- 20160321 by Peggy,for cust price issue-->
<!-- 20160805 by Peggy,�}��˫~rfq���L�Ϭd��-->
...
String salesAreaNo=request.getParameter("SAREANO");  // 2007/05/17 �W�[�̷~�ȤH���D�n�a��
...
if (orgID==null || orgID.equals("")) orgID = "41"; // �Y�S���ΨS����,�h�w�]�O�b����~�ȳ�
...
/*
//20110627 liling for excle �W�ǨS���~�Ȱ�
...
// 2007/05/17 �̨ϥΪ̿�ܪ��~�Ȧa�ϥN�����X�~�ȸs�է�X�i�ѬD�諸�Ȥ�
String salesChGroupID = userSalesGroupID; // �w�]�ȬO�ϥΪ̪��w�]�~�ȸs��
...
//modify by Peggy 20140314,�վ�code�g�k,���read db����
...
{
	salesAreaNo ="020"; //�}��L�Ϸ~�ȥi�d�˫~��rfq,add by Peggy 20160805
}
if (salesAreaNo==null || salesAreaNo.equals("") || salesAreaNo.equals("null"))
{
...
function sendToMainWindow(custID,custNo,custName,custActive,custAROverdue,funcname,custMarketGroup,ShipToOrg,shipAddress,shipCountry,billTo,billAddress,billCountry,payTermID,paymentTerm,fobPoint,firmPriceList,shipVia,curr,salesArea,Flag,deliveryTo,deliveryAddress,deliveryCountry,shipToContactid,shipToContact,taxCode,salesGroupId,SupplierFlag)
{
	...
	else
	{
		...
		if (funcname == "D1001")
		{
			...
			{
				window.opener.document.MYFORM.PREORDERTYPE.value="1763"; //1132�q�� add by Peggy 20210528
			}			
...
<%     
    ...
    { 
		...
		cs1.setString(1,PAR_ORG_ID);  // ���~�ȭ�����ParOrgID
		cs1.execute();
		cs1.close();	
		if (searchString!="" && searchString!=null) 
	   	{  	
			...
			if (UserRoles.equals("admin"))  // �]�w�޲z���i�ݱo��Ҧ��Ȥ�
			{
			...
			//modify by Peggy 20110623,��User��ܪ��~�Ȱϰ�a�X�������Ȥ�list
			...
			if (sql.length() >0)
			{
				...
				{	
					if (rowcnt ==0)
					{
						...
						//for (int i=2;i<=colCount;i++) // ����ܲĤ@����, �G for �� 2�}�l
						for (int i=2;i<=colCount-2;i++) // modify by Peggy 20111011,status,market group�����
						{
					...
					String sqla = //" select case when upper(a.site_use_code)='BILL_TO' then 1 else 2 end as segno,"+ //add by Peggy 20120914
					              " select case when upper(a.site_use_code)='BILL_TO' then 1 when upper(a.site_use_code)='SHIP_TO' then 2 else 3 end as segno,"+ //fob ����ship_to���D,�Y�L,�A��deliver_to���D,modify by Peggy 20121026
					...
					if (salesAreaNo.equals("009") || salesAreaNo.equals("008") || salesAreaNo.equals("018"))  //add by Peggy 20160321,for cust price issue //add TSCA ��ship to��price list by Peggy 20221219
					{
					...
					while (rsa.next())
					{
						if (rsa.getString("SITE_USE_CODE").equals("SHIP_TO"))
						{
							...
							if (!salesAreaNo.equals("001"))
							{
								...
								//if (fobPoint == null || fobPoint.equals("") || salesAreaNo.equals("001"))  //�ڬw�Ϩ�ship_to��fob���w�]��,modify by Peggy 20121026
						...
						else if (rsa.getString("SITE_USE_CODE").equals("DELIVER_TO"))  //add by Peggy 20121026
						{
							...
							{					
								if (fobPoint==null || fobPoint.equals(""))  //modify by Peggy 20220808 �ڬw��1156/1142/1141��deliver to ��fob
								{
									fobPoint = rsa.getString("FOB_POINT");
								}
					...
					if (rec_cnt ==0)  //add by Peggy 20120517
					{
						...
						while (rsx.next())
						{
							...
							else if (rsx.getString("SITE_USE_CODE").equals("DELIVER_TO"))  //add by Peggy 20121026
							{
								...
								deliveryAddress = rsa.getString("ADDRESS1");  //add by Peggy 20130218
								deliveryCountry = rsa.getString("COUNTRY");   //add by Peggy 20130218
								if (fobPoint==null || fobPoint.equals("") || (salesAreaNo.equals("001") && (ODRTYPE.equals("1022") || ODRTYPE.equals("1175") || ODRTYPE.equals("1322"))))  //modify by Peggy 20220808 �ڬw��1156/1142/1141��deliver to ��fob
								{
									fobPoint = rsx.getString("FOB_POINT");
								}
								taxCode = rsa.getString("TAX_CODE");          //add by Peggy 20130411
