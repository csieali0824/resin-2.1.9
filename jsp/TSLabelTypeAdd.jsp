...
<body>
<FORM NAME="MYFORMD" ACTION="../jsp/TSLabelTypeAdd.jsp" METHOD="POST"> 
<%
...
String LABEL_TYPE_SIZE = request.getParameter("LABEL_TYPE_SIZE");
if (LABEL_TYPE_SIZE==null || LABEL_TYPE_SIZE.equals("--")) LABEL_TYPE_SIZE="";
String LABEL_SIZE_DESC = request.getParameter("LABEL_SIZE_DESC");
if (LABEL_SIZE_DESC==null) LABEL_SIZE_DESC="";
...
{  
	if (STATUS.equals("UPD"))
	{
		if (ACODE.equals(""))
		{
			sql = " select a.LABEL_TYPE_NAME, a.DESCRIPTION,a.PRINT_NUM,a.LABEL_TYPE_SIZE,"+
			...
			if (rs.next())
			{
				...
				LABEL_TYPE_SIZE=rs.getString("LABEL_TYPE_SIZE");
				if (LABEL_TYPE_SIZE==null) LABEL_TYPE_SIZE="";
	...
	if (ACODE.equals("SAVE"))
	{
		try
		{
			if (STATUS.equals("UPD"))
			{
				sql= " update oraddman.ts_label_types "+
				     " set LABEL_TYPE_NAME=?"+
					 ",DESCRIPTION=?"+
					 ",PRINT_NUM=?"+
					 ",LABEL_TYPE_SIZE=?"+
				...
				st1.setString(4,LABEL_TYPE_SIZE);
				...
				st1.setString(8,LABEL_SIZE_DESC);  //add by Peggy 20211102
			...
			{
				sql = " select lpad(nvl(max(LABEL_TYPE_CODE),0)+1,3,'0') from oraddman.ts_label_types a ";
				//out.println(sql);
				PreparedStatement statement = con.prepareStatement(sql);
				ResultSet rs=statement.executeQuery();
				if (rs.next())
				{
					LABEL_TYPE_CODE = rs.getString(1);
				}
				rs.close();
				statement.close();
				
				sql = " insert into oraddman.ts_label_types"+
					  " (LABEL_TYPE_CODE"+
					  ",LABEL_TYPE_NAME"+
					  ",DESCRIPTION"+
					  ",PRINT_NUM"+
					  ...
					  ",last_update_date"+
					  ",last_updated_by"+
					  ",label_size_desc"+ //add by Peggy 20211102
					  ")"+
					  " values ("+
					  " ?"+
					  ",?"+
				...
				st1.setString(5,LABEL_TYPE_SIZE);
				...
				st1.setString(10,LABEL_SIZE_DESC);  //add by Peggy 20211102
				st1.executeUpdate();
				st1.close();
				s_res ="2";
			}
	...
	<tr>
		<td>
			<table width="100%"  align="CENTER" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#99CC99">
				...
				<tr>
					<td width="35%" height="30" bgcolor="#C9E2D0" nowrap><span style="font-size:12px;color:#006666">標籤尺吋說明</span>：</td>
					<td nowrap><INPUT TYPE="TEXT" name="LABEL_SIZE_DESC" value="<%=LABEL_SIZE_DESC%>" size="30" style="font-family: Tahoma,Georgia;"></td>
				</tr>	
				<tr>
					<td width="35%" height="30" bgcolor="#C9E2D0" nowrap><span style="font-size:12px;color:#006666">標籤容器</span>：</td>
					<td nowrap><select NAME="LABEL_TYPE_SIZE" style="font-family:Tahoma,Georgia; font-size: 12px ">
					<OPTION VALUE=-- <%if (LABEL_TYPE_SIZE.equals("")) out.println("selected");%>>--</OPTION>
					<OPTION VALUE="<%=v_reel%>" <%if (LABEL_TYPE_SIZE.equals(v_reel)) out.println("selected");%>><%=v_reel%></OPTION>
					<OPTION VALUE="<%=v_box%>" <%if (LABEL_TYPE_SIZE.equals(v_box)) out.println("selected");%>><%=v_box%></OPTION>
					<OPTION VALUE="<%=v_carton%>" <%if (LABEL_TYPE_SIZE.equals(v_carton)) out.println("selected");%>><%=v_carton%></OPTION>
					</select></td>					
				<tr>
	...
	<tr>
		<td>
			<table width="100%" border="0">
  				<tr align=center>
    				<td width="16%"> <input type="button"  name="btnSubmit" onClick='setSubmit("../jsp/TSLabelTypeAdd.jsp?STATUS=<%=STATUS%>&ACODE=SAVE")' value="存檔" style="font-family:'Tahoma,Georgia';font-size:12px">&nbsp;&nbsp;&nbsp;
