    ...
    //out.println("setResult="+setResult);
	 ...
	 {  
	    ...
	    { //out.println("0");
			...
			exSampleQty = rsInspNo.getString("SAMPLE_QTY");
			exProdName = rsInspNo.getString("PROD_NAME");
			exInspectQty = rsInspNo.getString("INSPECT_QTY");
			
			if (exProdName==null || exProdName.equals("") ||exProdName.equals("null")) exProdName = "";
			if (exSampleQty ==null || exSampleQty.equals("null")) exSampleQty = "0";
			if (exInspectQty ==null || exInspectQty.equals("null")) exInspectQty = "0";
...
<table cellSpacing="1" bordercolordark="#D0C8C1" cellPadding="1" width="97%" align="center" borderColorLight="#ffffff" border="0">
  ...
  /*
	...
	*/	
        ...
        ResultSet rsCNT=stateCNT.executeQuery("select count(DISTINCT ITEM_ID),count(DISTINCT EXAMINE_ITEM_ID) from ORADDMAN.TSCIQC_EXAMINE where CLASS_ID='"+classID+"' and IMATCODE ='"+iMatCode+"' ");	
       ...
       // 由檢驗項目表自動帶出內容
        ...
        String sqlInsp = "select * from ORADDMAN.TSCIQC_ITEM where CLASS_ID ='"+classID+"' and IMATCODE ='"+iMatCode+"' order by ITEM_ID ";
		//out.println("sqlInsp ="+sqlInsp);
	  ...
	  <tr bgcolor="#DDDBAA">
       ...
       <td nowrap>	
		<%
			...
			String sqlExam = "select * from ORADDMAN.TSCIQC_EXAMINE where CLASS_ID ='"+classID+"' and ITEM_ID = '"+rsInsp.getString("ITEM_ID")+"' and IMATCODE ='"+iMatCode+"' order by EXAMINE_ITEM_ID ";
			//out.println("sqlExam ="+sqlExam);
			...
			while (rsExam.next())
			{
			  ...
			  String examValue = "";
			  String sqlVal = "select EXAMINE_VALUE from ORADDMAN.TSCIQC_LOTEXAMINE_DETAIL where INSPLOT_NO = '"+inspLotNo+"' and CLASS_ID ='"+classID+"' and ITEM_ID = '"+rsInsp.getString("ITEM_ID")+"' and EXAMINE_ITEM_ID='"+rsExam.getString("EXAMINE_ITEM_ID")+"' and IMATCODE ='"+iMatCode+"' ";
			...
			/*
			  ...
			  arrIQC2DIMaterialBean.setArray2DString(wafer);
			*/
			if (rsInsp.getString("ITEM_ID").equals("01")) // 外觀
			{
			  switch (rsExam.getString("EXAMINE_ITEM_ID").charAt(1)) { // 利用Switch Case 條件決定要用那種內容檢驗的表單Include
			    case '1':   // 間接原物料外觀		   
	            {   
				  if (imSurface==null || imSurface.equals("")) imSurface=examValue;
				  ...
				  out.println("<td width='20%' nowrap>");
	              try
                  {       
                              Statement statement=con.createStatement();
                              ResultSet rs=statement.executeQuery("select RULE_ID as IMSURFACE,RULE_NAME from ORADDMAN.TSCIQC_RULE order by RULE_ID");
                              checkBoxBean.setRs(rs);
                              if (imSurface != null)
   	                          checkBoxBean.setChecked(imSurface);
	                          checkBoxBean.setFieldName("IMSURFACE");	   
	              ...
	              out.println("</td>");				  
			      wafer[itemNo][examNo]=imSurface;				  		   
			      arrIQC2DIMaterialBean.setArray2DString(wafer);
				  break;
			    }
				case '2':   //  外觀描述
				{
				     out.print("<td nowrap><input type='text' name='"+rsExam.getString("EXAMINE_ITEM_CODE")+"' value='"+chSurDsc+"' size=15> "+"</td>");
					 wafer[itemNo][examNo]=chSurDsc;				  		   
			         arrIQC2DIMaterialBean.setArray2DString(wafer);
					 break;
				}					   		  
			    default:
		 ...
		 else if (rsInsp.getString("ITEM_ID").equals("02"))   
		      {
			    switch (rsExam.getString("EXAMINE_ITEM_ID").charAt(1)) { // 利用Switch Case 條件決定要用那種內容檢驗的表單Include
			      case '1':   // 間接原物料重量			   
	              {   
				    if (imWeight==null) imWeight=examValue;
                    ...
                    {       
                              Statement statement=con.createStatement();
                              ResultSet rs=statement.executeQuery("select RULE_ID as IMWEIGHT,RULE_NAME from ORADDMAN.TSCIQC_RULE order by RULE_ID");
                              checkBoxBean.setRs(rs);
                              if (imWeight != null)
   	                          checkBoxBean.setChecked(imWeight);
	                          checkBoxBean.setFieldName("IMWEIGHT");	   
	                          checkBoxBean.setColumn(3); //傳參數給bean以回傳checkBox的列數
                              out.println(checkBoxBean.getRsString());
	                          statement.close();
                              rs.close();       
	                ...
	                out.println("</td>");
			        wafer[itemNo][examNo]=imWeight;				   
			        arrIQC2DIMaterialBean.setArray2DString(wafer);
				    break;
			      }
				  case '2':   //  重量描述
				  {
				     out.print("<td nowrap><input type='text' name='"+rsExam.getString("EXAMINE_ITEM_CODE")+"' value='"+chLiqDsc+"' size=15> "+"</td>");
					 wafer[itemNo][examNo]=chLiqDsc;				  		   
			         arrIQC2DIMaterialBean.setArray2DString(wafer);
					 break;
				  }	
			      default:
			      {
			  ...
			  else if (rsInsp.getString("ITEM_ID").equals("03"))
			       {
			             switch (rsExam.getString("EXAMINE_ITEM_ID").charAt(1)) { // 利用Switch Case 條件決定要用那種內容檢驗的表單Include
			               case '1':   // 間接原物料			   
	                       {   
						     if (imSizeDim==null || imSizeDim.equals("")) imSizeDim=examValue;
							 ...
							 out.println("<td width='15%' nowrap>");
	                         try
                             {       
                              Statement statement=con.createStatement();
                              ResultSet rs=statement.executeQuery("select RULE_ID as IMSIZEDIM,RULE_NAME from ORADDMAN.TSCIQC_RULE order by RULE_ID");
                              checkBoxBean.setRs(rs);
                              if (imSizeDim != null)
   	                          checkBoxBean.setChecked(imSizeDim);
	                          checkBoxBean.setFieldName("IMSIZEDIM");	   
	                          checkBoxBean.setColumn(3); //傳參數給bean以回傳checkBox的列數
                              out.println(checkBoxBean.getRsString());
	                          statement.close();
                              rs.close();       
	                         ...
	                         out.println("</td>");
			                 wafer[itemNo][examNo]=imSizeDim;				   
			                 arrIQC2DIMaterialBean.setArray2DString(wafer);
				             break;
			               }
						   case '2':   //  IPA描述
				           {
				             out.print("<td nowrap><input type='text' name='"+rsExam.getString("EXAMINE_ITEM_CODE")+"' value='"+chIPADsc+"' size=15> "+"</td>");
					         wafer[itemNo][examNo]=chIPADsc;				  		   
			                 arrIQC2DIMaterialBean.setArray2DString(wafer);
					         break;
				           }							   
			               default:
...
<table cellSpacing="1" bordercolordark="#D0C8C1" cellPadding="1" width="97%" align="center" borderColorLight="#ffffff" border="0">
  <tr bgcolor="#DDDBAA"> 
      <td width="14%">
	    <div align="center"><input type="button" name="EXRESULT" value="規格圖面上載" onClick='subSpecFileUpload("<%=inspLotNo%>","<%=classID%>")'><BR>		   
...
<%  
  try
  {
	...
	{  
	    String aIQCIMaterialCode[][]=arrIQC2DIMaterialBean.getArray2DContent();      // <!--取出夾入檢驗數據陣列內容-->
	
	    String sqlInspItem = "select * from ORADDMAN.TSCIQC_ITEM where CLASS_ID ='"+classID+"' and IMATCODE ='"+iMatCode+"' order by ITEM_ID ";
	    ...
	    {//out.println("0");
	        ...
	        String sqlExamItem = "select * from ORADDMAN.TSCIQC_EXAMINE where CLASS_ID ='"+classID+"' and ITEM_ID = '"+rsInspItem.getString("ITEM_ID")+"' and IMATCODE ='"+iMatCode+"' order by EXAMINE_ITEM_ID ";
			...
			while (rsExamItem.next())
			{
			 ...
			 {			 
			  ...
			  String sqlExist = "select EXAMINE_VALUE from ORADDMAN.TSCIQC_LOTEXAMINE_DETAIL where INSPLOT_NO = '"+inspLotNo+"' and CLASS_ID ='"+classID+"' and ITEM_ID = '"+rsInspItem.getString("ITEM_ID")+"' and EXAMINE_ITEM_ID='"+rsExamItem.getString("EXAMINE_ITEM_ID")+"' and IMATCODE ='"+iMatCode+"' ";
		      Statement stateExist=con.createStatement();
              ResultSet rsExist=stateExist.executeQuery(sqlExist);
			  if (rsExist.next())
			  {  // 已存在,先刪除,後面再新增
			       String sqlDel= "delete from ORADDMAN.TSCIQC_LOTEXAMINE_DETAIL where INSPLOT_NO = '"+inspLotNo+"' and CLASS_ID ='"+classID+"' and ITEM_ID='"+rsInspItem.getString("ITEM_ID")+"' and EXAMINE_ITEM_ID = '"+rsExamItem.getString("EXAMINE_ITEM_ID")+"' and IMATCODE ='"+iMatCode+"' ";
	         ...
	         String sqlDtl="insert into ORADDMAN.TSCIQC_LOTEXAMINE_DETAIL(INSPLOT_NO, LINE_NO, CLASS_ID, ITEM_ID, EXAMINE_ITEM_ID, "+
	                                                                     "EXAMINE_VALUE, EXAMINE_UNIT, DEFECT_REASON, LOT_RESULTS, "+ 
					           										     "DEFECT_REMARK, CREATED_BY, LAST_UPDATE_DATE, LAST_UPDATED_BY, IMATCODE) "+		  
                           " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	         PreparedStatement pstmt=con.prepareStatement(sqlDtl);      
							   ...
							   pstmt.setString(6,aIQCIMaterialCode[i][j]);   //out.println("aIQCWaferDiceCode[i][j]="+aIQCWaferDiceCode[i][j]);              //  檢驗測試項目值
							   ...
							   pstmt.setString(14,iMatCode); // 原料種類	                          
                               pstmt.executeUpdate(); 
                               pstmt.close();  
		...
		stateInsp.close();
		 ...
		 try
		 {		
		      String sqlExist = "select * from ORADDMAN.TSCIQC_LOTINSPECT_HEADER where INSPLOT_NO = '"+inspLotNo+"' and IQC_CLASS_CODE ='"+classID+"' and IMATCODE ='"+iMatCode+"' ";
		      Statement stateExist=con.createStatement();
			  ...
			  {  // 已存在,作更新
			       ...
			       String sqlUpdate= "update ORADDMAN.TSCIQC_LOTINSPECT_HEADER set SAMPLE_QTY=?, INSPECT_QTY=?, PROD_NAME=? where INSPLOT_NO = '"+inspLotNo+"' and IMATCODE ='"+iMatCode+"' ";
