...
{ 
	...
	String sql = " select DISTINCT SML.INVENTORY_ITEM_ID as \"半成品料號識別碼\", 'KPC' as \"單位\",  "+
		         "  OOH.ORDER_NUMBER MO單號, MSI.SEGMENT1 as \"製成品品號\", REPLACE(MSI.DESCRIPTION,'\''',' ') as \"品號規格說明\",  "+
    ...
    { 
		...
		semiItemID = rs.getString(1);	 // 取得前段半成品料號ID作為查MO單依據
		woQty = rs.getString(6);  //把未出貨數量給此次新增數量
		...
		invItem=rs.getString(4); //rs.getString("INV_ITEM");
		itemDesc=rs.getString(5);//rs.getString("DESCRIPTION");			 
		...
		customerId=rs.getString(13); //rs.getString("SOLD_TO_ORG_ID");	
		orderPartnoQty=rs.getString(14); //add by Peggy 20191007
		wipPartnoQty=rs.getString(15); //add by Peggy 20191007
		...
		String sqlWoLst = " select DISTINCT b.EXTEND_NO||'(數量='||b.EXTENDED_QTY||',單位='||WO_UOM||',單耗量='||a.WO_UNIT_QTY||')', (b.EXTENDED_QTY*a.WO_UNIT_QTY) as ACCWO_QTY "+
						  ...
						  " and a.ORDER_HEADER_ID = "+rs.getString(10)+" "+
						  " and a.ORDER_LINE_ID= '"+rs.getString(11)+"' "+
		...
		oeLineQty=rs.getString(6); //rs.getString("UNSHIP_QTY");
