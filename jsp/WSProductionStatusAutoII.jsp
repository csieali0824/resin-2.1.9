...
<FORM ACTION="../jsp/WSProductionStatusAutoII.jsp" METHOD="post" NAME="MYFORM" >
       ...
       %--> 
	    <% 		 		 		 
         ...
         {   
		  ...
		  out.println("<select NAME='REGION' onChange='setSubmit("+'"'+"../jsp/WSProductionStatusAutoII.jsp"+'"'+")'>");
      ...
      <td><font color="#000000" face="Arial Black"><strong>Model</strong></font>
	   <% 
         ...
         {   
		  ...
		  //sWhereRCenter = "where REPCENTERNO = lpad(to_char(REPCENTERNOR),3,'0') ";
		  sWhereModel = "where PROJECTCODE IS NOT NULL and SALESCODE IS NOT NULL ";		 
    ...
    <% // ***  Main Loop  Start   ***//
       //ResultSet RpRepair = StatementRpRepair.executeQuery(); 
		...
		try
		{
			 ...
			 // 2004/06/15 Update//  String sql = "select DISTINCT m.SALESCODE , m.PROJECTCODE, f.FMQTY, f.FMCOUN from WSADMIN.PSALES_FORE_MONTH f , WSADMIN.PIMASTER m, WSADMIN.PRODMODEL p ";
             // 2004/06/15 Update//  sWhere = "where m.PROJECTCODE = f.FMPRJCD(+) and m.PROJECTCODE =  p.MPROJ  and f.FMYEAR = '"+YearFr+"'  and f.FMMONTH=  '"+MonthFr+"'  "+
		    // 2004/06/15 Update//   "and to_number(f.FMVER) = (select max(to_number(q.FMVER)) from PSALES_FORE_MONTH q where q.FMPRJCD = f.FMPRJCD) ";     // 2003/12/08
		 
			  String sql = "select DISTINCT SALESCODE, PROJECTCODE from PIMASTER ,PRODMODEL  ";   
		          sWhere = "where trim(PROJECTCODE) =  trim(MPROJ) ";
		                       
		
		//if (locale==null || locale.equals("--")) {   sWhere = sWhere + "and to_char(f.FMCOUN) !=  'ALL'  "; }   // 2004/06/15 Update 
		//else { sWhere = sWhere + "and to_char(f.FMCOUN) =  '"+locale+"'  ";}                                    // 2004/06/15 Update 
        if (locale==null || locale.equals("--")) {   sWhere = sWhere + " "; }
		else { sWhere = sWhere + "and MCOUNTRY =  '"+locale+"'  ";}   
/*   // 2004/06/15 Update 
...
*/  // 2004/06/15 Update
		if (model==null || model.equals("--")) { sWhere = sWhere +  " "; }
		else { sWhere = sWhere +  "and SALESCODE =  '"+model+"' "; }
		
		String sOrder = "order by PROJECTCODE ";
		...
		//sql = "select SALESCODE, PROJECTCODE from PIMASTER , PRODMODEL where PROJECTCODE = MPROJ order by PROJECTCODE ";
...
// 加入分頁 - 迄 /		
		...
		{	
         try
         {
          Statement stateAQTY=con.createStatement();   
          String sqlAQTY = "select FMQTY, FMCOUN from PSALES_FORE_MONTH f ";
          String sWhereAQTY = "where f.FMPRJCD='"+rs.getString("PROJECTCODE")+"' "+
                             "and to_number(f.FMVER) = (select max(to_number(q.FMVER)) from PSALES_FORE_MONTH q where q.FMPRJCD = f.FMPRJCD) ";
          if (YearFr==null) { sWhereAQTY = sWhereAQTY + "and f.FMYEAR = '"+dateBean.getYearString()+"'  ";  } 
          else { sWhereAQTY = sWhereAQTY + "and f.FMYEAR = '"+YearFr+"' "; } 
          if (MonthFr==null) { sWhereAQTY = sWhereAQTY + "and f.FMMONTH = '"+dateBean.getMonthString()+"'  ";  } 
          else { sWhereAQTY = sWhereAQTY + "and f.FMMONTH = '"+MonthFr+"' "; }         
          if (locale==null || locale.equals("--")) {   sWhereAQTY = sWhereAQTY + "and to_char(f.FMCOUN) !=  'ALL'  "; }   // 2004/06/15 Update 
		  else { sWhereAQTY = sWhereAQTY + "and to_char(f.FMCOUN) =  '"+locale+"'  ";} 
          if (compNo==null || compNo.equals("--")) {   sWhereAQTY = sWhereAQTY + "and f.FMCOMP !=  'ALL'  "; }
		  else { sWhereAQTY = sWhereAQTY + "and f.FMCOMP =  '"+compNo+"'  "; }
          if (regionNo==null || regionNo.equals("--")) { sWhereAQTY = sWhereAQTY + "and to_char(f.FMREG) != 'ALL'  "; }
		  else { sWhereAQTY = sWhereAQTY +  "and to_char(lpad(f.FMREG,2,0))= '"+regionNo+"'  "; }  
          sqlAQTY = sqlAQTY + sWhereAQTY;  
          //out.println(sqlAQTY);             
          ResultSet rsFMQTY=stateAQTY.executeQuery(sqlAQTY); 
          if (rsFMQTY.next() ) 
          {
            FMQTY = rsFMQTY.getInt("FMQTY"); 
            FMCOUN = rsFMQTY.getString("FMCOUN");   
          }               
          rsFMQTY.close();
          stateAQTY.close();             
         } //end of try
         catch (Exception e)
         {
		  ...
		  {
		  // sumPlan = sumPlan + rs.getInt("FMQTY"); }  // 改取另外抓的數量 2004/06/14//
           sumPlan = sumPlan + FMQTY; }     
    ...
    <tr bordercolor="#000000"> 	   
	  ...
	  <td><div align="center"><font color="#000000" size="2">
	   <%
				...
				try
		        {    out.println(FMCOUN);
				    // String sqlCountry = "select LOCALE_ENG_NAME from WSADMIN.WSLOCALE  where LOCALE= '"+rs.getString("FMCOUN")+"' "; // 2004/06/14 //
                     String sqlCountry = "select LOCALE_ENG_NAME from WSADMIN.WSLOCALE  where LOCALE= '"+FMCOUN+"' "; 
      ...
      <td rowspan="<%=1%>"><div align="center"><font color="#000000" size="2">
	        <% 
			   //   out.println(rs.getInt("FMQTY"));  // 2004/06/14
			   //  sumFmQty = sumFmQty + rs.getInt("FMQTY");  // 2004/06/14
                  out.println(FMQTY);
				  sumFmQty = sumFmQty + FMQTY;
      ...
      <td rowspan="<%=rowSpanCnt%>"><div align="center"><font color="#000000" size="2"> 
          <% //Balance Plan Qty - Finish Qty//	    
			...
			//balanceQty = (rs.getInt("FMQTY") - finishQty)*1;   // 2004/06/14
			balanceQty = (FMQTY - finishQty)*1;
