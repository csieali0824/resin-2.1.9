...
{ 	
    ...
    sql = "select a.*"+
		  //SG PRICE拿掉/1.05公式,ADD BY PEGGY 20210114		  
		  ...
		  ",fair.cust_partno fairchild_cpn"+  //add by Peggy 20181221
		  ...
		  ",(select distinct pl_category from oraddman.ts_pl_category ttpc where ttpc.TSC_PROD_GROUP=a.TSC_PROD_GROUP and NVL(ttpc.TSC_PROD_CATEGORY,a.TSC_PROD_CATEGORY)=a.TSC_PROD_CATEGORY and ttpc.TSC_FAMILY=a.TSC_FAMILY and NVL(ttpc.TSC_PROD_FAMILY,nvl(a.TSC_PROD_FAMILY,'XXX'))=nvl(a.TSC_PROD_FAMILY,'XXX')) pl_category"+ //add by Peggy 20221108
		  ...
		  "       AND msii.ORGANIZATION_ID=43"+    //add by Peggy 20190215
          "       AND LENGTH(msii.SEGMENT1)>=22"+   //add by Peggy 20190215
		  "       AND msii.ITEM_TYPE='FG'"+ //add by Peggy 20190802
          "       AND msii.INVENTORY_ITEM_STATUS_CODE <>'Inactive'"+  //add by Peggy 20190215
		  ...
		  "       AND NVL(msi.CUSTOMER_ORDER_ENABLED_FLAG,'N')='Y'"+ //add by Peggy 20181023
		  "       AND NVL(msi.INTERNAL_ORDER_ENABLED_FLAG,'N')='Y'"+ //add by Peggy 20181023
		  "       and tsc_item_pcn_flag(43,msi.inventory_item_id,trunc(sysdate))='N'"+ //add by Peggy 20230214
		  "       and (((length(msi.segment1)=22 and substr(msi.segment1,21,1)='0') or (length(msi.segment1)=30 and substr(msi.segment1,21,1)='0' and substr(msi.segment1,29,2)='00'))"+  //30D料號末兩碼=00,add by Peggy 20190918
          "       and substr(msi.segment1,22,1) in ('0','A','B','C','D','F','G','H','I','J','K','L','N','O','P','Q','R','S','T','V','W','X','Y','Z'))"+ //add "N" for -ON型號,add by Peggy 20181101
		  "       and instr(msi.description,'/')<=0"+
		  ...
		  "       ,oraddman.tsc_sales_bottom_price tsbp"+ //add by Peggy 20181112
		  ...
		  "       ,(SELECT x.PCN_NUMBER,x.TSC_PART_NO,case when substr(y.segment1,21,1) not in ('0') or substr(y.segment1,22,1) not in ('0','E','M','N','U') THEN '' ELSE REPLACE_PART_NO END REPLACE_PART_NO"+
          "         FROM (SELECT a.*,ROW_NUMBER () OVER (PARTITION BY tsc_part_no ORDER BY pcn_number) row_seq FROM (SELECT DISTINCT pcn_number, tsc_part_no, replace_part_no FROM oraddman.tsqra_pcn_item_detail a WHERE source_type = '2' AND replace_part_no IS NOT NULL"+
          "                 AND TRIM (UPPER (replace_part_no)) NOT IN ('NO CHANGE', 'NONE')) a) x,(select a.* from (select description,segment1,row_number() over (partition by description order by creation_date desc) row_seq from inv.mtl_system_items_b where organization_id=43 and inventory_item_status_code<>'Inactive' ) a where row_seq=1) y "+
          "                 WHERE x.row_seq = 1 and x.REPLACE_PART_NO=y.description(+)) new_pn"+ //modify by Peggy 20200723
		  ...
		  "       AND a.description=new_pn.TSC_PART_NO(+)"+  //add by Peggy 20200715
		  "       AND a.part_id=tsbpt.part_id(+)"+ //add by Peggy 20210729
		  "       AND a.item_desc1=fair.tsc_partno(+)"+ //add by Peggy 20181221
...
//	out.println(sql);
	...
	{ 	
		...
		if (rs.getString("PACKAGE_CODE") ==null || rs.getString("part_spec")==null || rs.getInt("ITEM_CNT")!=1) continue;  //20230720 by Peggy 取消沒提供salse bottom 價格的條件 from Mabel 20230717 mail
		if (rs.getString("TSC_PROD_GROUP").equals("PMD") && rs.getString("SEGMENT1").length()==30 && rs.getString("SEGMENT1").substring(21,22).equals("V")) continue;  //20230720 by Peggy 在PROD GROUP下PMD的30D第22碼若為V，也屬於HsNr from Mabel 20230717 mail
		if (reccnt==0)
		{
			col=0;row=0;
			...
			ws.addCell(new jxl.write.Label(col, row, "Sales Website Status" , ACenterBLB));  //add by Peggy 20200421
			ws.setColumnView(col,12);	
			col++;	
			...
			//Inner Box Per Carton
			ws.addCell(new jxl.write.Label(col, row, "Inner Box per Carton" , ACenterBLB));
			ws.setColumnView(col,12);	
			col++;	
		...
		*/
		...
		//col++;		
		//if (rs.getString("part_id").equals("TESDH5V0U04P2Q1") ||
