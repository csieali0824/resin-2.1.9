...
<FORM ACTION="../jsp/TSCInvI7SubInventoryNBReport.jsp" METHOD="post" NAME="MYFORM">
...
<strong>I7 Inventory Non-Balance Report(第<%=currentWeek%>週 : <%=strFirstDayWeek%> ~ <%=strLastDayWeek%>)</strong></font>
<BR>
  <!--%/20040109/將Excel Veiw 夾在檔頭%-->
<%
...
if ((YearFr==null || YearFr.equals("")) && (YearTo==null || YearTo.equals("")))
{
   ...
   sWhere =  "WHERE T.INVENTORY_ITEM_ID = M.INVENTORY_ITEM_ID AND T.ORGANIZATION_ID = M.ORGANIZATION_ID "+
             "AND T.ORGANIZATION_ID=143 AND TRANSACTION_TYPE_ID != 52 "+	 
    ...
    <tr bgcolor="<%=colorStr%>"> 
      ...
      <td width="59%" nowrap><font size="-1">
			  ...
			  //out.println("wipEntID="+wipEntID);out.println("rsTC.getString(INVENTORY_ITEM_ID)="+rsTC.getString("INVENTORY_ITEM_ID"));
			       ...
			       {
			          out.print("<tr><td nowrap>"); out.println("<font size='-1'>"+rsTC.getString("SEGMENT1")+"</font>"); %><a href="../jsp/TSCInvI7SubInventoryNBReport.jsp?SPANNING=TRUE&WIPENTID=<%=rsTC.getString("INVENTORY_ITEM_ID")%>&DATESETBEGIN=<%=dateSetBegin%>&DATESETEND=<%=dateSetEnd%>"><img src="../image/PLUS.gif" width="14" height="15" border="0"></a><% out.println("</td></tr>");
			              ...
			              {			    
				             ...
				             { //out.println("wipEntID="+wipEntID);out.println("rsTC.getString(INVENTORY_ITEM_ID)="+rsTC.getString("INVENTORY_ITEM_ID"));
				               out.print("<tr><td nowrap>"); out.println("<font size='-1'>"+rsTC.getString("SEGMENT1")+"</font>");  %><a href="../jsp/TSCInvI7SubInventoryNBReport.jsp?SPANNING=FALSE&WIPENTID=<%=rsTC.getString("INVENTORY_ITEM_ID")%>&DATESETBEGIN=<%=dateSetBegin%>&DATESETEND=<%=dateSetEnd%>"><img src="../image/MINUS.gif" width="14" height="15" border="0"></a><% out.println("</td></tr>");
...
%>
    <tr bgcolor="#003366"> 
      <td height="23" colspan="10" ><font color="#FFFFFF" size="-1">本週I7庫存異常筆數</font> 
        <% 
	      if (CaseCount==0) 
...
<font color="#CC0066" size="2">說明 : <BR>1.本表為前一週I7庫存平衡狀況表,資料抓取區間如表頭所示.</font><BR>
