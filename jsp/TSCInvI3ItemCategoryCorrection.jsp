...
<%@ page import="bean.QryAllChkBoxEditBean,bean.ComboBoxAllBean,bean.DateBean,bean.WorkingDateBean,bean.ArrayComboBoxBean"%>
...
<FORM ACTION="../jsp/TSCInvI3ItemCategoryCorrection.jsp" METHOD="post" NAME="MYFORM">
...
<strong>I3 Organization Item Category Cost Account Setting Error Correction</strong></font>
<BR>
...
{
   sSql = "select C.ORGANIZATION_ID, I.INVENTORY_ITEM_ID, I.SEGMENT1 || '('||I.INVENTORY_ITEM_ID||')' as SEGMENT1, I.CREATED_BY, TO_CHAR(I.CREATION_DATE,'YYYY-MM-DD HH24:MI:SS') as CREATION_DATE, "+
   ...
   sSqlCNT = "select count(I.SEGMENT1) as CaseCount from MTL_ITEM_CATEGORIES_V C, MTL_SYSTEM_ITEMS_B I, MTL_PARAMETERS P, FND_USER U ";
   sWhere =  "where C.ORGANIZATION_ID <> 43 AND CATEGORY_SET_ID = 4 AND C.ORGANIZATION_ID = P.ORGANIZATION_ID "+
			 ...
			 "AND to_char(C.LAST_UPDATE_DATE,'YYYYMMDD') between '20050601' and '20050631' ";
...
{ 
  ...
  //out.println(sSql);    
   ...
   String sqlOrgCnt = "select count(I.SEGMENT1) as CaseCountORG from MTL_ITEM_CATEGORIES_V C, MTL_SYSTEM_ITEMS_B I, MTL_PARAMETERS P, FND_USER U  ";
...
%>  
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#FFFFFF">
	...
	<td width="68%" bordercolor="#CCCCFF">
		    <INPUT TYPE="button" align="middle"  value='查詢' onClick='setSubmit("../jsp/TSCInvI3ItemCategoryCorrection.jsp")' >   
	</td>
   </tr>
  </table>  
  ...
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#999999" bordercolordark="#FFFFFF">
    <tr bgcolor="#003366"> 
	  ...
	  <td width="5%"  height="19" nowrap><div align="center"><font color="#FFFFFF" size="2"><strong>Fix</strong></font></div></td>   
      <td width="8%" height="22" nowrap><div align="center"><font color="#FFFFFF" size="-1">Org. Information</font></div></td>          
      <td width="11%" nowrap><div align="center"><font color="#FFFFFF" size="-1">Item</font></div></td>
      <td width="12%" nowrap><div align="center"><font color="#FFFFFF" size="-1">Error Category</font></div></td>      
      <td width="14%" nowrap><div align="center"><font color="#FFFFFF" size="-1">Correct Category</font></div></td>
      <td width="11%" nowrap><div align="center"><font color="#FFFFFF" size="-1">Last Upate User</font></div></td>      
      <td width="16%" nowrap><div align="center"><font color="#FFFFFF" size="-1">Inform User Mail</font></div></td>
	  <td width="19%" nowrap><div align="center"><font color="#FFFFFF" size="-1">Last Update Date </font></div></td>	         
    </tr>
	     ...
	     <%//=rsTC.getString("CREATION_DATE")
		   ...
		   /*
                ...
                {   
		          ...
		          sSqlPerson = "select M.CATEGORY_ID, M.CATEGORY_ID||'('||C.SEGMENT2|| ',' || C.SEGMENT1 ||')' as COR_CATEGORY FROM CST_COST_GROUP_ASSIGNMENTS G, MTL_FISCAL_CAT_ACCOUNTS M, MTL_CATEGORIES C ";		  		  
		          sWherePerson  = "where G.COST_GROUP_ID=M.COST_GROUP_ID AND M.CATEGORY_ID = C.CATEGORY_ID "+
		                          "AND G.ORGANIZATION_ID = '"+rsTC.getString("ORGANIZATION_ID")+"' ";  
		          String sOrderPerson = "order by M.CATEGORY_ID ";
		     		              
		          sSqlPerson = sSqlPerson+sWherePerson+sOrderPerson;		         
		         
		          //out.println(sSqlPerson);
                  Statement statement=con.createStatement();
                  ResultSet rs=statement.executeQuery(sSqlPerson);
                  out.println("<select NAME='CORCATEGORY' onChange='setSubmit("+'"'+"../jsp/TSCInvItemCategoryCorrection.jsp"+'"'+")'>");
                  ...
                  {            
				   ...
				   if (corCategory==null)
				   {
				    out.println("<OPTION VALUE='"+categoryId+"' SELECTED>"+categoryCode);  
                   }      
                   else // if (s1.equals(corCategory)) 
                   {
...
<BR>
<div align="left"><input name="submit2" type="submit" value="BATCH CORRECT CATEGORY" onClick='return setSubmit2("../jsp/TSCInvI3ItemCategoryCorrectionCommit.jsp")'></div>
</FORM>
