...
{
   warray=new Array(document.MYFORM.RD_CODE.value,document.MYFORM.RDDPT_NO.value,document.MYFORM.RDDPT.value,document.MYFORM.RDDPR_DESC.value);   
   for (i=0;i<4;i++)
   {     
      if (warray[i]=="" || warray[i]==null || warray[i]=="--" )
     { 
      alert("Before you want to add , please do not let the any fields of Research Unit data be Null !!");   
      return(false);
      } 
...
{    
   ...
   {     
      for (j=1;j<dim2;j++)
	  {
		 ...
		 { 
           alert("Before you want to save , please do not let the any filed of Research Unit detail be Null !!");   
           return(false);
		 }  
	  } //enf for of jj             
   } //end of for null check
...
<title>WINS System-產品研發單位新增</title>
</head>
<body>
<FORM ACTION="WSMRDPTInput.jsp" METHOD="post" NAME="MYFORM">
  ...
  <strong> 產品編碼系統-產品研發單位新增 </strong></font> &nbsp; &nbsp; &nbsp; &nbsp;<BR>
  <A HREF="../WinsMainMenu.jsp">首頁</A> &nbsp;&nbsp;<A HREF="../jsp/WSModelEncodingSub.jsp">產品編碼管理作業</A> 
  &nbsp;&nbsp;<A HREF="../jsp/WSMRDPTInquiry.jsp?ENTRY=TRUE">產品研發單位查詢</A> 
  <%   
...
String rddpt_No=request.getParameter("RDDPT_NO"),rd_Code=request.getParameter("RD_CODE"),rddpt=request.getParameter("RDDPT"),rddpr_Desc=request.getParameter("RDDPR_DESC");
...
String [] allMonth={rd_Code,rddpt_No,rddpt,rddpr_Desc};
...
<table width="100%" border="0">
  ...
  <table width="79%" border="1" cellspacing="0" cellpadding="0">
    <tr> 
      <td width="24%" colspan="1" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>研發單位代碼</strong></font></div></td>
      <td width="17%" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>公司別</strong></font></div></td>
      <td width="29%" colspan="1" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>研發單位簡稱</strong></font></div></td>
      <td width="30%" bgcolor="#003366"><div align="center"><font face="Arial" color="#FFFFFF" size="3"><strong>研發單位說明</strong></font></div></td>
      <td width="18%" rowspan="3">
        <font color="#333399" face="Arial Black" size="2"><strong> <div align="center" >
        <input name="button3" type="button" onClick='setSubmit("../jsp/WSMRDPTInput.jsp")'  value="Add" >
	    </div></strong></font>
    </tr>
    <tr> 
      <td><input type="text" name="RD_CODE"  size="1" maxlength="1"   <%if (allMonth[0]!=null) out.println("value="); else out.println("value=");%> > 
      </td>
      <td> <input type="text" name="RDDPT_NO"  size="2" maxlength="2" <%if (allMonth[1]!=null) out.println("value="); else out.println("value=");%>> 
      </td>
      <td><input type="text" name="RDDPT"  size="15" maxlength="15"   <%if (allMonth[2]!=null) out.println("value="); else out.println("value=");%> > 
      </td>
      <td><input type="text" name="RDDPR_DESC"  size="25" maxlength="25" <%if (allMonth[3]!=null) out.println("value="); else out.println("value=");%>></td>
    </tr>
    <tr bgcolor="#003366"> 
	  ...
	  try
      {
        ...
        String oneDArray[]= {"增項","研發單位代碼","公司別","研發單位簡稱","研發單位說明"}; 		 	     			  
    	array2DimensionInputBean.setArrayString(oneDArray);
	     ...
	     if (rd_Code!=null && !rd_Code.equals("") && rddpt_No!=null && !rddpt_No.equals("") && rddpt!=null && !rddpt.equals("") && rddpr_Desc!=null && !rddpr_Desc.equals("")&& bringLast==null) //bringLast是用來識別是否帶出上一次輸入之最新版本資料
		 {  //out.println("step1");             		    
		   if (a!=null) 
		   { //out.println("step2");
			 ...
			 { //out.println("step3");
			  ...
			  { //out.println("step4");
			    b[i][j]=a[i][j];	
                if (a[k][1].equals(rd_Code)) { dupFLAG = "TRUE"; }				
			  }
			  k++;
			 }
			 //b[k][0]=chooseItem;
             ...
             String sqlIMDUP = "select RD_CODE,RDDPT_NO,RDDPT,RDDPR_DESC from MRDPT where RD_CODE='"+rd_Code+"' ";
             out.println(sqlIMDUP);     
             ResultSet rsIMDUP=stateIMDUP.executeQuery(sqlIMDUP);             
             if (rsIMDUP.next() )
              { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input1 :</font><font color='#FFFFFF' face='Arial Black'> "+rd_Code+"</font><font color='#FF0000' face='Arial Black'> on Database history"+"</font>"); }
             else
			  if (dupFLAG=="TRUE" || dupFLAG.equals("TRUE")) { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input :</font><font color='#FFFFFF' face='Arial Black'> "+rd_Code+"</font><font color='#FF0000' face='Arial Black'> on Your list"+"</font>"); }
               else
                {
                 b[k][0]=rd_Code;b[k][1]=rddpt_No;b[k][2]=rddpt;b[k][3]=rddpr_Desc;
             ...
             ResultSet rsIMDUP=stateIMDUP.executeQuery(sqlIMDUP);             
             if (rsIMDUP.next() )
             { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input2 :</font><font color='#FFFFFF' face='Arial Black'> "+rd_Code+"</font><font color='#FF0000' face='Arial Black'> on Database history"+"</font>"); }
             else if (dupFLAG=="TRUE" || dupFLAG.equals("TRUE")) { out.println("<font color='#FF0000' face='Arial Black'>"+"Duplicate last Input :</font><font color='#FFFFFF' face='Arial Black'> "+rd_Code+"</font><font color='#FF0000' face='Arial Black'> on your list2 "+"</font>"); }
             else
             {
              String c[][]={{rd_Code,rddpt_No,rddpt,rddpr_Desc}};						             			 
       ...
       {
        out.println("Exception: dd "+e.getMessage());
       }
       %>
          <%
	    try 
	 ...
	 %>
        ...
        */  
         ...
         {
          out.println("Exception: ee "+e.getMessage());		  
         }
	 %>
          </strong></div></td>
    </tr>
...
<% 
     int div1=0,div2=0;      //做為運算共有多少個row和column輸入欄位的變數
      ...
      {	
       ...
       {
        out.println("Exception: ff "+e.getMessage());
       }
       %>
<BR>
<INPUT name="button2" TYPE="button" onClick='setSubmit3("../jsp/WSMRDPTInput.jsp")'  value="DELETE" >
------------------------------------------------------------------------------------------------------------
<INPUT TYPE="button"  value="SAVE" onClick='setSubmit2("../jsp/WSMRDPTInsert.jsp",<%=div1%>,<%=div2%>)' >
---------------------
<!-- 表單參數 -->  
