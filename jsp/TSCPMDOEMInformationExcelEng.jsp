<!--20170124 Peggy ,D/C=HOLD 黃底紅字標示-->
...
<body>
<FORM ACTION="../jsp/TSCPMDOEMInformationExcelEng.jsp" METHOD="post" name="MYFORM">
<%
...
{ 	
	...
	{ 
		...
		//報表名稱平行置中    
		WritableCellFormat wRptName = new WritableCellFormat(new WritableFont(WritableFont.createFont("Arial"), 14, WritableFont.BOLD,false,UnderlineStyle.NO_UNDERLINE ,jxl.format.Colour.BLACK));   
		...
		String strName = "Commissioned Production Instruction";
		ws.mergeCells(col+2, row, col+9, row);     
		...
		if (!VERSION_ID.equals("0"))
		{
			...
			//ws.setColumnView(col+line,8);	
		}
		row+=2;
		ws.mergeCells(col, row, col+7, row);
    	//ws.addCell(new jxl.write.Label(col, row ,"  "+((ASSEMBLY.equals("Y"))?"[V] ":"    ")+"Assembly   "+((TESTING.equals("Y"))?"[V] ":"    ")+"Testing    "+((TAPING_REEL.equals("Y"))?"[V] ":"    ")+"T＆R   "+((LAPPING.equals("Y"))?"[V] ":"    ")+"Lapping   "+((OTHERS!=null)?"[V] ":"    ")+"Others" , ALeft9));
		ws.addCell(new jxl.write.Label(col, row ,"  "+((ASSEMBLY.equals("Y"))?"[V] ":"[ ] ")+"Assembly   "+((TESTING.equals("Y"))?"[V] ":"[ ] ")+"Testing    "+((TAPING_REEL.equals("Y"))?"[V] ":"[ ] ")+"T＆R   "+((LAPPING.equals("Y"))?"[V] ":"[ ] ")+"Lapping   "+((OTHERS!=null)?"[V] ":"[ ] ")+"Others" , ALeft9));
		...
		row++;
		line=1;
    	ws.addCell(new jxl.write.Label(col+line, row ," Item No." , ACenterBL));
    	...
    	ws.addCell(new jxl.write.Label(col+line, row , (rsh.getString("unit_price_uom").equals("片"))?"Pieces":"(Kpcs)" , ACenterBL));
		ws.setColumnView(col+line,10);	
		line++;
		...
		{
			line++;
			ws.addCell(new jxl.write.Label(col+line, row ,"TSC Device Name" ,ACenterBL));
		}					
		row++;//列+1
		...
		if (WIP_TYPE_NO.equals("01") || WIP_TYPE_NO.equals("02")) //add by Peggy 20140814,工程&量產工單顯示芯片尺吋
		{
			...
			{
				//ws.addCell(new jxl.write.Label(col+line, row ,DIE_ITEM_NAME.substring(8,9) , ACenterL));
    	...
    	ws.addCell(new jxl.write.Label(col+1, row,"Remarks:" , ALeft));
		...
		ws.mergeCells(col+1, row, col+2, row);     
    	ws.addCell(new jxl.write.Label(col+1, row,"TSC "+rsh.getString("inventory_item_name").length()+" Code:" , ALeft));
		ws.mergeCells(col+3, row, col+6, row);     
    	ws.addCell(new jxl.write.Label(col+3, row, rsh.getString("inventory_item_name") , new WritableCellFormat(new WritableFont(WritableFont.createFont("Arial"), 10, WritableFont.NO_BOLD,false))));
		row++;//列+1	
		...
		{ 
			if (cnt==0)
			{
				...
				if (VENDOR_CODE.equals("4056") || VENDOR_CODE.equals("4746"))  //add by Peggy 20180726
				{
					...
					ws.addCell(new jxl.write.Label(col+9+i_waferno, row, "Marking two" , ACenterL));
					ws.addCell(new jxl.write.Label(col+10+i_waferno, row, "Marking three" , ACenterL));
					if (i_addline==1)
					{
					...
					if (VENDOR_CODE.equals("4746"))
					{
						ws.addCell(new jxl.write.Label(col+11+i_addline+i_waferno, row, "Die Mode" , ACenterL));;
					}
				}	
			...
			sql = " select a.lot_number, a.wafer_qty, a.chip_qty,a.date_code, to_char(to_date(a.completion_date,'yyyymmdd'),'yyyy-mm-dd') completion_date,wafer_number,dc_yyww,a.die_mode "+
          		  " FROM oraddman.tspmd_oem_lines_all a"+
			...
			if (rsy.next())
			{
				...
				ORIG_DC_YYWW=rsy.getString("dc_yyww");
			...
			{					
				if (cnt==0)
				{
					ws.addCell(new jxl.write.Label(col+line+i_addline-1, row, "Marking" , ALeftBB));
				}
				else if (cnt==1)
				{
					v_making="Y";
						...
						if (v_making.equals("N"))
						{
							...
							{	
								...
								for (int k =0 ; k < str_marking.length() ; k++)
								{
									...
									if (str_marking.substring(k,k+1).equals("∫") || str_marking.substring(k,k+1).equals("<") || str_marking.equals("><"))
									{
										//out.println(str_marking.substring(k+1,k+2));
										if (str_marking.substring(k,k+1).equals("∫"))
										{
											imageFileName="tsclogo.PNG";
										}
    	...
    	ws.addCell(new jxl.write.Label(col+3, row, "Vendor Signature" , ACenterBL));
		...
		if(f.exists()) 
		{
			if (VENDOR_CODE.equals("4056") || VENDOR_CODE.equals("4746"))
			{			
				//jxl.write.WritableImage image1 = new jxl.write.WritableImage(Math.floor(col)+8.1,row+1,0.8,2, f); 
		...
		if(f.exists()) 
		{
			if (VENDOR_CODE.equals("4056") || VENDOR_CODE.equals("4746"))
			{		
				//jxl.write.WritableImage image2 = new jxl.write.WritableImage(Math.floor(col)+9.1,row+1,0.8,2, f); 
