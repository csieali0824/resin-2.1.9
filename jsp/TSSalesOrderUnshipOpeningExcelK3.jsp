...
{ 	
	...
	{	
		...
		else if (i==1)
		{
			...
			if (ACTTYPE.equals("AUTO2"))
			{
				sql +="   AND ooh.org_id IN (41)";
			}
			else if (ACTTYPE.equals("AUTO3"))
			{
            	sql+="       AND ool.PACKING_INSTRUCTIONS='T'"+ 
                     "       AND ooh.org_id IN (41,906)";
			}
             sql +="    AND ool.ship_to_org_id=hcsua.site_use_id  "+
                  //"       AND hcsua.location=tkale.erp_ship_to_location_id(+)"+ 
				  "       AND hcsua.location=tkale.sg_ship_to_location_id(+)";
                  //"       AND not exists (select 1 from ont.oe_order_headers_all sgh where sgh.org_id=906 and ooh.org_id<>906 and sgh.order_number=ooh.order_number)";
			if (ACTTYPE.equals("AUTO2"))
			...
			if (ACTTYPE.equals("AUTO2"))
			{
				sql +="        where (ooax.ORDER_NUMBER || '.'|| ooax.line_number =po.note_to_receiver";
			}
			else if (ACTTYPE.equals("AUTO3"))
			{			
				  //"        and ((substr(ooax.order_number,1,1)<>'8' and po.comments LIKE 'Opening PO%')"+
				sql +="       where substr(ooax.order_number,1,1)='8' and po.vendor_id='976232'";
			}
			sql +="        union all"+
				  "        SELECT a.segment1 ORDER_NUMBER"+
                  "       ,0 header_id"+
                  "       ,a.CREATION_DATE ORDERED_DATE"+
                  "       ,'' FOB_POINT_CODE"+
                  "       ,'' attribute11"+
                  "       ,0 SALESREP_ID"+
                  "       ,case b.ship_to_organization_id when 907 then 14980 when 908 then 20100 else 0 end SOLD_TO_ORG_ID"+
                  "       ,906 ORG_ID"+
                  "       ,a.APPROVED_DATE BOOKED_DATE"+
                  "       ,e.po_line_id line_id"+
                  "       ,e.line_num"+
                  "       ,b.SHIPMENT_NUM shipment_number"+
                  "       ,'' ITEM_IDENTIFIER_TYPE"+
                  "       ,'' ORDERED_ITEM"+
                  "       ,'FORECAST' CUSTOMER_LINE_NUMBER"+
                  "       ,b.quantity * case  b.unit_meas_lookup_code when 'KPC' then 1000 else 1 end ordered_quantity"+
                  "       ,b.PROMISED_DATE schedule_ship_date"+
                  "       ,b.NEED_BY_DATE REQUEST_DATE"+
                  "       ,'TRUCK' SHIPPING_METHOD_CODE"+
                  ...
                  "       ,'' k3_order_line_no"+
				  "       ,a.segment1 po_no"+
                  "       ,e.unit_price/case e.unit_meas_lookup_code when 'KPC' then 1000 else 1 end  po_unit_price"+
                  "       ,b.quantity * case b.unit_meas_lookup_code when 'KPC' then 1000 else 1 end po_qty"+
                  "       ,c.segment1 vendor_code"+
                  "       ,c.vendor_name"+
                  "       ,decode(a.currency_code,'CNY','RMB',a.currency_code) TRANSACTIONAL_CURR_CODE"+
                  ...
                  "       and d.vendor_id=c.vendor_id"+
                  "       and a.po_header_id=e.po_header_id"+
                  "       and substr(b.note_to_receiver,1,instr(b.note_to_receiver,'.')-1) is null"+
                  "       and a.org_id=906"+
				  "       and a.comments LIKE 'Opening PO%'"+
                  "       ) ooa"+
                  "     ,JTF_RS_SALESREPS jrs "+
                  "     ,MTL_SYSTEM_ITEMS_B msi"+
                  "     ,AR_CUSTOMERS ar,"+
