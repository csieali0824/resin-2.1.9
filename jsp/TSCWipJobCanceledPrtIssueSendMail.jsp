...
<%
    ...
    String sSqlMailUser = "select DISTINCT USERMAIL,USERNAME from ORADDMAN.WSUSER, ORADDMAN.WSGROUPUSERROLE where USERNAME=GROUPUSERNAME and ROLENAME in ('admin','sysaudit') and LOCKFLAG='N' ";
	...
	while(rsMail.next())
	{
	   try 
          {		     
		   ...
		   urAddress = serverHostName+":8080/oradds/jsp/TSCWipJobCanceledPrtIssueReport.jsp?DATEBEGIN="+strFirstDayWeek+"&DATEEND="+strLastDayWeek;		
        ...
        //   sendMailBean.setSubject(CodeUtil.unicodeToBig5("來自ORACLE系統的郵件:每週料件分類設定異常檢核表-("+strFirstDayWeek+"~"+strLastDayWeek+")")); 
		   sendMailBean.setSubject(CodeUtil.unicodeToBig5("Oracle ERP Alert E-Mail-Weekly WIP Job Canceled Partial Issue List("+strFirstDayWeek+"~"+strLastDayWeek+")"));         
		   sendMailBean.setUrlName("Dear "+UserID+",\n"+CodeUtil.unicodeToBig5("   請點擊來自ORACLE系統的郵件:每週WIP工單取消部份發料檢核表-("+strFirstDayWeek+"~"+strLastDayWeek+")"));     
