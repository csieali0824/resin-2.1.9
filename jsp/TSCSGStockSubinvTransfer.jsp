...
<title>SG Stock Subinventory Transfer</title>
...
<FORM ACTION="../jsp/TSCSGStockSubinvTransfer.jsp" METHOD="post" NAME="MYFORM">
<strong><font style="font-size:20px;color:#006666">SG Stock Subinventory Tranfer</font></strong>
<BR>
...
<table border="0" width="100%">
	<tr>
		<td align="center">
		    <INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>' onClick='setSubmit("../jsp/TSCSGStockSubinvTransfer.jsp")' > 
		</td>
   </tr>
</table>  
<hr>
...
{       	 
	...
	if (ATYPE.equals("Q"))
	{
		sql = " SELECT X.*,X.RECEIVED_QTY+X.ALLOCATE_IN_QTY-(X.ALLOCATE_OUT_QTY+X.RETURN_QTY+X.SHIPPED_QTY+X.PICK_QTY) ONHAND "+
			  ...
			  "       ,A.ORGANIZATION_ID AS to_organization_id"+
			  "       ,CASE A.ORGANIZATION_ID WHEN 907 THEN '內銷' WHEN 908 THEN '外銷' ELSE '??' END AS to_organization_name"+
			  "       ,CASE WHEN A.SUBINVENTORY_CODE='01' THEN '03' WHEN A.SUBINVENTORY_CODE='03' THEN '01' ELSE '' END as TO_SUBINVENTORY_CODE"+ //add by Peggy 20231226
			  ...
			  "       AND A.SUBINVENTORY_CODE in ('01','03')) X "+
			  "       WHERE 1=1";
		...
		{ 	
			...
			<table width="100%" border="1" cellpadding="1" cellspacing="0" borderColorLight="#CFDAD8"  bordercolordark="#5C7671">
				<tr bgcolor="#C9E2D0"> 
					...
					<td width="5%" style="color:#006666" align="center">移轉量(K)</td>            
					<td width="6%" style="color:#006666" align="center">移轉原因</td>
				</tr>
			<% 
			}
			%>
			<tr id="tr_<%=rs.getString("SG_STOCK_ID")%>">
				...
				<td align="center"><input type="text" name="TO_SUBINV_<%=rs.getString("SG_STOCK_ID")%>"  size="6" value="<%=rs.getString("TO_SUBINVENTORY_CODE")%>" style="text-align:center;font-family: Tahoma,Georgia;font-size:12px" readonly></td>
	...
	%>
		...
		<table width="100%">
			<tr><td align="center"><input type="button" name="save" value="Submit" style="font-family: Tahoma,Georgia;" onClick="setSubmit1('../jsp/TSCSGPOReceiveProcess.jsp?TRANSTYPE=SUBINVTRX')"></td></tr>
		</table>
	<%
		}
	}
