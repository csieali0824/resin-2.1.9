...
String custActive=request.getParameter("CUSTACTIVE");
    ...
    String orgCode="",fgItemId="",fgOhQty="",subItemId="",subItemNo="",subItemDesc="",subItemQty="",revsQty="",fwoQty="";
    float fgOhQtyf=0,revsQtyf=0,avaiQtyf=0;
...
<FORM ACTION="../jsp/TSCMfgINVFgStockQuery.jsp" METHOD="post" NAME="MYFORM">
...
<strong><a>成品庫存狀況查詢</a></strong></font><BR>
...
try
{
     ...
     sSql = " select MSI.ORGANIZATION_ID,MSI.INVENTORY_ITEM_ID,MSI.SEGMENT1 ITEM_NO,MSI.DESCRIPTION,MSI.PRIMARY_UNIT_OF_MEASURE UOM, "+
            "         (MOQ.PRIMARY_TRANSACTION_QUANTITY) FG_OH_QTY ,OPEN_MO.ORDER_NUMBER , OPEN_MO.ORDERED_QTY "+
			"   from MTL_ONHAND_QUANTITIES_DETAIL MOQ,MTL_SYSTEM_ITEMS MSI,   "+
            "         (select ooha.ORDER_NUMBER,oola.inventory_item_id, OOLA.LINE_NUMBER||'.'||OOLA.SHIPMENT_NUMBER AS LINE_NUMBER, "+
            "	              TO_CHAR(OOLA.SCHEDULE_SHIP_DATE,'YYYY/MM/DD') AS SCHEDULE_SHIP_DATE, OOLA.ORDERED_QUANTITY   AS ORDERED_QTY, "+
            "                  OOLA.ORDER_QUANTITY_UOM   AS ORDER_UOM,  OOLA.SHIP_FROM_ORG_ID ,OOLA.FLOW_STATUS_CODE  "+
	        "            from OE_ORDER_HEADERS_ALL OOHA,OE_ORDER_LINES_ALL OOLA  "+
            "           where OOHA.HEADER_ID = OOLA.HEADER_ID  and OOLA.FLOW_STATUS_CODE NOT IN ('CLOSED','CANCELLED') ) OPEN_MO ";;
   ...
   sWhere = "  where MOQ.INVENTORY_ITEM_ID(+)=MSI.INVENTORY_ITEM_ID  and MOQ.ORGANIZATION_ID(+)=MSI.ORGANIZATION_ID "+
  		   "    and OPEN_MO.INVENTORY_ITEM_ID(+)= MSI.INVENTORY_ITEM_ID  and OPEN_MO.SHIP_FROM_ORG_ID(+)=MSI.ORGANIZATION_ID and MOQ.ORGANIZATION_ID =43 ";
   
  // sWhereGP = " group by  MSI.ORGANIZATION_ID,MSI.INVENTORY_ITEM_ID,MSI.SEGMENT1,MSI.DESCRIPTION,MSI.PRIMARY_UNIT_OF_MEASURE  ";
   sWhereGP =  "";	
  ...
  catch (Exception e)
  {
   ...
   try
        {	
/*
	     out.println("<BR>sSqlCNT-1="+ sSqlCNT); 
...
*/		 
        ...
        {
          out.println("Exception B:"+e.getMessage());
        }
//out.print("<br>no wo_no input!!!");
}
else
{   
  try
 {
     ...
     sSql = " select MSI.ORGANIZATION_ID,MSI.INVENTORY_ITEM_ID,MSI.SEGMENT1 ITEM_NO,MSI.DESCRIPTION,MSI.PRIMARY_UNIT_OF_MEASURE UOM, "+
            "         SUM(MOQ.PRIMARY_TRANSACTION_QUANTITY) FG_OH_QTY ,OPEN_MO.ORDER_NUMBER , OPEN_MO.ORDERED_QTY "+
			"   from MTL_ONHAND_QUANTITIES_DETAIL MOQ,MTL_SYSTEM_ITEMS MSI,   "+
            "         (select ooha.ORDER_NUMBER,oola.inventory_item_id, OOLA.LINE_NUMBER||'.'||OOLA.SHIPMENT_NUMBER AS LINE_NUMBER, "+
            "	              TO_CHAR(OOLA.SCHEDULE_SHIP_DATE,'YYYY/MM/DD') AS SCHEDULE_SHIP_DATE, OOLA.ORDERED_QUANTITY   AS ORDERED_QTY, "+
            "                  OOLA.ORDER_QUANTITY_UOM   AS ORDER_UOM,  OOLA.SHIP_FROM_ORG_ID ,OOLA.FLOW_STATUS_CODE  "+
	        "            from OE_ORDER_HEADERS_ALL OOHA,OE_ORDER_LINES_ALL OOLA  "+
            "           where ooha.ORG_ID !=42 and OOHA.HEADER_ID = OOLA.HEADER_ID  and OOLA.FLOW_STATUS_CODE NOT IN ('CLOSED','CANCELLED') ) OPEN_MO ";
   ...
   sWhere ="  where MOQ.INVENTORY_ITEM_ID(+)=MSI.INVENTORY_ITEM_ID  and MOQ.ORGANIZATION_ID(+)=MSI.ORGANIZATION_ID "+
  		   "    and OPEN_MO.INVENTORY_ITEM_ID(+)= MSI.INVENTORY_ITEM_ID  and OPEN_MO.SHIP_FROM_ORG_ID(+)=MSI.ORGANIZATION_ID and MOQ.organization_id ='"+marketType+"' " ;
   			 
   sWhereSDRQ = "  ";
   
   sWhereGP = "  group by  MSI.ORGANIZATION_ID,MSI.INVENTORY_ITEM_ID,MSI.SEGMENT1,MSI.DESCRIPTION,MSI.PRIMARY_UNIT_OF_MEASURE,OPEN_MO.ORDER_NUMBER,OPEN_MO.ORDERED_QTY    ";
			 
   if (marketType==null || marketType.equals("--") || marketType.equals("")) {sWhere=sWhere+" and MOQ.organization_id in (326,327) ";}
   else {sWhere=sWhere+" and MOQ.organization_id ='"+marketType+"'"; }
...
//   else {sWhere=sWhere+" and YWA.WORKORDER_TYPE ='"+woType+"'"; }
   ...
   else {sWhere=sWhere+" and (  MSI.SEGMENT1 like upper('"+woNo+"%') or upper(MSI.DESCRIPTION) like upper('"+woNo+"%')  "+
                        "        or upper(OPEN_MO.ORDER_NUMBER) like upper('"+woNo+"%')) "  ; }   
...
%> 
  <table cellspacing="0" bordercolordark="#6699CC" cellpadding="1" width="100%" align="center" bordercolorlight="#ffffff" border="1">
     <tr>
	   ...
	   <td width="20%" nowrap><font color="#006666"><strong> &nbsp; &nbsp;成品料號、品名/規格或MO單號</strong></font></td>
	   <td width="15%"> &nbsp;
       <input type="text" name="WONO" value="<%=woNo%>"></td>
	<td  width="20%">
		    <INPUT TYPE="button" align="middle"  value='<jsp:getProperty name="rPH" property="pgQuery"/>' onClick='setSubmit("../jsp/TSCMfgINVFgStockQuery.jsp")' > 
			<input type="reset" name="RESET" align="middle"  value='<jsp:getProperty name="rPH" property="pgReset"/>' onClick='setSubmit2("../jsp/TSCMfgINVFgStockQuery.jsp")' >
...
<table cellspacing="0" bordercolordark="#6699CC" cellpadding="1" width="100%" align="center" bordercolorlight="#ffffff" border="1">	
	...
	/*	 
	     try
         {       
          String a[]={"2007","2008","2009","2010","2011","2012"};
       ...
       %>
       <font color="#006666"><strong><jsp:getProperty name="rPH" property="pgWorkOrder"/><jsp:getProperty name="rPH" property="pgCDate"/></strong></font>
        <%
         ...
         {
          out.println("Exception t1:"+e.getMessage());
         }
         ...
         {
          out.println("Exception t2:"+e.getMessage());
         }
         ...
         {
          out.println("Exception t3:"+e.getMessage());
         }
*/
       %>
    </td-->  
...
%>
  ...
  <table cellspacing="0" bordercolordark="#6699CC" cellpadding="1" width="100%" align="center" bordercolorlight="#ffffff" border="1">
    <tr bgcolor="#BBD3E1"> 
	  ...
	  <td width="3%" height="22" nowrap><div align="center"><font color="#006666" face="Arial">內/外銷</font></div></td>
	  <td width="15%" nowrap><div align="center"><font color="#006666" face="Arial">料號</font></div></td>
      <td width="8%" nowrap><div align="center"><font color="#006666" face="Arial">品名</font></div></td>           
      <td width="4%" nowrap><div align="center"><font color="#006666" face="Arial">單位</font></div></td> 
	  <td width="4%" nowrap><div align="center"><font color="#006666" face="Arial">庫存數量</font></div></td>
	  <td width="4%" nowrap><div align="center"><font color="#006666" face="Arial">保留數</font></div></td>
	  <td width="5%" nowrap><div align="center"><font color="#006666" face="Arial">可用數量</font></div></td>
      <td width="6%" nowrap><div align="center"><font color="#006666" face="Arial">訂單號</font></div></td>
	  <td width="5%" nowrap><div align="center"><font color="#006666" face="Arial">工令數</font></div></td>	 
	  <td width="10%" nowrap><div align="center"><font color="#006666" face="Arial">下階料號</font></div></td>  	   
	  <td width="12%" nowrap><div align="center"><font color="#006666" face="Arial">品名</font></div></td>
	  <td width="5%" nowrap><div align="center"><font color="#006666" face="Arial">庫存數</font></div></td>
	  <td width="5%" nowrap><div align="center"><font color="#006666" face="Arial">前段工令數</font></div></td>	
...
//
		...
		fgOhQty=rsTC.getString("FG_OH_QTY"); 
		fgOhQtyf=rsTC.getFloat("FG_OH_QTY");
		
        if (organizationId=="326" || organizationId.equals("326")) orgCode="內銷" ;
          else if (organizationId=="327" || organizationId.equals("327")) orgCode="外銷" ; 
            else orgCode="&nbsp;" ;
   ...
   //out.print("<br>"+invItem+"   "+itemDesc+"  "+fgOhQty);

    //顯示成品保留數
     String revSql =" SELECT SUM(PRIMARY_RESERVATION_QUANTITY) RESERVATION_QTY FROM MTL_RESERVATIONS "+
 				    "  WHERE ORGANIZATION_ID= "+organizationId+" AND INVENTORY_ITEM_ID= "+itemId+"  ";
    //out.print("revSql="+revSql);
    Statement revstate=con.createStatement(); 
    ResultSet revrs=revstate.executeQuery(revSql);	
    if  (revrs.next())
    {
     revsQty=revrs.getString("RESERVATION_QTY");
     revsQtyf=revrs.getFloat("RESERVATION_QTY");
    }
    revrs.close();   
    revstate.close();

   //可用庫存  on_hand_qty - reservation_qty
     avaiQtyf =  fgOhQtyf - revsQtyf;
 
    //顯示工單資訊
     String wkSql =" select PRIMARY_ITEM_ID,sum(START_QUANTITY) WO_QTY   from WIP_DISCRETE_JOBS  "+
				   "  where ORGANIZATION_ID = "+organizationId+" and PRIMARY_ITEM_ID = "+itemId+" "+ 
                   "    and STATUS_TYPE  =3 group by PRIMARY_ITEM_ID  ";
    //out.print("<br>wkSql="+wkSql);
    Statement wkstate=con.createStatement(); 
    ResultSet wkrs=wkstate.executeQuery(wkSql);	
    if  (wkrs.next())
    {
     fgItemId=wkrs.getString("PRIMARY_ITEM_ID");
     woQty=wkrs.getString("WO_QTY");
    }
    wkrs.close();   
    wkstate.close();
    ...
    //顯示半成品庫存
     String subSql =" select BOM.ASSEMBLY_ITEM_ID,MSI.SEGMENT1 SUB_ITEM_NO,MSI.DESCRIPTION SUB_ITEM_DESC,SUM(MOQ.PRIMARY_TRANSACTION_QUANTITY) SUB_ITEM_QTY,BIC.COMPONENT_ITEM_ID "+
					"  from BOM_BILL_OF_MATERIALS BOM,BOM_INVENTORY_COMPONENTS BIC,MTL_SYSTEM_ITEMS MSI,MTL_ONHAND_QUANTITIES_DETAIL MOQ "+
					"  where BOM.BILL_SEQUENCE_ID = BIC.BILL_SEQUENCE_ID and BIC.WIP_SUPPLY_TYPE !=6  "+
  					"    and MSI.INVENTORY_ITEM_ID = BIC.COMPONENT_ITEM_ID   and MSI.ORGANIZATION_ID = BOM.ORGANIZATION_ID "+
  					"    and MOQ.ORGANIZATION_ID(+) = MSI.ORGANIZATION_ID   and MOQ.INVENTORY_ITEM_ID(+) = MSI.INVENTORY_ITEM_ID "+
                    "    and  BOM.ASSEMBLY_ITEM_ID = "+itemId+"  and BOM.ORGANIZATION_ID = "+organizationId+"  "+
					"  group by BOM.ASSEMBLY_ITEM_ID,MSI.SEGMENT1,MSI.DESCRIPTION ,BIC.COMPONENT_ITEM_ID";
    //out.print("<BR>subSql="+subSql);
    Statement substate=con.createStatement(); 
    ResultSet subrs=substate.executeQuery(subSql);	
    while  (subrs.next())
    {
     if ((rs1__index % 2) == 0){
	       colorStr = "#D8E6E7"; 
     ...
     subItemId=subrs.getString("COMPONENT_ITEM_ID");
     subItemNo=subrs.getString("SUB_ITEM_NO");
     subItemDesc=subrs.getString("SUB_ITEM_DESC");
	 subItemQty=subrs.getString("SUB_ITEM_QTY");

    //顯示前段工單資訊
     String wkSqlb =" select PRIMARY_ITEM_ID,sum(START_QUANTITY) B_WO_QTY   from WIP_DISCRETE_JOBS  "+
				   "  where ORGANIZATION_ID = "+organizationId+" and PRIMARY_ITEM_ID = "+subItemId+" "+ 
                   "    and STATUS_TYPE  =3 group by PRIMARY_ITEM_ID  ";
    //out.print("<br>wkSql="+wkSql);
    Statement wkbstate=con.createStatement(); 
    ResultSet wkbrs=wkbstate.executeQuery(wkSqlb);	
    if  (wkbrs.next())
    {
    // fgItemId=wkrs.getString("PRIMARY_ITEM_ID");
     fwoQty=wkbrs.getString("B_WO_QTY");
    }
    wkbrs.close();   
    wkbstate.close();
  ...
  if (woQty==null || woQty.equals("")) woQty="&nbsp"; 
  if (subItemQty==null || subItemQty.equals("")) subItemQty="&nbsp"; 
  if (revsQty==null || revsQty.equals("")) revsQty="&nbsp"; 
  if (fwoQty==null || fwoQty.equals("")) fwoQty="&nbsp"; 
  if (oeOrderNo==null || oeOrderNo.equals("")) oeOrderNo="&nbsp"; 
...
//         else if (rsTC.getString("ALTERNATE_ROUTING").equals("2"))  clkDesc="(外購工令)"+"點擊連結查詢明細";     
    ...
    <tr bgcolor="<%=colorStr%>"> 
	  ...
	  <td ><div align="center"><font color="#006666" face="Arial"><%=orgCode%></font></div></td>
	  ...
	  <td ><div align="center"><font color="#006666" face="Arial"><a href="../jsp/TSCMfgStockDetail.jsp?ITEM_ID=<%=itemId%>&ORGANIZATION_ID=<%=organizationId%>&TYPEID=<%=1%>"><%=fgOhQty%></a></font></div></td>
	  <td ><div align="center"><font color="#006666" face="Arial"><a href="../jsp/TSCMfgStockDetail.jsp?ITEM_ID=<%=itemId%>&ORGANIZATION_ID=<%=organizationId%>&TYPEID=<%=2%>"><%=revsQty%></a></font></div></td>	  
	  <td ><div align="center"><font color="#006666" face="Arial"><%=avaiQtyf%></font></div></td>
      <td ><div align="center"><font color="#006666" face="Arial"><%=oeOrderNo%></font></div></td>
	  <td ><div align="center"><font color="#006666" face="Arial"><a href="../jsp/TSCMfgStockDetail.jsp?ITEM_ID=<%=itemId%>&ORGANIZATION_ID=<%=organizationId%>&TYPEID=<%=3%>"><%=woQty%></a></font></div></td>	  
	  <td ><div align="left"><font color="#006666" face="Arial"><%=subItemNo%></font></div></td>
	  <td ><div align="left"><font color="#006666" face="Arial"><%=subItemDesc%></font></div></td>
      <td ><div align="center"><font color="#006666" face="Arial"><a href="../jsp/TSCMfgStockDetail.jsp?SUB_ITEM_ID=<%=subItemId%>&ORGANIZATION_ID=<%=organizationId%>&TYPEID=<%=4%>"><%=subItemQty%></a></font></div></td> 
      <td ><div align="center"><font color="#006666" face="Arial"><a href="../jsp/TSCMfgStockDetail.jsp?SUB_ITEM_ID=<%=subItemId%>&ORGANIZATION_ID=<%=organizationId%>&TYPEID=<%=5%>"><%=fwoQty%></a></font></div></td> 
    </tr>
    <%
  rs1__index++;
  rs_hasDataTC = rsTC.next();
  ...
  subrs.close();     //半成品庫存 CLOSED
  substate.close();   //半成品庫存CLOSED
  } //endof while 
