	...
	{	
		...
		if (i==0)
		{
			sql = " SELECT upper(Tsc_Intercompany_Pkg.get_sales_group(ooa.header_id)) sales_group,"+  
                  ...
                  "       WHERE (exists (select 1 from oraddman.tssg_abroad_open_order taox where NVL(taox.UPDATED_FLAG,'N')='Y' AND taox.so_no=ooh.order_number and taox.line_no=ool.line_number||'.'||ool.shipment_number)"+
			      ...
			      "       and  (ool.attribute3 is null or instr(ool.attribute3,'-')=0)"+
				  "       ) ooa"+
		...
		else if (i==1)
		{
			sql =" select '' k3_order_no, "+
                 ...
                 " case when substr(odr.ORDER_NUMBER,1,1) ='8' then to_char(trunc(odr.ORDERED_DATE),'yyyy/mm/dd') else  case when trunc(b.PROMISED_DATE)<to_date('20200122','yyyymmdd') then to_char(b.PROMISED_DATE,'yyyy/mm/dd') else '2020/01/22' end end ordered_date,"+
                 ...
                 "        and a.org_id=906"+
                 "        and (a.comments LIKE 'Opening PO%' or substr(b.note_to_receiver,1,instr(b.note_to_receiver,'.')-1) in ('11310043440','11310043441','11310043442'))"+
                 "        and d.vendor_site_code like '%-E'"+
                 "        and b.note_to_receiver=odr.order_line_no(+)"+
                 "        and e.item_id=msi.inventory_item_id"+
                 "        and case when substr(b.note_to_receiver,1,1)<>'8' then case b.ship_to_organization_id when 907 then 14980 when 908 then 20100 else 0 end  else odr.sold_to_org_id end =ar.customer_id"+
