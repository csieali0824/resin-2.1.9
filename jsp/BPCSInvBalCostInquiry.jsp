...
function submitCheck() {
	...
	document.MYFORM.action="BPCSInvBalCostInquiry.jsp?SEL=1";
...
<font color="#009999" face="Times New Roman" size="+3"><strong>大霸電子庫存餘額查詢(含成本)</strong></font>
...
<tr>
	...
	<td align="right"><font face="Arial" color="#000000" size="1"><strong>單位標準成本(NTD)</strong></font></td>
	<td align="right"><font face="Arial" color="#000000" size="1"><strong>標準金額</strong></font></td>
</tr>

<%
try {
	String sql="SELECT lwhs,lloc||'--'||wdesc AS lloc,lprod,idesc||idsce as desc,iums,lopb+lrct-lissu+ladju as inv ,sum(cftlvl+cfplvl) as cost"
	+" FROM ILI,IIM,ilm,cmf "
	+" WHERE lid='LI' and lprod=iprod and lwhs=wwhs and lloc=wloc "
	+" and lprod=cfprod AND cffac='' AND cfcset=2 AND cfcbkt>=1 AND cfcbkt<=5 "
	+" and (lopb+lrct-lissu+ladju)!=0 "
	+" AND ((lwhs>='00' AND lwhs<='02') OR (lwhs>='22' AND lwhs<='22') OR (lwhs>='31' AND lwhs<='33') "
	+"   OR (lwhs>='50' AND lwhs<='59') OR (lwhs>='70' AND lwhs<='87') "
	+"   OR (lwhs>='K1' AND lwhs<='K9') OR (lwhs>='T1' AND lwhs<='T9') OR lwhs='P1') ";
	if (sPartNumber!=null && !sPartNumber.equals("") ) {
	...
	sql = sql+" GROUP BY 1,2,3,4,5,6 ORDER BY lwhs,lloc,lprod"; //out.println(sql);
	if (sel!=null) {
		rs = st.executeQuery(sql);
		boolean eof = !rs.next();
...
%>
	...
	<td align="right"><font face="Arial" color="#000000" size="1"><strong><%=rs.getString("cost")%></strong></font></td>
	<td align="right"><font face="Arial" color="#000000" size="1"><strong><%=rs.getInt("inv")*rs.getFloat("cost")%></strong></font></td>
	</tr>
<%
				eof = !rs.next();
			} // end while
