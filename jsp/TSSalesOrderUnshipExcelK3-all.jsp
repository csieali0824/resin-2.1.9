...
{ 	
	...
	for (int i =2 ; i < sheetname.length ; i++)
	...
	{	
		...
		else if (i==2)
		{
			...
			sql = " SELECT upper(TSC_OM_Get_Sales_Group(ooa.header_id)) sales_group,"+  
                  ...
                  "       AND ooh.ORDER_TYPE_ID not in ('1114','1091','1016','1020','1343','1017')"+ 
				  "       AND substr(ooh.order_number,1,3) not in ('819')"+ 
                  "       AND ool.INVENTORY_ITEM_ID not in  (29570,29562,29560)"+ 
			...
			//}	
			sql +="      AND 1=1 "+
			      //"      AND ooh.BOOKED_DATE  BETWEEN TO_DATE('"+SDATE+"','yyyymmdd')+(15/24) AND TO_DATE('"+ (EDATE.equals("")?SDATE:EDATE)+"','yyyymmdd')+(14.9996/24)"+
			      "      ) ooa"+
				  "     ,JTF_RS_SALESREPS jrs "+
				  "     ,MTL_SYSTEM_ITEMS_B msi"+
				  "     ,AR_CUSTOMERS ar,"+
		...
		else if (i==3)
		{
			sql =" select '' k3_order_no, "+
                 ...
                 " msi.description,  "+
                 " odr.CUST_ITEM,"+
                 " (b.quantity-nvl(b.QUANTITY_RECEIVED,0)-nvl(b.QUANTITY_CANCELLED,0)) * case  b.unit_meas_lookup_code when 'KPC' then 1000 else 1 end ordered_quantity,"+
