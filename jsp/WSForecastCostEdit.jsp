...
<title>WINS System - Forecast Cost Edit Page</title>
...
<strong>WINS System Forecast Cost Edit Page</strong></font>
<% //out.println("step0");    %>
...
%>
  <table width="100%" border="0">
    <tr bgcolor="#000066"> 
     ...
     {  
      ...
      sqlGlobal =  "select FCCOMP,FCREG,FCCOUN,FCCURR,FCPRJCD,FCCOST,FCYEAR,FCMONTH,FCTYPE,FCLUSER,FCMVER  "+
 ...
 <table width="100%" border="1" cellpadding="1" cellspacing="1">
  <tr>
      <td width="24%" nowrap bordercolor="#FFFFFF" bordercolordark="#000000" bgcolor="#FFFFCC"><font face="Arial"><strong>COST: 
        </strong></font> </td>
    <td> 
      <input name="FCCOST" type="text" value="<%=rs.getInt("FCCOST")%>" size="100%" maxlength="10">	
    </td>
  </tr>             
 </table>
 <table width="100%" border="1" cellpadding="1" cellspacing="1">
 ...
 <input name="submit1" type="submit" value="UPDATE" onClick='return setSubmit("../jsp/WSForecastCostMaintEditCommit.jsp?&FCCOMP=<%=comp%>&FCTYPE=<%=type%>&FCREG=<%=region%>&FCCOUN=<%=country%>&FCCURR=<%=curr%>&FCPRJCD=<%=model%>&FCYEAR=<%=year%>&FCMONTH=<%=month%>&FCMVER=<%=version%>")'> 
 <input name="submit2" type="submit" value="ABORT" onClick='return setSubmit("../jsp/WSForecastCostMaintenance.jsp?&FCCOMP=<%=comp%>&FCTYPE=<%=type%>&FCREG=<%=region%>&FCCOUN=<%=country%>&FCCURR=<%=curr%>&FCPRJCD=<%=model%>&FCYEAR=<%=year%>&FCMONTH=<%=month%>&FCMVER=<%=version%>")'>
 <%
       rs.close();
       statement.close();
      }
