...
String custActive=request.getParameter("CUSTACTIVE");
  ...
  if (order==null) order = "ST"; // 預設以站別排序
  
  
  
  String reportType=request.getParameter("REPORTTYPE");   
  if (reportType==null) reportType = "SI"; // 預設的報表類型(半成品)
...
//liling create
	...
	//if (order==null || order.equals("")) order = "ST"; // 預設值以站別排序
...
<strong><a>生產月報統計表-線上暫存</a></strong></font>
<BR>
...
if ( dateSetBegin==null || dateSetBegin.equals("") )
{
  ...
  {   // out.println("workingDateBean.getWeek()="+workingDateBean.getWeek());
     ...
     sSql = " select YWA.WO_NO, YWO.WIP_ENTITY_ID, YWA.INV_ITEM, YWA.ITEM_DESC, YWO.OPERATION_CODE, WOP.DESCRIPTION, "+
	        "        sum(YWO.QUANTITY_BEGIN_BALANCE) as QUANTITY_BEGIN_BALANCE, "+
			...
			"        sum(YWO.QUANTITY_END_BALANCE) as QUANTITY_END_BALANCE, sum(YWO.QTY_MOVEIN_TRANSACT) as QTY_MOVEIN_TRANSACT, "+
			"        sum(YWO.QTY_LASTOP_COMPLETE) as QTY_LASTOP_COMPLETE, sum(YWO.QTY_ONLINE_RETURN) as QTY_ONLINE_RETURN, "+
			"        sum(YWO.QTY_COMPON_ISSUED) as QTY_COMPON_ISSUED, sum(YWO.QTY_MOVEOUT_TRANSACT) as QTY_MOVEOUT_TRANSACT "+
	        "   from YEW_WIP_WIPIO YWO, ORG_ACCT_PERIODS OAP, YEW_WORKORDER_ALL YWA, WIP_OPERATIONS WOP ";	

   sSqlCNT = " select count(YWA.INV_ITEM) CaseCount from YEW_WIP_WIPIO YWO, ORG_ACCT_PERIODS OAP, YEW_WORKORDER_ALL YWA, WIP_OPERATIONS WOP  ";
   ...
   sWhere =  "  where YWO.ORGANIZATION_ID = OAP.ORGANIZATION_ID and YWO.ACCT_PERIOD_ID = OAP.ACCT_PERIOD_ID and YWO.ORGANIZATION_ID = "+marketType+" and to_char(OAP.PERIOD_START_DATE,'YYYYMM')= '"+dateBean.getYearString()+dateBean.getMonthString()+"' "+
             "    and YWO.WIP_ENTITY_ID = WOP.WIP_ENTITY_ID and YWO.OPERATION_SEQ_NUM = WOP.OPERATION_SEQ_NUM "+
             "    and YWO.WIP_ENTITY_ID = YWA.WIP_ENTITY_ID and YWO.OPERATION_CODE != '9998'  "+
			 "    and YWO.DEPT_NO in ('1','2','3') "; //  確保由生產系統(不含一月份舊系統麗玲手開工令)
			 
   sWhereSDRQ = " ";
   havingGrp = "group by YWA.INV_ITEM, YWA.ITEM_DESC, YWA.WO_NO, YWO.WIP_ENTITY_ID, YWO.OPERATION_CODE, WOP.DESCRIPTION ";               
   havingGrpSDRQ = "  ";
   sOrder = " order by YWO.OPERATION_CODE, WOP.DESCRIPTION, YWA.INV_ITEM, YWO.WIP_ENTITY_ID, YWA.ITEM_DESC, YWA.WO_NO  "; //預設以站別
...
{ 
  ...
  sSql = " select YWA.WO_NO, YWO.WIP_ENTITY_ID, YWA.INV_ITEM, YWA.ITEM_DESC, YWO.OPERATION_CODE, WOP.DESCRIPTION, sum(YWO.QUANTITY_BEGIN_BALANCE) as QUANTITY_BEGIN_BALANCE, "+
	        "        sum(YWO.QUANTITY_RECEIVED) as QUANTITY_RECEIVED, sum(YWO.QUANTITY_PROCESSED) as QUANTITY_PROCESSED, "+
			"        sum(YWO.QUANTITY_COMPLETED) as QUANTITY_COMPLETED, sum(YWO.QUANTITY_SCRAPPED) as QUANTITY_SCRAPPED, "+
			"        sum(YWO.QUANTITY_END_BALANCE) as QUANTITY_END_BALANCE, sum(YWO.QTY_MOVEIN_TRANSACT) as QTY_MOVEIN_TRANSACT, "+
			"        sum(YWO.QTY_LASTOP_COMPLETE) as QTY_LASTOP_COMPLETE, sum(YWO.QTY_ONLINE_RETURN) as QTY_ONLINE_RETURN, "+
   ...
   sWhere =  "  where YWO.ORGANIZATION_ID = OAP.ORGANIZATION_ID and YWO.ACCT_PERIOD_ID = OAP.ACCT_PERIOD_ID and YWO.ORGANIZATION_ID = "+marketType+" "+
	 ...
	 if (reportType==null || reportType.equals("SI")) { sWhere=sWhere+" and length(YWA.INV_ITEM) = 13 and substr(YWA.INV_ITEM,5,1) = '-' "; } // 13碼半成品
     else if (reportType.equals("GD")) { sWhere=sWhere+" and length(YWA.INV_ITEM) = 22 "; }     // 22 碼成品
   
     havingGrp = "group by YWA.INV_ITEM, YWA.ITEM_DESC, YWA.WO_NO, YWO.WIP_ENTITY_ID, YWO.OPERATION_CODE, WOP.DESCRIPTION ";      
     havingGrpSDRQ = "  ";      
	 
	 if (order.equals("ST")) sOrder = "  order by YWO.OPERATION_CODE, WOP.DESCRIPTION, YWA.INV_ITEM, YWA.ITEM_DESC, YWA.WO_NO, YWO.WIP_ENTITY_ID ";
     else  sOrder = "  order by YWA.INV_ITEM, YWA.ITEM_DESC, YWA.WO_NO, YWO.WIP_ENTITY_ID, YWO.OPERATION_CODE, WOP.DESCRIPTION ";
     ...
     String sqlOrgCnt = " select count(YWA.INV_ITEM) CaseCount from YEW_WIP_WIPIO YWO, ORG_ACCT_PERIODS OAP, YEW_WORKORDER_ALL YWA, WIP_OPERATIONS WOP  ";
...
<table cellspacing="0" bordercolordark="#6699CC" cellpadding="1" width="100%" align="center" bordercolorlight="#ffffff" border="1">	
     <tr>
       ...
       %>	   </td>
	   <td nowrap><font color="#006666"><strong>生產月份</strong></font><font color="#CC3366" face="Arial Black">&nbsp;</font> 
        <%
	...
	<td nowrap><font color="#006666"><strong>工令類別</strong></font><font color="#CC3366" face="Arial Black">&nbsp;</font>	  
	   <%
	             try
                 {  
				   //-----取工單類別  
				   ...
				   */
				   out.println("<select NAME='WOTYPE' onChange='setSubmit("+'"'+"../jsp/TSCMfgWIPOnlineTemporaryRpt.jsp"+'"'+")'>");
	...
	<td nowrap><font color="#006666"><strong>站別</strong></font><font color="#CC3366" face="Arial Black">&nbsp;</font>
	   <%
	             try
                 {  
				   //-----取站別  
	...
	<td width="22%">
		    <INPUT TYPE="button" align="middle"  value='查詢' onClick='setSubmit("../jsp/TSCMfgWIPOnlineTemporaryRpt.jsp")' > 
			<input type="reset" name="RESET" align="middle"  value='重置' onClick='setSubmit2("../jsp/TSCMfgWIPOnlineTemporaryEntry.jsp")' >
   ...
   <tr>
     <td><font color="#006666"><strong>報表類別</strong></font></td>
     <td colspan="2">	     	      
	      <input type="radio" name="REPORTTYPE" value="SI" <% if (reportType.equals("SI")) out.println("checked"); %>>半成品
		  <input type="radio" name="REPORTTYPE" value="GD" <% if (reportType.equals("GD")) out.println("checked"); %>>成品
	 </td>
     <td><font color="#006666"><strong>排序方式</strong></font></td>
     <td colspan="2">	     
	      <input type="radio" name="ORDER" value="ST" <% if (order.equals("ST")) out.println("checked"); %>>依站別排序
	      <input type="radio" name="ORDER" value="IM" <% if (order.equals("IM")) out.println("checked"); %>>依料號排序
	 </td>
   </tr>
  </table> 
<BR> 
...
<table cellspacing="0" bordercolordark="#6699CC" cellpadding="1" width="100%" align="center" bordercolorlight="#ffffff" border="1">
   ...
   <tr>
      <td colspan="11"><div align="center">線上暫存明細(<%=orgCode%>)</div></td>
   </tr>
   <tr bgcolor="#BBD3E1"> 	  
	  <td width="11%" height="22" nowrap><div align="center"><font color="#006666" face="Arial">工令</font></div></td>               
	  <td width="11%" nowrap><div align="center"><font color="#006666" face="Arial">站別</font></div></td>
      <td width="8%" nowrap><div align="center"><font color="#006666" face="Arial">料號</font></div></td>           
      <td width="9%" nowrap><div align="center"><font color="#006666" face="Arial">品名</font></div></td> 	 
	  <td width="11%" nowrap><div align="center"><font color="#006666" face="Arial">期初數</font></div></td>  
	  <td width="9%" nowrap><div align="center"><font color="#006666" face="Arial">領料數</font></div></td>
	  <td width="10%" nowrap><div align="center"><font color="#006666" face="Arial">轉入數</font></div></td>
	  <td width="10%" nowrap><div align="center"><font color="#006666" face="Arial">入退庫</font></div></td>	 
	  <td width="10%" nowrap><div align="center"><font color="#006666" face="Arial">轉出數</font></div></td>  	   
	  <td width="11%" nowrap><div align="center"><font color="#006666" face="Arial">期末數</font></div></td>	   	  
    </tr>	
    <% 
	   float beginBalTotal=0, qtyIssuedTotal=0, qtyMoveInTotal=0, qtyMoveOutTotal=0, qtyCompORRetnTotal=0, endBalTotal=0; 
    ...
    %>     
	  ...
	  %>     
    <tr bgcolor="<%=colorStr%>">  	  
	  <td nowrap><div align="center"><font color="#006666" face="Arial"><%=rsTC.getString("WO_NO")%></font></div></td>
	  <td nowrap><div align="center"><font color="#006666" face="Arial"><%=rsTC.getString("OPERATION_CODE")+"("+rsTC.getString("DESCRIPTION")+")"%></font></div></td>
	  <td nowrap><div align="center"><font color="#006666" face="Arial">
	           <% 		     
				   out.println(rsTC.getString("INV_ITEM"));
				   //
			   %></font></div></td>
	  <td nowrap><div align="center"><font color="#006666" face="Arial">
	           <%
	               out.println(rsTC.getString("ITEM_DESC"));
	  ...
	  <td nowrap><div align="right"><font color="#006666" face="Arial">
			   ...
			   <% 
			       ...
			       String sqlDTL = " select ISS_INV_ITEM, REPLACE(ISS_ITEM_DESC,'\"','”') as ISS_ITEM_DESC, QTY_REQUIRED, QTY_ISSUED, QTY_PER_ASSEMBLY    "+				                   
									 " from YEW_WIP_ISSUE_DTL ";
			       String whereDTL = " where WIP_ENTITY_ID = "+rsTC.getString("WIP_ENTITY_ID")+" "+
				                     "   and OPERATION_CODE = '"+rsTC.getString("OPERATION_CODE")+"' and ORGANIZATION_ID = "+marketType+"  "+
				                     "    ";						   						  
				   String orderDTL = "  "; 				   
				   ...
				   while (rsDTL.next())
				   {
				      detailLot = detailLot +"<tr><td>"+rsDTL.getString("ISS_INV_ITEM")+"</td><td><div align=right>"+rsDTL.getString("ISS_ITEM_DESC")+"</div></td><td><div align=right>"+rsDTL.getString("QTY_REQUIRED")+"</div></td><td><div align=right>"+rsDTL.getString("QTY_ISSUED")+"</div></td><td><div align=right>"+rsDTL.getString("QTY_PER_ASSEMBLY")+"</div></td></tr>";  // 組合前段予後段領用的批號
					  //out.println("1");
				   }
				   rsDTL.close();
				   stateDTL.close();
			   ...
			   %>
	  <td nowrap><div align="center"><font color="#006666" face="Arial"><a onmouseover='this.T_STICKY=true;this.T_WIDTH=400;this.T_CLICKCLOSE=false;this.T_BGCOLOR="#CCFF66";this.T_SHADOWCOLOR="#FFFF99";this.T_TITLE="領料料號&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;規格說明&nbsp;&nbsp;&nbsp;&nbsp;工令需求數&nbsp;&nbsp;&nbsp;&nbsp;實際領用數&nbsp;&nbsp;&nbsp;&nbsp;單位用量";this.T_OFFSETY=-32;return escape("<%=detailHdr%>")'><font color='#990000'>領料明細</font></a>
	           <!--%
			     if (rsTC.getFloat("QTY_COMPON_ISSUED")!=0)
				 {
			      out.println(rsTC.getFloat("QTY_COMPON_ISSUED")*-1);	
				 } else {
				          out.println("0");	
				        }
				 qtyIssuedTotal=qtyIssuedTotal+rsTC.getFloat("QTY_COMPON_ISSUED");		   
			   %--></font></div></td>
	  <td nowrap><div align="right"><font color="#006666" face="Arial">
	           <%    
			         out.println(rsTC.getFloat("QTY_MOVEIN_TRANSACT")); 	                 
					 qtyMoveInTotal=qtyMoveInTotal+rsTC.getFloat("QTY_MOVEIN_TRANSACT"); 
			   %></font></div></td>	  
	  <td nowrap><div align="right"><font color="#006666" face="Arial">
	           <% 
			     out.println(rsTC.getFloat("QTY_LASTOP_COMPLETE")); 				 
				 qtyCompORRetnTotal = qtyCompORRetnTotal + rsTC.getFloat("QTY_LASTOP_COMPLETE");
			     //frontEndBalTotal=frontEndBalTotal+rsTC.getFloat("QUANTITY_END_BALANCE"); 
			   %></font></div></td>
	  <td nowrap><div align="right"><font color="#006666" face="Arial">
	           <% 
			      out.println(rsTC.getFloat("QTY_MOVEOUT_TRANSACT")); 
			      //workTimeTotal=workTimeTotal+rsTC.getFloat("WORK_TIME");
				  qtyMoveOutTotal=qtyMoveOutTotal+rsTC.getFloat("QTY_MOVEOUT_TRANSACT");
			   %></font></div></td>		      
	  <td nowrap><div align="right"><font color="#006666" face="Arial">
	       <% 
		     out.println(rsTC.getFloat("QUANTITY_END_BALANCE"));
		     endBalTotal=endBalTotal+rsTC.getFloat("QUANTITY_END_BALANCE"); 
		   %></font></div></td>         
    </tr> 
    <%
         rs1__index++;
    ...
    %>
	<tr>
	   <td colspan="4"><div align="center"><font color="#006666" face="Arial">總計</font></div></td>	   
	   <td><div align="right"><font color="#006666" face="Arial"><%=beginBalTotal%></font></div></td>
	   <td><div align="right"><font color="#006666" face="Arial">&nbsp;<%//=(qtyIssuedTotal*-1)%></font></div></td>
	   <td><div align="right"><font color="#006666" face="Arial"><%=qtyMoveInTotal%></font></div></td>
	   <td><div align="right"><font color="#006666" face="Arial"><%=qtyCompORRetnTotal%></font></div></td>
	   <td><div align="right"><font color="#006666" face="Arial"><%=qtyMoveOutTotal%></font></div></td>
