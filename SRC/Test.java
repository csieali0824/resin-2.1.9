...
public class Test {
    ...
    public static Map<String, Map<String, String>> parseToMap(String data) {
        Map<String, Map<String, String>> resultMap = new LinkedHashMap<>();

        // 正則表達式匹配最外層的 key-value
        Pattern outerPattern = Pattern.compile("(\\w+[\\s\\w]*)=\\{(.*?)\\}");
        Matcher outerMatcher = outerPattern.matcher(data);

        while (outerMatcher.find()) {
            String outerKey = outerMatcher.group(1).trim();  // 取出 key
            String innerData = outerMatcher.group(2);  // 取出內部資料 (dateCode, marking, remarks)

            // 解析內部 Map
            Map<String, String> innerMap = new LinkedHashMap<>();
            Pattern innerPattern = Pattern.compile("(\\w+)=([^,}]+)");
            Matcher innerMatcher = innerPattern.matcher(innerData);

            while (innerMatcher.find()) {
                innerMap.put(innerMatcher.group(1).trim(), innerMatcher.group(2).trim());
            }
    ...
    public static void main(String[] args) {
        String str = "xxxxx,123456789op,TQM170NH10CR RLG";

        System.out.println("Xxx="+ Arrays.asList(str.split(",")));
        System.out.println("Xxx="+ Arrays.asList(str.split(",")).size());

    }
    public static void main2(String[] args) throws JsonProcessingException, SQLException {
        String data = "{123456789op={dateCode=YML123, marking=Y_M_L_, remarks=PEGGY_CHEN}, TQM170NH10LCR " +
                "RLG={dateCode=YWWLF4222, marking=YWWLF_, remarks=Mars Wang Create}, TQM170NH10CR " +
                "RLG={dateCode=YWWLF55555, marking=YWWLF_, remarks=Mars Wang Create}}";
        // 解析並轉換成 Map
        Map<String, Map<String, String>> resultMap = parseToMap(data);
...
//        System.out.println(resultMap);
        ...
        for (Map.Entry<String, Map<String, String>> entry : resultMap.entrySet()) {
            ...
            System.out.println("dateCode="+ parameterMap.get("dateCode"));
            System.out.println("marking="+ parameterMap.get("marking"));
            System.out.println("remarks="+ parameterMap.get("remarks"));
            try {
...
//            strPath = "D:\\resin-2.1.9\\webapps\\oradds\\TSCEPriceBook\\" + FileName;
//            File dir = new File("D:\\resin-2.1.9\\webapps\\oradds\\TSCEPriceBook");
            String dirPath = "D:\\resin-2.1.9\\webapps\\oradds\\TSCEPriceBook";
            strPath = dirPath + "\\" + FileName;
            File dir = new File(dirPath);
            deleteDirectory(dir);
//            if (deleteDirectory(dir)) {
//                System.out.println("目錄已成功刪除: " + strPath);
//            } else {
//                System.out.println("目錄刪除失敗或不存在: " + strPath);
//            }
            ...
            WritableFont font_bold = new WritableFont(WritableFont.createFont("Arial"), fontsize + 1,
                    WritableFont.BOLD, false, UnderlineStyle.NO_UNDERLINE, jxl.format.Colour.BLACK);
            WritableFont font_nobold = new WritableFont(WritableFont.createFont("Arial"), fontsize,
                    WritableFont.NO_BOLD, false, UnderlineStyle.NO_UNDERLINE, jxl.format.Colour.BLACK);
            WritableFont font_bold_red = new WritableFont(WritableFont.createFont("Arial"), fontsize,
                    WritableFont.BOLD, false, UnderlineStyle.NO_UNDERLINE, jxl.format.Colour.RED);
            ...
            sql = "with s1 as (\n" +
                    ...
                    "\n" +
                    "     case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.box_qty else replace(nvl(tpi" +
                    ".innerbox_pc,tpii.innerbox_pc),'-','')  end innerbox_pc,\n" +
                    "     case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.carton_qty else replace(nvl(tpi" +
                    ".carton_pc,tpii.carton_pc),'-','') end carton_pc,\n" +
                    "     case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.carton_size else nvl(tpi" +
                    ".CartonSize,tpii.CartonSize) end CartonSize_mm ,\n" +
                    "     case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.carton_weight else nvl(tpi.GW_KG," +
                    "tpii.GW_KG) end GW_KG_CARTON ,\n" +
                    "     (select to_char(max(oola.creation_date),'yyyy/mm/dd') from ont.oe_order_headers_all ooha," +
                    "ont.oe_order_lines_all oola \n" +
                    "     where ooha.header_id=oola.header_id \n" +
                    "      and oola.inventory_item_id=a.inventory_item_id \n" +
                    "        and substr(ooha.order_number ,1,4) in ('1121','1131','1141','1142','1156','1214','7141'," +
                    "'7214','7121')) last_order_date,\n" +
                    ...
                    "\n" +
                    "        case when substr(a.ITEM_DESC1,-3)='-ON' then null else nvl(tpi.PART_NO_LIST,tpii" +
                    ".PART_NO_LIST) end PART_NO_LIST1 ,\n" +
                    "        row_number() over(partition by a.SEGMENT1 order by decode(a.ITEM_DESC1,nvl(tpi" +
                    ".part_no_list,tpii.part_no_list),1,2)) item_cnt,\n" +
                    "        fair.cust_partno fairchild_cpn,case when  a.attribute3 ='005' and a.tw_vendor_flag='N' " +
                    "then 'CHINA' else a.COO_CODE end COO,\n" +
                    "        case a.TSC_PROD_GROUP when 'PRD' then case when substr(a.segment1,11,1) in ('2','H') " +
                    "then 'Yes' else '' end \n" +
                    ...
                    "        else '' end AEC_Q101,tsc_packing_info_preferred(a.inventory_item_id) " +
                    ...
                    "        and NVL(ttpc.TSC_PROD_FAMILY,nvl(a.TSC_PROD_FAMILY,'XXX'))=nvl(a.TSC_PROD_FAMILY,'XXX'))" +
                    " pl_category,\n" +
                    ...
                    "\n" +
                    "        tdpb.price_book_code,tdpb.design_registration,tdpb.recommend_replacement,tdpb" +
                    ".distribution_book_price price1,tdpb.distribution_mpp_price price2,\n" +
                    "        tdpb.design_registration_price price3 FROM (SELECT msi.organization_id,msi.segment1,msi" +
                    ".description,msi.inventory_item_id,msi.attribute3,\n" +
                    ...
                    "                CASE WHEN TSC_INV_CATEGORY(msi.inventory_item_id,43,1100000003) IN ('PMD','SSD'," +
                    "'SSP') \n" +
                    "                THEN TSC_INV_CATEGORY(msi.inventory_item_id,43,1100000004) ELSE '' END " +
                    "TSC_PROD_FAMILY,\n" +
                    ...
                    "                TSC_GET_ITEM_DESC_NOPACKING(MSI.ORGANIZATION_ID ,MSI.INVENTORY_ITEM_ID) " +
                    "ITEM_DESC1,\n" +
                    "                tm.ALENGNAME factory_code,pcn.pcn_list,to_char(pt.creation_date,'yyyy-mm-dd') " +
                    "parts_release_date,\n" +
                    "                case when TSC_INV_CATEGORY(msi.inventory_item_id,49,1100000003) in ('PRD'," +
                    "'PRD-Subcon') \n" +
                    "                THEN TO_CHAR(MIN(pt.CREATION_DATE) over (partition by substr(msi.segment1,1,10)" +
                    "||'1'||substr(msi.segment1,12)),'yyyy-mm-dd') \n" +
                    ...
                    "                when tm.manufactory_no in ('005','006','010','011') then 'TAIWAN' else '' end as" +
                    " COO_CODE,\n" +
                    ...
                    "                 AND nvl(z.active_flag,'N')='A')),(SELECT 'Y' FROM ORADDMAN.TSSG_VENDOR_TW_PARTS" +
                    " X\n" +
                    "                 WHERE X.PART_NAME=TSC_GET_ITEM_DESC_NOPACKING(MSI.ORGANIZATION_ID ,MSI" +
                    ".INVENTORY_ITEM_ID))),'N') tw_vendor_flag\n" +
                    "                 ,nvl(tpcl.default_packing_code,tsc_get_item_packing_code (43, msi" +
                    ".inventory_item_id))  default_packing_code\n" +
                    "                 ,case when instr(TSC_GET_ITEM_PACKING_CODE(43,msi.inventory_item_id),'QQ')>0 " +
                    "then msi.description \n" +
                    "                 else trim(substr(msi.description,0,length(msi.description)-length" +
                    "(TSC_GET_ITEM_PACKING_CODE(43,msi.inventory_item_id)))) end part_id\n" +
                    "                 FROM MTL_SYSTEM_ITEMS  msi,MTL_SYSTEM_ITEMS  msii,oraddman.tsprod_manufactory " +
                    "tm,\n" +
                    "                 (SELECT TSC_PART_NO,listagg(PCN_NUMBER,',') within group(order by PCN_NUMBER) " +
                    "pcn_list\n" +
                    "                 FROM (SELECT TSC_PART_NO,PCN_NUMBER FROM oraddman.tsqra_pcn_item_detail a WHERE" +
                    " SOURCE_TYPE=2\n" +
                    "                 GROUP BY TSC_PART_NO,PCN_NUMBER) X\n" +
                    "                 group by TSC_PART_NO) pcn,(SELECT distinct y.INVENTORY_ITEM_ID,y" +
                    ".ORGANIZATION_ID, x.SEGMENT1,x.CREATION_DATE\n" +
                    ...
                    "                 ,(select mc.inventory_item_id ,mc.organization_id , tc.cccode from " +
                    "mtl_item_categories mc, mtl_categories_tl mct,tsc_cccode tc \n" +
                    ...
                    "                 ,(SELECT * FROM oraddman.tsc_packing_conversion_list where nvl(INACTIVE_DATE," +
                    "to_date('20991231','yyyymmdd'))>TRUNC(SYSDATE)) tpcl\n" +
                    ...
                    "                 and (((length(msi.segment1)=22 and substr(msi.segment1,21,1)='0') or (length" +
                    "(msi.segment1)=30 \n" +
                    "                 and substr(msi.segment1,21,1)='0' and substr(msi.segment1,29,2)='00'))\n" +
                    "                 and substr(msi.segment1,22,1) in ('0','A','B','C','D','F','G','H','I','J','K'," +
                    "'L','N','O','P','Q','R','S','T','V','W','X','Y','Z'))\n" +
                    ...
                    "                 ,TABLE(TSC_ORDER_REVISE_PKG.GET_ITEM_TARGET_PRICE_NEW(43,a.inventory_item_id," +
                    "'PCE',7257,\n" +
                    "                 nvl(TO_DATE('','YYYY/MM/DD'),trunc(sysdate)),'USD',\n" +
                    "                 CASE a.attribute3 WHEN '002' THEN 'Y' WHEN '005' THEN 'T' WHEN '006' THEN 'I' " +
                    "WHEN '008' THEN 'T' WHEN '010' THEN 'A' WHEN '011' THEN 'E' \n" +
                    "                 ELSE 'X' END ,NULL)) pp\n" +
                    "                 ,TABLE(TSC_ORDER_REVISE_PKG.GET_ITEM_TARGET_PRICE_NEW(43,a.inventory_item_id," +
                    "'PCE',9455,\n" +
                    ...
                    "                 CASE a.attribute3 WHEN '002' THEN 'Y' WHEN '005' THEN 'T' WHEN '006' THEN 'I' " +
                    "WHEN '008' THEN 'T' WHEN '010' THEN 'A' WHEN '011' THEN 'E' \n" +
                    "                 ELSE 'X' END ,NULL)) pp_ssd,TABLE(TSC_ORDER_REVISE_PKG" +
                    ".GET_ITEM_TARGET_PRICE_NEW(43,a.inventory_item_id,'PCE',8508,\n" +
                    "                 nvl(TO_DATE('','YYYY/MM/DD'),trunc(sysdate)),'USD',\n" +
                    "                 CASE a.attribute3 WHEN '002' THEN 'Y' WHEN '005' THEN 'T' WHEN '006' THEN 'I' " +
                    "WHEN '008' THEN 'T' WHEN '010' THEN 'A' WHEN '011' THEN 'E' \n" +
                    "                 ELSE 'X' END ,NULL)) pp_prod,table(tsc_get_aecq_info(a.inventory_item_id)) " +
                    "aecq\n" +
                    "                 ,(select * from oraddman.tsc_packing_info_list where part_no_list is not null) " +
                    "tpi\n" +
                    "                 ,(select * from oraddman.tsc_packing_info_list where part_no_list is null) " +
                    "tpii\n" +
                    "                 ,oraddman.tsc_sales_bottom_price tsbp       ,oraddman" +
                    ".tsc_sales_bottom_price_tvs tsbpt\n" +
                    "                 ,(SELECT a.ALTERNATE_ROUTING TSC_PACKAGE, a.DEM_LOCATION_ID SPQ, a" +
                    ".EXP_LOCATION_ID MOQ, a.CODE_DESC REEL_QTY, a.CODE_DESC2 BOX_QTY,\n" +
                    "                 a.CODE_DESC3 CARTON_QTY,a.CODE_DESC4 CARTON_SIZE,a.CODE_DESC5 CARTON_WEIGHT,a" +
                    ".code_desc6 PACKAGINGDESCRIPTION\n" +
                    "                 FROM yew_mfg_defdata a WHERE DEF_TYPE='ON') on_packing\n" +
                    "                 ,(select  cust_partno, tsc_partno from oraddman.ts_label_onsemi_item where " +
                    "tsc_partno like '%-ON%') fair\n" +
                    "                 ,(SELECT x.PCN_NUMBER,x.TSC_PART_NO,case when substr(y.segment1,21,1) not in " +
                    "('0') \n" +
                    "                 or substr(y.segment1,22,1) not in ('0','E','M','N','U') THEN ''\n" +
                    "                  ELSE REPLACE_PART_NO END REPLACE_PART_NO\n" +
                    "                  FROM (SELECT a.*,ROW_NUMBER () OVER (PARTITION BY tsc_part_no ORDER BY " +
                    "pcn_number) row_seq\n" +
                    "                   FROM (SELECT DISTINCT pcn_number, tsc_part_no, replace_part_no FROM oraddman" +
                    ".tsqra_pcn_item_detail a \n" +
                    ...
                    "                    over (partition by description order by creation_date desc) row_seq from inv" +
                    ".mtl_system_items_b\n" +
                    "                     where organization_id=43 and inventory_item_status_code<>'Inactive' ) a " +
                    "where row_seq=1) y\n" +
                    ...
                    "                     AND case when UPPER(a.TSC_PROD_GROUP)='PRD-SUBCON' or upper(a.TSC_PACKAGE)" +
                    "='I2PAK'\n" +
                    ...
                    ") select SEGMENT1, PART_ID, PACKAGE_CODE, DESCRIPTION, PL_CATEGORY, TSC_PROD_GROUP," +
                    "TSC_PROD_CATEGORY,TSC_FAMILY,\n" +
                    "    TSC_PROD_FAMILY, TSC_PACKAGE, SPQ, MOQ, LEAD_TIME, PRICE, PRICE_LAST_UPDATE_DATE, " +
                    "item_creation_date, bottom_price_usd_pcs,\n" +
                    "    sales_head_price_usd_pcs, recommend_stock_in_channel, price_book_code, price1, price2, " +
                    "design_registration, price3, recommend_replacement,\n" +
                    "    factory_code, attribute3, coo, coo_code, part_spec, AEC_Q101,msl, website_status," +
                    "PACKAGINGDESCRIPTION, reel_pc, innerbox_pc, carton_pc,\n" +
                    "    CartonSize_mm, GW_KG_CARTON, pcn_list, cccode,HTS_CODE, NEW_PARTS_RELEASE_DATE, " +
                    "TW_VENDOR_FLAG, first_on_website_date, F400_PRODUCT,\n" +
                    "    TSC_prod_hierarchy_1, TSC_prod_hierarchy_2, TSC_prod_hierarchy_3, TSC_prod_hierarchy_4, " +
                    "fairchild_cpn, ITEM_CNT, prefeered_packing_code_flag \n" +
                    "   from s1\n";
//            System.out.println("sql="+sql);
            Statement statement = con.createStatement();
            ResultSet rs = statement.executeQuery(sql);
            while (rs.next()) {
                //if (rs.getString("PACKAGE_CODE") ==null || rs.getString("part_spec")==null || rs.getInt("ITEM_CNT")
                // !=1 || (rs.getString("TSC_PROD_GROUP").equals("PMD") && rs.getString("bottom_price")==null))
                // continue;
                ...
                if (reccnt == 0) {
                    ...
                    //Series AECQ
                    ws.addCell(new jxl.write.Label(col, row, "AEC-Q101", ACenterBLB)); //rename to AEC-Q101,add by
                    // Peggy 2
                    ws.setColumnView(col, 10);
                    col++;
                ...
                if (rs.getString("fairchild_cpn") != null)  //add by Peggy 20181221
                {
                    ...
                    //ws.addCell(new jxl.write.Label(col, row, rs.getString("DESCRIPTION").substring(0, rs.getString
                    // ("DESCRIPTION").length()-rs.getString("PACKAGE_CODE").length()).trim()  , ALeftL)); //add by
                    // Peggy 20180718
                    //ws.addCell(new jxl.write.Label(col, row, strPartName, ALeftL));
                    ws.addCell(new jxl.write.Label(col, row, rs.getString("part_id"), ALeftL));  //modify by Peggy
                    // 20210729
                }
                col++;
                // PACKAGE CODE
                if (rs.getString("fairchild_cpn") != null || rs.getString("PACKAGE_CODE").indexOf("QQ") >= 0)  //add
                    // by Peggy 20181221
                {
                    ws.addCell(new jxl.write.Label(col, row, "", ALeftL));
                } else {
                    ws.addCell(new jxl.write.Label(col, row, rs.getString("PACKAGE_CODE"), (rs.getString(
                            "prefeered_packing_code_flag").equals("Y") ? ALeftLR : ALeftL)));
                }
                col++;
                // TSC Ordering Code
                if (rs.getString("fairchild_cpn") != null)  //add by Peggy 20181221
                ...
                col++;
                // PL Category
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("PL_CATEGORY") == null ? "" : rs.getString(
                        "PL_CATEGORY")), ALeftL)); //add by Peggy 20221108
                col++;
                // PROD GROUP
                ...
                col++;
                // TSC Prod Category
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_PROD_CATEGORY") == null ? "" :
                        rs.getString("TSC_PROD_CATEGORY")), ALeftL));
                col++;
                // FAMILY
                ...
                col++;
                // PROD FAMILY
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_PROD_FAMILY") == null ? "" :
                        rs.getString("TSC_PROD_FAMILY")), ALeftL));
                col++;
                // PACKAGE
                ...
                if (rs.getString("SPQ") == null) {
                    ...
                    ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("SPQ")).doubleValue(),
                            ARightL));
                }
                col++;
                // MOQ
                if (rs.getString("MOQ") == null) {
                    ...
                    ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("MOQ")).doubleValue(),
                            ARightL));
                }
                col++;
                ...
                if (rs.getString("bottom_price_usd_pcs") == null) {
                    ...
                    //ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("BOTTOM_PRICE"))
                    // .doubleValue(), ARightL));
                    ws.addCell(new jxl.write.Number(col, row,
                            Double.valueOf(rs.getString("bottom_price_usd_pcs")).doubleValue(), ARightL));
                }
                col++;
                // Sales Head price (USD/PCS)
                if (rs.getString("sales_head_price_usd_pcs") == null) {
                    ...
                    //ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("SALES_HEAD_PRICE"))
                    // .doubleValue(), ARightL));
                    ws.addCell(new jxl.write.Number(col, row,
                            Double.valueOf(rs.getString("sales_head_price_usd_pcs")).doubleValue(), ARightL));
                }
                col++;
                // Recommended to Stock in Channel
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("recommend_stock_in_channel") == null ? "" :
                        rs.getString("recommend_stock_in_channel")), ACenterL));
                col++;
                // Price book Code
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("price_book_code") == null ? "" :
                        rs.getString("price_book_code")), ACenterL));
                col++;
                // Distribution Book Price
                if (rs.getString("price1") == null) {
                    ws.addCell(new jxl.write.Label(col, row, "", ACenterL));
                } else {
                    ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("price1")).doubleValue(),
                            ARightL));
                }
                col++;
                ...
                if (rs.getString("price2") == null) {
                    ...
                    ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("price2")).doubleValue(),
                            ARightL));
                }
                col++;
                // Design Registration
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("design_registration") == null ? "" :
                        rs.getString("design_registration")), ACenterL));
                col++;
                // Design Registration Price
                if (rs.getString("price3") == null) {
                    ws.addCell(new jxl.write.Label(col, row, "", ACenterL));
                } else {
                    ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("price3")).doubleValue(),
                            ARightL));
                }
                col++;
                // Recommended Replacement
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("recommend_replacement") == null ? "" :
                        rs.getString("recommend_replacement")), ACenterL));
                col++;
                // Factory
                ...
                col++;
                // COO(TSCA use only)
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("coo_code") == null ? "" : rs.getString(
                        "coo_code")), ACenterL)); //add by Peggy 20200203
                col++;
                // Description
                ...
                //AEC-Q101
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("AEC_Q101") == null ? "" : rs.getString(
                        "AEC_Q101")), ACenterL));
                col++;
                // MSL
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("msl") == null || rs.getString("msl").equals(
                        "0") ? "" : rs.getString("msl")), ALeftL));
                col++;
                // Website Status
                ...
                {
                    //if (rs.getString("TSC_PACKAGE")!=null && (rs.getString("TSC_PACKAGE").equals("C-SMA") || rs
                    // .getString("TSC_PACKAGE").equals("Folded SMA") || rs.getString("TSC_PACKAGE").equals("SMA") ||
                    // rs.getString("TSC_PACKAGE").equals("SMB") || rs.getString("TSC_PACKAGE").equals("SMC")))
                    ...
                    ws.addCell(new jxl.write.Label(col, row, (rs.getString("website_status") == null ? "" :
                            rs.getString("website_status")), ACenterL));
                }
                col++;
                // Packaging Description
                ...
                if (rs.getString("innerbox_pc") == null) {
                    ...
                    ws.addCell(new jxl.write.Number(col, row,
                            Double.valueOf(rs.getString("innerbox_pc")).doubleValue(), ARightL));
                }
                col++;
                // Carton(PC)
                ...
                col++;
                // TARIC Code
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("cccode") == null ? "" :
                        rs.getString("cccode")), ACenterL));
                col++;
                // HTS Code(TSCA local use)
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("HTS_CODE") == null ? "" : rs.getString(
                        "HTS_CODE")), ALeftL));
                col++;
                // Part Name create Date
                ws.addCell(new jxl.write.Label(col, row, rs.getString("NEW_PARTS_RELEASE_DATE"), ACenterL));  //add
                // by Peggy 20230421
                col++;
                // TW Vendor
                ws.addCell(new jxl.write.Label(col, row, rs.getString("TW_VENDOR_FLAG"), ACenterL));  //add by Peggy
                // 20200130
                col++;
                // NPI released to Web
                ...
                col++;
                // F400 PRODUCT
                ws.addCell(new jxl.write.Label(col, row, (rs.getString("F400_PRODUCT") == null ? "" : rs.getString(
                        "F400_PRODUCT")), ACenterL)); //add by Peggy 20230214
                col++;
                // SPG STATUS
                ...
                col++;
                //ws.addCell(new jxl.write.Label(col, row, (rs.getString("recommend_stock_item")==null?"":"Y") ,
                // ACenterL)); //add by Peggy 20230807
                //col++;
                // PROD HIERARCHY 1
...
//                    col++;
            ...
            System.out.println("Exception:" + e.getMessage() + " cnt:" + reccnt);
            throw new RuntimeException(e);
        }
    }
}