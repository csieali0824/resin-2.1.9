...
//        ResultSet rs=statement.executeQuery(sql);
        ...
        String inCondition = createInCondition(description);
        System.out.println("xxx="+ inCondition);
...
//        result.append(")");
        ...
        // 使用 Stream 和 String.join() 來拼接字串
//        return "(" + String.join(", ", wrapWithQuotes(values)) + ")";
    }

    private static String[] wrapWithQuotes(String[] values) {
        // 為每個值加上單引號
        return java.util.Arrays.stream(values)
                .map(value -> "'" + value + "'") // 防止 SQL 注入
                .toArray(String[]::new);
    }

    public static void main(String[] args) throws Exception {
        Connection con = ConnUtils.getConnectionProd();
...
//        String serverHostName=request.getServerName();
        ...
        String strAECQ = "", strDesc = "", strCartonSize = "", strGW = "", strwebstatus = "", strgroup = "",
                obsoletedate = "", strPath = "", strPartName = "";
        ...
        if (DATATYPE.equals("ALL")) {
            FileName =
                    RPTName + "(Include Inactive)" + "-" + dateBean.getYearMonthDay() + dateBean.getHourMinute() + (SYSDATE.equals("") ? "" : "(price date" + SYSDATE + ")") + ".xls";
        } else if (DATATYPE.equals("INACTIVE")) {
            FileName =
                    RPTName + "(Inactive)" + "-" + dateBean.getYearMonthDay() + dateBean.getHourMinute() + (SYSDATE.equals("") ? "" : "(price date" + SYSDATE + ")") + ".xls";
        } else {
            FileName = RPTName + "-" + dateBean.getYearMonthDay() + dateBean.getHourMinute() + (SYSDATE.equals("") ?
                    "" : "(price date" + SYSDATE + ")") + ".xls";
        }
        //FileName = RPTName+(DATATYPE.equals("ALL")?"(Include Inactive)":"")+"-"+dateBean.getYearMonthDay()+dateBean
        // .getHourMinute()+".xls";
        strPath = "D:\\resin-2.1.9\\webapps\\oradds\\TSCItemListUser\\" + FileName;
        ...
        sql = "select a.*" +
                ",case when a.attribute3 in ('005') and a.tw_vendor_flag='N' then round(decode( pp_ssd.TP_PRICE_UOM," +
                "'KPC',pp_ssd.TP_PRICE/1000,pp_ssd.TP_PRICE),5) when a.attribute3 in ('008') and a.tw_vendor_flag='N'" +
                " then round(decode( pp_prod.TP_PRICE_UOM,'KPC',pp_prod.TP_PRICE/1000,pp_prod.TP_PRICE),5) else " +
                "decode( pp.TP_PRICE_UOM,'KPC',pp.TP_PRICE/1000,pp.TP_PRICE) end AS PRICE" +
                ",case when a.attribute3 in ('005') and a.tw_vendor_flag='N' then to_char(pp_ssd.last_update_date," +
                "'yyyy-mm-dd')  when a.attribute3 in ('008') and a.tw_vendor_flag='N' then to_char(pp_prod" +
                ".last_update_date,'yyyy-mm-dd') else to_char(pp.last_update_date,'yyyy-mm-dd') end AS " +
                "price_last_update_date" +
                ",case when a.attribute3 in ('005') and a.tw_vendor_flag='N' then pp_ssd.TP_PRICE_UOM when a" +
                ".attribute3 in ('008') and a.tw_vendor_flag='N' then pp_prod.TP_PRICE_UOM else pp.TP_PRICE_UOM end " +
                "as PRODUCT_UOM_CODE" +
                ",'PCE' UOM" +
                ",qq.SPQ" +
                ",qq.MOQ" +
                ...
                ",TSC_INV_Prod_Group_7(a.inventory_item_id ,a.organization_id) Product_Group_8" +  //Product Group
                // 8同Forecast 及 BI 都使用 Function TSC_INV_Prod_Group_7,add by Peggy 20170510
                ",aecq.series_aecq" +
                ",aecq.part_spec" +
                ",aecq.website_status" +
                ...
                ",nvl(aecq.msl,'NA') msl" +
                ",case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.reel_qty else  replace(nvl(tpi.reel_pc,tpii" +
                ".reel_pc),'-','')  end reel_pc" +       //modify by Peggy 20181115
                ",case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.box_qty else replace(nvl(tpi.innerbox_pc," +
                "tpii.innerbox_pc),'-','')  end innerbox_pc" + //modify by Peggy 20181115
                ",case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.carton_qty else replace(nvl(tpi.carton_pc," +
                "tpii.carton_pc),'-','') end carton_pc" +   //modify by Peggy 20181115
                ",case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.carton_size else nvl(tpi.CartonSize,tpii" +
                ".CartonSize) end CartonSize_mm " +         //modify by Peggy 20181115
                ",case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.carton_weight else nvl(tpi.GW_KG,tpii" +
                ".GW_KG) end GW_KG_CARTON " +         //modify by Peggy 20181115
                ",(select to_char(max(oola.creation_date),'yyyy/mm/dd') from ont.oe_order_headers_all ooha,ont" +
                ".oe_order_lines_all oola where ooha.header_id=oola.header_id" +
                "  and oola.inventory_item_id=a.inventory_item_id" +
                "  and substr(ooha.order_number ,1,4) in ('1121','1131','1141','1142','1156','1214','7141','7214'," +
                "'7121')) last_order_date" + //add by Peggy 20180202
                ",case when substr(a.ITEM_DESC1,-3)='-ON' then on_packing.PACKAGINGDESCRIPTION else nvl(tpi" +
                ".PACKAGINGDESCRIPTION,tpii.PACKAGINGDESCRIPTION) end PACKAGINGDESCRIPTION " + //modify by Peggy
                // 20181115
                ",case when substr(a.ITEM_DESC1,-3)='-ON' then null else nvl(tpi.PART_NO_LIST,tpii.PART_NO_LIST) end " +
                "PART_NO_LIST1 " + //modify by Peggy 20181115
                ",row_number() over(partition by a.SEGMENT1 order by decode(a.ITEM_DESC1,nvl(tpi.part_no_list,tpii" +
                ".part_no_list),1,2)) item_cnt" +  //add by Peggy 20181023
                ",case when  a.attribute3 ='005' and a.tw_vendor_flag='N' then 'CHINA' else a.COO_CODE end COO" +
                ",case a.TSC_PROD_GROUP when 'PRD' then case when substr(a.segment1,11,1) in ('2','H') then 'Yes' " +
                "else '' end" +
                "                       when 'SSD' then case when substr(a.segment1,11,1) in ('2','H') then 'Yes' " +
                "else '' end" + //SSD add by Peggy 20240516
                "                       when 'PMD' then case when substr(a.segment1,11,2) ='TP' then 'Yes' else '' " +
                "end" +
                "                       else '' end AEC_Q101" + //add by Peggy 20200708
                ",tsc_packing_info_preferred(a.inventory_item_id) prefeered_packing_code_flag" +
                ",(select listagg(pl_category,',') within group(order by pl_category) xx  from oraddman" +
                ".ts_pl_category ttpc where ttpc.TSC_PROD_GROUP=a.TSC_PROD_GROUP and NVL(ttpc.TSC_PROD_CATEGORY,a" +
                ".TSC_PROD_CATEGORY)=a.TSC_PROD_CATEGORY and ttpc.TSC_FAMILY=a.TSC_FAMILY and NVL(ttpc" +
                ".TSC_PROD_FAMILY,nvl(a.TSC_PROD_FAMILY,'XXX'))=nvl(a.TSC_PROD_FAMILY,'XXX')) pl_category" + //add by
                // Peggy 20221108
                ",case when tfp.tsc_ordering_code is not null then 'YES' else '' end as F400_PRODUCT" + //add by
                // Peggy 20230214
                ...
                "       TSC_INV_CATEGORY(msi.inventory_item_id,43,1100000203) TSC_prod_hierarchy_1," +          //add
                // by Peggy 20230525
                "       TSC_INV_CATEGORY(msi.inventory_item_id,43,1100000204) TSC_prod_hierarchy_2," +          //add
                // by Peggy 20230525
                "       TSC_INV_CATEGORY(msi.inventory_item_id,43,1100000205) TSC_prod_hierarchy_3," +          //add
                // by Peggy 20230525
                "       TSC_INV_CATEGORY(msi.inventory_item_id,43,1100000206) TSC_prod_hierarchy_4," +          //add
                // by Peggy 20230525
                "       CASE WHEN TSC_INV_CATEGORY(msi.inventory_item_id,43,1100000003) IN ('PMD','SSD','SSP') THEN " +
                "TSC_INV_CATEGORY(msi.inventory_item_id,43,1100000004) ELSE '' END TSC_PROD_FAMILY," +
                ...
                "       case when TSC_INV_CATEGORY(msi.inventory_item_id,49,1100000003) in ('PRD','PRD-Subcon') THEN " +
                "TO_CHAR(MIN(pt.CREATION_DATE) over (partition by substr(msi.segment1,1,10)||'1'||substr(msi" +
                ".segment1,12)),'yyyy-mm-dd') ELSE to_char(pt.creation_date,'yyyy-mm-dd') END new_parts_release_date,"
                +//add by Peggy 20230421
                "       to_char(msi.creation_date,'yyyy-mm-dd') item_creation_date," + //add by Peggy 20171017
                "       case when tm.manufactory_no in ('002','008') then 'CHINA' when tm.manufactory_no in ('005'," +
                "'006','010','011') then 'TAIWAN' else '' end as COO_CODE," +//add by Peggy 20180810
                "       trim(substr(cc.cccode,-11)) cccode," + //add by Peggy 20180813
                "       msii.INVENTORY_ITEM_STATUS_CODE IM_STATUS," + //add by Peggy 20190218
                "       case when 'Y' IN (NVL(msi.CUSTOMER_ORDER_ENABLED_FLAG,'N'), NVL(msi" +
                ".INTERNAL_ORDER_ENABLED_FLAG,'N')) THEN 'Y' ELSE 'N' END AS ORDER_ENABLED_FLAG" + //20190218
                "       ,(SELECT PACKAGE_TYPE FROM ORADDMAN.TSC_PACKAGE_TYPE B WHERE TSC_PACKAGE=TSC_INV_CATEGORY(msi" +
                ".INVENTORY_ITEM_ID,msi.ORGANIZATION_ID,23)) PACKAGE_TYPE" + //20191209
                "       ,TSCA_GET_HTS_CODE(msi.inventory_item_id,trim(substr(cc.cccode,-11))) HTS_CODE" + //add by
                // Peggy 20191209
                "       ,NVL(NVL((SELECT DISTINCT 'Y' FROM po_headers_all x,po_lines_all y WHERE x" +
                ".TYPE_LOOKUP_CODE='BLANKET'" +
                "        AND x.ORG_ID in (41) AND NVL(x.cancel_flag,'N') = 'N' AND NVL(x.closed_code,'OPEN') NOT LIKE" +
                " '%CLOSED%' AND NVL(y.cancel_flag,'N') = 'N'" +
                "        AND NVL(y.closed_code,'OPEN') <> 'CLOSED'  AND NVL(y.closed_flag,'N') <> 'Y' AND y.item_id " +
                "=msi.inventory_item_id AND x.po_header_id=y.po_header_id" +
                "        AND EXISTS (SELECT 1 FROM oraddman.tssg_vendor_tw z WHERE z.vendor_site_id=x.VENDOR_SITE_ID " +
                "AND nvl(z.active_flag,'N')='A')),(SELECT 'Y' FROM ORADDMAN.TSSG_VENDOR_TW_PARTS X" +
                "        WHERE X.PART_NAME=TSC_GET_ITEM_DESC_NOPACKING(MSI.ORGANIZATION_ID ,MSI.INVENTORY_ITEM_ID)))," +
                "'N') tw_vendor_flag" +
                "       ,nvl(tpcl.default_packing_code,tsc_get_item_packing_code (43, msi.inventory_item_id))  " +
                "default_packing_code" +  //add by Peggy 20201021
                "       ,case when instr(TSC_GET_ITEM_PACKING_CODE(43,msi.inventory_item_id),'QQ')>0 then msi" +
                ".description else trim(substr(msi.description,0,length(msi.description)-length" +
                "(TSC_GET_ITEM_PACKING_CODE(43,msi.inventory_item_id)))) end part_id" + //add by Peggy 20210729
                ...
                "       ,(select mc.inventory_item_id ,mc.organization_id , tc.cccode from mtl_item_categories mc, " +
                "mtl_categories_tl mct,tsc_cccode tc where mc.CATEGORY_SET_ID=6 " +
                "        and mc.category_id = mct.category_id and mct.language = 'US' and tc.category_id = mct" +
                ".category_id and tc.language = mct.language) cc" +  //add by Peggy 20180813
                "       ,(SELECT * FROM oraddman.tsc_packing_conversion_list where nvl(INACTIVE_DATE,to_date" +
                "('20991231','yyyymmdd'))>TRUNC(SYSDATE)) tpcl" +
                "       WHERE msi.ORGANIZATION_ID=49" +
                "       AND LENGTH(msi.SEGMENT1)>=22" +
                "       AND TSC_INV_Category(msi.inventory_item_id,43, 23)=tpcl.tsc_package(+)" + //add by Peggy
                // 20201021
                "       AND tsc_get_item_packing_code (43, msi.inventory_item_id)=tpcl.packing_code(+)" +  //add by
                // Peggy 20201021
        ...
        sql += "       AND msii.ORGANIZATION_ID=43" +    //add by Peggy 20190215
                ...
                "       ,TABLE(TSC_ORDER_REVISE_PKG.GET_ITEM_TARGET_PRICE_NEW(43,a.inventory_item_id,'PCE',7257,nvl" +
                "(TO_DATE('" + SYSDATE + "','YYYY/MM/DD'),trunc(sysdate)),'USD',CASE a.attribute3 WHEN '002' THEN 'Y'" +
                " WHEN '005' THEN 'T' WHEN '006' THEN 'I' WHEN '008' THEN 'T' WHEN '010' THEN 'A' WHEN '011' THEN 'E'" +
                " ELSE 'X' END ,NULL)) pp" +
                "       ,TABLE(TSC_ORDER_REVISE_PKG.GET_ITEM_TARGET_PRICE_NEW(43,a.inventory_item_id,'PCE',9455,nvl" +
                "(TO_DATE('" + SYSDATE + "','YYYY/MM/DD'),trunc(sysdate)),'CNY',CASE a.attribute3 WHEN '002' THEN 'Y'" +
                " WHEN '005' THEN 'T' WHEN '006' THEN 'I' WHEN '008' THEN 'T' WHEN '010' THEN 'A' WHEN '011' THEN 'E'" +
                " ELSE 'X' END ,NULL)) pp_cny" +
                "       ,TABLE(TSC_ORDER_REVISE_PKG.GET_ITEM_TARGET_PRICE_NEW(43,a.inventory_item_id,'PCE',8534,nvl" +
                "(TO_DATE('" + SYSDATE + "','YYYY/MM/DD'),trunc(sysdate)),'USD',CASE a.attribute3 WHEN '002' THEN 'Y'" +
                " WHEN '005' THEN 'T' WHEN '006' THEN 'I' WHEN '008' THEN 'T' WHEN '010' THEN 'A' WHEN '011' THEN 'E'" +
                " ELSE 'X' END ,NULL)) pp_ssd" +
                "       ,TABLE(TSC_ORDER_REVISE_PKG.GET_ITEM_TARGET_PRICE_NEW(43,a.inventory_item_id,'PCE',8508,nvl" +
                "(TO_DATE('" + SYSDATE + "','YYYY/MM/DD'),trunc(sysdate)),'USD',CASE a.attribute3 WHEN '002' THEN 'Y'" +
                " WHEN '005' THEN 'T' WHEN '006' THEN 'I' WHEN '008' THEN 'T' WHEN '010' THEN 'A' WHEN '011' THEN 'E'" +
                " ELSE 'X' END ,NULL)) pp_prod" +
                "       ,table(tsc_get_aecq_info(a.inventory_item_id)) aecq" + //add by Peggy 20230802
                "       ,(select * from oraddman.tsc_packing_info_list where part_no_list is not null) tpi" + //add
                // by Peggy 20210419
                "       ,(select * from oraddman.tsc_packing_info_list where part_no_list is null) tpii" + //add by
                // Peggy 20210419
                ...
                "       ,(SELECT a.ALTERNATE_ROUTING TSC_PACKAGE, a.DEM_LOCATION_ID SPQ, a.EXP_LOCATION_ID MOQ, a" +
                ".CODE_DESC REEL_QTY, a.CODE_DESC2 BOX_QTY,a.CODE_DESC3 CARTON_QTY,a.CODE_DESC4 CARTON_SIZE,a" +
                ".CODE_DESC5 CARTON_WEIGHT,a.code_desc6 PACKAGINGDESCRIPTION" +
                ...
                "       AND case when UPPER(a.TSC_PROD_GROUP)='PRD-SUBCON' or upper(a.TSC_PACKAGE)='I2PAK' THEN 'PRD'" +
                " else a.TSC_PROD_GROUP END =tpii.GROUPTYPE(+)" +
        ...
        while (rs.next()) {
            ...
            //else
            //{
            //	strPartName=rs.getString("DESCRIPTION").substring(0, rs.getString("DESCRIPTION").length()-rs.getString
            //	("PACKAGE_CODE").length()).trim();
            //}
            ...
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("PACKAGE_CODE").indexOf("QQ") >= 0 ? "" :
                    rs.getString("PACKAGE_CODE")),
                    (rs.getString("prefeered_packing_code_flag") != null && rs.getString("prefeered_packing_code_flag"
                    ).equals("Y") ? ALeftLR : ALeftL)));
            col++;
            ws.addCell(new jxl.write.Label(col, row, rs.getString("DESCRIPTION"), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_PROD_GROUP") == null ? "" : rs.getString(
                    "TSC_PROD_GROUP")), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_PROD_CATEGORY") == null ? "" : rs.getString(
                    "TSC_PROD_CATEGORY")), ALeftL));  //add by Peggy 20180222
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_FAMILY") == null ? "" : rs.getString(
                    "TSC_FAMILY")), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_PROD_FAMILY") == null ? "" : rs.getString(
                    "TSC_PROD_FAMILY")), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_PACKAGE") == null ? "" : rs.getString(
                    "TSC_PACKAGE")), ALeftL));
            col++;
            //ws.addCell(new jxl.write.Label(col, row, (rs.getString("PACKAGE_TYPE")==null?"":rs.getString
            // ("PACKAGE_TYPE")) , ALeftL)); //add by Peggy 20191209
            //col++;
            ...
            if (rs.getString("SAMPLE_SPQ") == null) {
                ...
                ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("SAMPLE_SPQ")).doubleValue(),
                        ARightL));
            }
            col++;
            if (rs.getString("LEAD_TIME") == null) {
                ...
                ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("LEAD_TIME")).doubleValue(),
                        ARightL));
            }
            col++;
            ...
            //else
            //{
            //	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("NO_WAFER_LEAD_TIME"))
            //	.doubleValue(), ARightL));
            //}
            //col++;
            if (rs.getString("PRICE") == null) {
                ...
                ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("PRICE")).doubleValue(),
                        ARightL));
            }
            col++;
            ...
            //else
            //{
            //	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("TSCE_PRICE")).doubleValue(),
            //	ARightL));
            //}
            //col++;
            ...
            //else
            //{
            //	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("BOTTOM_PRICE")).doubleValue(),
            //	ARightL));
            //}
            //col++;
            ...
            //else
            //{
            //	ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("SALES_HEAD_PRICE")).doubleValue
            //	(), ARightL));
            //}
            //col++;
            ...
            col++;
            //if (rs.getString("DESCRIPTION").startsWith("HER104G-K") || (rs.getString("DESCRIPTION").startsWith
            // ("BZW04-") && !rs.getString("DESCRIPTION").startsWith("BZW04-171-01")))
            //{
            //	//ws.addCell(new jxl.write.Label(col, row, (strPartName.endsWith("H")?strPartName.substring(0,
            //	strPartName.length()-1) :strPartName), ALeftL)); //add by Peggy 20180718
            //	ws.addCell(new jxl.write.Label(col, row, (rs.getString("part_id").endsWith("H")?rs.getString
            //	("part_id").substring(0,rs.getString("part_id").length()-1) :rs.getString("part_id")), ALeftL)); //add
            //	by Peggy 20210729
            //}
            ...
            //}
            //col++;
            //ws.addCell(new jxl.write.Label(col, row, (rs.getString("series_aecq")==null?"":rs.getString
            // ("series_aecq")), ACenterL));
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("AEC_Q101") == null ? "" :
                    rs.getString("AEC_Q101")), ACenterL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("part_spec") == null ? "" : rs.getString(
                    "part_spec")), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("msl") == null || rs.getString("msl").equals("0")
                    ? "" : rs.getString("msl")), ALeftL));  //add by Peggy 20190328
            col++;
            //ws.addCell(new jxl.write.Label(col, row, (rs.getString("website_status")==null?"":rs.getString
            // ("website_status")) , ACenterL));
            ...
            //ws.addCell(new jxl.write.Label(col, row, (rs.getString("website_status")==null?"":rs.getString
            // ("website_status")) , ACenterL));
            //if (rs.getString("TSC_PROD_GROUP")!=null && rs.getString("TSC_PROD_GROUP").equals("PRD") && rs
            // .getString("PACKAGE_CODE").indexOf("QQ")<0)//add by Peggy 20200421
            //{
            //	if (rs.getString("TSC_PACKAGE")!=null && ((rs.getString("TSC_PACKAGE").equals("C-SMA") && rs.getString
            //	("TSC_FAMILY") != null && !rs.getString("TSC_FAMILY").equals("Trench SKY")) || rs.getString
            //	("TSC_PACKAGE").equals("Folded SMA") || rs.getString("TSC_PACKAGE").equals("SMA") || rs.getString
            //	("TSC_PACKAGE").equals("SMB") || (rs.getString("TSC_PACKAGE").equals("SMC") && !rs.getString
            ...
            //}
            //col++;
            //ws.addCell(new jxl.write.Label(col, row, (rs.getString("obsolete_timestamp")==null?"":rs.getString
            // ("obsolete_timestamp")) , ACenterL));
            //col++;
            ws.addCell(new jxl.write.Label(col, row, rs.getString("PACKAGINGDESCRIPTION"), ACenterL));  //add by
            // Peggy 20180928
            col++;
            if (rs.getString("reel_pc") == null) {
                ws.addCell(new jxl.write.Label(col, row, "", ACenterL));
            } else {
                ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("reel_pc")).doubleValue(),
                        ARightL));
            }
            col++;
            if (rs.getString("innerbox_pc") == null) {
                ...
                ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("innerbox_pc")).doubleValue(),
                        ARightL));
            }
            col++;
            if (rs.getString("carton_pc") == null) {
                ...
                ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("carton_pc")).doubleValue(),
                        ARightL));
            }
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("CartonSize_mm") == null ? "" : rs.getString(
                    "CartonSize_mm")), ACenterL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("GW_KG_CARTON") == null ? "" : rs.getString(
                    "GW_KG_CARTON")), ARightL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, rs.getString("pcn_list"), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, rs.getString("Product_Group_8"), ALeftL));  //modify by Peggy
            // 20170510
            col++;
            if (rs.getString("Product_Group_8") != null) {
                ...
                } else if (rs.getString("Product_Group_8").equals("MOSFET-Subcon") || rs.getString("Product_Group_8").equals("Super Junction") || rs.getString("Product_Group_8").equals("Super Junction-Subcon"))  //add Super Junction-Subcon by Peggy 20210721
                {
                    ...
                    ws.addCell(new jxl.write.Label(col, row, (rs.getString("Product_Group_8") == null ? "" :
                            rs.getString("Product_Group_8")), ALeftL));
                }
            } else {
                ws.addCell(new jxl.write.Label(col, row, "", ALeftL));  //modify by Peggy 20170510
            }
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("NEW_PARTS_RELEASE_DATE") == null ? "" :
                    rs.getString("NEW_PARTS_RELEASE_DATE")), ACenterL));
            col++;
            //ws.addCell(new jxl.write.Label(col, row, rs.getString("PARTS_RELEASE_DATE") , ACenterL));
            //col++;
            //ws.addCell(new jxl.write.Label(col, row, rs.getString("last_order_date") , ACenterL));  //add by Peggy
            // 20180202
            //col++;
            //ws.addCell(new jxl.write.Label(col, row,  (rs.getString("attribute3")==null?"":((rs.getString
            // ("attribute3").equals("002") || rs.getString("attribute3").equals("008"))?"85411000":"")) , ACenterL))
            // ;  //add by Peggy 20180602
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("cccode") == null ? "" : rs.getString("cccode")),
                    ACenterL));  //add by Peggy 20180813
            col++;
            //ws.addCell(new jxl.write.Label(col, row, (rs.getString("HTS_CODE")==null?"":rs.getString("HTS_CODE")) ,
            // ACenterL));  //add by Peggy 20191209
            //col++;
		/*ws.addCell(new jxl.write.Label(col, row, (rs.getString("im_status")==null?"":rs.getString("im_status")) ,
		ACenterL));  //add by Peggy 20190218
		col++;
		ws.addCell(new jxl.write.Label(col, row, rs.getString("ORDER_ENABLED_FLAG") , ACenterL));  //add by Peggy
		20190218
		col++;
		...
		{
			ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("arrow_book_cost")).doubleValue(),
			ARightL));
		}
		col++;
		...
		{
			ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("avnet_future_book_cost"))
			.doubleValue(), ARightL));
		}
		col++;
		...
		{
			ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("mpp_for_arrow_avnet_future"))
			.doubleValue(), ARightL));
		}
		col++;
		...
		{
			ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("moq_for_arrow_avnet_future"))
			.doubleValue(), ARightL));
		}
		col++;
		...
		{
			ws.addCell(new jxl.write.Number(col, row, Double.valueOf(rs.getString("catalog_price")).doubleValue(),
			ARightL));
		}
		col++;
		*/
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("first_on_website_date") == null ? "" :
                    rs.getString("first_on_website_date")), ACenterL)); //add by Peggy 20211130
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_prod_hierarchy_1") == null ? "" :
                    rs.getString("TSC_prod_hierarchy_1")), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_prod_hierarchy_2") == null ? "" :
                    rs.getString("TSC_prod_hierarchy_2")), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_prod_hierarchy_3") == null ? "" :
                    rs.getString("TSC_prod_hierarchy_3")), ALeftL));
            col++;
            ws.addCell(new jxl.write.Label(col, row, (rs.getString("TSC_prod_hierarchy_4") == null ? "" :
                    rs.getString("TSC_prod_hierarchy_4")), ALeftL));
            col++;
