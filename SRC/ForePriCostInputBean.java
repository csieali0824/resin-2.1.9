...
public class ForePriCostInputBean implements java.io.Serializable
{
  ...
  public String [][] getCheckContent()  
 {
  ...
  {          
    ...
    sb.append("<TABLE>");         
    if (arrayString!=null && arrayString.length>0) //­Y¦³table header,´N¸m¤J¤§
    {
    ...
    {   
      ...
      for (j=1;j<array2DString[i].length;j++)      
      {
        ...
        String fieldName="MONTH"+i+"-"+j; 
        String checkString=(String)array2DCheck[i][j];
        if (!checkString.equals("N") && j<=1) //¦]¬°¥u¦³¤@­Ó¤ë»Ý©T©wë
        { 
           sb.append("<TD BGCOLOR=RED><FONT SIZE=2 COLOR=WHITE>"+checkString+"<input type='HIDDEN' name="+fieldName+" value='"+checkString+"'></TD>");
        } else {
           sb.append("<TD><FONT SIZE=1><input type='text' name="+fieldName+" size='10' value='"+s2+"'></TD>");
        }
      } //end of for j
      sb.append("</TR>");
    } //end of for i
